...
const LazyTaskList: React.FC<LazyTaskListProps> = ({ projectId, taskTreeId, onTasksChanged }) => {
  ...
  const handleDeleteTask = useCallback(async (taskId: string) => {
    ...
    if (callbacks) {
      ...
      try {
        // STEP 2: Call API to delete task from backend IMMEDIATELY (don't wait for animation)
        console.log('🚀 DELETE DEBUG: About to call deleteTask API for taskId:', taskId);
        logger.info('Calling delete API while animation plays', { component: 'LazyTaskList', taskId });
        await deleteTask(taskId);
        console.log('🚀 DELETE DEBUG: deleteTask API call completed for taskId:', taskId);

        // STEP 3: Wait for animation to complete BEFORE refreshing UI (800ms + buffer)
        logger.info('API delete successful, waiting for animation to complete', { component: 'LazyTaskList', taskId });
        await new Promise(resolve => setTimeout(resolve, 850));

        // STEP 4: NOW refresh the list to remove the task from UI
        if (onTasksChanged) {
          console.log('🚀 DELETE DEBUG: Animation complete, calling onTasksChanged() to refresh list');
          onTasksChanged();
        }

        logger.info('Task successfully deleted and animation completed', { component: 'LazyTaskList', taskId });
      } catch (error) {
        logger.error('Failed to delete task', { component: 'LazyTaskList', taskId, error });

        // If API fails, still wait for animation to complete to avoid jarring UX
        await new Promise(resolve => setTimeout(resolve, 850));
