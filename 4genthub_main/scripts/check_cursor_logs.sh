...
# Search for 4genthub errors
echo "🔍 4genthub errors:"
find "$CURSOR_LOGS_DIR" -name "*.log" -mtime -1 -exec grep -l "4genthub" {} \; 2>/dev/null | while read logfile; do
    echo "📋 Found in: $logfile"
    grep -n -A2 -B2 "4genthub" "$logfile" | tail -20
    echo ""
done
...
if [[ -f "$MAIN_LOG" ]]; then
    ...
    echo "=============================="
    tail -50 "$MAIN_LOG" | grep -i -A2 -B2 "mcp\|4genthub\|server\|error" || echo "No MCP-related entries found in recent main.log"
    echo ""
fi
...
if [[ -n "$RENDERER_LOGS" ]]; then
    ...
    for log in $RENDERER_LOGS; do
        echo "📋 Checking: $log"
        tail -30 "$log" | grep -i -A2 -B2 "mcp\|4genthub\|server.*error" || echo "No MCP-related entries found"
        echo ""
    done
fi
