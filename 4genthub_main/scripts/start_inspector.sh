...
# MCP Inspector Launcher for 4genthub Server
# This script starts the MCP inspector with the correct configuration

echo "🚀 Starting MCP Inspector for 4genthub Server..."
echo "📍 Working directory: $(pwd)"

# Check if we're in the right directory
if [[ ! -d ".venv" ]]; then
    echo "❌ Error: .venv directory not found. Please run this script from 4genthub_main directory"
    exit 1
fi
