#!/usr/bin/env python3
"""
PostgreSQL + Keycloak Setup Script for 4genthub
...
class PostgresKeycloakSetup:
    """Configure 4genthub for PostgreSQL + Keycloak"""
    ...
    def create_env_file(self) -> bool:
        ...
        env_content = """# 4genthub Environment Configuration
...
# Connection URL
DATABASE_URL=postgresql://4genthub_user:SecurePassword2025!@localhost:5432/4genthub_prod?sslmode=prefer
...
# ===================================================================
DOCKER_NETWORK=4genthub_network
...
"""
    ...
    def remove_supabase_references(self) -> int:
        ...
        for root, dirs, files in os.walk(self.project_root / "4genthub_main"):
    ...
    def setup_database_init_script(self) -> bool:
        ...
        init_sql_path = self.project_root / "4genthub_main" / "scripts" / "init.sql"
        init_sql_path.parent.mkdir(parents=True, exist_ok=True)
        
        init_sql = """-- PostgreSQL Initialization Script for 4genthub

-- Create database if not exists (run as superuser)
SELECT 'CREATE DATABASE 4genthub_prod'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '4genthub_prod')\\gexec
...
GRANT ALL ON ALL TABLES IN SCHEMA public TO 4genthub_user;
GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO 4genthub_user;
...
"""
    ...
    def run_database_migrations(self) -> bool:
        ...
        migration_script = self.project_root / "4genthub_main" / "src" / "fastmcp" / "task_management" / "infrastructure" / "database" / "init_database.py"
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                cwd=self.project_root / "4genthub_main"
            )
    ...
    def run_setup(self):
        ...
        print("=" * 60)
        print("4genthub PostgreSQL + Keycloak Setup")
        print("=" * 60)
        ...
        print("  Connect to PostgreSQL: psql -h localhost -U 4genthub_user -d 4genthub_prod")
