...
if [ -f .env.production-keycloak-postgres ]; then
    ...
    sed -i "s|4genthub|${KEYCLOAK_REALM}|g" .env.production
    ...
    sed -i "s|https://your-keycloak-domain.com/realms/4genthub|${KEYCLOAK_URL}/realms/${KEYCLOAK_REALM}|g" .env.production
...
def main():
    src_dir = Path("4genthub_main/src")
    cleaned_files = []
...
if [[ "$REMOVE_DATA" =~ ^[Yy]$ ]]; then
    docker volume rm 4genthub_postgres_prod_data 2>/dev/null || true
    docker volume rm 4genthub_postgres_prod_backup 2>/dev/null || true
    print_message "$YELLOW" "📦 Removed existing PostgreSQL volumes"
fi

# Create network if it doesn't exist
docker network create 4genthub_prod_network 2>/dev/null || true
...
for i in {1..30}; do
    if docker-compose -f docker-compose.production.yml exec -T postgres pg_isready -U postgres -d 4genthub >/dev/null 2>&1; then
...
# Execute SQL
docker-compose -f docker-compose.production.yml exec -T postgres psql -U postgres -d 4genthub < init_db.sql
rm init_db.sql
...
docker-compose -f docker-compose.production.yml exec -T postgres psql -U postgres -d 4genthub -c "SELECT version();" >/dev/null 2>&1
if [ $? -eq 0 ]; then
