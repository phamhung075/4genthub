...
# ==============================================================================
# Clean Production Setup for 4genthub
...
DB_NAME="${DATABASE_NAME:-4genthub_prod}"
DB_USER="${DATABASE_USER:-4genthub_user}"
...
cleanup_old_containers() {
    ...
    if [ "$1" == "--clean-volumes" ]; then
        ...
        docker volume rm 4genthub_logs 2>/dev/null || true
        docker volume rm 4genthub_pgadmin_data 2>/dev/null || true
    fi
...
try:
    ...
    config.database_name = os.getenv("DATABASE_NAME", "4genthub_prod")
    config.database_user = os.getenv("DATABASE_USER", "4genthub_user")
...
main() {
    ...
    print_header "4genthub Clean Production Setup"
