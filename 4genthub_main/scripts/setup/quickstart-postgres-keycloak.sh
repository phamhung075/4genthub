...
read -p "Keycloak Realm [4genthub]: " KEYCLOAK_REALM
...
while [ $attempt -lt $max_attempts ]; do
    if docker exec 4genthub-postgres pg_isready -U postgres >/dev/null 2>&1; then
...
pip install -q -r 4genthub_main/requirements.txt
...
nohup python 4genthub_main/src/mcp_http_server.py > logs/mcp-server.log 2>&1 &
MCP_PID=$!
...
echo ""
echo -e "${GREEN}Your 4genthub system is ready to use!${NC}"
echo ""
