...
start_container() {
    log "🚀 Starting 4genthub container..."
...
stop_container() {
    log "🛑 Stopping 4genthub container..."
...
restart_container() {
    log "🔄 Restarting 4genthub container..."
    stop_container
    sleep 2
    start_container
}
...
cleanup_duplicates() {
    ...
    # Find all containers with 4genthub in the name
    DUPLICATE_CONTAINERS=$(docker ps -a --filter "name=4genthub" --format "{{.Names}}" | grep -v "^${CONTAINER_NAME}$" || true)
...
reset_container() {
    log "🔄 Resetting 4genthub container..."
...
show_help() {
    echo "4genthub Container Management"
    echo "================================"
    echo ""
