...
4GENTHUB_DIR="$PROJECT_ROOT/4genthub_main"
VENV_PATH="$4GENTHUB_DIR/.venv"
PYTHON_PATH="$VENV_PATH/bin/python"
SERVER_SCRIPT="$4GENTHUB_DIR/src/fastmcp/task_management/interface/consolidated_mcp_server.py"
...
# Add src to path
sys.path.insert(0, '/home/daihungpham/agentic-project/4genthub_main/src')
...
EOF
    ...
    local cmd="cd '$4GENTHUB_DIR' && source .venv/bin/activate && $test_env python '$test_script'"
...
if __name__ == "__main__":
    ...
    os.environ['PYTHONPATH'] = '/home/daihungpham/agentic-project/4genthub_main/src'
...
EOF
    ...
    local cmd="cd '$4GENTHUB_DIR' && source .venv/bin/activate && python '$tool_test_script'"
...
EOF
    ...
    local cmd="cd '$4GENTHUB_DIR' && source .venv/bin/activate && python '$protocol_test_script'"
...
test_cursor_connectivity() {
    ...
    for config_info in "${configs[@]}"; do
        ...
        if [ -f "$config_path" ]; then
            ...
            # Check if 4genthub is configured
            if grep -q '"4genthub"' "$config_path" 2>/dev/null; then
                echo -e "     ✅ 4genthub server configured"
...
try:
    with open('$config_path', 'r') as f:
        config = json.load(f)
    tm_config = config.get('mcpServers', {}).get('4genthub', {})
...
" 2>/dev/null)
                ...
                echo -e "     ❌ 4genthub server not configured"
            fi
        else
...
EOF
    ...
    local cmd="cd '$4GENTHUB_DIR' && source .venv/bin/activate && python '$response_test_script'"
...
EOF
    ...
    local cmd="cd '$4GENTHUB_DIR' && source .venv/bin/activate && python '$summary_script'"
...
validate_mcp_config() {
    ...
    # Check for 4genthub server
    if grep -q '"4genthub"' "$config_file"; then
        echo -e "  ✅ ${GREEN}4genthub server found in config${NC}"
        
        # Extract and validate 4genthub configuration
        echo -e "  🔍 4genthub configuration:"
        python3 -c "
import json
import os
import sys
...
try:
    ...
    if 'mcpServers' in config and '4genthub' in config['mcpServers']:
        tm_config = config['mcpServers']['4genthub']
        print(f'     Command: {tm_config.get(\"command\", \"NOT SET\")}')
        ...
        print('     ❌ 4genthub not found in mcpServers')
...
" 2>/dev/null || echo -e "     ❌ ${RED}Error parsing configuration${NC}"
        ...
        echo -e "  ❌ ${RED}4genthub server not found in config${NC}"
    fi
}
...
test_manual_startup() {
    ...
    local startup_cmd="cd '$4GENTHUB_DIR' && source .venv/bin/activate && PYTHONPATH='$4GENTHUB_DIR/src' TASKS_JSON_PATH='$TASKS_JSON_PATH' TASK_JSON_BACKUP_PATH='$BACKUP_PATH' python -m fastmcp.task_management.interface.consolidated_mcp_server"
...
provide_recommendations() {
    ...
    if [ ${#issues[@]} -eq 0 ]; then
        ...
        echo -e "  • Check if server starts manually: cd $4GENTHUB_DIR && source .venv/bin/activate && python -m fastmcp.task_management.interface.consolidated_mcp_server"
        ...
        echo -e "  2. Activate virtual environment: cd $4GENTHUB_DIR && source .venv/bin/activate"
...
EOF
    ...
    check_path "$4GENTHUB_DIR" "4genthub_main Directory" "required"
    ...
    check_path "$4GENTHUB_DIR/src/fastmcp" "FastMCP Package" "required"
    check_path "$4GENTHUB_DIR/src/fastmcp/task_management" "Task Management Package" "required"
    ...
    test_server_startup "PYTHONPATH='$4GENTHUB_DIR/src'" "Default Environment"
    test_server_startup "PYTHONPATH='$4GENTHUB_DIR/src' TASKS_JSON_PATH='$TASKS_JSON_PATH' TASK_JSON_BACKUP_PATH='$BACKUP_PATH'" "Full Environment"
...
EOF
    ...
    local cmd="cd '$4GENTHUB_DIR' && source .venv/bin/activate && python '$quick_test_script'"
