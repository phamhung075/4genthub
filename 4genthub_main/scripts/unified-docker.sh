...
# =============================================================================
# 4genthub Unified Docker Management Script
...
FRONTEND_CONTAINER="4genthub-frontend"
POSTGRES_CONTAINER="4genthub-postgres"
REDIS_CONTAINER="4genthub-redis"
...
print_banner() {
    echo -e "${CYAN}=================================================${NC}"
    echo -e "${CYAN}🚀 4genthub Unified Docker Manager${NC}"
    echo -e "${CYAN}=================================================${NC}"
    echo ""
}
...
prepare_environment() {
    ...
    case "$mode" in
        "production")
            ...
            export 4GENTHUB_AUTH_ENABLED="true"
            export 4GENTHUB_MVP_MODE="false"
            ;;
        "local"|"dev")
            export DATABASE_TYPE="postgresql"
            export 4GENTHUB_AUTH_ENABLED="false"
...
cmd_start() {
    ...
    print_banner
    log_info "Starting 4genthub in $mode mode..."
...
cmd_stop() {
    ...
    log_info "Stopping 4genthub containers..."
...
cmd_health() {
    ...
    if check_container_running "$CONTAINER_NAME"; then
        ...
        if check_container_running "$POSTGRES_CONTAINER"; then
            if docker exec $POSTGRES_CONTAINER pg_isready -U 4genthub_user -d 4genthub > /dev/null 2>&1; then
...
cmd_clean() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        docker volume rm 4genthub_data 4genthub_logs postgres_data redis_data 2>/dev/null || true
        
        # Remove images (optional)
        read -p "Remove Docker images too? (y/N): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker rmi 4genthub/mcp-server:latest 4genthub/frontend:latest 2>/dev/null || true
...
cmd_rebuild() {
    ...
    log_info "Rebuilding 4genthub environment..."
