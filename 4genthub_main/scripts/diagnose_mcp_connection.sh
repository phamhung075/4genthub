...
if [[ -f ".cursor/mcp.json" ]]; then
    ...
    echo "📋 4genthub configuration:"
    jq '.mcpServers.4genthub' .cursor/mcp.json 2>/dev/null || echo "❌ Error reading JSON"
else
...
if [[ -f "4genthub_main/.venv/bin/python" ]]; then
    echo "✅ Virtual environment exists"
    echo "📋 Python version: $(4genthub_main/.venv/bin/python --version)"
else
    echo "❌ Virtual environment not found at 4genthub_main/.venv/bin/python"
fi
echo ""
...
if 4genthub_main/.venv/bin/python -c "
import sys
sys.path.insert(0, '4genthub_main/src')
from fastmcp.server.mcp_entry_point import create_4genthub_server
server = create_4genthub_server()
...
" 2>/dev/null; then
    ...
    4genthub_main/.venv/bin/python -c "
import sys
sys.path.insert(0, '4genthub_main/src')
from fastmcp.server.mcp_entry_point import create_4genthub_server
server = create_4genthub_server()
...
echo "2. After restart, check if 4genthub tools appear in the MCP tools list"
