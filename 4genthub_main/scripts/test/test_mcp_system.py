...
Tests various components of the 4genthub system to identify issues
"""
...
def test_database_connection():
    ...
    try:
        ...
        result = subprocess.run(
            ["docker", "exec", "4genthub-postgres", "psql", "-U", "4genthub_user", 
             "-d", "4genthub", "-c", "\\dt"],
            capture_output=True,
            text=True
        )
...
def test_mcp_tools_availability():
    ...
    try:
        ...
        if "4genthub_http" in result.stdout:
            print("✅ MCP server '4genthub_http' is configured")
            return True
        else:
            print("⚠️ MCP server '4genthub_http' not found in configured servers")
