...
"""
MCP Bridge Script
Bridges Claude Desktop (stdio) to 4genthub HTTP Server
...
class MCPBridge:
    ...
    async def run(self):
        ...
        if not self.test_connection():
            ...
            logger.error("Run: docker ps | grep 4genthub-server")
            logger.error("If not running: cd /path/to/4genthub_main && docker-compose up -d")
            sys.exit(1)
