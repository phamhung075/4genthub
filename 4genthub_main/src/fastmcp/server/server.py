...
try:
    ...
    import fastmcp.server.context as context
...
try:
    ...
    from ..utilities.components import FastMCPComponent
...
# DDD tools will be imported lazily to avoid circular imports
...
class FastMCP(Generic[LifespanResultT]):
    def __init__(
        ...
        *,
        ...
        self.resource_prefix_format: Literal["protocol", "path"] = (
            resource_prefix_format or _settings.resource_prefix_format
        )
        ...
        if enable_task_management:
            try:
                ...
                import os
                disable_cursor_tools = os.environ.get("4GENTHUB_DISABLE_CURSOR_TOOLS", "false").lower() == "true"
                
                # Create configuration that respects environment variables
                config_overrides = {}
                if disable_cursor_tools:
                    logger.info("4GENTHUB_DISABLE_CURSOR_TOOLS=true - disabling cursor tools")
                    config_overrides = {
                        "enabled_tools": {
                            "manage_project": True,
                            "manage_task": True, 
        ...
        self.dependencies = dependencies or _settings.server_dependencies
    ...
    def _handle_deprecated_settings(
        ...
        for name, arg in [
            ...
            if arg is not None:
                # Deprecated in 2.8.0
                if _settings.deprecation_warnings:
                    warnings.warn(
        ...
        combined_settings = _settings.model_dump() | deprecated_settings
    ...
    def settings(self) -> Settings:
        ...
        if _settings.deprecation_warnings:
            warnings.warn(
                "Accessing `.settings` on a FastMCP instance is deprecated. Use the global `_settings` instead.",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    async def get_tools(self) -> list[Tool]:
        """Get all registered tools as a list for easier iteration in tests."""
        tools_dict = await self._tool_manager.get_tools()
        # Return list preserving original registration order
    ...
    async def _mcp_list_tools(self) -> list[MCPTool]:
        ...
        with context.Context(fastmcp=self):
    ...
    async def _call_tool(self, key: str, arguments: dict[str, Any]) -> list[MCPContent]:
        ...
        mw_context = MiddlewareContext(
            ...
            type="request",
            method="tools/call",
            fastmcp_context=dependencies.get_context(),
        )
        return await self._apply_middleware(mw_context, _handler)
...
def add_resource_prefix(
    ...
    if prefix_format is None:
        prefix_format = _settings.resource_prefix_format
