...
class CallAgentUseCase:
    ...
    def _find_project_root(self) -> Path:
        ...
        while current_path.parent != current_path:
            if (current_path / "agent-library").exists():
                return current_path
            if (current_path / "4genthub_main").exists():
                return current_path / "4genthub_main"
            current_path = current_path.parent
    ...
    def execute(self, name_agent: str, format: str = "default") -> Dict[str, Any]:
        ...
        if executable_agent:
            ...
            frontmatter = {
                ...
                "description": agent_info.get('description', 'Agent from 4genthub agent-library')
            }
    ...
    def _convert_to_claude_format(self, executable_agent: ExecutableAgent) -> str:
        """Convert 4genthub agent-library structure to Claude Code .claude/agents format"""
...
if "AGENT_LIBRARY_DIR_PATH" in os.environ:
    ...
    # find_project_root() returns the 4genthub_main directory
    project_root = _find_project_root()
    if project_root.name == "4genthub_main":
        AGENT_LIBRARY_DIR = project_root / "agent-library"
    else:
        AGENT_LIBRARY_DIR = project_root / "4genthub_main/agent-library"
