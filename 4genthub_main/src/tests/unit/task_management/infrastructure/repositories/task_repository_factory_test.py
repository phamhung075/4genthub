...
class TestFindProjectRoot:
    ...
    def test_find_project_root_from_file_location(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            4genthub_dir = temp_path / "4genthub_main"
            4genthub_dir.mkdir()
            
            # Mock __file__ to be in the temp directory structure
            mock_file_path = 4genthub_dir / "src" / "test_file.py"
    ...
    def test_find_project_root_from_4genthub_main_name(self):
        """Test finding project root when current path is 4genthub_main."""
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            4genthub_dir = temp_path / "4genthub_main"
            4genthub_dir.mkdir()
    ...
    @patch.dict(os.environ, {'4GENTHUB_DATA_PATH': '/custom/data'})
    def test_find_project_root_environment_variable(self):
        ...
        with patch('os.path.exists', side_effect=mock_exists):
            with patch('pathlib.Path.cwd', return_value=Path("/tmp")):  # No 4genthub_main here
                with patch('fastmcp.task_management.infrastructure.repositories.task_repository_factory.__file__', '/non/existent/path'):
                    result = _find_project_root()
    ...
    def test_find_project_root_absolute_fallback(self):
        ...
        with patch('os.path.exists', return_value=False):
            with patch('pathlib.Path.cwd', return_value=Path("/tmp")):
                with patch('fastmcp.task_management.infrastructure.repositories.task_repository_factory.__file__', '/non/existent/path'):
                    result = _find_project_root()
                    assert result == Path("/tmp/4genthub_project")
