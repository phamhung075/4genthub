"""
Global pytest configuration for 4genthub test suite
...
os.environ['KEYCLOAK_REALM'] = '4genthub'
...
def set_mcp_db_path_for_tests(request):
    ...
    try:
        ...
        os.environ["MCP_DB_PATH"] = "/tmp/4genthub_test.db"
        ...
        if os.path.exists("/tmp/4genthub_test.db"):
            try:
                os.remove("/tmp/4genthub_test.db")
            except:
                pass
...
def shared_test_db():
    ...
    shared_db_path = Path(__file__).parent.parent / "database" / "data" / "4genthub_shared_test.db"
...
def module_test_db():
    ...
    module_db_path = Path(__file__).parent.parent / "database" / "data" / f"4genthub_module_test_{os.getpid()}.db"
