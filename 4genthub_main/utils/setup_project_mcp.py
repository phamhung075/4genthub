...
This script sets up the 4genthub task management system for any project location.
...
This project uses the 4genthub task management system.
...
def create_mcp_config(project_path: Path, 4genthub_path: Optional[Path] = None) -> bool:
    """
    Create MCP configuration file for the project.
    
    Args:
        project_path: Path to the project root
        4genthub_path: Path to 4genthub_main directory (optional)
    ...
    try:
        # Auto-detect 4genthub_main if not provided
        if 4genthub_path is None:
            # Look for 4genthub_main in the project or parent directories
            search_paths = [
                project_path / "4genthub_main",
                project_path.parent / "4genthub_main",
                Path.cwd() / "4genthub_main"
            ]
            ...
            if 4genthub_path is None:
                print("❌ Could not find 4genthub_main directory")
                print("   Please ensure 4genthub_main is in the project or parent directory")
                return False
        
        print(f"Using 4genthub_main at: {4genthub_path}")
        
        # Check if virtual environment exists
        venv_python = 4genthub_path / ".venv" / "bin" / "python"
        if not venv_python.exists():
            print(f"❌ Virtual environment not found at: {venv_python}")
            print("   Please run: cd 4genthub_main && python -m venv .venv && source .venv/bin/activate && pip install -e .")
            return False
        ...
        mcp_config = {
            "mcpServers": {
                "4genthub": {
                    ...
                    "env": {
                        "PYTHONPATH": str(4genthub_path / "src"),
                        "PROJECT_ROOT_PATH": ".",
                        ...
                        "PROJECTS_FILE_PATH": ".cursor/rules/brain/projects.json",
                                    "AGENT_LIBRARY_DIR_PATH": str(4genthub_path / "agent-library")
                    },
                    "transport": "stdio",
                    "debug": True
                },
