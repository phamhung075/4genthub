...
- **Vite HMR in WSL2 Development Mode** (2025-10-16): Fixed Hot Module Replacement not working in development mode (docker-menu.sh options D/R). Root cause: HMR WebSocket host configured as 'localhost' while server bound to '0.0.0.0', causing connection failures in WSL2. Solution: Updated vite.config.ts to set hmr.host to '0.0.0.0' matching server.host (line 56), and increased stabilityThreshold from 100ms to 500ms (line 65) to prevent multiple rapid file change triggers. HMR now works properly with automatic browser updates on code changes. Files: agenthub-frontend/vite.config.ts:54-67
