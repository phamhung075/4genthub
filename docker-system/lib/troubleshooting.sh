...
diagnose_command() {
    ...
    if [[ "${DOCKER_CLI_TEST_MODE:-}" == "true" ]]; then
        ...
        echo "  agenthub-network is operational"
        echo ""
    ...
    if docker network ls | grep -q "agenthub-network"; then
        success "Network: Exists"
        local containers=$(docker network inspect agenthub-network -f '{{len .Containers}}' 2>/dev/null || echo "0")
    ...
    for service in postgres redis backend frontend; do
        local container="agenthub-$service"
    ...
    if docker ps --format '{{.Names}}' | grep -q "^agenthub-postgres$"; then
        if db_operation test_connection &>/dev/null; then
            ...
            docker logs agenthub-postgres --tail 10 2>&1 | sed 's/^/    /'
        fi
    else
...
fix_permissions_command() {
    ...
    if [[ -d "${PROJECT_ROOT}/agenthub_main/data" ]]; then
        echo "  Setting data directory permissions..."
        chmod -R 755 "${PROJECT_ROOT}/agenthub_main/data"
    fi
...
emergency_backup_command() {
    ...
    if docker ps --format '{{.Names}}' | grep -q "^agenthub-postgres$"; then
        ...
        docker exec agenthub-postgres pg_dumpall -U postgres > "$backup_dir/database/emergency-dump.sql" 2>/dev/null || \
...
support_bundle_command() {
    ...
    echo "  Collecting service status..."
    {
        ...
        for container in $(docker ps -a --format '{{.Names}}' | grep '^agenthub-'); do
            echo "--- $container ---"
    ...
    for service in postgres redis backend frontend; do
        docker logs "agenthub-$service" --tail 1000 > "$bundle_dir/logs/${service}.log" 2>&1
    done
