...
**Why**: Dev phase = clean breaks allowed. No production data. Clean slate > accommodation.

**When Tests Fail**: Fix code cleanly → Update tests. **Clean code > Passing tests**
...
**Without it**: No authorization | Just a visitor
**Response**: Read `system_prompt` field (YOUR complete instructions)
...
# ❌ Create without checking - might duplicate!
```
...
**Required Actions**:
1. READ `system_prompt` field
2. FOLLOW all rules
3. USE ONLY tools in `tools` array
...
**Best Practices**:
1. Call `call_agent` first
2. Check `tools` array in response
3. Switch agents when need unavailable tools
...
**Rules**:
1. List subtasks before complete
2. NEVER complete if subtasks pending
3. VERIFY all done/completed
4. UPDATE after verification
5. DOCUMENT verification in summary
...
```bash
# Opens terminal + WAITS + RETURNS JSON
...
**Agent Switching**: Token efficiency (70% savings) | Sequential only | Simple workflows | Production automation | Context preservation
...
**Changelog**:
```markdown
❌ We added new feature that allows users to export data
Added support for multiple formats including CSV and JSON
...
| Content | Techniques | Example |
|---------|-----------|---------|
| Comparisons | Tables, Bullets | Feature comparison |
| Workflows | Numbered steps, "Why" | Setup guides |
| Code docs | One example, Patterns | API docs |
| Errors | Concrete examples | Troubleshooting |
| Reference | Quick-lists, Scannable | Agent directory |
| Architecture | Inverted pyramid, Tables | System design |

### Checklist

- [ ] Tables for comparisons?
- [ ] Redundant examples → patterns?
- [ ] Removed decorative elements?
- [ ] "Why" explanations for critical decisions?
- [ ] Numbered steps not ASCII?
- [ ] One perfect example not multiple?
- [ ] Inverted pyramid structure?
- [ ] Scannable (bullets, bold, tables)?
