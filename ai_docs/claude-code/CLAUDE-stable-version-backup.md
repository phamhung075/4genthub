...
# NEW - Subtask Workflow
# 1. Create subtask for granular work
subtask = manage_subtask(action="create", task_id=task.id, title="Implement token validation", progress_notes="Starting validation logic")

# 2. Delegate subtask to separate terminal
bash: cclaude coding-agent "subtask_id: {subtask.id}, task_id: {task.id}"

# 3. Agent works on subtask specifically
# 4. Auto-updates subtask progress (not parent task)
# 5. Parent task progress recalculated automatically
```
