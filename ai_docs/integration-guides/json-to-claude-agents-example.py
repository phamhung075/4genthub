#!/usr/bin/env python3
"""
Example: Convert DhafnckMCP agents to .claude/agents files using the new JSON format
"""
...
def create_agent_from_dhafnck_mcp(agent_name: str, output_dir: str = ".claude/agents") -> str:
    """
    Complete workflow: Get agent from DhafnckMCP and create .claude/agents file
    ...
    # In real usage, this would be: mcp__dhafnck_mcp_http__call_agent(name_agent=f"@{agent_name}")
...
def batch_create_agents(agent_names: list, output_dir: str = ".claude/agents") -> Dict[str, str]:
    """
    Create multiple .claude/agents files from DhafnckMCP agents
    ...
    for name in agent_names:
        try:
            filepath = create_agent_from_dhafnck_mcp(name, output_dir)
...
def main():
    ...
    print("=== DhafnckMCP to Claude Code Agent Conversion ===\n")
    
    # Example 1: Single agent conversion
    print("1. Creating single agent...")
    try:
        filepath = create_agent_from_dhafnck_mcp("test-orchestrator-agent", "/tmp/claude-agents-demo")
