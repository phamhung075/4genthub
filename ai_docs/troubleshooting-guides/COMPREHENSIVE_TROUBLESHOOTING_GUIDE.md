...
This guide covers common issues, known problems, and systematic troubleshooting approaches for the agenthub system.
...
mcp__agenthub_http__manage_context(action="get", level="task", context_id="your-task-id", include_inherited=True)
...
mcp__agenthub_http__manage_agent(action="list", project_id="your-project-id")
```
...
docker ps
docker logs agenthub-container
```
...
docker run -v /path/to/data:/data agenthub

# Check database file exists
docker exec -it container-name ls -la /data/agenthub.db
```

#### Local Development Mode
```bash
# Ensure Docker database is accessible
# Database MUST be at /data/agenthub.db
...
#### MCP STDIN Mode
```bash
# Uses local database (agenthub_local.db)
...
health = mcp__agenthub_http__manage_connection(action="health_check")

# 2. Verify database connectivity
projects = mcp__agenthub_http__manage_project(action="list")

# 3. Check agent system
agent = mcp__agenthub_http__call_agent(name_agent="master-orchestrator-agent")
```

### Step 2: Context Validation
```bash
# 1. Get resolved context with inheritance
resolved_context = mcp__agenthub_http__manage_context(
    action="resolve",
    level="task",
    context_id="your-task-id"
)
...
cache_status = mcp__agenthub_http__manage_context(
    action="get_health"
)
...
project_contexts = mcp__agenthub_http__manage_context(
    action="list",
    level="project"
)
```
...
```bash
# Diagnosis:
context_check = mcp__agenthub_http__manage_context(
    action="get",
    level="task",
...
cp /data/agenthub.db /data/agenthub.db.backup

# 3. Clear cache
rm -rf /tmp/agenthub_cache/*
...
cp /data/agenthub.db.backup /data/agenthub.db
...
task_result = mcp__agenthub_http__manage_task(
    action="create",
...
This comprehensive guide should help diagnose and resolve most issues encountered in the agenthub system. For persistent issues, follow the systematic troubleshooting approach and use the emergency recovery procedures when necessary.
