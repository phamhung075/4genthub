...
class TestRoleEnforcer:
    ...
    def setup_method(self):
        ...
        self.sample_config = {
            ...
            "default_role": {
                "name": "uninitialized",
                "allowed_tools": ["mcp__dhafnck_mcp_http__call_agent", "Read", "Grep", "Glob"],
                "blocked_tools": "*",
                "warnings": {
                    "write_attempt": "[NO ROLE] Must call mcp__dhafnck_mcp_http__call_agent first!",
            ...
            "roles": {
                "master-orchestrator-agent": {
                    ...
                    "allowed_tools": ["Task", "Read", "mcp__dhafnck_mcp_http__manage_task", "TodoWrite"],
    ...
    def test_get_default_role_fallback(self):
        ...
        with patch.object(RoleEnforcer, '_load_config') as mock_load:
            ...
            assert "mcp__dhafnck_mcp_http__call_agent" in default_role["allowed_tools"]
    ...
    def test_check_tool_permission_uninitialized_agent(self, mock_get_current_agent):
        ...
        with patch.object(RoleEnforcer, '_load_config') as mock_load:
            ...
            assert allowed is False
            assert "Must call mcp__dhafnck_mcp_http__call_agent first" in message
...
class TestRoleEnforcerIntegration:
    ...
    def setup_method(self):
        ...
        self.integrated_config = {
            ...
            "default_role": {
                "name": "uninitialized",
                "allowed_tools": ["mcp__dhafnck_mcp_http__call_agent"],
                "blocked_tools": "*"
            },
            "roles": {
                "master-orchestrator-agent": {
