...
class TestAssigneesValidationFix:
    ...
    def test_single_agent_validation(self):
        ...
        # Test with @ prefix
        assert self.validator._is_valid_assignees_list(["@@coding_agent"])
    ...
    def test_multiple_agents_validation(self):
        ...
        assignees = ["@@coding_agent", "@security-auditor-agent", "@test-orchestrator-agent"]
        assert self.validator._is_valid_assignees_list(assignees)
        
        # Mixed with/without @ prefix
        mixed_assignees = ["@@coding_agent", "security-auditor-agent", "@test-orchestrator-agent"]
    ...
    def test_invalid_assignees_validation(self):
        ...
        # Not a list
        assert not self.validator._is_valid_assignees_list("@@coding_agent")
...
def test_string_to_list_conversion_logic():
    ...
    # Single agent
    assert convert_assignees("@@coding_agent") == ["@@coding_agent"]
    assert convert_assignees("user123") == ["user123"]
    
    # Comma-separated agents
    assert convert_assignees("@@coding_agent,@security-auditor-agent") == ["@@coding_agent", "@security-auditor-agent"]
    assert convert_assignees("@@coding_agent, @security-auditor-agent") == ["@@coding_agent", "@security-auditor-agent"]
