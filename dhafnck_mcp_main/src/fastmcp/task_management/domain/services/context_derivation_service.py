...
class ContextDerivationService:
    ...
    async def derive_context_from_git_branch(
        ...
        try:
            if self._git_branch_repository:
                ...
                git_branch = await self._git_branch_repository.find_by_id(git_branch_id)
    ...
    async def derive_context_hierarchy(
        ...
        # Derive task context
        if task_id:
            task_context = await self.derive_context_from_task(task_id, user_id)
    ...
    def _get_default_context(self, default_user_id: Optional[str] = None) -> Dict[str, str]:
        ...
        Business Rules:
        - Use default project when context derivation fails
