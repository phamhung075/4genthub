...
===============================================================
This script configures the DhafnckMCP system to use:
...
class SetupManager:
    def __init__(self):
        ...
        self.src_dir = self.root_dir / "dhafnck_mcp_main" / "src"
...
DATABASE_URL=postgresql://postgres:postgres_secure_password_2025@localhost:5432/dhafnck_mcp

# PostgreSQL Docker Configuration
POSTGRES_CONTAINER_NAME=dhafnck-postgres
POSTGRES_DATA_VOLUME=dhafnck_postgres_data
POSTGRES_BACKUP_VOLUME=dhafnck_postgres_backup
...
KEYCLOAK_TOKEN_ISSUER=https://your-keycloak-domain.com/realms/dhafnck-mcp
...
# DOCKER
# =============================================================================
COMPOSE_PROJECT_NAME=dhafnck-mcp-clean
DOCKER_NETWORK=dhafnck_clean_network
...
services:
  ...
  postgres:
    ...
    container_name: ${POSTGRES_CONTAINER_NAME:-dhafnck-postgres}
    environment:
    ...
    networks:
      - dhafnck_network
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${DATABASE_USER:-postgres} -d ${DATABASE_NAME:-dhafnck_mcp}"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  pgadmin:
    ...
    environment:
      PGADMIN_DEFAULT_EMAIL: ${PGADMIN_EMAIL:-admin@dhafnck.com}
  ...
  backend:
    ...
    environment:
      ...
      DATABASE_PORT: 5432
      DATABASE_NAME: ${DATABASE_NAME:-dhafnck_mcp}
      DATABASE_USER: ${DATABASE_USER:-postgres}
      DATABASE_PASSWORD: ${DATABASE_PASSWORD:-postgres_secure_password_2025}
      DATABASE_URL: postgresql://${DATABASE_USER:-postgres}:${DATABASE_PASSWORD:-postgres_secure_password_2025}@postgres:5432/${DATABASE_NAME:-dhafnck_mcp}
...
networks:
  dhafnck_network:
    driver: bridge
    name: ${DOCKER_NETWORK:-dhafnck_clean_network}

volumes:
  postgres_data:
    name: ${POSTGRES_DATA_VOLUME:-dhafnck_postgres_data}
  postgres_backup:
    name: ${POSTGRES_BACKUP_VOLUME:-dhafnck_postgres_backup}
  pgadmin_data:
    name: dhafnck_pgadmin_data
"""
...
echo "=========================================="
echo " Starting DhafnckMCP Clean Setup"
echo "=========================================="
...
docker-compose -f docker-compose.clean.yml exec postgres psql -U postgres -c "CREATE DATABASE dhafnck_mcp;" 2>/dev/null || echo "Database already exists"
...
"""
    ...
    def run(self):
        ...
        self.print_header("DhafnckMCP Clean Setup - PostgreSQL + Keycloak")
