#!/usr/bin/env python3
"""
Configure and validate PostgreSQL Docker + Keycloak Cloud setup for DhafnckMCP.
...
class PostgresKeycloakConfigurator:
    ...
    def __init__(self):
        ...
        self.db_name = os.getenv("DATABASE_NAME", "dhafnck_mcp_prod")
        self.db_user = os.getenv("DATABASE_USER", "dhafnck_user")
    ...
    def validate_postgresql_connection(self) -> bool:
        ...
        try:
            ...
            logger.info("3. Check container logs: docker logs dhafnck-postgres")
    ...
    async def test_mcp_authentication(self) -> bool:
        ...
        try:
            ...
            logger.info("   or")
            logger.info("   python dhafnck_mcp_main/src/mcp_http_server.py")
            return False
        except Exception as e:
