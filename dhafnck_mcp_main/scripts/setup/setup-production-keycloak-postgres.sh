...
# ============================================================================
# This script sets up DhafnckMCP with:
...
echo -e "${BLUE}============================================================================${NC}"
echo -e "${BLUE}DhafnckMCP Production Setup - PostgreSQL Docker + Keycloak Cloud${NC}"
echo -e "${BLUE}============================================================================${NC}"
...
cat > config/pgadmin/servers.json << 'EOF'
{
    "Servers": {
        "1": {
            ...
            "SSLMode": "prefer",
            "Comment": "DhafnckMCP Production Database"
        }
    }
}
EOF
...
for i in {1..30}; do
    if docker-compose -f "$DOCKER_COMPOSE_FILE" exec postgres pg_isready -U dhafnck_user -d dhafnck_mcp_prod &>/dev/null; then
...
from dhafnck_mcp_main.src.fastmcp.task_management.infrastructure.database.database_initializer import DatabaseInitializer
from dhafnck_mcp_main.src.fastmcp.task_management.infrastructure.database.database_config import DatabaseConfig
...
if [ "$start_pgadmin" = "y" ]; then
    ...
    echo -e "                Email: admin@dhafnck.com"
...
echo -e "  DB shell:     ${BLUE}docker-compose -f $DOCKER_COMPOSE_FILE exec postgres psql -U dhafnck_user -d dhafnck_mcp_prod${NC}"
