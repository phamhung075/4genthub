...
read -p "Keycloak Realm [dhafnck-mcp]: " KEYCLOAK_REALM
...
while [ $attempt -lt $max_attempts ]; do
    if docker exec dhafnck-postgres pg_isready -U postgres >/dev/null 2>&1; then
...
pip install -q -r dhafnck_mcp_main/requirements.txt
...
nohup python dhafnck_mcp_main/src/mcp_http_server.py > logs/mcp-server.log 2>&1 &
MCP_PID=$!
...
echo ""
echo -e "${GREEN}Your DhafnckMCP system is ready to use!${NC}"
echo ""
