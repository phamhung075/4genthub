...
if [ -f .env.production-keycloak-postgres ]; then
    ...
    read -p "Keycloak Realm (default: dhafnck-mcp): " KEYCLOAK_REALM
    ...
    sed -i "s|dhafnck-mcp|${KEYCLOAK_REALM}|g" .env.production
    ...
    sed -i "s|https://your-keycloak-domain.com/realms/dhafnck-mcp|${KEYCLOAK_URL}/realms/${KEYCLOAK_REALM}|g" .env.production
...
def main():
    src_dir = Path("dhafnck_mcp_main/src")
    cleaned_files = []
...
if [[ "$REMOVE_DATA" =~ ^[Yy]$ ]]; then
    docker volume rm dhafnck_postgres_prod_data 2>/dev/null || true
    docker volume rm dhafnck_postgres_prod_backup 2>/dev/null || true
    print_message "$YELLOW" "📦 Removed existing PostgreSQL volumes"
fi

# Create network if it doesn't exist
docker network create dhafnck_prod_network 2>/dev/null || true
...
for i in {1..30}; do
    if docker-compose -f docker-compose.production.yml exec -T postgres pg_isready -U postgres -d dhafnck_mcp >/dev/null 2>&1; then
...
# Execute SQL
docker-compose -f docker-compose.production.yml exec -T postgres psql -U postgres -d dhafnck_mcp < init_db.sql
rm init_db.sql
...
docker-compose -f docker-compose.production.yml exec -T postgres psql -U postgres -d dhafnck_mcp -c "SELECT version();" >/dev/null 2>&1
if [ $? -eq 0 ]; then
