...
class AuthenticationSetup:
            ...
            for _ in range(30):
                result = subprocess.run(
                    ["docker-compose", "exec", "-T", "postgres", 
                     "pg_isready", "-U", os.getenv("DATABASE_USER", "dhafnck_user")],
                    cwd=self.project_root,
                    capture_output=True,
                    check=False
                )
    ...
    def initialize_database(self) -> bool:
        ...
        try:
            ...
            init_script = self.project_root / "dhafnck_mcp_main" / "src" / "fastmcp" / "task_management" / "infrastructure" / "database" / "init_database.py"
    ...
    def cleanup_backward_compatibility(self) -> None:
        ...
        files_to_remove = [
            "dhafnck_mcp_main/src/fastmcp/auth/supabase_service.py",
            "dhafnck_mcp_main/src/fastmcp/auth/supabase_auth.py",
            "dhafnck_mcp_main/scripts/setup_supabase.py",
            "dhafnck_mcp_main/scripts/migrate_to_supabase.py"
        ]
    ...
    def generate_mcp_config(self, keycloak_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "database": {
                ...
                "host": "localhost",
                "port": 5432,
                "name": os.getenv("DATABASE_NAME", "dhafnck_mcp_prod"),
                "user": os.getenv("DATABASE_USER", "dhafnck_user")
            },
            "server": {
                "host": "0.0.0.0",
                "port": 8001,
    ...
    def save_mcp_config(self, config: Dict[str, Any]) -> None:
        ...
        config_file = self.project_root / "dhafnck_mcp_main" / "config" / "mcp_config.json"
