#!/usr/bin/env python3
"""
Secure Token Generator for DhafnckMCP
...
def main():
    print("🔐 DhafnckMCP Secure Token Generator")
    print("=" * 50)
    print(f"Generated on: {datetime.now().isoformat()}")
    print()
    
    # Generate new tokens
    new_dhafnck_token = generate_mcp_token()
    new_api_token = generate_api_token()
    new_jwt_secret = generate_jwt_secret()
    
    print("🔑 GENERATED SECURE TOKENS:")
    print("-" * 30)
    print(f"DHAFNCK_TOKEN={new_dhafnck_token}")
    print(f"DHAFNCK_API_TOKEN={new_api_token}")  
    ...
    with open(env_file, 'w') as f:
        ...
        f.write(f"DHAFNCK_TOKEN={new_dhafnck_token}\n")
        f.write(f"DHAFNCK_API_TOKEN={new_api_token}\n")
