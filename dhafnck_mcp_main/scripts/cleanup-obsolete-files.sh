...
show_header() {
    echo -e "${BLUE}"
    echo "╔══════════════════════════════════════════════════════════════════════════════╗"
    echo "║                    DhafnckMCP Project Cleanup Script                         ║"
    echo "║                                                                              ║"
...
create_backup() {
    ...
    cp "$PROJECT_ROOT/dhafnck_mcp_main/logs/dhafnck_mcp.log" "$BACKUP_DIR/" 2>/dev/null || log "Note: dhafnck_mcp.log not found"
    cp "$PROJECT_ROOT/dhafnck_mcp_main/logs/dhafnck_mcp_errors.log" "$BACKUP_DIR/" 2>/dev/null || log "Note: dhafnck_mcp_errors.log not found"
...
safety_checks() {
    ...
    if [ ! -f "$PROJECT_ROOT/CLAUDE.md" ]; then
        log "${RED}Error: Not in DhafnckMCP project root${NC}"
        exit 1
    fi
    ...
    # Check Docker status
    if docker ps 2>/dev/null | grep -q dhafnck; then
        log "${YELLOW}Warning: DhafnckMCP Docker containers are running${NC}"
        docker ps --filter name=dhafnck
...
cleanup_logs() {
    ...
    for i in {2..9}; do
        local log_file="$PROJECT_ROOT/dhafnck_mcp_main/logs/backend.log.$i"
        if [ -f "$log_file" ]; then
...
show_summary() {
    ...
    local logs_size=$(du -sh "$PROJECT_ROOT/dhafnck_mcp_main/logs/" 2>/dev/null | cut -f1 || echo "N/A")
...
rollback() {
    ...
    if [ ! -d "$BACKUP_DIR" ]; then
        ...
        ls -la /tmp/dhafnck-cleanup-backup-* 2>/dev/null || echo "No backups found"
        exit 1
    fi
    
    # Restore critical logs
    if [ -f "$BACKUP_DIR/dhafnck_mcp.log" ]; then
        cp "$BACKUP_DIR/dhafnck_mcp.log" "$PROJECT_ROOT/dhafnck_mcp_main/logs/"
        log "Restored dhafnck_mcp.log"
    fi
    
    if [ -f "$BACKUP_DIR/dhafnck_mcp_errors.log" ]; then
        cp "$BACKUP_DIR/dhafnck_mcp_errors.log" "$PROJECT_ROOT/dhafnck_mcp_main/logs/"
        log "Restored dhafnck_mcp_errors.log"
    fi
...
verify_cleanup() {
    ...
    if [ -f "$PROJECT_ROOT/CLAUDE.md" ] && [ -f "$PROJECT_ROOT/dhafnck_mcp_main/src/fastmcp/__init__.py" ]; then
...
dry_run() {
    ...
    find "$PROJECT_ROOT/dhafnck_mcp_main/logs/" -name "backend.log.[2-9]" 2>/dev/null | while read -r file; do
...
case "${1:-}" in
    ...
    --rollback)
        ...
        if [ -n "$backup_path" ]; then
            ...
            BACKUP_DIR=$(ls -td /tmp/dhafnck-cleanup-backup-* 2>/dev/null | head -1)
        fi
        rollback
        exit 0
        ;;
