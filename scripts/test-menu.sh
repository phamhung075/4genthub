    ...
    while true; do
        ...
        case $choice in
            ...
            20)
                ...
                python -m pytest "${TEST_DIR}" --cov=dhafnck_mcp_main/src --cov-report=html:${COVERAGE_DIR} --cov-report=term --cov-report=json:${COVERAGE_DIR}/coverage.json

                # Save text report too
                python -m pytest "${TEST_DIR}" --cov=dhafnck_mcp_main/src --cov-report=term > "${COVERAGE_DIR}/coverage.txt" 2>&1

                echo -e "${GREEN}✓ Coverage report generated!${NC}"
                echo -e "${CYAN}  HTML Report: ${COVERAGE_DIR}/index.html${NC}"
                echo -e "${CYAN}  JSON Report: ${COVERAGE_DIR}/coverage.json${NC}"
                echo -e "${CYAN}  Text Report: ${COVERAGE_DIR}/coverage.txt${NC}"
                ;;
            21)
                find_obsolete_tests
                ;;
