...
def fix_boolean_comparisons(content: str) -> str:
    ...
    content = re.sub(
        r"while\s+(\w+[\.\w]*)\s*==\s*False\s*:", r"while not \1:", content
    )
...
def main():
    ...
    parser.add_argument(
        "--directory", type=Path, default=Path("src"), help="Directory to process"
    )
    ...
    for py_file in args.directory.rglob("*.py"):
        try:
            ...
            if content != original:
                ...
                files_modified += 1
                print(
                    f"{'[DRY RUN] Would modify' if args.dry_run else 'Modified'}: {py_file.relative_to(args.directory.parent)}"
                )
