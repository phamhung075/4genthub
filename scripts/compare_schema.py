...
class ColumnDifference:
    ...
    column_name: str
    difference_type: str  # 'missing_in_db', 'missing_in_orm', 'type_mismatch', 'nullable_mismatch'
    db_value: Any = None
    orm_value: Any = None
...
def compare_columns(db_columns: dict, orm_table: Table) -> list[ColumnDifference]:
    ...
    for col_name, orm_col in orm_columns.items():
        if col_name not in db_columns:
            differences.append(ColumnDifference(
...
def generate_migration_sql(differences: list[TableDifference]) -> str:
    ...
    sql_statements.append(f"-- Date: {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def main():
    ...
    for table_name, orm_table in sorted(orm_tables.items()):
        try:
            ...
            if differences:
                all_differences.append(TableDifference(
                ...
                for diff in differences:
                    if diff.difference_type == 'missing_in_db':
                        print(f"   ❌ Column missing in DB: {diff.column_name} ({diff.orm_value})")
                    elif diff.difference_type == 'missing_in_orm':
                        print(f"   ⚠️  Column in DB but not ORM: {diff.column_name} ({diff.db_value})")
