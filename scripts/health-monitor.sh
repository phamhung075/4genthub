#!/bin/bash
# DhafnckMCP Health Monitoring System
...
check_frontend_health() {
    ...
    if response=$(curl -s -f -m "$TIMEOUT" "$url" 2>&1); then
        ...
        if echo "$response" | grep -q "DhafnckMCP" || echo "$response" | grep -q "vite" || echo "$response" | grep -q "<!DOCTYPE html>"; then
...
check_database_health() {
    ...
    if docker exec "$DB_CONTAINER" pg_isready -U "${DATABASE_USER:-dhafnck_user}" -d "${DATABASE_NAME:-dhafnck_mcp}" > /dev/null 2>&1; then
        # Get additional database metrics
        if connection_count=$(docker exec "$DB_CONTAINER" psql -U "${DATABASE_USER:-dhafnck_user}" -d "${DATABASE_NAME:-dhafnck_mcp}" -t -c "SELECT count(*) FROM pg_stat_activity;" 2>/dev/null | xargs); then
...
show_status() {
    ...
    echo -e "\n${BOLD}${BLUE}=== DhafnckMCP System Status ===${RESET}"
    echo -e "${BLUE}================================${RESET}\n"
