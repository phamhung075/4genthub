...
def test_connection():
    ...
    try:
        ...
        print(
            f"  DATABASE_PASSWORD: {'SET' if os.getenv('DATABASE_PASSWORD') else 'NOT SET'}"
        )
        ...
        if not os.getenv("DATABASE_HOST"):
            ...
            print(
                "Please ensure .env or .env.dev file exists and contains database configuration"
            )
            return False
        ...
        print(
            ...
            if db.database_url
            else "❌ Database URL configured: No"
        )
