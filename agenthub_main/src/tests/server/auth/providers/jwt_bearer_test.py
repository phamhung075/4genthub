...
class TestJWTBearerAuthProvider:
    ...
    def test_init_with_env_secret(self, secret_key):
        ...
        with patch.dict(os.environ, {"JWT_SECRET_KEY": secret_key}):
            ...
            assert provider.secret_key == secret_key
            assert provider.issuer == "agenthub"
    ...
    @pytest.mark.asyncio
    async def test_verify_token_delegates_to_backend(
        self, secret_key, mock_jwt_backend
    ):
        """Test verify_token delegates to JWT backend"""
        with patch(
            "fastmcp.auth.mcp_integration.jwt_auth_backend.create_jwt_auth_backend",
            return_value=mock_jwt_backend,
        ):
    ...
    @pytest.mark.asyncio
    async def test_load_access_token_delegates_to_verify(
        self, secret_key, mock_jwt_backend
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_user_token_with_user_id_fallback(
    ...
    @pytest.mark.asyncio
    async def test_validate_token_in_database_valid(
        self, mock_db_session, mock_api_token
    ):
        """Test validation of valid token in database"""
        # Mock both get_db and APIToken import
        with (
            patch("fastmcp.server.auth.providers.jwt_bearer.get_db") as mock_get_db,
            patch(
                "fastmcp.task_management.infrastructure.database.models.APIToken"
            ) as MockAPIToken,
        ):
            ...
            with patch.dict(
                "sys.modules",
                {"fastmcp.server.routes.token_router": Mock(APIToken=MockAPIToken)},
            ):
    ...
    async def test_validate_token_in_database_not_found(self, mock_db_session):
        ...
        with (
            ...
            with patch.dict(
                ...
                provider = JWTBearerAuthProvider(secret_key="test-key")
                result = await provider._validate_token_in_database(
                    "non-existent-token"
                )
    ...
    @pytest.mark.asyncio
    async def test_validate_token_in_database_inactive(
        self, mock_db_session, mock_api_token
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_in_database_expired(
        self, mock_db_session, mock_api_token
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_in_database_with_rate_limit(
        self, mock_db_session, mock_api_token
    ):
    ...
    async def test_integration_with_jwt_backend_factory(self):
        ...
        with patch(
            ...
            provider = JWTBearerAuthProvider(
                secret_key="test-key", required_scopes=["mcp:admin", "mcp:write"]
            )
    ...
    async def test_database_check_disabled(self, secret_key, mock_jwt_backend):
        ...
        with patch(
            ...
            provider = JWTBearerAuthProvider(
                secret_key=secret_key, check_database=False
            )
            ...
            expected_token = AccessToken(
                token="test-token", client_id="test-client", scopes=["mcp:read"]
            )
            mock_jwt_backend.verify_token.return_value = expected_token
