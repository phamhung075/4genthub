"""
Simplified pytest configuration for agenthub test suite
...
def test_database(request):
    ...
    try:
        ...
        initialize_database(
            None
        )  # Will use DATABASE_URL or MCP_DB_PATH from environment
...
def pytest_sessionstart(session):
    ...
    """
    print("\n🧪 Starting agenthub Test Suite (Simplified)")
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "unit: mark test as unit test (skips database setup)"
    )
