...
def isolated_test_environment(
    test_id: str,
) -> Generator[IsolatedTestEnvironmentConfig, None, None]:
    """
...
def is_test_data_file(file_path: Path) -> bool:
    ...
    return (
        file_path.name.endswith(".test.json")
        or file_path.name.endswith(".test.db")
        or "test" in file_path.name.lower()
        or "temp" in str(file_path).lower()
        or file_path.name.startswith("agenthub_test_")
        or "pytest" in str(file_path).lower()
    )
...
def cleanup_test_data_files_only(test_root: Path) -> int:
    ...
    for dir_path in test_root.rglob("*"):
        if (
            dir_path.is_dir()
            and "test" in dir_path.name.lower()
