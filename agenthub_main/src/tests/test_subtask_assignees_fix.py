...
def test_assignees_string_conversion():
    ...
    try:
        if (
            "assignees" in test_kwargs_single
            and test_kwargs_single["assignees"] is not None
            and isinstance(test_kwargs_single["assignees"], str)
        ):
            assignees = test_kwargs_single["assignees"]
            if "," in assignees:
                # Comma-separated assignees - convert to list
                test_kwargs_single["assignees"] = [
    ...
    try:
        ...
        if (
            "assignees" in test_kwargs_multi
            and test_kwargs_multi["assignees"] is not None
            and isinstance(test_kwargs_multi["assignees"], str)
        ):
            assignees = test_kwargs_multi["assignees"]
            if "," in assignees:
                # Comma-separated assignees - convert to list
                test_kwargs_multi["assignees"] = [
                    a.strip() for a in assignees.split(",") if a.strip()
                ]
            else:
                # Single assignee - convert to list
