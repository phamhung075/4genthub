...
class TestGetCurrentMCPUser:
    ...
    async def test_get_current_mcp_user_expired_token(self):
        ...
        expired_token = jwt.encode(
            expired_payload, "test-secret", algorithm=FRONTEND_JWT_ALGORITHM
        )
    ...
    async def test_get_current_mcp_user_missing_user_id(self):
        ...
        token = jwt.encode(
            payload_no_id, "test-secret", algorithm=FRONTEND_JWT_ALGORITHM
        )
    ...
    async def test_get_current_mcp_user_user_id_fallback(self):
        ...
        token = jwt.encode(
            payload_with_user_id, "test-secret", algorithm=FRONTEND_JWT_ALGORITHM
        )
    ...
    async def test_get_current_mcp_user_minimal_payload(self):
        ...
        token = jwt.encode(
            minimal_payload, "test-secret", algorithm=FRONTEND_JWT_ALGORITHM
        )
    ...
    async def test_get_current_mcp_user_with_auth_provider(self):
        ...
        for provider in providers:
            ...
            token = jwt.encode(payload, "test-secret", algorithm=FRONTEND_JWT_ALGORITHM)
    ...
    async def test_get_current_mcp_user_http_exception_passthrough(self):
        ...
        with patch("jwt.decode") as mock_decode:
            ...
            custom_exc = HTTPException(status_code=403, detail="Custom forbidden error")
...
class TestGetOptionalMCPUser:
    ...
    @pytest.mark.asyncio
    async def test_get_optional_mcp_user_with_valid_credentials(
    ...
    async def test_get_optional_mcp_user_exception_handling(self):
        ...
        with patch(
            "fastmcp.auth.mcp_dependencies.get_current_mcp_user"
        ) as mock_get_user:
...
class TestIntegration:
    ...
    async def test_authentication_with_different_algorithms(self):
        ...
        with patch("fastmcp.auth.mcp_dependencies.FRONTEND_JWT_ALGORITHM", "RS256"):
            ...
            credentials = HTTPAuthorizationCredentials(
                scheme="Bearer", credentials="fake.rs256.token"
            )
