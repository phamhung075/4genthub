...
class TestKeycloakJWKSClient:
    ...
    @patch(
        "fastmcp.auth.keycloak_dependencies.KEYCLOAK_URL",
    ...
    @patch.dict(
        os.environ,
        {"KEYCLOAK_URL": "https://keycloak.test.com", "KEYCLOAK_REALM": "test-realm"},
    )
    def test_get_keycloak_jwks_client_correct_url(self):
        ...
        with patch("jwt.PyJWKClient") as mock_jwks_class:
            ...
            mock_jwks_class.assert_called_with(
                expected_url, cache_keys=True, lifespan=3600
            )
...
class TestUniversalUserAuthentication:
        ...
        with patch.dict(os.environ, {"KEYCLOAK_URL": "https://keycloak.example.com"}):
            with patch(
                ...
                with patch("jwt.decode") as mock_decode:
                    ...
                    with patch(
                        "fastmcp.auth.keycloak_dependencies.validate_keycloak_token"
                    ) as mock_validate:
                        mock_user = User(
    ...
    async def test_get_current_user_local_token(self, mock_credentials):
        ...
        with patch("jwt.decode") as mock_decode:
            ...
            with patch(
                "fastmcp.auth.keycloak_dependencies.validate_local_token"
            ) as mock_validate:
                mock_user = User(
                    id="local-user-456",
                    email="user@local.com",
...
class TestKeycloakTokenValidation:
    ...
    async def test_validate_keycloak_token_success(self):
        ...
        with patch.dict(
            ...
            with patch(
                "fastmcp.auth.keycloak_dependencies.get_keycloak_jwks_client"
            ) as mock_get_client:
                # Mock JWKS client
                mock_client = Mock()
...
class TestIntegration:
    ...
    async def test_keycloak_to_local_fallback(self):
        ...
        credentials = HTTPAuthorizationCredentials(
            scheme="Bearer", credentials="ambiguous.token"
        )

        # First decode returns non-Keycloak issuer
        with patch("jwt.decode") as mock_decode:
            mock_decode.return_value = {"sub": "user-123", "iss": "local-issuer"}
