...
class TestSupabaseConnectionFix:
    ...
    def test_current_environment_configuration(self):
        ...
        logger.info(
            f"SUPABASE_DB_PASSWORD: {'SET' if supabase_db_password else 'NOT SET'}"
        )
    ...
    @patch(
        "fastmcp.task_management.infrastructure.database.supabase_config.SupabaseConfig._initialize_database"
    )
    def test_fix_with_database_url(self, mock_init_db):
        ...
        os.environ["DATABASE_URL"] = (
            "postgresql://postgres.PLACEHOLDER_SUPABASE_REF:PLACEHOLDER_SUPABASE_PASSWORD@aws-0-eu-north-1.pooler.supabase.com:5432/postgres?sslmode=require"
        )
    ...
    @patch(
        "fastmcp.task_management.infrastructure.database.supabase_config.SupabaseConfig._get_supabase_database_url"
    )
    ...
    def test_database_config_with_fix(self):
        ...
        with patch.dict(
            ...
            with patch.object(
                DatabaseConfig, "_get_secure_database_url", return_value=test_url
            ):
                # Mock is_supabase_configured to return True for test purposes
                with patch(
                    "fastmcp.task_management.infrastructure.database.supabase_config.is_supabase_configured",
                    return_value=True,
                ):
                    ...
                    with patch(
                        "fastmcp.task_management.infrastructure.database.supabase_config.get_supabase_config",
                        return_value=mock_supabase_config,
                    ):
                        ...
                        with patch(
                            ...
                            with patch(
                                "fastmcp.task_management.infrastructure.database.database_config.sessionmaker"
                                ...
                                with patch.object(
                                    ...
                                    logger.info(
                                        f"Database type: {db_config.database_type}"
                                    )
                                    logger.info(
                                        f"Database URL set: {'Yes' if db_config.database_url else 'No'}"
                                    )
                                    logger.info(
                                        f"Engine created: {'Yes' if db_config.engine else 'No'}"
                                    )
                                    logger.info(
                                        f"Session factory created: {'Yes' if db_config.SessionLocal else 'No'}"
                                    )
                                    ...
                                    logger.info(
                                        "DatabaseConfig correctly configured for Supabase PostgreSQL"
                                    )
