...
class DatabaseConnectionAnalyzer:
    ...
    def analyze_database_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                results["engine_type"] = (
                    str(type(db_config.engine)) if db_config.engine else None
                )
                results["session_factory"] = (
                    str(type(db_config.SessionLocal))
                    if db_config.SessionLocal
                    else None
                )
                ...
                if db_config.engine:
                    ...
                    if (
                        db_type.lower() == "supabase"
                        and "postgresql" in engine_url.lower()
                    ):
    ...
    def analyze_repository_session_usage(
        self, repo_class, repo_name: str
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if "BaseORMRepository" in base_classes:
                ...
                results["session_source"] = (
                    "BaseORMRepository.get_db_session() -> get_session() from database_config"
                )
            else:
    ...
    def analyze_all_repositories(self) -> dict[str, Any]:
        ...
        for repo_name, repo_class in repositories_to_check.items():
            results["repositories"][repo_name] = self.analyze_repository_session_usage(
                repo_class, repo_name
            )
    ...
    def generate_fix_recommendations(
        ...
        if db_config.get("database_type") == "supabase" and not db_config.get(
            "supabase_configured"
        ):
        ...
        if any(
            issue["type"] == "potential_sqlite_usage" for issue in global_context_issues
        ):
            recommendations.append(
                {
                    "priority": "HIGH",
                    ...
                    "files_to_modify": [
                        "agenthub_main/src/fastmcp/task_management/infrastructure/repositories/global_context_repository.py",
                        "agenthub_main/src/fastmcp/task_management/infrastructure/repositories/global_context_repository_user_scoped.py",
                    ],
                }
            )
...
class TestDatabaseConnectionAnalysis:
    ...
    def test_root_cause_analysis(self):
        ...
        for i, cause in enumerate(root_causes, 1):
            ...
            print(
                f"     Impact: {cause['impact']}, Affects: {cause['affected_components']}"
            )
    ...
    def test_complete_analysis_report(self):
        ...
        print(
            f"   - Critical Issues: {sum(1 for cause in root_causes if cause['impact'] == 'HIGH')}"
        )
