...
class TestRepositoryFactory:
    ...
    def test_get_environment_config_delegates_to_utils(self):
        ...
        with patch('fastmcp.task_management.infrastructure.repositories.repository_factory.get_repository_config') as mock_config:
            ...
            assert result['redis_enabled'] == False
            assert result['use_cache'] == False
            mock_config.assert_called_once()

    @patch('fastmcp.task_management.infrastructure.repositories.repository_factory.RepositoryFactory.get_environment_config')
    def test_get_task_repository_test_environment(self, mock_config):
        ...
        with patch('fastmcp.task_management.infrastructure.repositories.repository_factory.logger') as mock_logger:
    ...
    def test_get_project_repository_cache_import_error(self, mock_config):
        ...
        with patch('fastmcp.task_management.infrastructure.repositories.repository_factory.logger') as mock_logger:
            # Simulate ImportError by preventing import of CachedProjectRepository
            import builtins
            original_import = builtins.__import__

            def mock_import(name, *args, **kwargs):
                if 'cached_project_repository' in name:
    ...
    def test_caching_disabled_in_test_environment(self, mock_config):
        ...
        # Test all repository types - only test working mock repositories
        # Note: Some mock repositories may have unimplemented abstract methods
        repos_to_test = [
            (RepositoryFactory.get_task_repository, ()),
            (RepositoryFactory.get_project_repository, ()),
            (RepositoryFactory.get_subtask_repository, ('user-123',)),
            (RepositoryFactory.get_agent_repository, ()),
            (RepositoryFactory.get_context_repository, ())
        ]

        for repo_func, args in repos_to_test:
            try:
                repo = repo_func(*args)
                ...
                # Skip if mock repository has unimplemented abstract methods
    ...
    def test_task_repository_cache_temporarily_disabled(self):
        ...
        with patch('fastmcp.task_management.infrastructure.repositories.repository_factory.RepositoryFactory.get_environment_config') as mock_config:
            mock_config.return_value = {
                'environment': 'production',
                'database_type': 'sqlite',
                'redis_enabled': True,
