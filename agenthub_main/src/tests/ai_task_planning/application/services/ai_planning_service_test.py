...
class TestAITaskPlanningService:
    ...
    @pytest.mark.asyncio
    async def test_create_intelligent_plan_basic(
        self, service, sample_planning_request
    ):
    ...
    @pytest.mark.asyncio
    async def test_plan_contains_expected_patterns(
        self, service, sample_planning_request
    ):
    ...
    async def test_dependency_generation(self, service, sample_planning_request):
        ...
        if arch_tasks and impl_tasks:
            ...
            dep_pairs = [
                (d.dependent_task_id, d.prerequisite_task_id) for d in plan.dependencies
            ]

            # At least some implementation tasks should depend on architecture tasks
            phase_deps_exist = any(
                impl.id in [d[0] for d in dep_pairs]
                and any(arch.id in [d[1] for d in dep_pairs] for arch in arch_tasks)
    ...
    async def test_technical_considerations(self, service, sample_planning_request):
        ...
        tasks_with_tech_reqs = [
            task for task in plan.tasks if task.technical_requirements
        ]
    ...
    async def test_workload_distribution(self, service, sample_planning_request):
        ...
        for task in plan.tasks:
            if task.agent_assignment:
                ...
                calculated_workload[agent] = (
                    calculated_workload.get(agent, 0) + task.estimated_hours
                )

        # Verify workload calculation
        for agent in calculated_workload:
            assert (
                abs(plan.agent_workload.get(agent, 0) - calculated_workload[agent])
                < 0.01
            )
