...
class TestAITaskPlanningMCPController:
    ...
    async def test_create_ai_plan_with_comma_separated_requirements(self, controller):
        ...
        with patch.object(
            controller.planning_service, "create_intelligent_plan"
        ) as mock_plan:
            with patch.object(
                controller.planning_service, "execute_plan_with_mcp"
            ) as mock_execute:
                # Setup mocks
            ...
            with patch.object(
                ...
                requirements_json = json.dumps(
                    [
                        ...
                        {"description": "Add user dashboard", "priority": "medium"},
                    ]
                )
    ...
    async def test_analyze_requirements_json_format(self, controller):
        ...
        requirements_json = json.dumps(
            [
                ...
                {"description": "Add database schema", "priority": "medium"},
            ]
        )
        ...
        assert (
            "api_integration" in detailed[0]["detected_patterns"]
            or "crud_operations" in detailed[0]["detected_patterns"]
        )
    ...
    async def test_suggest_agents_basic(self, controller):
        ...
        assert (
            ...
            in result["agent_suggestions"]["recommended_agents"]
        )
    ...
    def test_generate_recommendations(self, controller):
        ...
        assert any(
            "detailed requirements" in r for r in recommendations
        )  # Low confidence
    ...
    def test_estimate_agent_workload(self, controller):
        ...
        agent_recommendations = {"coding-agent": 4, "test-agent": 2, "ui-agent": 2}
        total_hours = 40

        workload = controller._estimate_agent_workload(
            agent_recommendations, total_hours
        )
    ...
    def test_identify_specialization_needs(self, controller):
        ...
        specializations = controller._identify_specialization_needs(
            pattern_distribution
        )
...
class TestEdgeCases:
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = await controller.create_ai_plan(
                    ...
                    requirements=[
                        "Simple string",
                        {"description": "Object format", "priority": "high"},
                    ],
                    git_branch_id="branch_123",
                )
    ...
    async def test_analyze_requirements_with_invalid_json(self, controller):
        ...
        result = await controller.analyze_requirements(requirements='{"broken": json')
    ...
    async def test_suggest_agents_empty_available_list(self, controller):
        ...
        result = await controller.suggest_agents(
            requirements="Build complex system", available_agents=""
        )
    ...
    async def test_estimate_effort_extreme_values(self, controller):
        ...
        with patch.object(
            controller.planning_service.requirement_analyzer,
            ...
            with patch.object(
                ...
                mock_analyzed = [Mock()]
                mock_analyzed[0].original_requirement = Mock(
                    id="req_1", description="Test"
                )
                ...
                result = await controller.estimate_effort(
                    requirements="Extremely complex requirement", include_breakdown=True
                )

                assert result["success"]
                assert result["effort_estimate"]["total_hours"] == 10000
                assert (
                    result["effort_estimate"]["confidence_level"] == "low"
                )  # Should be low for extreme values
    ...
    async def test_create_plan_with_all_optional_parameters(self, controller):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                assert any(
                    "smaller phases" in r for r in recommendations
                )  # Large project
                assert any("coordination" in r for r in recommendations)  # Many agents
                assert any(
                    "risk" in r.lower() for r in recommendations
                )  # Critical risk
                assert any(
                    "overloaded" in r for r in recommendations
                )  # Overloaded agents
