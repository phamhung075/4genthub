...
class TestRequirementAnalyzer:
    ...
    def test_complexity_analysis(self, analyzer):
        ...
        simple_req = RequirementItem(
            id="req_simple", description="Add a simple button to the homepage"
        )
        ...
        assert (
            complex_result.estimated_effort_hours > simple_result.estimated_effort_hours
        )
    ...
    def test_performance_requirement_pattern(self, analyzer):
        ...
        assert any(
            "caching" in consideration.lower()
            for consideration in result.technical_considerations
        )
    ...
    def test_dependency_inference(self, analyzer):
        ...
        assert RequirementPattern.UI_COMPONENT in result.detected_patterns
        assert (
            RequirementPattern.API_INTEGRATION.value in result.dependencies
            or "api_integration" in result.dependencies
        )
    ...
    def test_effort_estimation(self, analyzer):
        ...
        req1 = RequirementItem(id="req_e1", description="Add simple text label")
    ...
    def test_analyze_requirements_batch(self, analyzer):
        ...
        requirements = [
            RequirementItem(
                id="req_b1", description="Create REST API endpoints for user management"
            ),
            RequirementItem(
                id="req_b2",
    ...
    def test_generate_planning_insights(self, analyzer):
        ...
        requirements = [
            RequirementItem(
                id="req_i1", description="Design database schema for new features"
            ),
            RequirementItem(
                id="req_i2", description="Implement CRUD operations for user management"
            ),
            RequirementItem(
                id="req_i3",
            ...
            RequirementItem(
                id="req_i4", description="Setup deployment pipeline with Docker"
            ),
        ]
    ...
    def test_technical_considerations(self, analyzer):
        ...
        assert any(
            "owasp" in consideration.lower() or "security" in consideration.lower()
            for consideration in result.technical_considerations
        )
        assert any(
            "index" in consideration.lower() or "query" in consideration.lower()
            for consideration in result.technical_considerations
        )

    def test_empty_requirement_handling(self, analyzer):
        """Test handling of minimal/empty requirements"""
        requirement = RequirementItem(id="req_empty", description="Fix bug")
