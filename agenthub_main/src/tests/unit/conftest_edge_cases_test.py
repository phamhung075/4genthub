...
class TestPytestSessionfinishCleanup:
    ...
    def test_sessionfinish_calls_cleanup_function(self):
        ...
        with patch('tests.conftest.cleanup_test_data_files_only', return_value=5) as mock_cleanup:
            with patch('tests.conftest.Path') as mock_path:
                ...
                assert mock_cleanup.called, "cleanup_test_data_files_only should be called"
        ...
        with patch('tests.conftest.cleanup_test_data_files_only', return_value=10):
            with patch('tests.conftest.Path') as mock_path_class:
                ...
                with patch('tests.conftest.shutil.rmtree') as mock_rmtree:
                    ...
                    assert mock_rmtree.call_count == 2, "Should cleanup 2 temp directories"
    ...
    def test_sessionfinish_temp_directory_cleanup_error_handling(self):
        ...
        with patch('tests.conftest.cleanup_test_data_files_only', return_value=0):
            with patch('tests.conftest.Path') as mock_path_class:
                ...
                with patch('tests.conftest.shutil.rmtree', side_effect=OSError("Permission denied")):
                    with patch('builtins.print') as mock_print:
                        ...
                        error_printed = any("Could not remove temp dir" in str(call) for call in print_calls)
                        assert error_printed, "Should print error message for cleanup failure"
...
class TestPytestConfigureMarkers:
    ...
    def test_configure_registers_memory_marker(self):
        ...
        memory_marker_registered = any("memory" in str(call) and "mark test as memory usage test" in str(call)
                                       for call in calls)
    ...
    def test_configure_registers_vision_marker(self):
        ...
        # Check vision marker
        calls = [str(call) for call in mock_config.addinivalue_line.call_args_list]
        vision_marker = any("vision" in str(call) and "mark test as vision system test" in str(call)
                           for call in calls)
    ...
    def test_configure_registers_context_marker(self):
        ...
        calls = [str(call) for call in mock_config.addinivalue_line.call_args_list]
        context_marker = any("context" in str(call) and "hierarchical context test" in str(call)
                            for call in calls)
    ...
    def test_configure_registers_migration_marker(self):
        ...
        calls = [str(call) for call in mock_config.addinivalue_line.call_args_list]
        migration_marker = any("migration" in str(call) and "repository migration test" in str(call)
                              for call in calls)
    ...
    def test_configure_registers_database_marker(self):
        ...
        calls = [str(call) for call in mock_config.addinivalue_line.call_args_list]
        database_marker = any("database" in str(call) and "requiring database" in str(call)
                             for call in calls)
    ...
    def test_configure_registers_all_required_markers(self):
        ...
        for marker_name, marker_desc_fragment in expected_markers:
            marker_found = any(marker_name in str(call) and marker_desc_fragment in str(call)
...
class TestDatabaseInitializationEdgeCases:
    ...
    def test_initialize_database_git_branch_sql_insert(self):
        ...
        mock_session = Mock()
        mock_db_config = Mock()
        mock_db_config.get_session.return_value.__enter__ = Mock(return_value=mock_session)
        ...
        with patch('fastmcp.task_management.infrastructure.database.database_config.get_db_config', return_value=mock_db_config):
            with patch('sqlalchemy.text') as mock_text:
                with patch('uuid.uuid4', return_value='test-branch-uuid'):
                    ...
                    assert mock_text.called, "SQL text() should be called for INSERT statements"

                    # Check that git branch INSERT was attempted
                    sql_calls = [str(call) for call in mock_text.call_args_list]
                    git_branch_insert = any("project_git_branchs" in str(call) for call in sql_calls)
                    assert git_branch_insert, "Git branch INSERT statement should be executed"
    ...
    def test_initialize_database_git_branch_description_field(self):
        ...
        with patch('fastmcp.task_management.infrastructure.database.database_config.get_db_config', return_value=mock_db_config):
            with patch('sqlalchemy.text', side_effect=lambda x: x):
                ...
                git_branch_params = [p for p in execute_params if 'name' in p and p.get('name') == 'main']
                assert len(git_branch_params) > 0, "Git branch parameters should include description field"
    ...
    def test_initialize_database_error_handling_rollback(self):
        ...
        with patch('fastmcp.task_management.infrastructure.database.database_config.get_db_config', return_value=mock_db_config):
            with patch('builtins.print') as mock_print:
                ...
                print_calls = [str(call) for call in mock_print.call_args_list]
                error_printed = any("Error initializing test data" in str(call) for call in print_calls)
...
class TestPostgreSQLSessionFixture:
    ...
    def test_postgresql_session_fixture_import_error_handling(self):
        ...
        assert "get_test_database_config" in source, "Fixture should try to get test config"
        assert "install_missing_dependencies" in source, "Fixture should try to install dependencies"
    ...
    def test_postgresql_session_fixture_general_error_handling(self):
        ...
        assert "failed" in source.lower(), "Fixture should include 'failed' in error message"
