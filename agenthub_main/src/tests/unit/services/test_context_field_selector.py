...
class TestContextFieldSelector(unittest.TestCase):
    ...
    def test_optimal_field_set_selection(self):
        ...
        self.assertEqual(
            self.selector.get_optimal_field_set("list", "task"), FieldSet.MINIMAL
        )
        self.assertEqual(
            self.selector.get_optimal_field_set("status", "project"), FieldSet.MINIMAL
        )

        # Summary operations
        self.assertEqual(
            self.selector.get_optimal_field_set("get", "task"), FieldSet.SUMMARY
        )
        self.assertEqual(
            self.selector.get_optimal_field_set("search", "context"), FieldSet.SUMMARY
        )

        # Detail operations
        self.assertEqual(
            self.selector.get_optimal_field_set("update", "task"), FieldSet.DETAIL
        )
        self.assertEqual(
            self.selector.get_optimal_field_set("workflow", "project"), FieldSet.DETAIL
        )

        # Debug operations
        self.assertEqual(
            self.selector.get_optimal_field_set("debug", "task"), FieldSet.FULL
        )
    ...
    def test_caching_behavior(self):
        ...
        cached = self.selector.get_cached_fields(
            "task-cache", ["id", "title", "status"]
        )
    ...
    def test_default_field_set(self):
        ...
        self.assertEqual(
            len(result["fields"]),
            len(self.selector.TASK_FIELD_SETS[FieldSet.SUMMARY]) + 2,
        )  # +2 for dependencies (assignee_ids, label_ids)
