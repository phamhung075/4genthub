...
class TestResponseFormatterOptimization(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_data = {
            "task": {"id": "task-123", "title": "Test Task", "status": "todo"}
        }
    ...
    def test_legacy_mode_via_header(self):
        ...
        request_context = {"headers": {"X-Response-Format": "legacy"}}

        response = self.formatter.create_success_response(
            operation="create", data=self.sample_data, request_context=request_context
        )
    ...
    def test_optimized_response_structure(self):
        ...
        response = self.formatter.create_success_response(
            operation="create", data=self.sample_data
        )
    ...
    def test_profile_selection_via_parameter(self):
        ...
        response = self.formatter.create_success_response(
            operation="list", data={"items": [1, 2, 3]}, profile=ResponseProfile.MINIMAL
        )
    ...
    def test_auto_profile_selection_for_high_frequency(self):
        ...
        response = self.formatter.create_success_response(
            operation="list", data={"items": [1, 2, 3, 4, 5]}
        )
    ...
    def test_error_response_optimization(self):
        ...
        response = self.formatter.create_error_response(
            operation="create", error="Validation failed", error_code="VALIDATION_ERROR"
        )
    ...
    def test_format_success_with_optimization(self):
        ...
        response = self.formatter.format_success(data=raw_data, operation="create")
    ...
    def test_format_context_response_with_optimization(self):
        ...
        context_data = {
            "success": True,
            "context": {"id": "ctx-123", "data": {"key": "value"}},
            "level": "task",
        }

        response = self.formatter.format_context_response(
            data=context_data, operation="context.get"
        )
    ...
    def test_partial_success_response_optimization(self):
        ...
        partial_failures = [{"item": "item1", "error": "Failed to process"}]
