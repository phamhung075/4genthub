...
class TestResponseProfiles(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_response = {
            ...
            "workflow_guidance": {
                ...
                "autonomous_guidance": {"confidence": 0.85},
            },
            "metadata": {"created_by": "user-789"},
        }
    ...
    def test_profile_auto_selection_ai_agent(self):
        ...
        request_context = {"params": {"agent": "autonomous"}}
        profile = self.optimizer.auto_select_profile(
            {"operation": "update", "data": {}}, request_context
        )
        self.assertEqual(profile, ResponseProfile.DETAILED)
    ...
    def test_profile_auto_selection_debug(self):
        ...
        profile = self.optimizer.auto_select_profile(
            {"operation": "create", "data": {}}, request_context
        )
        self.assertEqual(profile, ResponseProfile.DEBUG)

        # Test with debug in headers
        request_context = {"headers": {"X-Debug": "true"}}
        profile = self.optimizer.auto_select_profile(
            {"operation": "update", "data": {}}, request_context
        )
        self.assertEqual(profile, ResponseProfile.DEBUG)
    ...
    def test_minimal_profile_fields(self):
        ...
        optimized = self.optimizer.optimize_response(
            self.sample_response.copy(), ResponseProfile.MINIMAL
        )
        ...
        self.assertTrue(
            "operation" in optimized
            or (optimized.get("meta", {}).get("operation") == "create")
        )
    ...
    def test_standard_profile_fields(self):
        ...
        optimized = self.optimizer.optimize_response(
            self.sample_response.copy(), ResponseProfile.STANDARD
        )
    ...
    def test_detailed_profile_fields(self):
        ...
        optimized = self.optimizer.optimize_response(
            self.sample_response.copy(), ResponseProfile.DETAILED
        )
        ...
        self.assertTrue(
            ...
            or optimized["hints"]["required"] == "add_description"
        )
    ...
    def test_debug_profile_fields(self):
        ...
        optimized = self.optimizer.optimize_response(
            self.sample_response.copy(), ResponseProfile.DEBUG
        )
    ...
    def test_profile_size_differences(self):
        ...
        large_response = {
            ...
            "workflow_guidance": {
                ...
                "validation": {"errors": [], "warnings": ["warning1", "warning2"]},
            },
            "metadata": {"total_count": 5, "page": 1, "per_page": 10},
        }

        # Get sizes for each profile
        minimal = self.optimizer.optimize_response(
            large_response.copy(), ResponseProfile.MINIMAL
        )
        standard = self.optimizer.optimize_response(
            large_response.copy(), ResponseProfile.STANDARD
        )
        detailed = self.optimizer.optimize_response(
            large_response.copy(), ResponseProfile.DETAILED
        )
        debug = self.optimizer.optimize_response(
            large_response.copy(), ResponseProfile.DEBUG
        )
        ...
        self.assertLess(
            minimal_size, standard_size, "MINIMAL should be smaller than STANDARD"
        )
        self.assertLess(
            standard_size, detailed_size, "STANDARD should be smaller than DETAILED"
        )
        self.assertLess(
            detailed_size, debug_size, "DETAILED should be smaller than DEBUG"
        )
    ...
    def test_auto_profile_with_optimization(self):
        ...
        request_context = {"params": {"agent": "coding-agent"}}
    ...
    def test_profile_with_error_response(self):
        ...
        for profile in ResponseProfile:
            optimized = self.optimizer.optimize_response(error_response.copy(), profile)
