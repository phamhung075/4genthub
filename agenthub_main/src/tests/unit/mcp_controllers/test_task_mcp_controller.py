...
class TestTaskMCPController:
    ...
    def mock_authentication(self, sample_user_id):
        ...
        with (
            ...
            patch(
                "fastmcp.task_management.interface.mcp_controllers.task_mcp_controller.task_mcp_controller.log_authentication_details"
            ) as mock_log,
        ):
    ...
    def test_create_task_missing_required_fields(
        ...
        assert any(
            ...
            for keyword in ["required", "validation_error", "missing", "expected"]
        )
    ...
    def test_create_task_invalid_assignees(
        self,
        controller,
        sample_task_data,
        invalid_assignees,
    ...
    def test_get_task_not_found(
        ...
        result = controller.manage_task_sync(action="get", task_id="non-existent-id")

        assert result["success"] is False
        assert "not found" in self._extract_error_message(result).lower()

    def test_get_task_missing_task_id(
        self, controller, mock_authentication, mock_permissions
    ):
    ...
    def test_list_tasks_success(
        ...
        task_list = [
            sample_task_data,
            {**sample_task_data, "task_id": str(uuid.uuid4()), "title": "Another Task"},
        ]
        expected_response = {
            "success": True,
            "data": {
    ...
    def test_list_tasks_pagination(
        ...
        result = controller.manage_task_sync(action="list", limit=limit, offset=offset)
    ...
    def test_search_tasks_success(
        ...
        expected_response = {
            "success": True,
            "data": {"tasks": [sample_task_data], "total": 1, "query": "test"},
            "message": "Search completed successfully",
        }
        mock_facade.search_tasks.return_value = expected_response

        result = controller.manage_task_sync(action="search", query="test")
    ...
    def test_search_tasks_empty_query(
        self, controller, mock_authentication, mock_permissions
    ):
    ...
    def test_insufficient_permissions(
        self, controller, mock_authentication, sample_task_data
    ):
    ...
    def test_workflow_hint_enhancement(
        self,
        controller,
        mock_facade_service,
        mock_workflow_hint_enhancer,
        sample_task_data,
        mock_authentication,
        mock_permissions,
    ):
    ...
    def test_workflow_enhancement_failure_graceful_degradation(
        self,
        controller,
        mock_facade_service,
        mock_workflow_hint_enhancer,
        ...
        mock_workflow_hint_enhancer.enhance_response.side_effect = Exception(
            "Enhancement failed"
        )
