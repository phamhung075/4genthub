...
class TestJWTAuthBackend:
    ...
    @pytest.mark.asyncio
    async def test_validate_token_dual_auth_local_success(
        self, auth_backend, jwt_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_dual_auth_supabase_success(
        self, auth_backend, jwt_service, monkeypatch
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_dual_auth_both_fail(
        self, auth_backend, jwt_service, monkeypatch
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_dual_auth_no_supabase_secret(
        self, auth_backend, jwt_service, monkeypatch
    ):
    ...
    def test_init_requires_jwt_secret(self, monkeypatch):
        ...
        with pytest.raises(
            ValueError, match="JWT_SECRET_KEY environment variable not set"
        ):
            JWTAuthBackend()
    ...
    async def test_verify_token_valid(self, auth_backend, jwt_service):
        ...
        auth_backend._validate_token_dual_auth.assert_called_once_with(
            "valid.token.here"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_user_context_from_repository(
        self, auth_backend, user_repository
    ):
        ...
        context = await auth_backend._get_user_context(
    ...
    async def test_get_user_context_cache(self, auth_backend, user_repository):
        ...
        context1 = await auth_backend._get_user_context(
            "12345678-1234-1234-1234-123456789abc"
        )
        assert user_repository.find_by_id.call_count == 1

        # Second call should use cache
        context2 = await auth_backend._get_user_context(
            "12345678-1234-1234-1234-123456789abc"
        )
    ...
    async def test_get_user_context_cache_expiry(self, auth_backend, user_repository):
        ...
        auth_backend._cache_timestamps["12345678-1234-1234-1234-123456789abc"] = (
            time.time() - 400
        )  # Expired
    ...
    @pytest.mark.asyncio
    async def test_get_user_context_repository_error(
        self, auth_backend, user_repository
    ):
    ...
    async def test_user_context_middleware_integration(self, auth_backend, jwt_service):
        ...
        user_context = await auth_backend._get_user_context(
            "12345678-1234-1234-1234-123456789abc"
        )
