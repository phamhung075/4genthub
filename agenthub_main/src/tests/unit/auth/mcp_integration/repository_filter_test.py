...
class TestUserFilteredRepository:
    ...
    def test_get_current_user_id_with_user(self):
        ...
        with patch(
            "fastmcp.auth.mcp_integration.repository_filter.get_current_user_id",
...
class TestUserFilteredTaskRepository:
    ...
    def test_find_all(self, task_repo, base_repo):
        ...
        with patch(
            ...
            assert result == tasks
            base_repo.find_all.assert_called_once_with(
                status="active", user_id="user-123"
            )
    ...
    def test_save_existing_task_other_user(self, task_repo, base_repo):
        ...
        with patch(
            ...
            with pytest.raises(
                RuntimeError, match="Cannot save task belonging to another user"
            ):
                task_repo.save(task)
