...
class TestMCPAuthMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_http_request_with_valid_token(
        self, middleware, mock_app, mock_jwt_backend
    ):
    ...
    @pytest.mark.asyncio
    async def test_http_request_with_invalid_token(
        self, middleware, mock_app, mock_jwt_backend
    ):
        ...
        with patch(
            "fastmcp.auth.mcp_integration.mcp_auth_middleware.logger.warning"
        ) as mock_logger:
            await middleware(scope, receive, send)
    ...
    @pytest.mark.asyncio
    async def test_http_request_no_user_context_found(
        self, middleware, mock_app, mock_jwt_backend
    ):
        ...
        with patch(
            ...
            # Verify warning was logged
            mock_logger.assert_called_once()
            assert "Could not get user context for user_id: test-user-456" in str(
    ...
    @pytest.mark.asyncio
    async def test_context_reset_after_request(
        self, middleware, mock_app, mock_jwt_backend
    ):
        ...
        scope = {
            "type": "http",
            "headers": [(b"authorization", b"Bearer admin-jwt-token")],
        }
        receive = AsyncMock()
        send = AsyncMock()
    ...
    async def test_middleware_error_handling(self, middleware, mock_app):
        ...
        with patch(
            "fastmcp.auth.mcp_integration.mcp_auth_middleware.logger.error"
        ) as mock_logger:
            await middleware(scope, receive, send)
    ...
    async def test_non_bearer_auth_header(self, middleware, mock_app):
        ...
        with patch(
            "fastmcp.auth.mcp_integration.mcp_auth_middleware.logger.debug"
        ) as mock_logger:
            await middleware(scope, receive, send)

            # Verify debug message about no Bearer token
            mock_logger.assert_called()
            assert "No Bearer token found in Authorization header" in str(
                mock_logger.call_args
            )
    ...
    @pytest.mark.asyncio
    async def test_existing_scope_state_preserved(
        self, middleware, mock_app, mock_jwt_backend
    ):
        ...
        scope = {
            "type": "http",
            "headers": [(b"authorization", b"Bearer special-jwt-token")],
