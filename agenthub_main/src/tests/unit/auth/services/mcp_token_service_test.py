...
class TestMCPTokenService:
    ...
    async def test_generate_mcp_token_with_custom_expiration(self, service):
        ...
        # Act
        token = await service.generate_mcp_token_from_user_id(
            user_id="user_456", expires_in_hours=48
        )
    ...
    async def test_generate_mcp_token_with_metadata(self, service):
        ...
        # Act
        token = await service.generate_mcp_token_from_user_id(
            user_id="user_789", metadata=metadata
        )
    ...
    async def test_generate_mcp_token_stores_in_memory(self, service):
        ...
        # Act
        token = await service.generate_mcp_token_from_user_id(user_id="user_abc")
    ...
    async def test_validate_mcp_token_valid(
        self, mock_get_session, service, sample_token
    ):
    ...
    @pytest.mark.asyncio
    @patch("fastmcp.auth.services.mcp_token_service.get_session")
    async def test_validate_mcp_token_inactive(
        self, mock_get_session, service, sample_token
    ):
