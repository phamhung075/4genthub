...
class TestPasswordService:
    ...
    def test_verify_password_incorrect_password(self):
        ...
        assert (
            PasswordService.verify_password("securepassword123!", hashed) is False
        )  # Case sensitive
        assert (
            PasswordService.verify_password("SecurePassword123", hashed) is False
        )  # Missing char
    ...
    def test_hash_password_validates_min_length(self):
        ...
        with pytest.raises(
            ValueError,
            match=f"Password must be at least {PasswordService.MIN_PASSWORD_LENGTH} characters",
        ):
    ...
    def test_hash_password_validates_max_length(self):
        ...
        with pytest.raises(
            ValueError,
            match=f"Password cannot exceed {PasswordService.MAX_PASSWORD_LENGTH} characters",
        ):
    ...
    def test_generate_secure_password_custom_character_sets(self):
        ...
        # Only lowercase
        password = PasswordService.generate_secure_password(
            include_uppercase=False, include_digits=False, include_special=False
        )
    ...
    def test_needs_rehash_handles_malformed_rounds(self, mock_logger):
        ...
        assert "Could not check if hash needs rehash" in str(
