...
class TestJWTService:
    ...
    def test_create_access_token_with_additional_claims(self, jwt_service):
        ...
        token = jwt_service.create_access_token(
            "user123", "test@example.com", ["user"], additional_claims=additional_claims
        )
    ...
    def test_create_refresh_token_with_family(self, jwt_service):
        ...
        token, returned_family = jwt_service.create_refresh_token(
            user_id, token_family=existing_family, token_version=token_version
        )
    ...
    def test_verify_token_api_token_compatibility(self, jwt_service):
        ...
        token = jwt.encode(
            token_payload, jwt_service.secret_key, algorithm=jwt_service.ALGORITHM
        )
    ...
    def test_verify_token_wrong_secret(self, jwt_service):
        ...
        token = jwt.encode(
            token_payload, "wrong-secret", algorithm=jwt_service.ALGORITHM
        )
    ...
    def test_extract_token_from_header(self, jwt_service):
        ...
        # Valid Bearer token
        token = jwt_service.extract_token_from_header(
    ...
    def test_verify_supabase_token_simulation(self, jwt_service):
        ...
        token = jwt.encode(
            supabase_payload, jwt_service.secret_key, algorithm=jwt_service.ALGORITHM
        )
