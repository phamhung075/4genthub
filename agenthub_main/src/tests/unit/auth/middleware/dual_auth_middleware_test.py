...
class TestDualAuthMiddleware:
    ...
    def test_should_skip_auth_static_path(self, middleware, mock_request):
        """Test skipping auth for static file paths."""
        mock_request.url.path = "/static/styles.css"
        result = middleware._should_skip_auth(mock_request)
        assert result is True
    ...
    def test_extract_token_from_query_param(self, middleware, mock_request):
        ...
        with patch(
            "fastmcp.auth.middleware.dual_auth_middleware.logger"
        ) as mock_logger:
            result = middleware._extract_token(mock_request)
    ...
    @pytest.mark.asyncio
    async def test_authenticate_request_supabase_success(
        ...
        # Create a mock Supabase auth service and set it on middleware
        ...
        mock_supabase_auth.verify_token = AsyncMock(return_value=mock_result)
    ...
    @pytest.mark.asyncio
    async def test_authenticate_request_mcp_token_success(
        self, middleware, mock_request
    ):
        ...
        with patch.object(
            middleware.token_validator, "validate_token"
        ) as mock_validate:
    ...
    @pytest.mark.asyncio
    async def test_dispatch_skip_auth_path(
    ...
    async def test_dispatch_with_mock_token_user(
        self, middleware, mock_request, mock_call_next
    ):
        ...
        # Mock the JWT validation to accept our test token
        with patch("jwt.decode") as mock_decode:
            mock_decode.return_value = payload
            response = await middleware.dispatch(mock_request, mock_call_next)

        assert response.status_code == 200
        assert mock_request.state.user_id == mock_user_id
        assert mock_request.state.auth_type == "local_jwt"

    @pytest.mark.asyncio
    @patch.dict(os.environ, {"PRODUCTION": "false", "AUTH_ENABLED": "true"})
    async def test_dispatch_authenticated_request(
        self, middleware, mock_request, mock_call_next
    ):
    ...
    @pytest.mark.asyncio
    @patch.dict(os.environ, {"PRODUCTION": "false", "AUTH_ENABLED": "true"})
    async def test_dispatch_token_validation_error(
        self, middleware, mock_request, mock_call_next
    ):
    ...
    @pytest.mark.asyncio
    @patch.dict(os.environ, {"PRODUCTION": "false", "AUTH_ENABLED": "true"})
    async def test_dispatch_rate_limit_error(
        self, middleware, mock_request, mock_call_next
    ):
