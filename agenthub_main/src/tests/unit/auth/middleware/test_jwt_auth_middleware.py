...
class TestJWTAuthMiddleware:
    ...
    def test_extract_user_from_valid_token_with_user_id(
        self, middleware, valid_token_with_user_id
    ):
    ...
    def test_extract_user_from_token_without_user_claim(
        self, middleware, secret_key, algorithm
    ):
    ...
    def test_create_user_scoped_repository_without_user_support(
        self, middleware, caplog
    ):
    ...
    def test_create_user_scoped_service_with_user_id_in_init(self, middleware):
        ...
        service = middleware.create_user_scoped_service(MockService, user_id, dep1=dep1)
    ...
    def test_create_user_scoped_service_with_with_user_method(self, middleware):
        ...
        service = middleware.create_user_scoped_service(MockService, user_id)
    ...
    @pytest.mark.asyncio
    async def test_require_auth_decorator_with_valid_token(
...
class TestUserContextManager:
    ...
    def test_get_repository_creates_new(self, manager, user_id):
        ...
        with patch(
            "fastmcp.auth.middleware.jwt_auth_middleware.JWTAuthMiddleware"
        ) as mock_middleware:
            mock_instance = Mock()
            mock_instance.create_user_scoped_repository.return_value = MockRepository(
                session, user_id
            )
