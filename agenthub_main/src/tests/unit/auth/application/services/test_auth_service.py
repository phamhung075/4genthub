...
def mock_jwt_service():
    ...
    service.create_refresh_token = Mock(
        return_value=("test_refresh_token", "test_family")
    )
    ...
    service.verify_refresh_token = Mock(
        return_value={"sub": "test_user_id", "family": "test_family", "version": 0}
    )
    service.verify_reset_token = Mock(
        return_value={"sub": "test_user_id", "email": "test@example.com"}
    )
    return service
...
def mock_password_service():
    ...
    service.check_password_strength = Mock(
        return_value={"strength": "strong", "suggestions": []}
    )
...
class TestAuthServiceRegistration:
    ...
    @pytest.mark.asyncio
    async def test_registration_with_existing_username(
        self, auth_service, mock_user_repository, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_registration_with_weak_password(
        self, auth_service, mock_password_service
    ):
...
class TestAuthServiceLogin:
    ...
    async def test_login_with_wrong_password(
        self, auth_service, mock_user_repository, mock_password_service, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_login_with_suspended_account(
        self, auth_service, mock_user_repository, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_login_with_password_rehash(
        self, auth_service, mock_user_repository, mock_password_service, sample_user
    ):
...
class TestAuthServiceEmailVerification:
    ...
    async def test_successful_email_verification(
        self, auth_service, mock_user_repository, mock_jwt_service, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_email_verification_with_non_existent_user(
        self, auth_service, mock_user_repository, mock_jwt_service
    ):
...
class TestAuthServicePasswordReset:
    ...
    async def test_successful_password_reset_request(
        ...
        # Act
        success, token, error = await auth_service.request_password_reset(
            "test@example.com"
        )
...
class TestAuthServiceTokenOperations:
    ...
    async def test_successful_token_refresh(
        ...
        mock_jwt_service.create_refresh_token.return_value = (
            "new_refresh_token",
            "test_family",
        )
    ...
    @pytest.mark.asyncio
    async def test_token_refresh_with_non_existent_user(
        self, auth_service, mock_user_repository, mock_jwt_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_token_refresh_with_wrong_family(
        self, auth_service, mock_user_repository, mock_jwt_service, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_token_refresh_with_old_version(
        self, auth_service, mock_user_repository, mock_jwt_service, sample_user
    ):
...
class TestAuthServiceLogout:
    ...
    @pytest.mark.asyncio
    async def test_logout_with_token_revocation(
        self, auth_service, mock_user_repository, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_logout_with_non_existent_user(
        self, auth_service, mock_user_repository
    ):
...
class TestAuthServiceExceptionHandling:
    ...
    @pytest.mark.asyncio
    async def test_registration_handles_repository_exception(
        self, auth_service, mock_user_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_email_verification_handles_exception(
        self, auth_service, mock_jwt_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_password_reset_handles_exception(
        self, auth_service, mock_user_repository
    ):
