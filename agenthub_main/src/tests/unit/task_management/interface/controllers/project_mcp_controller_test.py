...
class TestProjectMCPController:
    ...
    def test_register_tools(self):
        ...
        with (
            patch(
                "fastmcp.task_management.interface.mcp_controllers.project_mcp_controller.project_mcp_controller.get_manage_project_description"
            ) as mock_get_desc,
            patch(
                "fastmcp.task_management.interface.mcp_controllers.project_mcp_controller.project_mcp_controller.get_manage_project_parameters"
            ) as mock_get_params,
        ):
            ...
            mock_mcp.tool.assert_called_once_with(
                description="Test project management description"
            )
    ...
    def test_get_facade_for_request_no_facade_service_raises_error(self):
        ...
        assert "Either facade_factory or facade_service is required" in str(
            exc_info.value
        )
    ...
    async def test_manage_project_health_check_action(self, mock_get_auth_user_id):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = await self.controller._manage_project_async(
                    action="project_health_check", project_id="test-project"
                )

                assert result == {"success": True, "health_score": 85}
                mock_handle_operation.assert_called_once()
                mock_permissions.assert_called_once_with(
                    "project_health_check", "test-user-123", "test-project"
                )

    @patch(
        "fastmcp.task_management.interface.mcp_controllers.project_mcp_controller.validate_user_id"
    )
    @patch(
        "fastmcp.task_management.interface.mcp_controllers.project_mcp_controller.get_current_user_id"
    )
    @patch(
        "fastmcp.task_management.interface.mcp_controllers.project_mcp_controller.get_authenticated_user_id"
    )
    def test_manage_project_unknown_action(
        self, mock_get_auth_user_id, mock_get_current_user_id, mock_validate_user_id
    ):
        ...
        result = self.controller.manage_project(
            action="invalid_action", name="test-project"
        )
    ...
    def test_handle_crud_operations_get_missing_identifier(self):
        ...
        assert "meta" in result
        assert (
            "project_id" in result["error"]["message"]
            and "name" in result["error"]["message"]
        )
    ...
    def test_handle_crud_operations_list_success(self):
        ...
        result = self.controller.handle_crud_operations(
            "list", None, None, None, "test-user-123", False
        )
    ...
    def test_handle_crud_operations_update_success(self):
        ...
        # Mock the facade to return success response
        self.mock_facade.update_project.return_value = {
            "success": True,
            "project": {"id": "project-123"},
        }
    ...
    def test_handle_maintenance_operations_health_check_success(self):
        ...
        with patch.object(
            self.controller, "_handle_maintenance_action"
        ) as mock_maintenance:
    ...
    def test_get_project_management_descriptions(self):
        ...
        with patch(
            "fastmcp.task_management.interface.mcp_controllers.project_mcp_controller.description_loader"
        ) as mock_loader:
    ...
    def test_include_project_context(self):
        ...
        with patch(
            "fastmcp.task_management.infrastructure.factories.unified_context_facade_factory.UnifiedContextFacadeFactory"
        ) as mock_factory:
            ...
            mock_context_facade.get_context.assert_called_once_with(
                "project", "project-123", include_inherited=True
            )
    ...
    async def test_complete_health_check_workflow(self, mock_get_auth_user_id):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = await self.controller._manage_project_async(
                    action="project_health_check", project_id="project-123"
                )
