...
def create_mock_with_spec(spec_class):
    ...
    if (
        ...
        or isinstance(spec_class, (Mock, MagicMock))
        or type(spec_class).__name__ in ("MagicMock", "Mock", "NonCallableMock")
    ):
...
class TestAgentMCPController:
    ...
    def setup_method(self):
        ...
        with patch(
            "fastmcp.task_management.interface.mcp_controllers.agent_mcp_controller.agent_mcp_controller.AgentWorkflowFactory"
        ):
            self.controller = AgentMCPController(
                self.mock_facade_service, self.mock_config
            )
    ...
    def test_register_tools(self):
        ...
        with patch.object(
            self.controller, "_get_agent_management_descriptions"
        ) as mock_get_desc:
    ...
    @patch(
        "fastmcp.task_management.interface.mcp_controllers.agent_mcp_controller.agent_mcp_controller.get_current_user_id"
    )
    def test_get_facade_for_request_with_user_context(self, mock_get_user_id):
        ...
        with patch(
            "fastmcp.task_management.interface.mcp_controllers.agent_mcp_controller.agent_mcp_controller.validate_user_id"
        ) as mock_validate:
            ...
            mock_validate.assert_called_once_with(
                "jwt-user-123", "Agent facade creation"
            )
            self.mock_facade_service.get_agent_facade.assert_called_once_with(
                project_id=project_id, user_id="jwt-user-123"
            )
    ...
    def test_get_facade_for_request_no_auth_raises_error(self, mock_get_user_id):
        ...
        assert "Agent facade creation requires user authentication" in str(
            exc_info.value
        )
    ...
    def test_manage_agent_register_action(self, mock_get_user_id):
        ...
        with patch.object(
            self.controller._operation_factory, "handle_operation"
        ) as mock_factory:
            mock_factory.return_value = {"success": True}

            result = self.controller.manage_agent(
                action="register", project_id="test-project", name="test-agent"
            )
    ...
    def test_manage_agent_rebalance_action(self, mock_get_user_id):
        ...
        with patch.object(
            ...
            result = self.controller.manage_agent(
                action="rebalance", project_id="test-project"
            )
    ...
    def test_manage_agent_unknown_action(self, mock_get_user_id):
        ...
        with patch(
            ...
            result = self.controller.manage_agent(
                action="invalid_action", project_id="test-project"
            )
            ...
            error_data = result.get(
                "error", result.get("message", result.get("data", {}))
            )
            ...
            assert (
                "operation failed" in str(error_data).lower()
                or "invalid" in str(error_data).lower()
                or "unknown" in str(error_data).lower()
            )
    ...
    def test_handle_crud_operations_get_success(self):
        ...
        with patch.object(
            ...
            assert result["success"] is True
            assert "data" in result or "meta" in result
            self.mock_facade.get_agent.assert_called_once_with(
                "test-project", "agent-123"
            )
    ...
    def test_handle_crud_operations_get_missing_agent_id(self):
        ...
        with patch.object(
            ...
            result = self.controller.handle_crud_operations("get", "test-project", None)
    ...
    def test_handle_crud_operations_list_success(self):
        ...
        with patch.object(
            ...
            result = self.controller.handle_crud_operations("list", "test-project")
    ...
    def test_enhance_response_extract_agent_id_from_register(self):
        ...
        expected_context = {"project_id": "test-project", "agent_id": "new-agent-123"}
        self.controller._workflow_guidance.generate_guidance.assert_called_once_with(
...
class TestAgentMCPControllerIntegration:
    ...
    def test_complete_register_workflow(self, mock_get_user_id):
        ...
        with patch(
            ...
            workflow_guidance = result.get(
                "workflow_guidance", result.get("data", {}).get("workflow_guidance")
            )
            assert agent_data.get("id") == "agent-123" or "agent-123" in str(result)
            assert (
                workflow_guidance == mock_guidance
                or "workflow_guidance" in result
                or "data" in result
            )
