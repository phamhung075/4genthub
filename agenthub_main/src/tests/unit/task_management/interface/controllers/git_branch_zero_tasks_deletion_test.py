...
class TestGitBranchZeroTasksDeletion:
    ...
    def test_delete_empty_branch_should_succeed(self, mock_auth):
        ...
        mock_facade.delete_git_branch.return_value = (
            True  # Return True for successful deletion
        )
        ...
        assert (
            ...
            or result.get("data", {}).get("deleted") is True
        )
    ...
    def test_git_branch_service_delete_empty_branch_integration(self):
        ...
        mock_project_repo.with_user = Mock(
            return_value=mock_project_repo
        )  # For user-scoped repository
        ...
        mock_repo.delete_branch = AsyncMock(
            return_value=True
        )  # The actual method used by GitBranchService
        ...
        result = asyncio.run(
            service.delete_git_branch(self.project_id, self.empty_branch_id)
        )
