...
class TestTaskUserIdParameter:
    ...
    def controller(self, mock_facade_service):
        ...
        return TaskMCPController(
            facade_service_or_factory=mock_facade_service, workflow_hint_enhancer=None
        )

    @pytest.mark.asyncio
    async def test_manage_task_passes_user_id_through_authentication_chain(
        self, controller
    ):
        ...
        with patch(
            ...
            if hasattr(controller, "_derive_context_from_identifiers"):
                project_id, git_branch_name, user_id = (
                    controller._derive_context_from_identifiers(
                        task_id=None,
