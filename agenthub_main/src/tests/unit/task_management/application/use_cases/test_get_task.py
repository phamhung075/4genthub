...
class TestGetTaskUseCase:
    ...
    def mock_task_repository(self):
        ...
        if (
            hasattr(TaskRepository, "_mock_name")
            or hasattr(TaskRepository, "_spec_class")
            or hasattr(TaskRepository, "_mock_methods")
        ):
    ...
    def use_case_with_sync_context(self, mock_task_repository, mock_context_service):
        ...
        return GetTaskUseCase(
            task_repository=mock_task_repository, context_service=mock_context_service
        )

    @pytest.fixture
    def use_case_with_async_context(
        self, mock_task_repository, mock_async_context_service
    ):
    ...
    def use_case_without_context(self, mock_task_repository):
        ...
        return GetTaskUseCase(
            task_repository=mock_task_repository, context_service=None
        )
    ...
    def sample_task(self):
        ...
        if (
            hasattr(Task, "_mock_name")
            or hasattr(Task, "_spec_class")
            or hasattr(Task, "_mock_methods")
        ):
            # It's already a Mock, don't use spec
            task = Mock()
        else:
    ...
    @pytest.mark.asyncio
    async def test_execute_successful_without_context(
        self, use_case_without_context, mock_task_repository, sample_task
    ):
        ...
        with patch(
            ...
            mock_response.from_domain.assert_called_once_with(
                sample_task, git_branch_repository=None, context_data=None
            )

    @pytest.mark.asyncio
    async def test_execute_successful_with_sync_context(
        self,
        use_case_with_sync_context,
        ...
        with patch(
            ...
            # Act
            result = await use_case_with_sync_context.execute(
                task_id, include_context=True
            )
            ...
            mock_context_service.get_context.assert_called_once_with(
                level="task", context_id=task_id, include_inherited=True
            )
    ...
    @pytest.mark.asyncio
    async def test_execute_successful_with_async_context(
        self,
        use_case_with_async_context,
        mock_task_repository,
        sample_task,
        mock_async_context_service,
        sample_context_data,
    ):
        ...
        with patch(
            ...
            # Act
            result = await use_case_with_async_context.execute(
                task_id, include_context=True
            )
    ...
    @pytest.mark.asyncio
    async def test_execute_task_not_found_raises_exception(
        self, use_case_without_context, mock_task_repository
    ):
        """Test TaskNotFoundError is raised when task is not found"""
        # Arrange
        task_id = "12345678-1234-5678-1234-567812345999"  # Valid UUID format
    ...
    @pytest.mark.asyncio
    async def test_execute_with_generate_rules_flag(
        self, use_case_without_context, mock_task_repository, sample_task
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_sync_context_with_data_field(
        self,
        use_case_with_sync_context,
        mock_task_repository,
        sample_task,
    ...
    @pytest.mark.asyncio
    async def test_execute_sync_context_no_success(
        self,
        use_case_with_sync_context,
        mock_task_repository,
        sample_task,
    ...
    @pytest.mark.asyncio
    async def test_execute_async_context_with_data_field(
        self,
        use_case_with_async_context,
        mock_task_repository,
        sample_task,
    ...
    @pytest.mark.asyncio
    async def test_execute_async_context_with_to_dict_method(
        self,
        use_case_with_async_context,
        mock_task_repository,
        sample_task,
    ...
    @pytest.mark.asyncio
    async def test_execute_domain_event_creation(
        self, use_case_without_context, mock_task_repository, sample_task
    ):
        ...
        with patch(
            ...
            with patch(
                "fastmcp.task_management.application.use_cases.get_task.TaskRetrieved"
            ) as mock_event:
                mock_task_response = Mock()
                mock_response.from_domain.return_value = mock_task_response
    ...
    @pytest.mark.asyncio
    async def test_execute_unexpected_exception_handling(
        self, use_case_without_context, mock_task_repository, caplog
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_no_context_service_warning(
        self, use_case_without_context, mock_task_repository, sample_task, caplog
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_various_task_id_formats(
        self,
        use_case_without_context,
        mock_task_repository,
        sample_task,
        task_id_input,
        should_succeed,
        ...
        if should_succeed:
            ...
            with pytest.raises((ValueError, TaskNotFoundError)):
                await use_case_without_context.execute(task_id_input)
