...
class TestDeleteTaskUseCase:
    ...
    @pytest.fixture
    def use_case(
        self, mock_task_repository, mock_db_session_factory, mock_logging_service
    ):
    ...
    def test_execute_successful_deletion_with_string_id(
    ...
    def test_execute_successful_deletion_with_integer_id(
        self, use_case, mock_task_repository, sample_task
    ):
    ...
    def test_execute_task_not_found(self, use_case, mock_task_repository):
        ...
        # Arrange
        task_id = (
            "99999999-9999-9999-9999-999999999999"  # Valid UUID format but non-existent
        )
    ...
    def test_execute_repository_delete_fails(
        self, use_case, mock_task_repository, sample_task
    ):
    ...
    def test_execute_with_git_branch_id_update_success(
        self,
        use_case,
    ...
    def test_execute_with_git_branch_id_update_exception(
        self,
        use_case,
        mock_task_repository,
        sample_task,
        ...
        assert result["success"] is True
        assert (
            result["task_deleted"] is True
        )  # Should still succeed despite branch update failure
    ...
    def test_execute_without_git_branch_id(
        self, use_case, mock_task_repository, sample_task, mock_logger
    ):
    ...
    def test_logging_initialization(
        self, mock_task_repository, mock_logging_service, mock_logger
    ):
        """Test proper logger initialization"""
        # Arrange & Act
        DeleteTaskUseCase(
            task_repository=mock_task_repository, logging_service=mock_logging_service
        )
    ...
    def test_execute_task_without_hasattr_git_branch_id(
    ...
    def test_execute_with_none_git_branch_id(
        self, use_case, mock_task_repository, sample_task, mock_logger
    ):
