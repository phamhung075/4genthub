...
class TestSearchTasksUseCase:
    ...
    def test_execute_successful_search_with_results(
        self, use_case, mock_task_repository, sample_tasks
    ):
        ...
        with patch(
            "fastmcp.task_management.application.use_cases.search_tasks.TaskListResponse"
        ) as mock_response:
            ...
            mock_response.from_domain_list.assert_called_once_with(
                sample_tasks, query=query
            )

    def test_execute_successful_search_with_no_results(
        self, use_case, mock_task_repository, empty_task_list
    ):
        ...
        with patch(
            ...
            mock_response.from_domain_list.assert_called_once_with(
                empty_task_list, query=query
            )

    def test_execute_search_with_different_limits(
        self, use_case, mock_task_repository, sample_tasks
    ):
        ...
        for limit in test_cases:
            ...
            mock_task_repository.search.return_value = (
                sample_tasks[:limit] if limit < len(sample_tasks) else sample_tasks
            )
    ...
    def test_execute_various_query_formats(
        self, use_case, mock_task_repository, sample_tasks, query, expected_calls
    ):
    ...
    def test_repository_interaction_error_handling(
        self, use_case, mock_task_repository
    ):
    ...
    def test_execute_with_git_branch_filtering(
        self, use_case, mock_task_repository, sample_tasks
    ):
        ...
        with patch(
            ...
            mock_task_repository.search.assert_called_once_with(query, limit=10)
            mock_response.from_domain_list.assert_called_once_with(
                auth_tasks, query=query
            )

    def test_execute_preserves_query_in_response(
        self, use_case, mock_task_repository, sample_tasks
    ):
    ...
    def test_execute_with_large_result_set(self, use_case, mock_task_repository):
        ...
        for i in range(50):
            task = Mock(spec=Task)
            task.id = TaskId(f"12345678-1234-5678-1234-{i:012d}")  # Valid UUID format
        ...
        with patch(
            ...
            mock_response.from_domain_list.assert_called_once_with(
                large_task_list, query=query
            )

    @pytest.mark.parametrize("limit_value", [0, -1, -10, 1000000])
    def test_execute_edge_case_limits(
        self, use_case, mock_task_repository, sample_tasks, limit_value
    ):
        ...
        with patch(
            ...
            # Assert
            assert result == mock_task_list_response
            mock_task_repository.search.assert_called_once_with(
    ...
    def test_multiple_consecutive_searches(
        self, use_case, mock_task_repository, sample_tasks
    ):
        ...
        for i, query in enumerate(queries):
            request = SearchTasksRequest(query=query, limit=5)
            mock_task_repository.search.return_value = (
                sample_tasks[i : i + 1] if i < len(sample_tasks) else []
            )
