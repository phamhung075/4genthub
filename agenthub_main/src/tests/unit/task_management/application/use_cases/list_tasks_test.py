...
class TestListTasksUseCase:
    ...
    def test_list_tasks_with_git_branch_filter(
        ...
        mock_task_repository.find_by_criteria.assert_called_once_with(
            {"git_branch_id": "branch-123"}, limit=None
        )
    ...
    def test_list_tasks_with_status_filter(
        ...
        # Arrange
        request = ListTasksRequest(git_branch_id="branch-123", status="todo")
        mock_task_repository.find_by_criteria.return_value = [sample_task]
    ...
    def test_list_tasks_with_priority_filter(
        self, use_case, mock_task_repository, sample_task
    ):
        """Test listing tasks with priority filter"""
        # Arrange
        request = ListTasksRequest(git_branch_id="branch-123", priority="high")
        mock_task_repository.find_by_criteria.return_value = [sample_task]
        ...
        mock_task_repository.find_by_criteria.assert_called_once_with(
            {"git_branch_id": "branch-123", "priority": Priority.high()}, limit=None
        )

    def test_list_tasks_with_assignees_filter(
        self, use_case, mock_task_repository, sample_task
    ):
        """Test listing tasks with assignees filter"""
        # Arrange
        request = ListTasksRequest(
            git_branch_id="branch-123", assignees=["user-1", "user-2"]
        )
        mock_task_repository.find_by_criteria.return_value = [sample_task]
    ...
    def test_list_tasks_with_labels_filter(
        self, use_case, mock_task_repository, sample_task
    ):
        """Test listing tasks with labels filter"""
        # Arrange
        request = ListTasksRequest(git_branch_id="branch-123", labels=["bug", "urgent"])
        mock_task_repository.find_by_criteria.return_value = [sample_task]
        ...
        mock_task_repository.find_by_criteria.assert_called_once_with(
            {"git_branch_id": "branch-123", "labels": ["bug", "urgent"]}, limit=None
        )

    def test_list_tasks_with_limit(self, use_case, mock_task_repository, sample_task):
        """Test listing tasks with limit"""
        # Arrange
        request = ListTasksRequest(git_branch_id="branch-123", limit=10)
        mock_task_repository.find_by_criteria.return_value = [sample_task]
        ...
        assert isinstance(response, TaskListResponse)
        assert len(response.tasks) == 1
        assert response.filters_applied == {"git_branch_id": "branch-123", "limit": 10}

        # Verify repository was called with limit
        mock_task_repository.find_by_criteria.assert_called_once_with(
            {"git_branch_id": "branch-123"}, limit=10
        )

    def test_list_tasks_with_all_filters(
        self, use_case, mock_task_repository, sample_task
    ):
    ...
    def test_list_tasks_empty_result(self, use_case, mock_task_repository):
        ...
        # Arrange
        request = ListTasksRequest(git_branch_id="branch-999", status="done")
    ...
    def test_list_tasks_preserves_task_details(
        self, use_case, mock_task_repository, sample_task
    ):
    ...
    def test_list_tasks_legacy_assignee_field_support(
        self, use_case, mock_task_repository, sample_task
    ):
        ...
        mock_task_repository.find_by_criteria.assert_called_once_with(
            {"git_branch_id": "branch-123", "assignee": "user-1"}, limit=None
        )
