...
class TestAddSubtaskWithInheritance:
    ...
    def test_subtask_creation_with_mixed_parent_assignees(self):
        ...
        assert response.agent_inheritance_applied is True
        assert (
            len(response.inherited_assignees) == 4
        )  # All assignees including empty string
    ...
    def test_subtask_creation_inheritance_preserves_other_fields(self):
        ...
        assert (
            saved_subtask_call.description
            == "Test field preservation during inheritance"
        )
    ...
    def test_inheritance_logging_behavior(self):
        ...
        with patch(
            "fastmcp.task_management.application.use_cases.add_subtask.logging"
        ) as mock_logging:
            # Execute use case
            response = self.use_case.execute(request)
            ...
            )  # At least 2 info logs for inheritance
...
class TestAddSubtaskInheritanceErrorCases:
    ...
    def test_inheritance_with_get_next_id_failure(self):
        ...
        self.subtask_repository.get_next_id.side_effect = Exception(
