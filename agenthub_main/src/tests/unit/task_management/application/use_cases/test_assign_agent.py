...
class TestAssignAgentUseCase:
    ...
    def use_case(self, mock_agent_repository):
        ...
        with patch(
            "fastmcp.task_management.application.use_cases.assign_agent.RepositoryFactory"
        ) as mock_factory:
    ...
    def test_execute_successful_assignment(
        self, use_case, valid_request, mock_agent_repository
    ):
    ...
    def test_execute_agent_not_found_error(
        self, use_case, valid_request, mock_agent_repository
    ):
        """Test handling of AgentNotFoundError"""
        # Arrange
        error_message = "Agent agent-123 not found"
        mock_agent_repository.assign_agent_to_tree.side_effect = AgentNotFoundError(
            error_message
        )
    ...
    def test_execute_project_not_found_error(
        self, use_case, valid_request, mock_agent_repository
    ):
        """Test handling of ProjectNotFoundError"""
        # Arrange
        error_message = "Project 12345678-1234-5678-1234-567812345678 not found"
        mock_agent_repository.assign_agent_to_tree.side_effect = ProjectNotFoundError(
            error_message
        )
    ...
    def test_execute_with_different_request_data(self, use_case, mock_agent_repository):
        ...
        mock_agent_repository.assign_agent_to_tree.assert_called_once_with(
            "different-project-id", "different-agent", "different-branch"
        )

    def test_logging_on_agent_not_found(
        self, use_case, valid_request, mock_agent_repository, caplog
    ):
    ...
    def test_logging_on_unexpected_error(
        ...
        mock_agent_repository.assign_agent_to_tree.side_effect = Exception(
            error_message
        )
    ...
    def test_request_dto_creation(self):
        ...
        # Arrange & Act
        request = AssignAgentRequest(
            project_id="proj-123", agent_id="agent-456", git_branch_id="branch-789"
        )
    ...
    def test_response_dto_error_creation(self):
        ...
        # Arrange & Act
        response = AssignAgentResponse(
            success=False, agent_id="agent-123", error="Agent not found"
        )
    ...
    def test_execute_with_various_input_formats(
        self, use_case, mock_agent_repository, project_id, agent_id, git_branch_id
    ):
