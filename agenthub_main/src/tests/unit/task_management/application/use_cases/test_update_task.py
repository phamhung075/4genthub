...
class TestUpdateTaskUseCase:
    ...
    def sample_task(self):
        ...
        task.append_progress = (
            Mock()
        )  # Changed from update_details to match implementation
    ...
    def basic_update_request(self):
        ...
        return UpdateTaskRequest(
            task_id="12345678-1234-5678-1234-567812345678", title="Updated Task Title"
        )
    ...
    def test_execute_successful_title_update(
        self, use_case, mock_task_repository, sample_task, basic_update_request
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                # Verify task was found (may be called multiple times during update process)
                assert mock_task_repository.find_by_id.called
                # Check the first call has the right task ID
                first_call_args = mock_task_repository.find_by_id.call_args_list[0][0]
                assert isinstance(first_call_args[0], TaskId)
                ...
                mock_task_response.from_domain.assert_called_once_with(
                    sample_task, git_branch_repository=None
                )
                mock_update_response.success_response.assert_called_once_with(
                    mock_response_instance
                )

    def test_execute_comprehensive_update(
        self, use_case, mock_task_repository, sample_task, comprehensive_update_request
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                sample_task.update_description.assert_called_once_with(
                    "Updated description"
                )
                sample_task.append_progress.assert_called_once_with(
                    "Updated details"
                )  # Changed from update_details to match implementation
                sample_task.update_estimated_effort.assert_called_once_with("4 hours")
                sample_task.update_assignees.assert_called_once_with(
                    ["user-1", "user-2"]
                )
                sample_task.update_labels.assert_called_once_with(
                    ["updated", "important"]
                )
    ...
    def test_execute_task_not_found(
        self, use_case, mock_task_repository, basic_update_request
    ):
    ...
    def test_execute_with_string_task_id(
        self, use_case, mock_task_repository, sample_task
    ):
        """Test task ID conversion from string"""
        # Arrange
        request = UpdateTaskRequest(
            task_id="12345678-1234-5678-1234-567812345678", title="Updated"
        )
        mock_task_repository.find_by_id.return_value = sample_task
        mock_task_repository.save.return_value = None
    ...
    def test_execute_with_integer_task_id(
        self, use_case, mock_task_repository, sample_task
    ):
    ...
    def test_execute_with_task_id_object(
        self, use_case, mock_task_repository, sample_task
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                assert result is not None
                assert mock_task_repository.find_by_id.called
                # Verify the TaskId object was used
                called_task_id = mock_task_repository.find_by_id.call_args_list[0][0][0]
                assert called_task_id == task_id_obj

    def test_execute_context_sync_success(
        self, use_case, mock_task_repository, sample_task, basic_update_request
    ):
    ...
    def test_execute_context_sync_failure_doesnt_fail_update(
        self, use_case, mock_task_repository, sample_task, basic_update_request, caplog
    ):
        ...
        with patch.object(use_case, "_sync_task_context_after_update") as mock_sync:
    ...
    def test_execute_domain_events_processing(
        self, use_case, mock_task_repository, sample_task, basic_update_request
    ):
    ...
    def test_execute_context_id_set_last(
        self, use_case, mock_task_repository, sample_task
    ):
        ...
        sample_task.set_context_id.side_effect = lambda x: call_order.append(
            "context_id"
        )
        ...
        with patch(
            ...
            with patch(
                ...
                assert call_order == [
                    "title",
                    "context_id",
                ]  # context_id should be last

    def test_execute_logging_context_id_updates(
        self, use_case, mock_task_repository, sample_task, caplog
    ):
        """Test logging of context_id updates"""
        # Arrange
        request = UpdateTaskRequest(
            task_id="12345678-1234-5678-1234-567812345678", context_id="new-context-456"
        )
    ...
    def test_execute_individual_field_updates(
        self, use_case, mock_task_repository, sample_task, field_name, field_value
    ):
    ...
    def test_execute_none_values_not_updated(
        self, use_case, mock_task_repository, sample_task
    ):
    ...
    def test_sync_task_context_after_update_no_event_loop(
        self, use_case, mock_task_repository, sample_task, basic_update_request
    ):
    ...
    def test_sync_task_context_in_async_context(
        self, use_case, mock_task_repository, sample_task, basic_update_request, caplog
    ):
