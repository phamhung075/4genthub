...
class TestCreateGitBranchUseCaseExecute:
        ...
        with patch(
            ...
            with patch("fastmcp.config.auth_config.AuthConfig") as mock_auth_config:
                with patch(
                    ...
                    assert (
                        result["git_branch"]["description"] == "Test branch description"
                    )
                    assert result["git_branch"]["project_id"] == "project-123"
                    assert (
                        "Git branch 'feature/test-branch' created successfully"
                        in result["message"]
                    )
                    ...
                    mock_project_repository.save.assert_called_once_with(
                        mock_project_entity
                    )
    ...
    @pytest.mark.asyncio
    async def test_execute_domain_validation_error(
        self, use_case, mock_project_repository, mock_project_entity
    ):
        ...
        mock_project_entity.create_git_branch.side_effect = ValueError(
    ...
    @pytest.mark.asyncio
    async def test_execute_success_without_description(
        self,
        use_case,
        mock_project_repository,
        mock_project_entity,
    ...
    @pytest.mark.asyncio
    async def test_execute_includes_branch_statistics(
        self,
        use_case,
        mock_project_repository,
        mock_project_entity,
...
class TestCreateGitBranchUseCaseContextCreation:
    ...
    @pytest.mark.asyncio
    async def test_execute_context_creation_success(
        self,
        use_case,
        mock_project_repository,
        mock_project_entity,
    ...
    @pytest.mark.asyncio
    async def test_execute_context_creation_with_user_authentication(
        self,
        use_case,
        mock_project_repository,
        mock_project_entity,
            ...
            with patch("fastmcp.config.auth_config.AuthConfig") as mock_auth_config:
                with patch(
                    ...
                    with patch.dict("sys.modules", {"flask": mock_flask_module}):
                        ...
                        assert result["success"] is True
                        mock_validate_user.assert_called_once_with(
                            "authenticated_user", "Branch context creation"
                        )

    @pytest.mark.asyncio
    async def test_execute_context_creation_authentication_required_error(
        self,
        use_case,
        mock_project_repository,
        mock_project_entity,
    ...
    @pytest.mark.asyncio
    async def test_execute_context_creation_failure_does_not_fail_branch_creation(
        self,
        use_case,
        mock_project_repository,
        mock_project_entity,
    ...
    @pytest.mark.asyncio
    async def test_execute_context_creation_exception_does_not_fail_branch_creation(
        self,
        use_case,
        mock_project_repository,
        mock_project_entity,
...
class TestCreateGitBranchUseCaseErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_execute_repository_find_error(
        self, use_case, mock_project_repository
    ):
        """Test git branch creation when repository find raises exception"""
        mock_project_repository.find_by_id.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_execute_repository_update_error(
        self, use_case, mock_project_repository, mock_project_entity
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_invalid_branch_parameters(
        self, use_case, mock_project_repository, mock_project_entity
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_duplicate_branch_name(
        self, use_case, mock_project_repository, mock_project_entity
    ):
        ...
        mock_project_entity.create_git_branch.side_effect = ValueError(
            "Branch name 'main' already exists"
        )

        result = await use_case.execute(
            project_id="project-123", git_branch_name="main", branch_name="main"
        )
