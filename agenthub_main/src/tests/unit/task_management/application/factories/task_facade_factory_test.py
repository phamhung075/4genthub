...
class TestTaskFacadeFactory:
    ...
    @patch(
        "fastmcp.task_management.application.factories.task_facade_factory.ContextServiceFactory"
    )
    def test_singleton_pattern(
        self, mock_context_factory_class, mock_repository_provider
    ):
    ...
    @patch(
        "fastmcp.task_management.application.factories.task_facade_factory.RepositoryProviderService"
    )
    @patch(
        "fastmcp.task_management.application.factories.task_facade_factory.ContextServiceFactory"
    )
    def test_get_instance_first_call(
        self, mock_context_factory_class, mock_provider_service
    ):
    ...
    def test_get_instance_creates_provider(
        self, mock_context_factory_class, mock_provider_service
    ):
    ...
    def test_get_instance_subsequent_calls(
        self, mock_context_factory_class, mock_provider_service
    ):
    ...
    def test_initialization_with_context_factory(
        self,
        ...
        # Setup mock
        mock_context_factory_class.get_instance.return_value = (
            mock_context_service_factory
        )
    ...
    def test_initialization_context_factory_failure(
        self, mock_context_factory_class, mock_repository_provider, caplog
    ):
        """Test initialization when context service factory fails"""
        # Setup mock to raise exception
        mock_context_factory_class.get_instance.side_effect = Exception(
    ...
    def test_singleton_prevents_reinitialization(
        self, mock_context_factory_class, mock_repository_provider
    ):
        ...
        assert factory1 is factory2
        assert (
            factory2._repository_provider == mock_repository_provider
        )  # Original provider
