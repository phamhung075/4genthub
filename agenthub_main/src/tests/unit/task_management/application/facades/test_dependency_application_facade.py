...
class TestDependencyApplicationFacade:
    ...
    @pytest.fixture
    def facade_with_mocked_service(
        self, mock_task_repository, mock_dependency_app_service
    ):
    ...
    def test_add_dependency_success(
        self, facade_with_mocked_service, mock_dependency_app_service
    ):
    ...
    def test_add_dependency_missing_dependency_data(self, facade_with_mocked_service):
        ...
        result = facade_with_mocked_service.manage_dependencies(
            action="add_dependency", task_id="task-123", dependency_data=None
        )
    ...
    def test_remove_dependency_success(
        ...
        mock_dependency_app_service.remove_dependency.assert_called_once_with(
            "task-123", "dep-456"
        )

    def test_remove_dependency_missing_dependency_data(
        self, facade_with_mocked_service
    ):
        """Test remove_dependency with missing dependency_data"""
        result = facade_with_mocked_service.manage_dependencies(
            action="remove_dependency", task_id="task-123", dependency_data=None
        )
    ...
    def test_get_dependencies_success(
        ...
        result = facade_with_mocked_service.manage_dependencies(
            action="get_dependencies", task_id="task-123"
        )
    ...
    def test_clear_dependencies_success(
        self, facade_with_mocked_service, mock_dependency_app_service
    ):
        ...
        result = facade_with_mocked_service.manage_dependencies(
            action="clear_dependencies", task_id="task-123"
        )
        ...
        mock_dependency_app_service.clear_dependencies.assert_called_once_with(
            "task-123"
        )

    def test_get_blocking_tasks_success(
        self, facade_with_mocked_service, mock_dependency_app_service
    ):
        ...
        result = facade_with_mocked_service.manage_dependencies(
            action="get_blocking_tasks", task_id="task-123"
        )
        ...
        mock_dependency_app_service.get_blocking_tasks.assert_called_once_with(
            "task-123"
        )
    ...
    def test_unknown_action(self, facade_with_mocked_service):
        ...
        result = facade_with_mocked_service.manage_dependencies(
            action="unknown_action", task_id="task-123"
        )
    ...
    def test_task_not_found_error(
        ...
        mock_dependency_app_service.get_dependencies.side_effect = TaskNotFoundError(
            "Task not found"
        )

        result = facade_with_mocked_service.manage_dependencies(
            action="get_dependencies", task_id="nonexistent-task"
        )
    ...
    def test_value_error_handling(
        ...
        mock_dependency_app_service.add_dependency.side_effect = ValueError(
            "Invalid dependency"
        )
    ...
    def test_unexpected_exception_handling(
        ...
        mock_dependency_app_service.get_dependencies.side_effect = Exception(
            "Unexpected error"
        )

        with patch(
            "fastmcp.task_management.application.facades.dependency_application_facade.logger"
        ) as mock_logger:
            result = facade_with_mocked_service.manage_dependencies(
                action="get_dependencies", task_id="task-123"
            )
    ...
    def test_all_actions_with_valid_data(
        self, facade_with_mocked_service, mock_dependency_app_service
    ):
        ...
        mock_dependency_app_service.remove_dependency.return_value = (
            mock_remove_response
        )
        mock_dependency_app_service.clear_dependencies.return_value = (
            mock_clear_response
        )
        mock_dependency_app_service.get_dependencies.return_value = {
            "dependencies": ["dep-1"]
        ...
        for action, dependency_data in actions_and_data:
            result = facade_with_mocked_service.manage_dependencies(
                action=action, task_id="task-123", dependency_data=dependency_data
            )
...
class TestDependencyApplicationFacadeIntegration:
    ...
    def test_dependency_lifecycle(self):
        ...
        with (
            patch.object(facade._dependency_app_service, "add_dependency") as mock_add,
            patch.object(
                facade._dependency_app_service, "get_dependencies"
            ) as mock_get,
            patch.object(
                facade._dependency_app_service, "remove_dependency"
            ) as mock_remove,
        ):
            # Configure responses
            mock_add.return_value = MockDependencyResponse(
                True, "task-1", ["dep-1"], "Added"
            )
            mock_get.return_value = {"dependencies": ["dep-1"], "dependency_count": 1}
            mock_remove.return_value = MockDependencyResponse(
                True, "task-1", [], "Removed"
            )
            ...
            get_result = facade.manage_dependencies(
                action="get_dependencies", task_id="task-1"
            )
