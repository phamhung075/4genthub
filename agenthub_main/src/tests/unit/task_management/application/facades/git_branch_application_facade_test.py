...
class TestGitBranchApplicationFacade:
    ...
    @patch('fastmcp.task_management.domain.services.git_branch_name_validator.GitBranchNameValidator')
    async def test_create_tree_async(self, mock_validator_class, mock_repo_provider_class, facade, mock_git_branch_service):
        ...
        # Make validate_branch_name an async mock that returns None
        async def mock_validate(*args, **kwargs):
            return None
        mock_validator.validate_branch_name = mock_validate
    ...
    def test_update_git_branch(self, facade):
        ...
        # Mock the WebSocket notification service static method to prevent database access
        with patch('fastmcp.task_management.application.facades.git_branch_application_facade.WebSocketNotificationService.sync_broadcast_branch_event') as mock_broadcast:
            # Mock get_git_branch_by_id to return a valid branch
            facade.get_git_branch_by_id = MagicMock(return_value={
                "success": True,
                "git_branch": {
                    "id": "branch-id-123",
                    "name": "test-branch",
    ...
    def test_delete_git_branch_sync_success(self, facade, mock_git_branch_service):
        ...
        # Mock the async delete method
        async def mock_delete_func(*args, **kwargs):
            return expected_result
        
        mock_git_branch_service.delete_git_branch = mock_delete_func
        
        # Mock WebSocket notification
        with patch('fastmcp.task_management.application.facades.git_branch_application_facade.WebSocketNotificationService.sync_broadcast_branch_event'):
            with patch('asyncio.get_running_loop', side_effect=RuntimeError):
                with patch('asyncio.run', return_value=expected_result):
