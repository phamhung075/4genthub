...
class TestTaskCountRegression(unittest.TestCase):
    ...
    def test_repository_has_correct_method(self):
        ...
        with patch(
            "fastmcp.task_management.infrastructure.repositories.orm.task_repository.ORMTaskRepository"
        ) as MockRepo:
    ...
    def test_facade_uses_correct_method_call(self):
        ...
        source1 = inspect.getsource(
            GitBranchApplicationFacade.get_branches_with_task_counts
        )
    ...
    def test_task_counting_logic(self):
        ...
        self.assertEqual(
            in_progress_tasks, 1, "In progress task count should be correct"
        )
        ...
        self.assertGreater(
            total_tasks, 0, "CRITICAL: Task counts must not be 0 when tasks exist"
        )
