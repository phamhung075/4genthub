...
class TestContextCacheServiceInit:
    ...
    def test_initialization_with_parameters(self):
        ...
        mock_repo = Mock()
        service = ContextCacheService(
            repository=mock_repo, default_ttl_hours=4, user_id="test_user_123"
        )
...
class TestSyncWrapperMethods:
    ...
    def test_get_context_sync_wrapper_running_loop(self):
        ...
        with patch("asyncio.get_event_loop", return_value=mock_loop):
            with patch(
                "fastmcp.task_management.application.services.context_cache_service.logger"
            ) as mock_logger:
                result = service.get_context("task", "test_123")
...
class TestCacheStorage:
    ...
    async def test_cache_resolved_context_default_ttl(self):
        ...
        assert (
            abs(time_diff.total_seconds() - (4 * 3600)) < 60
...
class TestCacheInvalidation:
    ...
    async def test_invalidate_context_cache_success(self):
        ...
        result = await service.invalidate_context_cache(
            "task", "test_123", "manual_test"
        )

        assert result is True
        mock_repo.invalidate_cache_entry.assert_called_once_with(
            "task", "test_123", "manual_test"
        )
    ...
    async def test_invalidate_dependent_caches_global_changes(self):
        ...
        invalidated = await service.invalidate_dependent_caches(
            "global", "global_singleton", "global_update"
        )
    ...
    async def test_invalidate_dependent_caches_project_changes(self):
        ...
        invalidated = await service.invalidate_dependent_caches(
            "project", "proj123", "project_update"
        )
...
class TestCacheStatistics:
    ...
    async def test_get_cache_stats_success(self):
        ...
        mock_stats = {
            ...
            "expired_count": 5,
            "invalidated_count": 2,
            "entries_by_level": {"global": 1, "project": 10, "task": 89},
        }
        ...
        assert (
            perf_metrics["cache_efficiency"] == "medium"
        )  # hit_rate calculation changed

    @pytest.mark.asyncio
    async def test_get_cache_stats_empty_cache(self):
        """Test cache statistics with empty cache."""
        mock_repo = AsyncMock()
        mock_stats = {"total_entries": 0, "total_size_bytes": 0, "total_hits": 0}
        mock_repo.get_cache_statistics.return_value = mock_stats
...
class TestCacheOptimization:
    ...
    async def test_optimize_cache_comprehensive(self):
        ...
        cleanup_expired_result = {
            "success": True,
            "removed_count": 10,
            "size_freed_bytes": 5000,
        }
        cleanup_invalidated_result = {
            "success": True,
            "removed_count": 3,
            "size_freed_bytes": 1500,
        }
        ...
        with patch.object(
            service, "cleanup_expired", return_value=cleanup_expired_result
        ):
            with patch.object(
                service, "cleanup_invalidated", return_value=cleanup_invalidated_result
            ):
    ...
    async def test_optimize_cache_no_pressure(self):
        ...
        assert result["success"] is True
        assert (
            len(result["actions_taken"]) == 2
        )  # Only cleanup actions, no low-hit removal
    ...
    async def test_optimize_cache_exception(self):
        ...
        with patch.object(
            service, "cleanup_expired", side_effect=Exception("Cleanup failed")
        ):
            result = await service.optimize_cache()
