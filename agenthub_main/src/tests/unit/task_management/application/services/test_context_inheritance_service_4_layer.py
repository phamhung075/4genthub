...
class TestContextInheritanceService4Layer:
    ...
    def setup_method(self):
        ...
        self.global_context = {
            "autonomous_rules": {"agent_switching": True, "context_resolution": True},
            "security_policies": {"mfa_required": True, "audit_logging": True},
            "coding_standards": {"python_style": "PEP8", "test_coverage": 80},
            "delegation_rules": {
                "auto_delegate": {"patterns": True, "best_practices": True},
                "thresholds": {"complexity": 5, "impact": 3},
            },
            "metadata": {"version": 1},
        }
        ...
        self.project_context = {
            ...
            "project_workflow": {"git_flow": True, "automated_testing": True},
            "local_standards": {"max_line_length": 120},
            "global_overrides": {"coding_standards.test_coverage": 90},
            "delegation_rules": {"auto_delegate": {"code_patterns": True}},
            "metadata": {"version": 2},
        }

        # Sample branch context
        self.branch_context = {
            "branch_workflow": {"feature_flags": True, "continuous_integration": True},
            "branch_standards": {
        ...
        # Sample task context
        self.task_context = {
            "task_data": {"implementation_approach": "TDD", "estimated_complexity": 4},
            "local_overrides": {
            ...
            "delegation_triggers": {"auto_delegate_patterns": True},
    ...
    def test_inherit_branch_from_project(self):
        ...
        assert (
            result["coding_standards"]["test_coverage"] == 90
    ...
    def test_inherit_task_from_branch(self):
        ...
        assert (
            result["security_policies"]["mfa_required"] is False
        )  # From branch override
        ...
        assert (
            result["implementation_notes"]["special_requirements"]
    ...
    def test_full_inheritance_chain(self):
        ...
        assert (
            final_result["implementation_notes"]["special_requirements"]
            == "Performance critical"
        )

        # Verify override precedence (most specific wins)
        assert (
            final_result["coding_standards"]["test_coverage"] == 90
        )  # Project override
        assert (
            final_result["security_policies"]["mfa_required"] is False
        )  # Branch override
        assert (
            final_result["coding_standards"]["max_line_length"] == 100
        )  # Task override

        # Verify delegation rules are properly merged
        assert (
            final_result["delegation_rules"]["auto_delegate"]["patterns"] is True
        )  # Global
        assert (
            final_result["delegation_rules"]["auto_delegate"]["code_patterns"] is True
        )  # Project
        assert (
            final_result["delegation_rules"]["thresholds"]["complexity"] == 3
        )  # Branch override
    ...
    def test_complex_override_scenarios(self):
        ...
        global_ctx = {"settings": {"timeout": 30, "retries": 3, "debug": False}}

        project_ctx = {
            "global_overrides": {"settings.timeout": 60, "settings.debug": True}
        }

        branch_ctx = {"local_overrides": {"settings.retries": 5}}

        task_ctx = {
            "local_overrides": {"settings.timeout": 90, "settings.debug": False}
        }
    ...
    def test_empty_contexts(self):
        ...
        assert task_inherited["inheritance_metadata"]["inheritance_chain"] == [
            "global",
            "project",
            "branch",
            "task",
