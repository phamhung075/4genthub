...
class TestGitBranchService:
    ...
    def service(
        ...
        mock_hierarchical_context_service,
        monkeypatch,
    ):
    ...
    def test_init_with_user_id(
        self, mock_project_repo, mock_git_branch_repo, monkeypatch
    ):
    ...
    async def test_create_git_branch_success(
        ...
        mock_git_branch_repo.find_by_name.assert_called_once_with(
            "project-123", "feature/test"
        )
        mock_git_branch_repo.create_branch.assert_called_once_with(
            "project-123", "feature/test", "Test branch"
        )

    @pytest.mark.asyncio
    async def test_create_git_branch_project_not_found(
        self, service, mock_project_repo, mock_git_branch_repo
    ):
        """Test git branch creation when project is not found"""
        mock_project_repo.find_by_id.return_value = None

        result = await service.create_git_branch(
            project_id="project-999", branch_name="feature/test"
        )
    ...
    async def test_create_git_branch_already_exists(
        ...
        result = await service.create_git_branch(
            project_id="project-123", branch_name="feature/test"
        )

        assert result["success"] is False
        assert (
            result["error"]
            == "Git branch 'feature/test' already exists in project project-123"
        )
    ...
    @pytest.mark.asyncio
    async def test_get_git_branch_by_id_success(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_git_branch_success(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_git_branch_name_only(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
        ...
        result = await service.update_git_branch(
            git_branch_id="branch-456", branch_name="feature/new-name"
        )
    ...
    @pytest.mark.asyncio
    async def test_update_git_branch_description_only(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
        ...
        result = await service.update_git_branch(
            git_branch_id="branch-456", description="New description"
        )
    ...
    async def test_update_git_branch_not_found(self, service, mock_git_branch_repo):
        ...
        result = await service.update_git_branch(
            git_branch_id="branch-999", branch_name="feature/updated"
        )
    ...
    @pytest.mark.asyncio
    async def test_update_git_branch_no_fields(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
        ...
        assert (
            result["error"]
            == "No fields to update. Provide branch_name and/or description."
        )
    ...
    async def test_update_git_branch_exception(self, service, mock_git_branch_repo):
        ...
        result = await service.update_git_branch(
            git_branch_id="branch-456", branch_name="feature/updated"
        )
    ...
    @pytest.mark.asyncio
    async def test_delete_git_branch_success(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
    ...
    @pytest.mark.asyncio
    async def test_delete_git_branch_failed(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
    ...
    @pytest.mark.asyncio
    async def test_assign_agent_to_branch_success(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
        ...
        result = await service.assign_agent_to_branch(
            "project-123", "agent-1", "feature/test"
        )
        ...
        mock_git_branch_repo.assign_agent.assert_called_once_with(
            "project-123", "branch-456", "agent-1"
        )

    @pytest.mark.asyncio
    async def test_assign_agent_to_branch_not_found(
        self, service, mock_git_branch_repo
    ):
        """Test agent assignment when branch is not found"""
        mock_git_branch_repo.find_by_name.return_value = None

        result = await service.assign_agent_to_branch(
            "project-123", "agent-1", "feature/nonexistent"
        )

        assert result["success"] is False
        assert (
            result["error"]
            == "Git branch feature/nonexistent not found in project project-123"
        )

    @pytest.mark.asyncio
    async def test_assign_agent_to_branch_failed(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
        ...
        assert (
            result["error"]
            == "Failed to assign agent agent-1 to git branch feature/test"
        )

    @pytest.mark.asyncio
    async def test_assign_agent_to_branch_exception(
        self, service, mock_git_branch_repo
    ):
    ...
    @pytest.mark.asyncio
    async def test_unassign_agent_from_branch_success(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
        ...
        result = await service.unassign_agent_from_branch(
            "project-123", "agent-1", "feature/test"
        )

        assert result["success"] is True
        assert (
            result["message"] == "Agent agent-1 unassigned from git branch feature/test"
        )

        mock_git_branch_repo.find_by_name.assert_called_once_with(
            "project-123", "feature/test"
        )
        mock_git_branch_repo.unassign_agent.assert_called_once_with(
            "project-123", "branch-456"
        )

    @pytest.mark.asyncio
    async def test_unassign_agent_from_branch_not_found(
        self, service, mock_git_branch_repo
    ):
    ...
    @pytest.mark.asyncio
    async def test_unassign_agent_from_branch_failed(
        self, service, mock_git_branch_repo, mock_git_branch
    ):
        ...
        assert (
            result["error"]
            == "Failed to unassign agent agent-1 from git branch feature/test"
        )

    @pytest.mark.asyncio
    async def test_unassign_agent_from_branch_exception(
        self, service, mock_git_branch_repo
    ):
    ...
    async def test_get_branch_statistics_success(self, service, mock_git_branch_repo):
        ...
        mock_git_branch_repo.get_branch_statistics.assert_called_once_with(
            "project-123", "branch-456"
        )
    ...
    async def test_get_branch_statistics_error(self, service, mock_git_branch_repo):
        ...
        mock_git_branch_repo.get_branch_statistics.return_value = {
    ...
    async def test_get_branch_statistics_exception(self, service, mock_git_branch_repo):
        ...
        mock_git_branch_repo.get_branch_statistics.side_effect = Exception(
            "Database error"
        )
    ...
    async def test_archive_branch_success(self, service, mock_git_branch_repo):
        ...
        mock_git_branch_repo.archive_branch.assert_called_once_with(
            "project-123", "branch-456"
        )
    ...
    async def test_restore_branch_success(self, service, mock_git_branch_repo):
        ...
        mock_git_branch_repo.restore_branch.assert_called_once_with(
            "project-123", "branch-456"
        )
