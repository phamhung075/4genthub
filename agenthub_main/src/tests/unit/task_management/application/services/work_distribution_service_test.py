...
class TestWorkDistributionServiceInitialization:
    ...
    def test_initialization_minimal(self):
        ...
        service = WorkDistributionService(task_repository=self.task_repository)
...
class TestUserScopedRepository:
    ...
    def setup_method(self):
        ...
        self.service = WorkDistributionService(
            task_repository=self.task_repository, user_id="test_user"
        )
    ...
    def test_get_user_scoped_repository_with_user_id_attribute(self):
        """Test getting user-scoped repository with user_id attribute and different user"""
        ...
        with patch("builtins.type", return_value=mock_repo_class):
            ...
            mock_repo_class.assert_called_once_with(
                mock_repo.session, user_id="test_user"
            )
...
class TestTaskDistribution:
    ...
    async def test_distribute_tasks_round_robin_strategy(self):
        ...
        with patch.object(self.service, "_execute_distribution_plan") as mock_execute:
            plan = await self.service.distribute_tasks(
                ["task1", "task2"], strategy=DistributionStrategy.ROUND_ROBIN
            )
    ...
    async def test_distribute_tasks_load_balanced_strategy(self):
        ...
        with patch.object(self.service, "_execute_distribution_plan"):
            plan = await self.service.distribute_tasks(
                ["task1", "task2"], strategy=DistributionStrategy.LOAD_BALANCED
            )
    ...
    async def test_distribute_tasks_skill_matched_strategy(self):
        ...
        with patch.object(self.service, "_execute_distribution_plan"):
            plan = await self.service.distribute_tasks(
                ["task1"], strategy=DistributionStrategy.SKILL_MATCHED
            )
    ...
    async def test_distribute_tasks_priority_based_strategy(self):
        ...
        self.task_repository.get.side_effect = [
            high_priority_task,
        ...
        with patch.object(self.service, "_execute_distribution_plan"):
            plan = await self.service.distribute_tasks(
                ["high_task", "med_task"], strategy=DistributionStrategy.PRIORITY_BASED
            )

            assert len(plan.assignments) == 2
            # High priority task should go to better agent first
            high_task_assignment = next(
                a for a in plan.assignments if a[0] == "high_task"
            )
    ...
    async def test_distribute_tasks_hybrid_strategy(self):
        ...
        with patch.object(self.service, "_execute_distribution_plan") as mock_execute:
            plan = await self.service.distribute_tasks(
                ["priority", "skilled", "regular"], strategy=DistributionStrategy.HYBRID
            )
    ...
    async def test_distribute_tasks_with_project_filter(self):
        ...
        with patch.object(self.service, "_execute_distribution_plan"):
            plan = await self.service.distribute_tasks(["task1"], project_id=project_id)
...
class TestDistributionStrategies:
    ...
    async def test_distribute_skill_matched(self):
        ...
        self.service.coordination_service.find_best_agent_for_task.side_effect = [
            self.agents[0],
            self.agents[1],
            None,  # Third task has no matching agent
        ]
        ...
        assert (
            self.service.coordination_service.find_best_agent_for_task.call_count == 3
        )
    ...
    async def test_distribute_priority_based(self):
        ...
        await self.service._distribute_priority_based(
            self.tasks, self.agents, self.plan
        )
    ...
    async def test_distribute_hybrid(self):
        ...
        # Mock agent matching
        with patch.object(
            self.service, "_agent_matches_requirements", return_value=True
        ):
...
class TestAgentPerformanceAndMatching:
    ...
    def test_get_agent_performance_score_cached(self):
        ...
        agent.average_task_duration = None  # No duration data
    ...
    def test_agent_matches_requirements_excluded(self):
        ...
        requirements = TaskRequirements(task_id="task1", excluded_agents=["agent1"])
    ...
    def test_agent_matches_requirements_preferred(self):
        ...
        requirements = TaskRequirements(task_id="task1", preferred_agents=["agent1"])
    ...
    def test_agent_matches_requirements_developer_role(self):
        ...
        requirements = TaskRequirements(
            task_id="task1", required_role=AgentRole.DEVELOPER
        )
    ...
    def test_agent_matches_requirements_tester_role(self):
        ...
        requirements = TaskRequirements(task_id="task1", required_role=AgentRole.TESTER)
...
class TestPlanExecution:
    ...
    def setup_method(self):
        ...
        self.coordination_service = AsyncMock()
        self.service = WorkDistributionService(
            task_repository=Mock(), coordination_service=self.coordination_service
        )
...
class TestDistributionHistory:
    ...
    async def test_get_distribution_analytics_with_history(self):
        ...
        assert analytics["strategy_success_rates"]["round_robin"] == (
            5 / 7 * 100
        )  # 71.43%
        assert analytics["strategy_success_rates"]["skill_matched"] == (
            2 / 2 * 100
        )  # 100%
...
class TestIntegration:
    ...
    async def test_full_distribution_workflow(self):
        ...
        plan = await self.service.distribute_tasks(
            ["task1"], strategy=DistributionStrategy.LOAD_BALANCED
        )
    ...
    async def test_complex_hybrid_distribution(self):
        ...
        skilled_task = self._create_task(
            "skilled", PriorityLevel.MEDIUM.label, {"required_role": "developer"}
        )
        regular_task = self._create_task("regular", PriorityLevel.LOW.label, {})

        # Create agents with different capabilities
        expert_agent = self._create_agent(
            "expert", 95.0, 4.0, [AgentCapability.BACKEND_DEVELOPMENT]
        )
        regular_agent = self._create_agent(
            "regular", 80.0, 6.0, [AgentCapability.FRONTEND_DEVELOPMENT]
        )
        ...
        plan = await self.service.distribute_tasks(
            ["critical", "skilled", "regular"], strategy=DistributionStrategy.HYBRID
        )
    ...
    def _create_task(
        self, task_id: str, priority: str, metadata: dict[str, Any]
    ) -> Mock:
