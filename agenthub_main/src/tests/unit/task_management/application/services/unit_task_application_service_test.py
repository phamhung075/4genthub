...
class TestTaskApplicationService:
    ...
    def service(
        ...
        with patch(
            ...
            service = TaskApplicationService(
                mock_task_repository, mock_context_service, user_id="test-user-123"
            )
    ...
    async def test_create_task_success(
        self, service, mock_use_cases, mock_hierarchical_context_service, mock_task
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_task_no_context_on_failure(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
    ...
    async def test_get_task_not_found(self, service, mock_use_cases):
        ...
        mock_use_cases["get"].execute.side_effect = TaskNotFoundError(
    ...
    async def test_update_task_success(
        ...
        request = UpdateTaskRequest(
            task_id="task-123", title="Updated Task", status="in_progress"
        )
    ...
    @pytest.mark.asyncio
    async def test_delete_task_no_context_cleanup_on_failure(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
    ...
    async def test_complete_task(self, service, mock_use_cases):
        ...
        result = await service.complete_task(
            task_id, completion_summary=completion_summary
        )
    ...
    @pytest.mark.asyncio
    async def test_update_task_no_context_update_on_failure(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
        """Test that context is not updated when task update fails"""
        request = UpdateTaskRequest(task_id="task-123", title="Updated Task")
    ...
    async def test_complete_task_with_testing_notes(self, service, mock_use_cases):
        ...
        testing_notes = (
            "Added unit tests for auth service, integration tests for login flow"
        )
        expected_result = {
            "success": True,
            "task_id": task_id,
        ...
        # The complete_task method now accepts additional parameters
        result = await service.complete_task(
            task_id, completion_summary=completion_summary, testing_notes=testing_notes
        )
    ...
    @pytest.mark.asyncio
    async def test_create_task_handles_task_without_value_attribute(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_task_with_empty_response(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
        """Test task creation with empty response"""
        request = CreateTaskRequest(title="Test Task", git_branch_id="branch-123")
    ...
    @pytest.mark.asyncio
    async def test_create_task_with_none_task(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_task_with_none_task(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_task_handles_task_without_value_attribute(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_service_methods_with_different_parameter_combinations(
        self, service, mock_use_cases
    ):
    ...
    @pytest.mark.asyncio
    async def test_delete_task_with_different_parameters(
        self, service, mock_use_cases, mock_hierarchical_context_service
    ):
    ...
    def test_repository_scoping_with_user_id_property_same_user(
    ...
    @pytest.mark.asyncio
    async def test_context_service_error_handling(
        self, service, mock_use_cases, mock_hierarchical_context_service, mock_task
    ):
        ...
        mock_hierarchical_context_service.create_context.side_effect = Exception(
    ...
    async def test_multiple_concurrent_operations(self, service, mock_use_cases):
        ...
        mock_use_cases["list"].execute.return_value = TaskListResponse(
            tasks=[], count=0
        )
    ...
    def test_service_initialization_with_different_configurations(
        self, mock_task_repository
    ):
    ...
    async def test_full_task_lifecycle(
        ...
        create_request = CreateTaskRequest(
            title="Lifecycle Test", git_branch_id="branch-123"
        )
