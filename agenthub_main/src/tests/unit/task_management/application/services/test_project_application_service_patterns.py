...
class TestProjectServiceBehavior:
    ...
    async def test_project_lifecycle_orchestration(self):
        ...
        class ProjectService:
            ...
            async def create_git_branch(
                self, project_id, git_branch_name, tree_name, tree_description=""
            ):
                return await tree_use_case.execute(
    ...
    async def test_agent_management_workflow(self):
        ...
        class ProjectService:
            ...
            async def unregister_agent(self, project_id, agent_id):
                ...
                trees_to_unassign = [
                    tree
                    for tree, aid in project.agent_assignments.items()
                    if aid == agent_id
                ]
    ...
    async def test_error_handling_patterns(self):
        ...
        result = await register_agent_with_error_handling(
            "non-existent", "agent-1", "Test"
        )
        ...
        result = await assign_with_error_handling(
            "project-1", "agent-1", "non-existent"
        )
        assert result["success"] is False
