...
class TestUserScopedRepository:
    ...
    def test_get_user_scoped_repository_with_user_id_attribute(self):
        ...
        with patch("builtins.type", return_value=MockRepoClass):
            ...
            MockRepoClass.assert_called_once_with(
    ...
    async def test_analyze_task_workflow_success(self):
        ...
        with patch.object(self.service, "_get_related_tasks", return_value=[]):
            with patch.object(self.service, "_detect_patterns", return_value=[]):
                with patch.object(
                    self.service, "_identify_bottlenecks", return_value=[]
                ):
                    with patch.object(
                        self.service,
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                return_value=["test risk"],
                            ):
                                with patch.object(
                                    self.service,
                                    "_identify_success_indicators",
                                    return_value=["test indicator"],
                                ):
            ...
            with patch.object(self.service, "_detect_patterns", return_value=[]):
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            self.service, "_predict_completion_time", return_value=None
                        ):
                            with patch.object(
                                self.service, "_assess_risk_factors", return_value=[]
                            ):
                                with patch.object(
                                    self.service,
                                    "_identify_success_indicators",
    ...
    async def test_analyze_task_workflow_without_related_tasks(self):
        ...
        with patch.object(self.service, "_detect_patterns", return_value=[]):
            with patch.object(self.service, "_identify_bottlenecks", return_value=[]):
                with patch.object(
                    self.service, "_find_optimization_opportunities", return_value=[]
                ):
                    with patch.object(
                        self.service, "_predict_completion_time", return_value=None
                    ):
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                result = await self.service.analyze_task_workflow(
                                    self.task_id, include_related=False
                                )
...
class TestTaskContextAndRelatedTasks:
    ...
    async def test_get_task_context_exception(self):
        ...
        self.context_repository.get_by_task_id.side_effect = Exception(
            "Context not found"
        )
...
class TestPatternDetection:
    ...
    def test_detect_completion_patterns_extended_duration(self):
        ...
        task = Mock()
        task.id = self.task_id
        task.created_at = datetime.now(UTC) - timedelta(
            hours=20
        )  # Current duration: 20h
        ...
        for i in range(5):
            ...
            related_task.updated_at = datetime.now(UTC) - timedelta(
                hours=16
            )  # 8h duration
    ...
    def test_detect_collaboration_patterns_multiple_indicators(self):
        ...
        context = Mock()
        context.notes = (
            "Need help with this implementation, stuck on review"  # Contains keywords
        )
...
class TestOptimizationOpportunities:
    ...
    async def test_find_optimization_opportunities_task_decomposition(self):
        ...
        opportunities = await self.service._find_optimization_opportunities(
            task, None, []
        )
    ...
    async def test_find_optimization_opportunities_automation(self):
        ...
        context = Mock()
        context.notes = (
            "This task involves manual copying and repetitive work every time"
        )
...
class TestWorkflowRecommendations:
    ...
    async def test_get_workflow_recommendations(self):
        ...
        with patch.object(self.service, "analyze_task_workflow", return_value=analysis):
            recommendations = await self.service.get_workflow_recommendations(
                self.task_id
            )
        ...
        bottleneck_rec = next(
            r for r in recommendations if r["source"] == "bottleneck_progress_stall"
        )
        ...
        opt_rec = next(
            r for r in recommendations if r["source"] == "optimization_automation"
        )
...
class TestIntegration:
    ...
    async def test_full_workflow_analysis(self):
        ...
        task.created_at = datetime.now(UTC) - timedelta(
            days=15
        )  # Long running (>14 days for collaboration indicator)
        ...
        assert any(
            "High priority task with low progress" in risk
    ...
    async def test_caching_behavior(self):
        ...
        task.created_at = datetime.now(UTC) - timedelta(
            days=5
        )  # Add created_at for datetime operations
