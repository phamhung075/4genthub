...
class TestAgentInheritanceService:
    ...
    def setup_method(self):
        ...
        self.service = AgentInheritanceService(
            self.task_repository, self.subtask_repository
        )
    ...
    def test_apply_agent_inheritance_parent_task_no_assignees(self):
        ...
        # Execute inheritance
        result = self.service.apply_agent_inheritance(
            self.subtask_no_assignees, parent_no_assignees
        )
    ...
    def test_apply_inheritance_to_all_subtasks(self):
        ...
        updated_subtasks = self.service.apply_inheritance_to_all_subtasks(
            self.parent_task.id
        )

        # Verify repositories were called
        self.task_repository.find_by_id.assert_called_once_with(self.parent_task.id)
        self.subtask_repository.find_by_parent_task_id.assert_called_once_with(
            self.parent_task.id
        )
    ...
    def test_inheritance_with_logging_verification(self):
        ...
        with patch(
            "fastmcp.task_management.application.services.agent_inheritance_service.logger"
        ) as mock_logger:
            # Apply inheritance
            self.service.apply_agent_inheritance(
                self.subtask_no_assignees, self.parent_task
            )
