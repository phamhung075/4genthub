...
class TestTaskContextSyncService:
    ...
    def mock_unified_context_facade_factory(self, mock_context_service):
        ...
        with patch(
            "fastmcp.task_management.application.services.facade_service.FacadeService.get_unified_context_facade"
        ) as mock_get_facade:
    ...
    def service(
        ...
        mock_unified_context_facade_factory,
    ):
        """Create service instance with mocked dependencies"""
        with patch(
            "fastmcp.task_management.application.services.task_context_sync_service.GetTaskUseCase"
        ):
    ...
    def service_with_user(
        ...
        with patch(
            ...
            service = TaskContextSyncService(
                mock_task_repository, mock_context_service, user_id="user-123"
            )
            return service
    ...
    def test_service_initialization(
        self, mock_task_repository, mock_unified_context_facade_factory
    ):
    ...
    def test_service_initialization_with_user(
        self, mock_task_repository, mock_unified_context_facade_factory
    ):
...
class TestTaskContextSyncServiceSyncContext:
    ...
    @pytest.fixture
    def service(
        self, mock_task_repository, mock_context_service, mock_get_task_use_case
    ):
        """Create service instance with mocked dependencies"""
        with patch(
            "fastmcp.task_management.application.services.facade_service.FacadeService.get_unified_context_facade"
            ...
            with patch(
                ...
                service = TaskContextSyncService(
                    mock_task_repository, mock_context_service
                )
                return service
    ...
    @patch(
        "fastmcp.task_management.application.services.task_context_sync_service.validate_user_id"
    )
    @pytest.mark.asyncio
    async def test_sync_context_and_get_task_success_create_context(
        self,
        mock_validate_user,
        service,
        mock_task_repository,
        mock_context_service,
        mock_get_task_use_case,
        mock_task_entity,
    ):
        ...
        with patch(
            "fastmcp.task_management.infrastructure.repositories.orm.git_branch_repository.ORMGitBranchRepository"
            ...
            mock_context_service.get_context.assert_called_once_with(
                level="task", context_id="12345678-1234-5678-1234-567812345678"
            )
    ...
    @pytest.mark.asyncio
    async def test_sync_context_and_get_task_success_update_context(
        self,
        mock_validate_user,
        service,
        mock_task_repository,
        ...
        result = await service.sync_context_and_get_task(
            "task-123", user_id="user-123", project_id="project-456"
        )
    ...
    @pytest.mark.asyncio
    async def test_sync_context_and_get_task_task_not_found(
        self, mock_validate_user, service, mock_task_repository
    ):
        ...
        result = await service.sync_context_and_get_task(
            "12345678-1234-1234-1234-123456789012", user_id="user-123"
        )
    ...
    @pytest.mark.asyncio
    async def test_sync_context_and_get_task_project_id_from_git_branch(
        self,
        mock_validate_user,
        service,
        mock_task_repository,
    ...
    @pytest.mark.asyncio
    async def test_sync_context_and_get_task_default_project_id(
        self,
        mock_validate_user,
        service,
        mock_task_repository,
        ...
        with patch(
            ...
            result = await service.sync_context_and_get_task(
                "task-123", user_id="user-123", project_id="test-project"
            )
    ...
    @pytest.mark.asyncio
    async def test_sync_context_and_get_task_error_handling(
        self, mock_validate_user, service, mock_task_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_context_and_get_task_context_data_structure(
        self,
        mock_validate_user,
        service,
        mock_task_repository,
        ...
        await service.sync_context_and_get_task(
            "task-123", user_id="user-123", project_id="test-project"
        )
    ...
    @pytest.mark.asyncio
    async def test_sync_context_and_get_task_with_user_scoped_repository(
        self, mock_validate_user, mock_task_repository
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                service = TaskContextSyncService(
                    mock_task_repository, user_id="user-123"
                )
                await service.sync_context_and_get_task(
                    "task-123", user_id="user-123", project_id="user-project"
                )
...
class TestTaskContextSyncServiceErrorScenarios:
    ...
    @pytest.mark.asyncio
    async def test_sync_context_user_authentication_error(
        self, mock_validate_user, service
    ):
    ...
    @patch(
        "fastmcp.task_management.application.services.task_context_sync_service.TaskId"
    )
    @pytest.mark.asyncio
    async def test_sync_context_invalid_task_id(
        self, mock_task_id_class, mock_validate_user, service
    ):
        ...
        mock_task_id_class.from_string.side_effect = ValueError(
            "Invalid task ID format"
        )

        with pytest.raises(ValueError) as exc_info:
            await service.sync_context_and_get_task(
                "invalid-task-id", user_id="user-123"
            )
    ...
    @pytest.mark.asyncio
    async def test_sync_context_git_branch_repository_error(
        self, mock_validate_user, service, mock_task_repository
    ):
...
class TestTaskContextSyncServiceSyncSubtaskCounts:
    ...
    @pytest.mark.asyncio
    async def test_sync_subtask_counts_calculates_correctly(
        self, service, mock_subtask_repository, mock_context_service, mock_subtasks
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_subtask_counts_updates_progress_percentage(
        self, service, mock_subtask_repository, mock_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_subtask_counts_builds_items_array(
        self, service, mock_subtask_repository, mock_context_service, mock_subtasks
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_subtask_counts_handles_empty_subtasks(
        self, service, mock_subtask_repository, mock_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_subtask_counts_handles_missing_context_gracefully(
        self, service, mock_subtask_repository, mock_context_service, mock_subtasks
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_subtask_counts_handles_repository_error(
        self, service, mock_subtask_repository, mock_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_subtask_counts_with_user_scoped_repository(
        self,
        mock_task_repository,
...
class TestTaskContextSyncServiceSyncTaskStatus:
    ...
    @pytest.mark.asyncio
    async def test_sync_task_status_updates_metadata(
        self, service, mock_context_service
    ):
        ...
        mock_context_service.get_context.return_value = {
            "success": True,
            "context_data": {"metadata": {"status": "todo"}},
        }
    ...
    @pytest.mark.asyncio
    async def test_sync_task_status_creates_metadata_section(
        self, service, mock_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_task_status_handles_missing_context(
        self, service, mock_context_service
    ):
        """Test that sync handles missing context gracefully"""
        task_id = "12345678-1234-5678-1234-567812345678"
    ...
    async def test_sync_task_status_handles_error(self, service, mock_context_service):
        ...
        mock_context_service.get_context.side_effect = Exception(
...
class TestTaskContextSyncServiceSyncTaskMetadata:
    ...
    @pytest.mark.asyncio
    async def test_sync_task_metadata_copies_all_fields(
        self, service, mock_context_service, mock_task_entity
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_task_metadata_adds_at_prefix_to_assignees(
        self, service, mock_context_service, mock_task_entity
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_task_metadata_creates_sections_if_missing(
        self, service, mock_context_service, mock_task_entity
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_task_metadata_handles_missing_optional_fields(
        self, service, mock_context_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_task_metadata_handles_missing_context(
        self, service, mock_context_service, mock_task_entity
    ):
        """Test that sync handles missing context gracefully"""
        task_id = "12345678-1234-5678-1234-567812345678"
    ...
    @pytest.mark.asyncio
    async def test_sync_task_metadata_handles_error(
        self, service, mock_context_service, mock_task_entity
    ):
