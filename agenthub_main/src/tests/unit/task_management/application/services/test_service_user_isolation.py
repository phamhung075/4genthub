...
class TestServiceUserIsolation:
    ...
    def test_task_service_creates_user_scoped_repository(
        self, mock_task_repository, user1_id
    ):
        """Test that TaskApplicationService creates user-scoped repositories."""
        # Create service with user context
        service = TaskApplicationService(
            task_repository=mock_task_repository, context_service=None, user_id=user1_id
        )

        # Verify service has user context
        assert service._user_id == user1_id

    def test_task_service_with_user_creates_new_instance(
        self, mock_task_repository, user1_id, user2_id
    ):
    ...
    def test_project_service_creates_user_scoped_repository(
        self, mock_project_repository, user1_id
    ):
        """Test that ProjectApplicationService creates user-scoped repositories."""
        # Create service with user context
        service = ProjectApplicationService(
            project_repository=mock_project_repository, user_id=user1_id
        )

        # Verify service has user context
        assert service._user_id == user1_id

    def test_project_service_with_user_creates_new_instance(
        self, mock_project_repository, user1_id, user2_id
    ):
        """Test that with_user() creates a new service instance with different user."""
        # Create initial service
    ...
    def test_project_service_operations_use_scoped_repo(
        self, mock_project_repository, user1_id
    ):
    ...
    def test_agent_service_creates_user_context(
        self, mock_task_repository, mock_agent_repository, user1_id
    ):
    ...
    def test_agent_service_repository_scoping(
        self, mock_task_repository, mock_agent_repository, user1_id
    ):
    ...
    def test_subtask_service_creates_user_context(
        self, mock_task_repository, mock_subtask_repository, user1_id
    ):
    ...
    def test_subtask_service_repository_scoping(
        self, mock_task_repository, mock_subtask_repository, user1_id
    ):
    ...
    def test_services_isolate_data_between_users(
        ...
        user2_task_service = TaskApplicationService(
            task_repository=mock_task_repository, context_service=None, user_id=user2_id
        )
        ...
        user2_project_service = ProjectApplicationService(
            project_repository=mock_project_repository, user_id=user2_id
        )
    ...
    def test_service_without_user_context_works_normally(self, mock_task_repository):
        ...
        service = TaskApplicationService(
            task_repository=mock_task_repository, context_service=None, user_id=None
        )
    ...
    def test_repository_without_user_support_returns_original(self, user1_id):
        ...
        # Create service with user context
        service = TaskApplicationService(
            task_repository=mock_repo, context_service=None, user_id=user1_id
        )
    ...
    def test_service_propagates_user_context_through_operations(
        self, mock_task_repository, user1_id
    ):
