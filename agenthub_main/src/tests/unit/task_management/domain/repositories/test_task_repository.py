...
class MockTaskRepository(TaskRepository):
    ...
    def find_by_status(self, status: TaskStatus) -> list[Task]:
        ...
        return [
            task for task in self.tasks.values() if task.status.value == status.value
        ]
    ...
    def get_next_id(self) -> TaskId:
        ...
        task_id = TaskId.from_string(
            f"550e8400-e29b-41d4-a716-{self.next_id_counter:012d}"
        )
        self.next_id_counter += 1
        return task_id
    ...
    def find_by_criteria(
        ...
        if "git_branch_id" in filters:
            results = [
                t for t in results if t.git_branch_id == filters["git_branch_id"]
            ]
...
class TestTaskRepositorySaveOperation:
    ...
    def test_save_updates_existing_task(self):
        ...
        # Save initial task
        task = Task.create(
            id=task_id, title="Original Title", description="Original Description"
        )
        repo.save(task)
...
class TestTaskRepositorySearchOperation:
    ...
    def test_search_by_title(self, searchable_repo):
        ...
        assert all(
            "payment" in task.title.lower()
            or "payment" in task.description.lower()
            or "payment" in task.labels
            for task in results
        )
    ...
    def test_search_with_limit(self, searchable_repo):
        ...
        for i in range(15):
            task = Task.create(
                id=TaskId.from_string(
                    f"550e8400-e29b-41d4-a716-4466554400{i + 10:02d}"
                ),
                title=f"Task {i}",
...
class TestTaskRepositoryDeleteOperation:
    ...
    def test_delete_existing_task(self):
        ...
        task = Task.create(
            id=task_id, title="Task to Delete", description="This will be deleted"
        )
        repo.save(task)
...
class TestTaskRepositoryUtilityOperations:
    ...
    def test_exists_for_existing_task(self):
        ...
        task = Task.create(id=task_id, title="Existing Task", description="This exists")
        repo.save(task)
