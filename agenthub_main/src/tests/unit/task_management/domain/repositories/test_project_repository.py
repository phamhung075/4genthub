...
class MockProjectRepository(ProjectRepository):
    ...
    async def get_project_health_summary(self) -> dict[str, Any]:
        ...
        total_sessions = sum(
            len(p.active_work_sessions) for p in self.projects.values()
        )
        ...
        return {
            ...
            "projects_with_cross_tree_deps": sum(
                1 for p in self.projects.values() if p.cross_tree_dependencies
            ),
        }
...
class TestProjectRepositoryFindOperations:
    ...
    async def populated_repo(self):
        ...
        agent2 = Agent(
            id="agent-2", name="DevOps Agent", capabilities={AgentCapability.DEVOPS}
        )
        agent3 = Agent(
            id="agent-3", name="QA Agent", capabilities={AgentCapability.TESTING}
        )
...
class TestProjectRepositoryUtilityOperations:
    ...
    async def test_count_with_projects(self):
        ...
        for i in range(5):
            project = Project.create(
                name=f"Project {i + 1}", description=f"Description {i + 1}"
            )
            await repo.save(project)
    ...
    async def test_get_project_health_summary(self):
        ...
        agent2 = Agent(
            id="agent-2", name="Agent 2", capabilities={AgentCapability.TESTING}
        )
        project1.register_agent(agent1)
        project1.register_agent(agent2)
...
class TestProjectRepositoryIntegration:
    ...
    async def test_complete_project_lifecycle(self):
        ...
        feature_tree = saved_project.create_git_branch(
            "feature-payment", "Payment Feature"
        )
        infra_tree = saved_project.create_git_branch(
        ...
        # Unassign an agent
        unassign_result = await repo.unassign_agent_from_tree(
            project_id, dev_agent.id, feature_tree.id
        )
