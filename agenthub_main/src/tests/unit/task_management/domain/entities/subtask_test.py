...
class TestSubtaskCreation:
    ...
    def test_subtask_validation_title_too_long(self):
        ...
        with pytest.raises(
            ...
            Subtask(
                title=long_title, description="Valid", parent_task_id=parent_task_id
            )
...
class TestSubtaskStatusUpdates:
    ...
    def test_update_status_valid_transition(self):
        ...
        assert events[0].task_id == str(parent_task_id)
        assert "subtask_status" in events[0].changes
        assert f"{subtask_id}:todo" in str(events[0].changes["subtask_status"]["old"])
        assert f"{subtask_id}:in_progress" in str(
            events[0].changes["subtask_status"]["new"]
        )
    ...
    def test_update_priority(self):
        ...
        events = subtask.get_events()
        assert len(events) == 1
        assert "subtask_priority" in events[0].changes
...
class TestSubtaskFieldUpdates:
    ...
    def test_update_title(self):
        ...
        events = subtask.get_events()
        assert len(events) == 1
        assert "subtask_title" in events[0].changes
    ...
    def test_update_title_validation(self):
        ...
        subtask = Subtask(
            title="Original", description="Test", parent_task_id=parent_task_id
        )
    ...
    def test_update_description(self):
        ...
        events = subtask.get_events()
        assert len(events) == 1
        assert "subtask_description" in events[0].changes
...
class TestSubtaskAssigneeManagement:
    ...
    def test_update_assignees(self):
        ...
        with patch(
            "fastmcp.task_management.domain.enums.agent_roles.AgentRole.is_valid_role",
            return_value=True,
        ):
        ...
        events = subtask.get_events()
        assert len(events) == 1
        assert "subtask_assignees" in events[0].changes
    ...
    def test_add_assignee_enum(self):
        ...
        subtask = Subtask(
            title="Test", description="Test", parent_task_id=parent_task_id
        )
...
class TestSubtaskProgressTracking:
    ...
    def test_update_progress_percentage_valid(self):
        ...
        events = subtask.get_events()
        assert len(events) == 1
        assert "subtask_progress" in events[0].changes
    ...
    def test_update_progress_percentage_invalid(self):
        ...
        with pytest.raises(
            ValueError, match="Progress percentage must be integer between 0-100"
        ):
...
class TestSubtaskSerialization:
    ...
    def test_from_dict_minimal(self):
        ...
        data = {"title": "Minimal", "description": "Minimal subtask"}
    ...
    def test_roundtrip_serialization(self):
        ...
        assert (
            restored_subtask.progress_percentage == original_subtask.progress_percentage
        )
...
class TestSubtaskDomainEvents:
    ...
    def test_event_content(self):
        ...
        assert event.task_id == str(parent_task_id)
        assert "subtask_title" in event.changes
        assert f"{subtask_id}:Original Title" in str(
            event.changes["subtask_title"]["old"]
        )
        assert f"{subtask_id}:Updated Title" in str(
            event.changes["subtask_title"]["new"]
        )
