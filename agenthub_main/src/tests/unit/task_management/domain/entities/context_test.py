...
class TestTaskContextCreation:
    ...
    def test_create_task_context_full_data(self):
        ...
        context = TaskContext(
            metadata=metadata, objective=objective, requirements=requirements
        )
    ...
    def test_task_context_timezone_handling(self):
        ...
        metadata = ContextMetadata(
            task_id="task-123", created_at=utc_dt, updated_at=utc_dt
        )
...
class TestContextMetadata:
    ...
    def test_context_metadata_value_objects(self):
        ...
        metadata = ContextMetadata(
            task_id="task-456", status=TaskStatus.done(), priority=Priority.urgent()
        )
...
class TestContextProgress:
    ...
    def test_context_progress_with_actions(self):
        ...
        assert (
            progress.current_session_summary == "Made good progress on authentication"
        )
...
class TestTaskContextSerialization:
    ...
    def test_from_dict_with_custom_fields(self):
        ...
        custom_section = next(
            (
                cs
                for cs in context.custom_sections
                if cs.name == "root_level_custom_fields"
            ),
            None,
        )
    ...
    def test_roundtrip_serialization(self):
        ...
        assert (
            restored_context.metadata.status.value
            == original_context.metadata.status.value
        )
        assert (
            restored_context.metadata.priority.value
            == original_context.metadata.priority.value
        )
        assert (
            restored_context.metadata.assignees == original_context.metadata.assignees
        )
        assert restored_context.metadata.labels == original_context.metadata.labels
        assert (
            restored_context.progress.completion_summary
            == original_context.progress.completion_summary
        )
...
class TestContextSchema:
    ...
    def test_validate_context_valid(self):
        ...
        data = {
            "metadata": {"task_id": "task-123", "status": "todo", "priority": "medium"},
            "objective": {"title": "Valid Task"},
        }
