...
class TestTaskEntity:
    ...
    def test_task_creation_with_empty_title_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Task title cannot be empty"):
            Task.create(id=self.task_id, title="", description=self.valid_description)
    ...
    def test_task_creation_with_empty_description_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Task description cannot be empty"):
            Task.create(id=self.task_id, title=self.valid_title, description="")

    def test_task_creation_with_whitespace_title_raises_error(self):
        """Task cannot be created with whitespace-only title"""
        with pytest.raises(ValueError, match="Task title cannot be empty"):
            Task.create(
                id=self.task_id, title="   ", description=self.valid_description
            )
    ...
    def test_task_equality_based_on_id(self):
        ...
        # Arrange
        task1 = Task.create(
            id=self.task_id, title="Task 1", description="Description 1"
        )
        task2 = Task.create(
            id=self.task_id, title="Task 2", description="Description 2"
        )
    ...
    def test_valid_status_transitions(self):
        ...
        # Arrange
        task = Task.create(
            id=self.task_id, title=self.valid_title, description=self.valid_description
        )
    ...
    def test_task_completion_with_context_timestamp_validation(self):
        ...
        # Act & Assert
        with pytest.raises(
            ValueError, match="Context must be updated AFTER the task was last modified"
        ):
            task.complete_task(
                completion_summary="Task completed", context_updated_at=old_timestamp
            )
    ...
    def test_update_title(self):
        ...
        )  # Context preserved in hierarchical system

        # Verify event
        events = task.get_events()
        update_event = next(
            e for e in events if isinstance(e, TaskUpdated) and "title" in e.changes
        )
