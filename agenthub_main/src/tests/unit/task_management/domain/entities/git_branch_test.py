...
class TestTaskHierarchyManagement:
    ...
    def test_add_root_task(self):
        ...
        task = Task(
            id=TaskId("task-123"), title="Root Task", description="A root level task"
        )
    ...
    def test_add_child_task(self):
        ...
        # Add parent task
        parent_task = Task(
            id=TaskId("parent-123"), title="Parent Task", description="Parent task"
        )
        git_branch.add_root_task(parent_task)

        # Add child task
        child_task = Task(
            id=TaskId("child-123"), title="Child Task", description="Child task"
        )
    ...
    def test_add_child_task_parent_not_found(self):
        ...
        with pytest.raises(
            ValueError, match="Parent task nonexistent-parent not found"
        ):
    ...
    def test_remove_task_root_task(self):
        ...
        task = Task(
            id=TaskId("task-123"), title="Root Task", description="Task to remove"
        )
        git_branch.add_root_task(task)
    ...
    def test_remove_task_with_children(self):
        ...
        parent_task = Task(
            id=TaskId("parent-123"), title="Parent Task", description="Parent"
        )
        git_branch.add_root_task(parent_task)

        child_task = Task(
            id=TaskId("child-123"), title="Child Task", description="Child"
        )
        git_branch.add_child_task(parent_task.id.value, child_task)
...
class TestTaskQueries:
    ...
    def test_get_task(self):
        ...
        task = Task(id=TaskId("task-123"), title="Test Task", description="A test task")
        git_branch.add_root_task(task)
...
class TestSerialization:
    ...
    def test_to_dict(self):
        ...
        assert data["id"] == str(
            git_branch.id
        )  # Convert GitBranchId to string for comparison
