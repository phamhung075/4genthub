...
class TestGlobalContextNestedData:
    ...
    def test_get_nested_value(self):
        ...
        assert nested_data.get_nested_value("organization.standards") == {
            "coding_style": "PEP8"
        }

        # Test getting category.subcategory.field
        assert (
            nested_data.get_nested_value("security.authentication.provider") == "oauth2"
        )
    ...
    def test_to_dict_and_from_dict(self):
        ...
        assert restored_data.get_nested_value("organization.standards") == {
            "test": "value"
        }
...
class TestGlobalContextMigrator:
    ...
    def setup_method(self):
        ...
        self.sample_flat_data = {
            ...
            "security_policies": {"authentication": "oauth2", "encryption": "AES256"},
            "compliance_requirements": {"gdpr": True, "hipaa": False},
            "shared_resources": {
                "api_keys": ["key1", "key2"],
                "service_accounts": ["sa1"],
            },
            "reusable_patterns": {"singleton": "pattern1", "factory": "pattern2"},
            "global_preferences": {"theme": "dark", "notifications": True},
            "delegation_rules": {"auto_delegate": True, "threshold": 5},
        }
    ...
    def test_migrate_to_nested_basic(self):
        ...
        assert (
            nested_data.get_nested_value("organization.standards")
            == self.sample_flat_data["organization_standards"]
        )
        assert (
            nested_data.get_nested_value("security.access_control")
            == self.sample_flat_data["security_policies"]
        )
    ...
    def test_migrate_to_flat_basic(self):
        ...
        assert (
            flat_data["organization_standards"]
            == self.sample_flat_data["organization_standards"]
        )
    ...
    def test_migration_with_unmapped_fields(self):
        ...
        assert nested_data._custom_categories["legacy_fields"]["custom_field"] == {
            "custom": "value"
        }
        assert (
            nested_data._custom_categories["legacy_fields"]["another_custom"]
            == "string_value"
        )
...
class TestGlobalContextEntity:
    ...
    def test_convenience_methods(self):
        ...
        context = GlobalContext(
            id="test-id", organization_name="Test Org", global_settings={}
        )
    ...
    def test_update_global_settings_nested(self):
        ...
        context = GlobalContext(id="test-id", organization_name="Test Org")

        # Update with nested structure
        nested_updates = {"organization": {"standards": {"new_standard": "value"}}}
        context.update_global_settings(nested_updates, use_nested=True)

        # Verify update
        assert (
            context.get_nested_value("organization.standards.new_standard") == "value"
        )
    ...
    def test_update_global_settings_flat(self):
        ...
        nested_updates = {"organization": {"standards": {"flat_standard": "value"}}}
        context.update_global_settings(nested_updates, use_nested=True)

        # Verify update
        assert (
            context.get_nested_value("organization.standards")["flat_standard"]
            == "value"
        )
...
class TestCompatibilityLayer:
    ...
    def test_ensure_backward_compatibility(self):
        ...
        compatible_context = self.compatibility_layer.ensure_backward_compatibility(
            context
        )
    ...
    def test_legacy_field_access(self):
        ...
        org_standards = self.compatibility_layer.handle_legacy_field_access(
            context, "organization_standards"
        )
        assert org_standards["coding"] == "pep8"
    ...
    def test_legacy_field_update(self):
        ...
        self.compatibility_layer.handle_legacy_field_update(
            context, "organization_standards", new_value
        )
    ...
    def test_migrate_existing_context(self):
        ...
        migrated_context = self.compatibility_layer.migrate_existing_context(
            self.sample_flat_data
        )
...
class TestValidator:
    ...
    def test_validate_flat_structure(self):
        ...
        flat_data = {
            "id": "test-id",
            "global_settings": {"organization_standards": {}, "security_policies": {}},
        }
