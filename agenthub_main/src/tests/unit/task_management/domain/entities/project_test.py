...
class TestGitBranchManagement:
    ...
    async def test_create_git_branch_async_duplicate(self):
        ...
        with pytest.raises(
            ValueError, match="Git branch feature/existing already exists"
        ):
...
class TestAgentManagement:
    ...
    def test_assign_agent_to_tree(self):
        ...
        git_branch = project.create_git_branch(
            git_branch_name="feature/test", name="Test Branch", description="Test"
        )
...
class TestCrossTreeDependencies:
    ...
    def test_add_cross_tree_dependency(self):
        ...
        branch1 = project.create_git_branch(
            git_branch_name="feature/auth", name="Auth Branch", description="Auth"
        )
        branch2 = project.create_git_branch(
            git_branch_name="feature/ui", name="UI Branch", description="UI"
        )
        ...
        with patch.object(project, "_find_git_branch") as mock_find:
            mock_find.side_effect = (
                lambda task_id: branch1
                if task_id == project._normalize_task_id(task1_id)
                else branch2
            )
    ...
    def test_add_cross_tree_dependency_same_tree_fails(self):
        ...
        with patch.object(project, "_find_git_branch") as mock_find:
            ...
            with pytest.raises(
                ValueError,
                match="Use regular task dependencies for tasks within the same tree",
            ):
                project.add_cross_tree_dependency(task2_id, task1_id)
    ...
    def test_add_cross_tree_dependency_nonexistent_task(self):
        ...
        with patch.object(project, "_find_git_branch") as mock_find:
            ...
            with pytest.raises(
                ValueError, match="One or both tasks not found in project"
            ):
                project.add_cross_tree_dependency(task2_id, task1_id)
    ...
    def test_coordinate_cross_tree_dependencies(self):
        ...
        project.cross_tree_dependencies = {task2_id: {task1_id}}
        ...
        with patch.object(project, "_find_git_branch") as mock_find:
            mock_find.side_effect = (
                lambda task_id: mock_branch1 if task_id == task1_id else mock_branch2
            )
...
class TestWorkSessionManagement:
    ...
    def test_start_work_session(self):
        ...
        with patch.object(project, "_find_git_branch") as mock_find:
            ...
            with patch(
                "fastmcp.task_management.domain.entities.work_session.WorkSession.create_session"
            ) as mock_create:
                mock_session = Mock()
    ...
    def test_start_work_session_agent_not_assigned(self):
        ...
        with patch.object(project, "_find_git_branch", return_value=git_branch):
            with pytest.raises(
                ValueError,
                match=f"Agent {agent.id} not assigned to tree {git_branch.id}",
            ):
    ...
    def test_get_orchestration_status_with_data(self):
        ...
        with patch.object(git_branch, "get_task_count", return_value=5):
            with patch.object(git_branch, "get_completed_task_count", return_value=2):
                with patch.object(
                    git_branch, "get_progress_percentage", return_value=40.0
                ):
                    status = project.get_orchestration_status()
                    ...
                    assert (
                        AgentCapability.PROJECT_MANAGEMENT.value
                        in agent_info["capabilities"]
                    )
...
class TestGetAvailableWork:
    ...
    def test_get_available_work_with_assignments(self):
        ...
        with patch.object(
            git_branch, "get_available_tasks", return_value=available_branch_tasks
        ):
