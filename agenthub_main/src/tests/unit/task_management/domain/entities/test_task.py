...
class TestTaskCreation:
    ...
    def test_create_task_with_required_fields(self):
        ...
        assert task.priority.value in [
            p.label for p in PriorityLevel
        ]  # Any valid priority
    ...
    def test_create_task_with_subtasks(self):
        ...
        task = Task(
            title="Parent Task", description="Task with subtasks", subtasks=subtask_ids
        )
...
class TestTaskCompletion:
    ...
    def test_complete_task(self):
        ...
        status_events = [
            e
            for e in events
            if isinstance(e, TaskUpdated) and "status" in getattr(e, "changes", {})
        ]
...
class TestTaskDomainEvents:
    ...
    def test_get_events(self):
        ...
        # TaskUpdated now uses changes dict instead of field_name
        assert any(
            isinstance(e, TaskUpdated) and "status" in getattr(e, "changes", {})
            for e in events
        )
        assert any(
            isinstance(e, TaskUpdated) and "priority" in getattr(e, "changes", {})
            for e in events
        )
        assert any(
            isinstance(e, TaskUpdated) and "assignees" in getattr(e, "changes", {})
            for e in events
        )
...
class TestTaskCompletionSerialization:
    ...
    def test_to_dict_includes_empty_completion_summary_for_incomplete_tasks(self):
        ...
        task = Task(title="Test Task", description="Test incomplete task serialization")
    ...
    def test_to_dict_includes_testing_notes_field(self):
        ...
        task = Task(title="Test Task", description="Test testing_notes serialization")
