...
class TestContextProgress:
    ...
    def test_progress_section(self):
        ...
        action1 = ContextProgressAction(
            timestamp="2024-01-01T10:00:00Z", action="Started task", agent="agent-1"
        )
...
class TestContextCustomSection:
    ...
    def test_custom_section_creation(self):
        ...
        custom = ContextCustomSection(
            ...
            data={"response_time": 250, "throughput": 1000, "error_rate": 0.01},
            schema_version="1.0",
        )
...
class TestTaskContextSerialization:
    ...
    def test_to_dict_with_value_objects(self):
        ...
        metadata = ContextMetadata(
            task_id="vo-task", status=TaskStatus.in_progress(), priority=Priority.high()
        )
        objective = ContextObjective(title="Value Object Test")
        req = ContextRequirement(
            id="r1", title="Test Req", priority=Priority.critical()
        )
        context = TaskContext(metadata=metadata, objective=objective)
    ...
    def test_to_dict_with_datetime(self):
        ...
        now = datetime.now(UTC)
        metadata = ContextMetadata(task_id="dt-task", created_at=now, updated_at=now)
        objective = ContextObjective(title="DateTime Test", due_date=now)
        context = TaskContext(metadata=metadata, objective=objective)
    ...
    def test_from_dict_with_all_sections(self):
        ...
        data = {
            ...
            "custom_sections": [
                {"name": "metrics", "data": {"speed": 100}, "schema_version": "1.0"}
            ],
        }
    ...
    def test_round_trip_serialization(self):
        ...
        objective = ContextObjective(
            title="Round Trip Test", description="Testing serialization"
        )
        context = TaskContext(metadata=metadata, objective=objective)
...
class TestContextSchema:
    ...
    def test_validate_context_invalid(self):
        ...
        is_valid, errors = ContextSchema.validate_context(
            {"metadata": {"task_id": "123"}, "objective": {}}
        )
        assert is_valid is False
    ...
    def test_create_empty_context(self):
        ...
        context = ContextSchema.create_empty_context(
            task_id="empty-123", title="Empty Context Test"
        )
...
class TestContextIntegration:
    ...
    def test_full_context_workflow(self):
        ...
        context.requirements.checklist.append(
            ContextRequirement(
                id="req-1", title="Implement feature", priority=Priority.high()
            )
        )
        ...
        context.subtasks.items.append(
            ContextSubtask(id="sub-1", title="Write tests", status=TaskStatus.todo())
        )
        ...
        data = context.to_dict()
        assert (
            data["progress"]["completion_summary"]
