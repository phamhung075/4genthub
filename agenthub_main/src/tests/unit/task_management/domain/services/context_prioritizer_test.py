...
class TestScoringWeights:
    ...
    def test_normalize_weights(self):
        ...
        total = (normalized.semantic_relevance + normalized.recency + 
                normalized.frequency + normalized.completeness +
                normalized.user_preference + normalized.project_priority +
...
class TestContextScoreEdgeCases:
    ...
    def test_missing_id_in_context(self, prioritizer):
        ...
        scores = prioritizer.score_contexts_batch(
            contexts=[context],
            query='test',
            semantic_similarities={'fallback-id': 0.5}
        )
