...
class TestTaskStateTransitionService:
    ...
    def test_can_transition_to_valid_transition(self, service, mock_task):
        ...
        can_transition, reason = service.can_transition_to(
            mock_task, TaskStatus("in_progress")
        )
    ...
    def test_can_transition_to_invalid_transition(self, service, mock_task):
        ...
        can_transition, reason = service.can_transition_to(
            mock_task, TaskStatus("done")
        )

        assert can_transition is False
        assert "Cannot transition from 'todo' to 'done'" in reason

    def test_can_transition_to_done_with_incomplete_subtasks(
        self, service, mock_task, mock_subtask_repository
    ):
    ...
    def test_can_transition_to_done_with_completed_subtasks(
        self, service, mock_task, mock_subtask_repository
    ):
    ...
    def test_transition_to_success(self, service, mock_task):
        ...
        success, message = service.transition_to(mock_task, TaskStatus("in_progress"))
    ...
    def test_transition_to_failure_invalid_transition(self, service, mock_task):
        ...
        success, message = service.transition_to(mock_task, TaskStatus("done"))
    ...
    def test_suggest_next_status_blocked(
        self, service, mock_task, mock_subtask_repository
    ):
    ...
    def test_handle_dependency_completion_unblocks_task(
    ...
    def test_handle_dependency_completion_multiple_dependencies(
        self, service, mock_task_repository
    ):
    ...
    def test_transition_context_types(self, service, mock_task):
        ...
        for context in contexts:
            success, message = service.transition_to(
                mock_task, TaskStatus("in_progress"), context=context
            )
    ...
    def test_suggest_next_status_blocked_with_alternative_suggestions(
        self, service, mock_task
    ):
    ...
    def test_check_transition_prerequisites_review_from_non_in_progress(
        self, service, mock_task
    ):
    ...
    def test_partial_branch_251_249_blocked_dependency_not_all_satisfied(
        self, service, mock_task, mock_task_repository
    ):
    ...
    def test_partial_branch_261_249_all_dependencies_not_satisfied(
        self, service, mock_task, mock_task_repository
    ):
    ...
    def test_partial_branch_296_304_done_with_no_subtask_repo(self, service, mock_task):
        ...
        service_no_subtasks = TaskStateTransitionService(
            subtask_repository=None, task_repository=None
        )
    ...
    def test_partial_branch_330_exit_no_completion_summary(self, service, mock_task):
        ...
        success, message = service.transition_to(
            mock_task, TaskStatus("done"), metadata={}
        )

        assert success is True
        assert "Status changed" in message

    def test_partial_branch_341_345_post_transition_with_task_repo(
        self, service, mock_task, mock_task_repository
    ):
    ...
    def test_partial_branch_372_378_all_dependencies_satisfied_status_check(
        self, service
    ):
