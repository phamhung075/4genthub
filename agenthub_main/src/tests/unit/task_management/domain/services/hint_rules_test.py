...
class TestStalledProgressRule:
    ...
    def test_evaluate_recent_progress(self, rule, mock_task):
        ...
        mock_task.progress_timeline = [{"timestamp": recent_time.isoformat()}]
    ...
    def test_evaluate_stalled_blocked(self, rule, mock_task):
        ...
        stalled_time = datetime.now(UTC) - timedelta(
            hours=49
        )  # More than 48 hours for CRITICAL priority
        mock_task.progress_timeline = [{"timestamp": stalled_time.isoformat()}]
...
class TestMissingContextRule:
    ...
    def test_evaluate_complete_context(self, rule, mock_task):
        ...
        task_context.data = {"completion_summary": "Task completed successfully"}
    ...
    def test_evaluate_missing_test_notes(self, rule, mock_task):
        ...
        task_context = Mock(spec=TaskContext)
        task_context.notes = {"design_decisions": "Some design decisions"}
        task_context.data = {}
