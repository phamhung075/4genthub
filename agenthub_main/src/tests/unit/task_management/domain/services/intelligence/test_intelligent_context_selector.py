...
class TestIntelligentContextSelector:
    ...
    @patch(
        "fastmcp.task_management.domain.services.intelligence.semantic_matcher.SentenceTransformer"
    )
    def test_select_context_performance_target(
        self, mock_transformer, intelligent_selector, sample_contexts
    ):
    ...
    def test_select_context_relevance(self, intelligent_selector, sample_contexts):
        ...
        with patch(
            ...
            with patch.object(
                selector.semantic_matcher,
                "find_similar_contexts",
                return_value=mock_results,
            ):
                # Test authentication query
                result = selector.select_context(
                    "user authentication JWT login", max_tokens=1500
                )
    ...
    def test_select_context_with_user_preferences(
        self, intelligent_selector, sample_contexts
    ):
        ...
        with patch(
            ...
            result = selector.select_context(
                "authentication system", max_tokens=1000, user_preferences=user_prefs
            )

            assert result.total_tokens_used <= 1000  # Respects user max size
            assert len(result.selected_contexts) > 0

    def test_select_context_progressive_expansion(
        self, intelligent_selector, sample_contexts
    ):
    ...
    def test_caching_functionality(self):
        ...
        selector = IntelligentContextSelector(enable_caching=True, cache_ttl_seconds=10)
    ...
    def test_hit_rate_estimation(self, intelligent_selector, sample_contexts):
        ...
        with patch(
            ...
            result = selector.select_context(
                "JWT authentication login system", max_tokens=1000
            )
...
class TestIntelligentContextSelectorPerformance:
    ...
    def test_selection_time_under_200ms(self):
        ...
        with patch(
            ...
            selector = IntelligentContextSelector(
                max_selection_time_ms=200.0, enable_caching=False
            )
            ...
            start_time = time.time()
            result = selector.select_context(
                "task implementation system", max_tokens=2000
            )
    ...
    def test_hit_rate_target_90_percent(self):
        ...
        with patch(
            ...
            with patch.object(
                ...
                result = selector.select_context(
                    "user authentication JWT login system", max_tokens=1500
                )
