...
class TestProgressiveExpander:
    ...
    def test_calculate_expansion_priority_with_access_patterns(
        self, progressive_expander
    ):
    ...
    def test_identify_expansion_candidates(
        ...
        assert any(
            c.trigger == ExpansionTrigger.SIMILARITY_MATCH for c in ctx_3_candidates
        )
        ...
        assert any(
            c.trigger == ExpansionTrigger.DEPENDENCY_CHAIN for c in task_3_candidates
        )

    def test_has_dependency_relationship(
        ...
        # Same branch
        same_branch_context = {"id": "task_10", "git_branch_id": "branch_123"}
        assert progressive_expander._has_dependency_relationship(
            same_branch_context, sample_current_context
        )

        # No relationship
        unrelated_context = {"id": "task_99", "git_branch_id": "branch_999"}
    ...
    def test_matches_usage_pattern(self, progressive_expander):
        ...
        assert progressive_expander._matches_usage_pattern(
            context_id, "implement JWT auth"
        )
    ...
    def test_record_context_access(self, progressive_expander):
        ...
        assert (
            progressive_expander.context_access_patterns[context_id]["access_count"]
            == 1
        )
        assert (
            progressive_expander.context_access_patterns[context_id]["last_accessed"]
            is not None
        )
    ...
    def test_expand_context_progressive_basic(self, progressive_expander):
        ...
        candidates = [
            ...
            ExpansionCandidate(
                ...
                metadata={
                    "context_data": {"id": "low_priority", "data": "less important"}
                },
            ),
        ]
...
class TestEdgeCases:
    ...
    def test_expansion_with_zero_budget(self, progressive_expander):
        ...
        result = progressive_expander.expand_context_progressive(
            {"loaded_contexts": []}, candidates, token_budget=0
        )
    ...
    def test_token_estimation_with_special_values(self, progressive_expander):
        ...
        # With datetime
        context_with_datetime = {"created_at": datetime.now(UTC), "data": "test"}
        tokens = progressive_expander.estimate_context_tokens(context_with_datetime)
        assert tokens > 0

        # With None values
        context_with_none = {"id": "test", "value": None, "list": [1, None, 3]}
    ...
    def test_skip_already_loaded_contexts(self, progressive_expander):
        ...
        current_context = {"loaded_contexts": ["ctx_already_loaded", "ctx_another"]}
        ...
        available_contexts = [
            ...
            {"id": "ctx_new", "context_id": "ctx_new", "context_type": "task"},
        ]

        candidates = progressive_expander.identify_expansion_candidates(
            current_context, "test query", available_contexts
        )
    ...
    def test_pattern_based_trigger_identified(self, progressive_expander):
        ...
        current_context = {"loaded_contexts": []}
        available_contexts = [
            {"id": context_id, "context_id": context_id, "context_type": "task"}
        ]
        ...
        pattern_candidates = [
            ...
            and c.trigger == ExpansionTrigger.PATTERN_BASED
        ]
    ...
    def test_high_priority_small_items_prefetch_on_budget_exceeded(
        self, progressive_expander
    ):
    ...
    def test_similarity_below_prefetch_threshold(self, progressive_expander):
        ...
        available_contexts = [
            {"id": "low_sim", "context_id": "low_sim", "context_type": "task"}
        ]
        ...
        candidates = progressive_expander.identify_expansion_candidates(
            current_context, "test query", available_contexts, similarity_scores
        )
    ...
    def test_token_availability_not_small_enough(self, progressive_expander):
        ...
        assert (
            len(large_candidates) > 0
        )  # Should still create candidate with default trigger
    ...
    def test_keyword_match_no_match_found(self, progressive_expander):
        ...
        result = progressive_expander._matches_usage_pattern(
            context_id, "implement frontend authentication with react"
        )
