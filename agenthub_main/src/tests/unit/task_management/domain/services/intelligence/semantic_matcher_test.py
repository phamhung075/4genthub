...
class TestSemanticMatcher:
    ...
    def test_generate_embedding_with_cache(self, semantic_matcher):
        ...
        with patch.object(
            semantic_matcher, "_load_cached_embedding", return_value=embedding1
        ) as mock_load:
    ...
    def test_generate_embeddings_batch_with_cache(self, semantic_matcher):
        ...
        with patch.object(semantic_matcher.model, "encode") as mock_encode:
            ...
            if HAS_NUMPY:
                ...
                mock_encode.return_value = np.random.random(
                    (2, semantic_matcher.embedding_dim)
                )
            else:
                mock_encode.return_value = [
                    [0.1] * semantic_matcher.embedding_dim,
    ...
    def test_add_context_items(self, semantic_matcher, sample_context_items):
        ...
        assert all(
            item.embedding is not None for item in semantic_matcher.context_items
        )
    ...
    @pytest.mark.skipif(not HAS_FAISS, reason="FAISS not installed")
    def test_find_similar_contexts_with_threshold(
        self, semantic_matcher, sample_context_items
    ):
        ...
        results_high = semantic_matcher.find_similar_contexts(
            query, top_k=10, min_similarity=0.9
        )
        results_low = semantic_matcher.find_similar_contexts(
            query, top_k=10, min_similarity=0.1
        )
    ...
    def test_get_context_similarity_matrix(
        self, semantic_matcher, sample_context_items
    ):
...
class TestContextItem:
    ...
    def test_context_item_creation(self):
        ...
        item = ContextItem(id="test_id", content="Test content", context_type="task")
    ...
    def test_context_item_with_metadata(self):
        ...
        item = ContextItem(
            id="test_id", content="Test content", context_type="task", metadata=metadata
        )
...
class TestEdgeCases:
    ...
    def test_ivf_index_type(self, temp_cache_dir):
        """Test IVF index type"""
        matcher = SemanticMatcher(cache_dir=temp_cache_dir, faiss_index_type="ivf")
    ...
    def test_unsupported_index_type(self, temp_cache_dir):
        ...
        matcher = SemanticMatcher(
            cache_dir=temp_cache_dir, faiss_index_type="unsupported"
        )

        with pytest.raises(ValueError, match="Unsupported FAISS index type"):
            matcher.add_context_items(
                [ContextItem(id="test", content="test", context_type="task")]
            )

    def test_similarity_search_without_faiss(self):
        """Test behavior when FAISS is not available"""
        with patch(
            "fastmcp.task_management.domain.services.intelligence.semantic_matcher.HAS_FAISS",
            False,
        ):
            matcher = SemanticMatcher()
