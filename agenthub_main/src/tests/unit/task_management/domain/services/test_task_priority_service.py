...
class TestTaskPriorityService:
    ...
    def test_calculate_priority_score_with_context_factors(self):
        ...
        score_with_context = self.service.calculate_priority_score(
            self.medium_priority_task, context_factors
        )
    ...
    def test_calculate_priority_score_in_progress_task(self):
        ...
        todo_task = self._create_test_task(
            task_id="todo-task", title="Todo Task", priority="medium", status="todo"
        )
    ...
    def test_order_tasks_by_priority(self):
        ...
        # Arrange
        tasks = [self.medium_priority_task, self.high_priority_task, self.old_task]
        ...
        for i in range(len(ordered_tasks) - 1):
            assert (
                ordered_tasks[i]["priority_score"]
                >= ordered_tasks[i + 1]["priority_score"]
            )
    ...
    def test_get_next_task_recommendation_with_exclusions(self):
        ...
        done_task = self._create_test_task(
            task_id="done-task", title="Completed Task", status="done"
        )
        ...
        recommendation = self.service.get_next_task_recommendation(
            "branch-1", exclude_statuses=["done", "cancelled"]
        )
    ...
    def test_adjust_priority_for_dependencies_with_incomplete_deps(self):
        ...
        task_with_deps = self._create_test_task(
            task_id="dependent-task", title="Task with Dependencies"
        )
        task_with_deps.dependencies = [
            TaskId.from_string("11111111-1111-1111-1111-111111111111"),
            TaskId.from_string("22222222-2222-2222-2222-222222222222"),
        ]
    ...
    def test_adjust_priority_for_dependencies_task_blocks_others(self):
        ...
        blocking_task = self._create_test_task(
            task_id="33333333-3333-3333-3333-333333333333", title="Blocking Task"
        )

        dependent_task = self._create_test_task(
            task_id="dependent-task", title="Dependent Task"
        )
        dependent_task.dependencies = [
            TaskId.from_string("33333333-3333-3333-3333-333333333333")
        ]
    ...
    def test_calculate_urgency_score_various_due_dates(self):
        ...
        today_task = self._create_test_task(
            task_id="today-task", title="Due today", due_date=now
        )
    ...
    def test_calculate_urgency_score_no_due_date(self):
        ...
        # Arrange
        task = self._create_test_task(
            task_id="no-due-date-task", title="Task without due date"
        )
        task.due_date = None
    ...
    def test_get_priority_factors_complete_breakdown(self):
        ...
        # Act
        factors = self.service._get_priority_factors(
            self.high_priority_task, context_factors
        )
        ...
        assert factors["base_priority"]["value"] == str(
            self.high_priority_task.priority
        )
        assert factors["progress_factor"]["status"] == str(
            self.high_priority_task.status
        )
    ...
    def test_generate_recommendation_reason(self):
        ...
        # Arrange
        high_score_task_info = {"task": self.high_priority_task, "priority_score": 85.0}
...
class TestTaskPriorityServiceIntegration:
    ...
    def test_complete_workflow_scenario(self):
        ...
        assert recommendation is not None
        assert (
            recommendation["task"].title == "Critical Production Bug"
    ...
    def test_dependency_chain_prioritization(self):
        ...
        foundation_task = self._create_task_with_id(
            "foundation", "Foundation Work", "medium"
        )
