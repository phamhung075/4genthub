...
class TestGitBranchNameValidator:
    ...
    async def test_validate_name_format_too_long(self, validator):
        ...
        long_name = "a" * 101
        with pytest.raises(
            ValidationException, match="Branch name cannot exceed 100 characters"
        ):
    ...
    async def test_validate_name_format_forbidden_chars(self, validator):
        ...
        for test_name, expected_char in forbidden_tests:
            if expected_char == "spaces":
                ...
                with pytest.raises(
                    ValidationException, match="cannot contain the character"
                ):
                    await validator.validate_name_format(test_name)
    ...
    async def test_validate_name_format_consecutive_dots(self, validator):
        ...
        with pytest.raises(
            ValidationException, match="cannot contain consecutive dots"
        ):
    ...
    async def test_validate_unique_name_empty_after_trim(self, validator):
        ...
        with pytest.raises(
            ValidationException,
            match="Branch name cannot be empty|must be at least 1 character",
        ):
    ...
    @pytest.mark.asyncio
    async def test_validate_unique_name_no_duplicates(
        self, validator, mock_git_branch_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_unique_name_with_existing_branches(
        self, validator, mock_git_branch_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_unique_name_duplicate_found(
        self, validator, mock_git_branch_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_unique_name_exclude_self(
        self, validator, mock_git_branch_repository
    ):
        ...
        await validator.validate_unique_name(
            "main", "project-123", exclude_branch_id="branch-1"
        )

    @pytest.mark.asyncio
    async def test_validate_unique_name_duplicate_with_different_exclude(
        self, validator, mock_git_branch_repository
    ):
        ...
        with pytest.raises(ValidationException, match="already exists in this project"):
            await validator.validate_unique_name(
                "main", "project-123", exclude_branch_id="branch-2"
            )

    @pytest.mark.asyncio
    async def test_validate_unique_name_trimmed(
        self, validator, mock_git_branch_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_branch_name_valid(
        self, validator, mock_git_branch_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_branch_name_invalid_format(
        self, validator, mock_git_branch_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_branch_name_duplicate(
        self, validator, mock_git_branch_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_branch_name_with_exclude(
        self, validator, mock_git_branch_repository
    ):
        ...
        await validator.validate_branch_name(
            "main", "project-123", exclude_branch_id="branch-1"
        )
        ...
        with pytest.raises(ValidationException, match="already exists in this project"):
            await validator.validate_branch_name(
                "develop", "project-123", exclude_branch_id="branch-1"
            )

    @pytest.mark.asyncio
    async def test_repository_call_parameters(
        self, validator, mock_git_branch_repository
    ):
        ...
        mock_git_branch_repository.find_all_by_project.assert_called_once_with(
            "project-123"
        )
