...
class TestIntelligentContextSelector:
    ...
    def selector(
        ...
        mock_progressive_expander,
        mock_predictive_loader,
        mock_context_prioritizer,
    ):
        """Create selector with mocked dependencies."""
        with (
            patch(
                "fastmcp.task_management.domain.services.intelligence.intelligent_context_selector.SemanticMatcher",
                return_value=mock_semantic_matcher,
            ),
            patch(
                "fastmcp.task_management.domain.services.intelligence.intelligent_context_selector.ProgressiveExpander",
                return_value=mock_progressive_expander,
            ),
            patch(
                "fastmcp.task_management.domain.services.intelligence.intelligent_context_selector.PredictiveLoader",
                return_value=mock_predictive_loader,
            ),
            patch(
                "fastmcp.task_management.domain.services.intelligence.intelligent_context_selector.ContextPrioritizer",
                return_value=mock_context_prioritizer,
            ),
        ):
    ...
    def sample_contexts(self):
        ...
        return [
            ...
            },
            {
                ...
                "context_type": "task",
                "metadata": {"priority": "high", "tags": ["api", "docs"]},
    ...
    def test_select_context_with_semantic_matches(self, selector, sample_contexts):
        ...
        selector.progressive_expander.expand_context_progressive.return_value = (
            mock_expansion
        )

        # Execute
        result = selector.select_context(
            query="How to implement user authentication?", max_tokens=1000
        )
    ...
    def test_select_context_with_predictive_fallback(self, selector, sample_contexts):
        ...
        # Execute
        result = selector.select_context(
            query="What's next?", max_tokens=500, current_task={"id": "current_task"}
        )
        ...
        assert (
            result.total_tokens_used == 200
        )  # The mock returns 100 per context, so 2 contexts = 200
    ...
    def test_select_context_error_handling(self, selector, sample_contexts):
        ...
        selector.semantic_matcher.generate_embedding.side_effect = Exception(
            "Embedding error"
        )
    ...
    def test_estimate_size_reduction(self, selector, sample_contexts):
        ...
        assert (
            selector._estimate_size_reduction(sample_contexts, sample_contexts) == 0.0
        )
    ...
    def test_session_management(self, selector):
        ...
        selector.predictive_loader.start_session.assert_called_once_with(
            "session123", "user456"
        )
        ...
        selector.predictive_loader.record_tool_usage.assert_called_once_with(
            "Read", "ctx1"
        )
        selector.context_prioritizer.record_context_access.assert_called_once_with(
            "ctx1"
        )
    ...
    def test_performance_metrics_tracking(self, selector, sample_contexts):
        ...
        assert (
            selector.metrics.avg_size_reduction == 0.1 * result.size_reduction_percent
        )

        # Performance history should have an entry
        assert len(selector.performance_history) == 1
        assert (
            selector.performance_history[0]["selection_time_ms"]
    ...
    def test_optimize_performance_slow_selection(self, selector):
        ...
        assert any(
            "similarity threshold" in action
    ...
    def test_optimize_performance_poor_cache(self, selector):
        ...
        selector.result_cache = {
            f"key{i}": (Mock(), datetime.now(UTC)) for i in range(20)
        }
    ...
    def test_cache_size_limiting(self, selector):
        ...
        assert (
            75 <= len(selector.result_cache) <= 90
        )  # Allow some variance for hash collisions
    ...
    def test_select_context_with_user_preferences(self, selector, sample_contexts):
        ...
        result = selector.select_context(
            query="test", max_tokens=1000, user_preferences=user_prefs
        )
    ...
    def test_select_context_aggressive_expansion(self, selector, sample_contexts):
        ...
        mock_similar = [
            SimilarityResult(
                item=ContextItem(
                    "ctx1", "content", "task", {"context_data": sample_contexts[0]}
                ),
        ...
        selector.select_context(
            query="test", max_tokens=2000, aggressive_expansion=True
        )
    ...
    def test_performance_warning_logs(self, selector, sample_contexts, caplog):
        ...
        with patch("time.time") as mock_time:
            ...
            mock_time.side_effect = [0, 0.25] + [
                0.25
            ] * 10  # Extra values for logging calls
        ...
        assert any(
            "Selection time" in record.message and "exceeded target" in record.message
            for record in caplog.records
            if record.levelname == "WARNING"
        )
        assert any(
            "Hit rate estimate" in record.message and "below target" in record.message
            for record in caplog.records
            if record.levelname == "WARNING"
        )
    ...
    def test_expansion_trigger_determination(self, selector, sample_contexts):
        ...
        assert (
            candidates[0].trigger == ExpansionTrigger.SIMILARITY_MATCH
        )  # High semantic
        assert (
            candidates[1].trigger == ExpansionTrigger.DEPENDENCY_CHAIN
        )  # High dependency
    ...
    def test_context_level_mapping(self, selector, sample_contexts):
        ...
        mock_similar = [
            SimilarityResult(
                item=ContextItem(
                    ctx["id"], ctx["title"], ctx["context_type"], {"context_data": ctx}
                ),
                similarity_score=0.7,
                rank=idx + 1,
            )
    ...
    def test_moving_average_metrics(self, selector, sample_contexts):
        ...
        assert (
            abs(selector.metrics.avg_size_reduction - 0.08) < 0.01
        )  # 0.1 * 0.8 + 0.9 * 0
