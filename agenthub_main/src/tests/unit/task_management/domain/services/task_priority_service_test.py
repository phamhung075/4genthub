...
class TestTaskPriorityService:
    ...
    def test_get_next_task_recommendation_all_done(
        self, service, mock_repository, basic_task
    ):
    ...
    def test_get_next_task_recommendation_exclude_statuses(
        ...
        result = service.get_next_task_recommendation(
            "branch-123", exclude_statuses=["review", "done"]
        )

        assert result is not None
        assert result["task_id"] == "task-2"  # Todo task should be recommended

    def test_get_next_task_recommendation_error_handling(
        self, service, mock_repository
    ):
    ...
    def test_adjust_priority_for_dependencies_incomplete_deps(
        self, service, basic_task
    ):
    ...
    def test_adjust_priority_for_dependencies_task_is_blocker(
        self, service, basic_task
    ):
    ...
    def test_calculate_urgency_score_due_dates(
        self, mock_datetime, service, basic_task
    ):
    ...
    def test_generate_recommendation_reason(self, service, basic_task):
        ...
        recommended = {"task": basic_task, "priority_score": 85.0}
