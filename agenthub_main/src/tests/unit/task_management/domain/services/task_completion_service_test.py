...
def service_without_context(mock_subtask_repository):
    ...
    return TaskCompletionService(
        subtask_repository=mock_subtask_repository, task_context_repository=None
    )
...
class TestCanCompleteTask:
    ...
    def test_can_complete_task_with_no_subtasks(
        self, service, valid_task, mock_subtask_repository, mock_task_context_repository
    ):
        ...
        mock_subtask_repository.find_by_parent_task_id.assert_called_once_with(
            valid_task.id
        )

    def test_can_complete_task_with_all_subtasks_completed(
        self, service, valid_task, completed_subtask, mock_subtask_repository
    ):
    ...
    def test_cannot_complete_task_with_incomplete_subtasks(
        self,
        service,
        valid_task,
        completed_subtask,
    ...
    def test_task_without_context_id_but_has_context_in_repository(
        self, service, valid_task, mock_subtask_repository, mock_task_context_repository
    ):
    ...
    def test_task_without_context_logs_info(
        self,
        service,
        valid_task,
        mock_subtask_repository,
    ...
    def test_exception_handling_in_can_complete_task(
        self, service, valid_task, mock_subtask_repository
    ):
    ...
    def test_multiple_incomplete_subtasks(
        self, service, valid_task, incomplete_subtask, mock_subtask_repository
    ):
...
class TestValidateTaskCompletion:
    ...
    def test_validate_task_completion_success(
        self, service, valid_task, mock_subtask_repository
    ):
    ...
    def test_validate_task_completion_raises_exception(
        self, service, valid_task, incomplete_subtask, mock_subtask_repository
    ):
        ...
        assert (
            exception.context["incomplete_subtasks"][0]["title"]
            == "Add session management"
        )

    def test_validate_task_completion_with_context_data(
        self,
        service,
        valid_task,
        incomplete_subtask,
...
class TestGetCompletionBlockers:
    ...
    def test_no_blockers_when_completable(
        self, service, valid_task, mock_subtask_repository, mock_task_context_repository
    ):
    ...
    def test_blocker_for_incomplete_subtasks(
        self,
        service,
        valid_task,
        incomplete_subtask,
    ...
    def test_blocker_with_many_incomplete_subtasks(
        self, service, valid_task, mock_subtask_repository
    ):
    ...
    def test_blocker_exception_handling(
        self, service, valid_task, mock_subtask_repository
    ):
...
class TestGetSubtaskCompletionSummary:
    ...
    def test_summary_with_no_subtasks(
        self, service, valid_task, mock_subtask_repository
    ):
    ...
    def test_summary_with_all_completed(
        self, service, valid_task, completed_subtask, mock_subtask_repository
    ):
...
class TestHelperMethods:
    ...
    def test_create_incomplete_subtasks_error(
        self, service, valid_task, incomplete_subtask, mock_subtask_repository
    ):
        ...
        error_info = service._create_incomplete_subtasks_error(
            valid_task, [incomplete_subtask]
        )

        assert (
            error_info["error"]
            == "Cannot complete task while subtasks remain incomplete."
        )
        ...
        assert (
            error_info["details"]["incomplete_subtask_titles"][0]
            == "Add session management"
        )
        assert len(error_info["recovery_instructions"]) == 3
        assert len(error_info["step_by_step_fix"]) == 3
...
class TestServiceWithoutContextRepository:
    ...
    def test_can_complete_without_context_repository(
        self, service_without_context, valid_task, mock_subtask_repository
    ):
    ...
    def test_blockers_without_context_repository(
        self, service_without_context, valid_task, mock_subtask_repository
    ):
...
class TestEdgeCases:
    ...
    def test_subtask_with_non_value_object_id(
        self, service, valid_task, mock_subtask_repository
    ):
    ...
    def test_completion_percentage_rounding(
        self, service, valid_task, mock_subtask_repository
    ):
        ...
        assert (
            summary["completion_percentage"] == 33.3
        )  # 1/3 = 33.333... rounded to 33.3

    def test_empty_subtask_titles_in_blockers(
        self, service, valid_task, mock_subtask_repository
    ):
...
class TestUncoveredLines:
    ...
    def test_context_none_but_exists_in_repository_logs_info(
        self, service, mock_subtask_repository, mock_task_context_repository, caplog
    ):
        ...
        auto_create_logs = [
            ...
            if "has no context but will be auto-created during completion"
            in log.message
        ]
    ...
    def test_context_repository_get_called_when_context_id_none(
        self, service, mock_subtask_repository, mock_task_context_repository
    ):
    ...
    def test_partial_branch_183_to_191_many_incomplete_subtasks(
        ...
        assert (
            "(including: Incomplete 1, Incomplete 2, Incomplete 3, and 2 more)"
            in blocker_msg
        )
    ...
    def test_context_check_with_valid_context_no_logging(
        ...
        info_logs = [record for record in caplog.records if record.levelname == "INFO"]
        auto_create_logs = [log for log in info_logs if "auto-created" in log.message]
