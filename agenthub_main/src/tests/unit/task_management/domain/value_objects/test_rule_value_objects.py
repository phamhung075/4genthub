...
class TestCompositionResult:
    ...
    def test_composition_result_validation_empty_content(self):
        ...
        with pytest.raises(
            ValueError, match="Successful composition must have content"
        ):
            CompositionResult(
                composed_content="",
                source_rules=["rule1"],
...
class TestCacheEntry:
    ...
    def test_create_valid_cache_entry(self):
        ...
        entry = CacheEntry(
            content=mock_content, timestamp=current_time, access_count=0, ttl=3600.0
        )
    ...
    def test_cache_entry_validation_invalid_ttl(self):
        ...
        with pytest.raises(ValueError, match="TTL must be positive"):
            CacheEntry(
                content=mock_content, timestamp=time.time(), access_count=0, ttl=0
            )
    ...
    def test_cache_entry_validation_negative_access_count(self):
        ...
        with pytest.raises(ValueError, match="Access count cannot be negative"):
            CacheEntry(
                content=mock_content, timestamp=time.time(), access_count=-1, ttl=3600.0
            )
    ...
    def test_cache_expiration(self, mock_time):
        ...
        entry = CacheEntry(
            content=mock_content, timestamp=1000.0, access_count=0, ttl=3600.0
        )
    ...
    def test_increment_access_count(self):
        ...
        entry = CacheEntry(
            content=mock_content, timestamp=time.time(), access_count=0, ttl=3600.0
        )
...
class TestValueObjectIntegration:
    ...
    def test_sync_workflow(self, sample_client_config, sample_sync_request):
        ...
        assert (
            f"Sync {result.request_id} completed" in sample_client_config.sync_history
        )
