...
class TestEventStore:
    ...
    async def test_get_latest_snapshot(self, event_store):
        ...
        await event_store.create_snapshot(
            aggregate_id, aggregate_type, {"version": 1}, 1
        )
        await asyncio.sleep(0.01)  # Ensure different timestamps
        await event_store.create_snapshot(
            aggregate_id, aggregate_type, {"version": 2}, 2
        )
        await asyncio.sleep(0.01)
        await event_store.create_snapshot(
            aggregate_id, aggregate_type, {"version": 3}, 3
        )
    ...
    async def test_get_events_after_snapshot(self, event_store):
        ...
        await event_store.create_snapshot(
            aggregate_id, aggregate_type, {"checkpoint": 1}, 1
        )
        ...
        events = await event_store.get_events(
            aggregate_id=aggregate_id, from_timestamp=snapshot_time
        )
    ...
    async def test_event_serialization_deserialization(self, event_store):
        ...
        class ComplexEvent:
            def __init__(self):
                ...
                self.occurred_at = datetime.now(UTC)
                self.complex_data = {
                    "nested": {"value": 123, "list": [1, 2, 3], "bool": True},
