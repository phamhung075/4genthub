...
class TestLoggingNotificationChannel:
    ...
    async def test_send_notification_low_priority(self, channel):
        ...
        with patch(
            "fastmcp.task_management.infrastructure.notification_service.logger"
        ) as mock_logger:
            notification = Notification(
                id="test_123",
                type="test_info",
...
class TestNotificationService:
    ...
    async def test_initialization(self, service):
        ...
        has_in_memory = any(
            isinstance(ch, InMemoryNotificationChannel) for ch in service.channels
        )
        has_logging = any(
            isinstance(ch, LoggingNotificationChannel) for ch in service.channels
        )
    ...
    async def test_notify_all_channels(self, service):
        ...
        notification_id = await service.notify(
            type="test_all", data={"message": "Broadcast"}, priority="high"
        )
    ...
    async def test_notify_with_retry(self, service):
        ...
        notification_id = await service.notify(
            type="test_retry", data={"message": "Retry"}, priority="high"
        )
    ...
    async def test_notify_batch(self, service):
        ...
        notifications = [
            {"type": "test1", "data": {"msg": "1"}, "priority": "high"},
            {"type": "test2", "data": {"msg": "2"}, "priority": "medium"},
        ]
    ...
    async def test_max_retries_exceeded(self, service):
        ...
        with patch(
            ...
            notification_id = await service.notify(
                type="test", data={"msg": "fail"}, priority="high"
            )
    ...
    async def test_get_in_memory_notifications_filtered(self, service):
        ...
        # Filter by type
        type_a_notifications = in_memory_channel.get_notifications(
            notification_type="type_a"
        )
        assert len(type_a_notifications) == 2

        # Filter by priority
        high_priority = in_memory_channel.get_notifications(
            priority=NotificationPriority.HIGH
        )
    ...
    async def test_concurrent_notifications(self, service):
        ...
        for i in range(10):
            tasks.append(
                service.notify(
                    type=f"concurrent_{i}", data={"index": i}, priority="medium"
                )
            )
