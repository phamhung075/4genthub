...
class TestBranchContextRepository:
    ...
    def setup_method(self):
        ...
        self.repository = BranchContextRepository(
            self.mock_session_factory, user_id=self.user_id
        )
    ...
    def test_update_success(self):
        ...
        assert (
            existing_model.branch_workflow
            == self.test_branch_settings["branch_workflow"]
        )
    ...
    def test_update_not_found(self):
        ...
        assert f"Branch context not found: {self.test_context_id}" in str(
            exc_info.value
        )
    ...
    def test_delete_success(self):
        ...
        self.mock_session.get.assert_called_once_with(
            BranchContextModel, self.test_context_id
        )
    ...
    def test_list_without_filters(self):
        ...
        db_models = [Mock(id=f"context-{i}", branch_id=f"branch-{i}") for i in range(3)]
    ...
    @patch(
        "fastmcp.task_management.infrastructure.repositories.branch_context_repository.select"
    )
    ...
    def test_update_sets_updated_at_timestamp(self):
        ...
        with patch.object(self.repository, "_to_entity") as mock_to_entity:
            ...
            with patch(
                "fastmcp.task_management.infrastructure.database.timestamp_events._before_update_handler",
                side_effect=mock_before_update,
            ):
                self.repository.update(self.test_context_id, self.test_entity)
    ...
    def test_create_uses_metadata_user_id_fallback(self):
        ...
        with pytest.raises(
            ValueError, match="user_id is required for branch context creation"
        ):
    ...
    def test_user_isolation_boundary(self):
        ...
        user1_repo = BranchContextRepository(
            self.mock_session_factory, user_id="user-001"
        )
        user2_repo = BranchContextRepository(
            self.mock_session_factory, user_id="user-002"
        )
