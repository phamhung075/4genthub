...
class TestORMGitBranchRepository:
    ...
    async def test_assign_agent_success(self):
        ...
        with patch.object(self.repo, "get_db_session") as mock_session_context:
            ...
            result = await self.repo.assign_agent(
                self.project_id, self.branch_id, self.agent_id
            )
    ...
    async def test_assign_agent_failure(self):
        ...
        with patch.object(self.repo, "get_db_session") as mock_session_context:
            ...
            result = await self.repo.assign_agent(
                self.project_id, "nonexistent", self.agent_id
            )
    ...
    async def test_get_project_branch_summary(self):
        ...
        with patch.object(self.repo, "get_db_session") as mock_session_context:
            ...
            self.mock_session.query.side_effect = [
                Mock(
                    filter=Mock(return_value=Mock(first=Mock(return_value=mock_stats)))
                ),
                Mock(
                    filter=Mock(
                        return_value=Mock(
                            group_by=Mock(
                                return_value=Mock(
                                    all=Mock(return_value=mock_status_rows)
                                )
                            )
                        )
                    )
    ...
    async def test_create_branch(self):
        ...
        with patch.object(self.repo, "save") as mock_save:
            ...
            result = await self.repo.create_branch(
                self.project_id, "feature/new", "New feature"
            )
    ...
    async def test_create_git_branch_success(self):
        ...
        with patch.object(self.repo, "create_branch") as mock_create:
            ...
            result = await self.repo.create_git_branch(
                self.project_id, "feature/test", "Test branch"
            )
        ...
        with patch.object(self.repo, "find_by_name") as mock_find:
            ...
            with patch.object(self.repo, "assign_agent") as mock_assign:
                ...
                result = await self.repo.assign_agent_to_branch(
                    self.project_id, self.agent_id, "feature/test"
                )
    ...
    async def test_get_branch_statistics_success(self):
        ...
        # Create mock for task count query result
        mock_task_count_result = Mock()
        mock_task_count_result.total = 10
        mock_task_count_result.completed = 6

        with patch.object(self.repo, "get_db_session") as mock_session_context:
            mock_session_context.return_value.__enter__.return_value = self.mock_session

            # Mock the two queries - first for branch, second for task counts
            mock_branch_query = Mock()
            mock_branch_query.filter.return_value.first.return_value = self.mock_model

            mock_task_query = Mock()
            mock_task_query.filter.return_value.first.return_value = (
                mock_task_count_result
            )

            # Return different query objects for each call
            self.mock_session.query.side_effect = [mock_branch_query, mock_task_query]

            result = await self.repo.get_branch_statistics(
