...
class TestPerformanceMonitor:
    ...
    def mock_cache_manager(self):
        ...
        cache_manager.get_performance_metrics.return_value = {
            "cache_statistics": {"hit_rate": 0.85, "miss_rate": 0.15},
            "performance_metrics": {
            ...
            "eviction_statistics": {"total_evictions": 50},
        }
    ...
    async def test_capture_snapshot_error(self, monitor, mock_cache_manager):
        ...
        mock_cache_manager.get_performance_metrics.side_effect = Exception(
            "Metrics error"
        )
    ...
    def test_get_performance_summary_multiple_snapshots(self, monitor):
        ...
        assert (
            abs(summary["averages"]["hit_rate"] - 0.84) < 0.0001
        )  # Average of 0.80 to 0.88
        assert abs(summary["trends"]["hit_rate_change"] - 0.08) < 0.0001  # 0.88 - 0.80
        assert (
            abs(summary["trends"]["response_time_change_ms"] - (-8.0)) < 0.0001
        )  # 22 - 30
...
class TestCacheBenchmark:
    ...
    def mock_cache_manager(self):
        ...
        cache_manager.get_performance_metrics = Mock(
            return_value={
                "cache_statistics": {"hit_rate": 0.85, "miss_rate": 0.15},
                "performance_metrics": {
                    "average_response_time_ms": 25.5,
...
class TestBenchmarkDataClasses:
    ...
    def test_benchmark_result_initialization(self):
        ...
        result = BenchmarkResult(
            config=config, start_time=time.time(), end_time=time.time() + 60
        )
