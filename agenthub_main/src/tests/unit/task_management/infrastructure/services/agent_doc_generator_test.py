...
class TestFindProjectRoot:
    ...
    def test_find_project_root_exists_in_parent(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('fastmcp.task_management.infrastructure.services.agent_doc_generator.__file__', str(deep_file)):
    ...
    def test_find_project_root_in_cwd(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('pathlib.Path.cwd', return_value=project_root):
                with patch('fastmcp.task_management.infrastructure.services.agent_doc_generator.__file__', '/some/other/path.py'):
                    result = _find_project_root()
                    assert result == project_root
    ...
    def test_find_project_root_is_agenthub_main(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('fastmcp.task_management.infrastructure.services.agent_doc_generator.__file__', str(file_path)):
                result = _find_project_root()
...
class TestAgentDocGenerator:
    ...
    def mock_project_root(self, temp_project_dir):
        ...
        with patch('fastmcp.task_management.infrastructure.services.agent_doc_generator._find_project_root', 
    ...
    def test_init_default_paths(self, mock_project_root):
        ...
        assert generator.agent_yaml_lib == mock_project_root / "agenthub_main" / "agent-library"
        assert generator.agents_output_dir == mock_project_root / ".cursor" / "rules" / "agents"
    ...
    def test_init_custom_paths(self, mock_project_root):
        ...
        generator = AgentDocGenerator(
            agent_yaml_lib=custom_yaml,
            agents_output_dir=custom_output
        )
    ...
    def test_init_relative_paths(self, mock_project_root):
        ...
        generator = AgentDocGenerator(
            agent_yaml_lib="relative/yaml",
            agents_output_dir="relative/output"
        )
    ...
    def test_init_env_variables(self, mock_project_root):
        ...
        with patch.dict(os.environ, {
            'AGENT_LIBRARY_DIR_PATH': 'env/yaml',
            'AGENTS_OUTPUT_DIR': 'env/output'
        }):
            generator = AgentDocGenerator()
    ...
    def test_generate_agent_docs_nonexistent(self, mock_project_root):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_called_with("Agent directory 'nonexistent_agent' not found.")
...
class TestModuleFunctions:
    ...
    @patch('fastmcp.task_management.infrastructure.services.agent_doc_generator.AgentDocGenerator')
    ...
    @patch('fastmcp.task_management.infrastructure.services.agent_doc_generator.AgentDocGenerator')
    @patch('fastmcp.task_management.infrastructure.services.agent_doc_generator.AGENT_YAML_LIB', Path("/mock/yaml"))
    @patch('fastmcp.task_management.infrastructure.services.agent_doc_generator.AGENTS_OUTPUT_DIR', Path("/mock/output"))
    def test_generate_agent_docs_function(self, mock_generator_class):
        ...
        mock_generator_class.assert_called_once_with(
            agent_yaml_lib=Path("/mock/yaml"),
            agents_output_dir=Path("/mock/output")
        )
        mock_instance.generate_agent_docs.assert_called_once_with("test", True)
    
    @patch('fastmcp.task_management.infrastructure.services.agent_doc_generator.AgentDocGenerator')
    @patch('logging.getLogger')
    def test_generate_docs_for_assignees_error_handling(self, mock_logger, mock_generator_class):
