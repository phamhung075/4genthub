...
class TestRuleParserService:
    ...
    def test_parse_rule_file_markdown_with_frontmatter(
        self, mock_stat, mock_exists, parser
    ):
...
"""
    ...
    def test_parse_rule_file_json(self, mock_stat, mock_exists, parser):
        ...
        content = json.dumps(
            {
                "name": "test_rule",
                "sections": {"config": {"key": "value"}, "data": {"items": [1, 2, 3]}},
                "variables": {"env": "test"},
            }
        )
    ...
    def test_parse_rule_file_yaml(self, mock_stat, mock_exists, parser):
        ...
        content = yaml.dump(
            {
                "name": "test_rule",
                "sections": {"config": {"timeout": 30}, "rules": ["rule1", "rule2"]},
...
"""
    ...
    def test_parse_json_valid(self, parser):
        ...
        assert (
            refs == []
        )  # References are extracted from content, not from a direct field
