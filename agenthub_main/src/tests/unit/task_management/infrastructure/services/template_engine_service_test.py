...
class TestTemplateEngineService(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        with patch(
            "fastmcp.task_management.infrastructure.services.template_engine_service.pybars"
        ):
    ...
    @patch(
        "fastmcp.task_management.infrastructure.services.template_engine_service.time"
    )
    ...
    async def test_render_template_cache_hit(self):
        ...
        template_id = TemplateId.generate_new()
        request = TemplateRenderRequest(
            template_id=template_id, variables={"test": "value"}, force_regenerate=False
        )
    ...
    async def test_render_template_force_regenerate(self):
        ...
        template_id = TemplateId.generate_new()
        request = TemplateRenderRequest(
            template_id=template_id, variables={"force": True}, force_regenerate=True
        )
    ...
    async def test_render_template_not_found(self):
        ...
        # Arrange
        template_id = TemplateId.generate_new()
        request = TemplateRenderRequest(template_id=template_id, variables={})
    ...
    async def test_render_template_render_error(self):
        ...
        template_id = TemplateId.generate_new()
        request = TemplateRenderRequest(
            template_id=template_id, variables={"test": "value"}
        )

        self.mock_registry_service.get_template.return_value = {"content": "{{test}}"}
    ...
    async def test_resolve_variables_hierarchy(self):
        ...
        # Act
        resolved = await self.service._resolve_variables(
            template_id, variables, task_context
        )
    ...
    async def test_clear_cache_all(self):
        ...
        # Arrange
        self.service._compiled_templates = {"template1": Mock(), "template2": Mock()}
    ...
    async def test_check_cache_redis_error_handling(self):
        ...
        # Arrange
        request = TemplateRenderRequest(
            template_id=TemplateId.generate_new(), variables={}
        )
    ...
    async def test_init_without_pybars(self):
        ...
        with self.assertRaises(ImportError) as context:
            TemplateEngineService(registry_service=self.mock_registry_service)
    ...
    async def test_check_cache_without_redis(self):
        ...
        with patch(
            ...
            service_no_redis = TemplateEngineService(
                registry_service=self.mock_registry_service, redis_client=None
            )

        # Arrange
        request = TemplateRenderRequest(
            template_id=TemplateId.generate_new(), variables={"test": "value"}
        )
    ...
    async def test_clear_cache_specific_template_not_in_compiled(self):
        ...
        self.service._compiled_templates = {"other-template": Mock()}

        # Mock Redis with keys to delete
        redis_keys = [
            b"template:non-existent-template:hash1",
            b"template:non-existent-template:hash2",
        ]
