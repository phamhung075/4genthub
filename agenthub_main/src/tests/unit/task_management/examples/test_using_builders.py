...
class TestBuilderExamples:
    ...
    def test_project_with_agents_and_branches(self):
        ...
        # Create agents
        backend_agent = (
            an_agent().with_name("Backend Dev").with_capabilities(["backend"]).build()
        )

        frontend_agent = (
            an_agent().with_name("Frontend Dev").with_capabilities(["frontend"]).build()
        )
    ...
    def test_multiple_builders_in_test(self):
        ...
        session = a_work_session().for_agent(agent.id).for_task(task.id.value).build()
...
class TestBuilderPatterns:
    ...
    def test_builder_as_fixture(self):
        ...
        def high_priority_task():
            return a_task().high_priority().with_labels(["urgent"]).build()
    ...
    def test_builder_factory_pattern(self):
        ...
        def a_bug_task():
            return a_task().with_labels(["bug"]).high_priority()

        def a_feature_task():
            return a_task().with_labels(["feature"]).with_estimated_effort("1 week")
