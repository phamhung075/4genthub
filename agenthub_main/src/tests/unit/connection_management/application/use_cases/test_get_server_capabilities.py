...
class TestGetServerCapabilitiesUseCase(unittest.TestCase):
    ...
    def test_execute_with_existing_server(self):
        ...
        self.assertEqual(
            response.available_actions, self.mock_capabilities.available_actions
        )
    ...
    @patch(
        "fastmcp.connection_management.application.use_cases.get_server_capabilities.logger"
    )
    ...
    def test_execute_with_mvp_mode_enabled(self):
        ...
        mock_mvp_capabilities.available_actions = {"task": ["create", "list"]}
