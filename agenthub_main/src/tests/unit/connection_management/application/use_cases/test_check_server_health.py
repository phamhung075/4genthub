...
def check_server_health_use_case(mock_server_repository, mock_health_service):
    ...
    return CheckServerHealthUseCase(
        server_repository=mock_server_repository, health_service=mock_health_service
    )
...
class TestCheckServerHealthUseCase:
    ...
    def test_successful_health_check_with_existing_server(
        self, check_server_health_use_case, mock_server_repository, sample_server
    ):
    ...
    def test_health_check_creates_server_if_none_exists(
        self, check_server_health_use_case, mock_server_repository, mock_health_service
    ):
        ...
        new_server.name = "agenthub - Task Management & Agent Orchestration"
        new_server.version = "0.0.2c"
        new_server.environment = {
            "platform": "linux",
            "python_version": "3.10.0",
            "working_directory": "/test",
        }
        new_server.authentication = {
            "enabled": True,
            "method": "JWT",
            "status": "configured",
        }
        new_server.task_management = {
            "active_tasks": 5,
            "completed_tasks": 10,
            "total_tasks": 20,
        }
        ...
        assert response.success is True
        assert (
            response.server_name == "agenthub - Task Management & Agent Orchestration"
        )
    ...
    def test_health_check_handles_connection_info_error(
        ...
        # Arrange
        mock_server_repository.get_current_server.return_value = sample_server
        mock_health_service.validate_server_configuration.side_effect = Exception(
            "Connection error"
        )
        request = HealthCheckRequest(include_details=True)
    ...
    def test_health_check_handles_server_health_check_failure(
        ...
        # Arrange
        sample_server.check_health.side_effect = ServerHealthCheckFailedError(
            "Health check failed"
        )
        mock_server_repository.get_current_server.return_value = sample_server
        request = HealthCheckRequest(include_details=False)
        ...
        assert response.error == "Server health check failed: Health check failed"
        assert response.uptime_seconds == 0
        assert response.restart_count == 0

    def test_health_check_handles_unexpected_exception(
        self, check_server_health_use_case, mock_server_repository
    ):
        """Test health check handles unexpected exceptions"""
        # Arrange
        mock_server_repository.get_current_server.side_effect = Exception(
            "Unexpected error"
        )
        request = HealthCheckRequest(include_details=False)
