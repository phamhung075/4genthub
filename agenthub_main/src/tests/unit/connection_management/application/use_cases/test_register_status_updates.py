...
class TestRegisterStatusUpdatesUseCase(unittest.TestCase):
    ...
    def test_execute_with_empty_client_info(self):
        ...
        # Arrange
        session_id = str(uuid4())
        request = RegisterUpdatesRequest(session_id=session_id, client_info=None)
    ...
    def test_execute_handles_service_exception(self):
        ...
        self.mock_broadcasting_service.register_client_for_updates.side_effect = (
            Exception("Broadcasting service unavailable")
        )
    ...
    @patch(
        "fastmcp.connection_management.application.use_cases.register_status_updates.logger"
    )
    ...
    @patch(
        "fastmcp.connection_management.application.use_cases.register_status_updates.datetime"
    )
    ...
    def test_execute_with_large_client_info(self):
        ...
        # Arrange
        session_id = str(uuid4())
        large_client_info = {f"field_{i}": f"value_{i}" * 100 for i in range(100)}
        request = RegisterUpdatesRequest(
            session_id=session_id, client_info=large_client_info
        )
