...
class MockStatusBroadcastingService(StatusBroadcastingService):
    ...
    def register_client_for_updates(
    ...
    def broadcast_status_update(self, update: StatusUpdate) -> bool:
        ...
        if (
            hasattr(update, "session_id")
            and update.session_id in self.broadcast_failures
        ):
            return False
...
class TestMockStatusBroadcastingService:
    ...
    def test_get_registered_clients_count(self):
        ...
        # Register clients
        for i in range(3):
            self.service.register_client_for_updates(
                f"session_{i}", {"name": f"client_{i}"}
            )
    ...
    def test_service_call_logging(self):
        ...
        # Call methods
        update_result = self.service.register_client_for_updates(
