...
class TestSecureHealthCheck:
    ...
    def test_controller_sanitizes_response(self):
        ...
        if "environment" in result:
            assert (
                "auth_enabled" in result["environment"]
                or "database_configured" in result["environment"]
            )
        ...
        if "connections" in result:
            assert (
                "active_connections" in result["connections"]
                or "status" in result["connections"]
            )
    ...
    def test_error_handling_doesnt_expose_details(self):
        ...
        mock_facade = Mock(spec=ConnectionApplicationFacade)
        mock_facade.check_server_health.side_effect = Exception(
            "Database password is incorrect: mypassword123"
        )
