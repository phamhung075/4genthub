...
class TestConnectionMCPController:
    ...
    def test_health_check_with_user_id(self, controller, mock_connection_facade):
        ...
        assert result["success"] is True
        mock_connection_facade.check_server_health.assert_called_once_with(
            True, test_user_id
        )

    def test_health_check_exception_handling(self, controller, mock_connection_facade):
        """Test health check exception handling"""
        mock_connection_facade.check_server_health.side_effect = Exception(
            "Health check failed"
        )
        ...
        assert (
            result["message"] == "Health check temporarily unavailable"
        )  # Sanitized message
    ...
    def test_response_formatting_success(self, controller):
        ...
        assert formatted["server_name"] == "agenthub"
        ...
        assert (
            formatted["task_management"]["enabled"] is True
        )  # Sanitized response only includes "enabled"
    ...
    def test_response_formatting_error(self, controller):
        ...
        assert (
            formatted["message"] == "Health check failed"
        )  # Sanitized to generic message
        assert formatted["timestamp"] == 123456789

    @patch(
        "fastmcp.connection_management.interface.controllers.connection_mcp_controller.logger"
    )
    def test_initialization_logging(self, mock_logger, mock_connection_facade):
        ...
        mock_logger.info.assert_called_with(
            "Simplified ConnectionMCPController initialized with health check only"
        )
