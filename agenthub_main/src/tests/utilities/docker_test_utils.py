#!/usr/bin/env python3
"""
Docker Test Utilities for agenthub E2E Testing
...
class DockerTestManager:
    ...
    async def build_test_image(self, force_rebuild: bool = False) -> dict:
        ...
        try:
            ...
            logger.info(
                f"Docker directory exists: {(self.project_root / 'docker').exists()}"
            )
            ...
            if not dockerfile_path.exists():
                ...
                for alt_path in alternative_paths:
                    ...
                    if alt_path.exists():
                        ...
                        logger.info(
                            f"Found Dockerfile at alternative location: {dockerfile_path}"
                        )
                        break
                else:
            ...
            build_context = (
                ...
                and dockerfile_path.parent.name == "docker"
    ...
    async def start_test_container(
        ...
        try:
            ...
            logger.info(
                f"🚀 Starting test container: {container_name} on port {self.test_port}"
            )
    ...
    async def wait_for_container_ready(
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                response = requests.get(
                    f"http://localhost:{test_port}/health", timeout=5
                )
                if response.status_code == 200:
                    logger.info(
                        f"✅ Container {container_name} is ready on port {test_port}"
                    )
                    return True
        ...
        logger.error(
            f"❌ Container {container_name} failed to become ready within {timeout}s"
        )
        return False
    ...
    async def test_container_health(self, container_name: str) -> dict:
        ...
        try:
            ...
            health_response = requests.get(
                f"http://localhost:{test_port}/health", timeout=10
            )
            mcp_response = requests.get(
                f"http://localhost:{test_port}/mcp/", timeout=10
            )
        ...
        try:
            ...
            task_response = requests.post(
                ...
                json={
                    ...
                    "params": {
                        "name": "manage_task",
                        "arguments": {"action": "list", "project_id": "test_project"},
                    },
                },
                timeout=10,
            )
