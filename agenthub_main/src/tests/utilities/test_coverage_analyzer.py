...
class CoverageAnalyzer:
    ...
    def get_source_files(self, domain: str = None) -> list[Path]:
        ...
        for file_path in self.src_root.glob(pattern):
            ...
            if (
                not any(part.startswith("test") for part in file_path.parts)
        ...
        for category, files in untested.items():
            if files:
                ...
                for file_path in files:
                    ...
                    report.append(
                        f"  Test:   {expected_test.relative_to(self.tests_root)}"
                    )
                    report.append("")
...
def main():
    ...
    src_root = script_dir.parent.parent  # agenthub_main/src
    tests_root = script_dir.parent  # agenthub_main/src/tests
