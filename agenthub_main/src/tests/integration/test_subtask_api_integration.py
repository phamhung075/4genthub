...
@pytest.mark.skipif(
    not check_server_available(), reason="API server not running on localhost:8000"
)
class TestSubtaskAPIIntegration:
    ...
    def setup_test_task(self, api_headers, setup_test_project_and_branch):
        ...
        response = requests.post(
            f"{API_V2_URL}/tasks", json=task_payload, headers=api_headers
        )

        # If the endpoint doesn't exist or returns error, skip the test
        if response.status_code not in [200, 201]:
            pytest.skip(
                f"Could not create test task: {response.status_code} - {response.text}"
            )

        task_data = response.json()

        # Return test data including the created task
        return {**test_data, "task_id": task_id, "task_data": task_data}

    def test_create_subtask_with_correct_parent_task_id(
        self, api_headers, setup_test_task
    ):
    ...
    def test_fetch_subtask_via_parent_task_endpoint(self, api_headers, setup_test_task):
        ...
        fetch_response = requests.get(
            f"{API_V2_URL}/tasks/{task_id}/subtasks/{subtask_id}", headers=api_headers
        )
    ...
    def test_list_subtasks_for_task(self, api_headers, setup_test_task):
        ...
        list_response = requests.get(
            f"{API_V2_URL}/tasks/{task_id}/subtasks", headers=api_headers
        )
    ...
    def test_update_subtask_maintains_parent_relationship(
        self, api_headers, setup_test_task
    ):
