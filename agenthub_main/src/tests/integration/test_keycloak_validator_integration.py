...
def valid_jwt_claims():
    ...
    return {
        ...
        "resource_access": {"test-client": {"roles": ["mcp-access"]}},
    }
...
class TestHappyPath:
    ...
    @pytest.mark.asyncio
    async def test_validate_mcp_token_success(
        self, validator, valid_jwt_claims, mock_jwks, create_test_token
    ):
        ...
        with patch.object(validator, "_get_jwks", return_value=mock_jwks):
            # Mock token decoding
            with patch.object(
                validator, "_decode_token", return_value=valid_jwt_claims
            ):
    ...
    @pytest.mark.asyncio
    async def test_role_extraction_from_realm_and_client(
        self, validator, valid_jwt_claims
    ):
    ...
    @pytest.mark.asyncio
    async def test_token_caching_mechanism(
        self, validator, valid_jwt_claims, mock_jwks, create_test_token
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_mcp_request_success(
        self, create_test_token, valid_jwt_claims, mock_jwks
    ):
        ...
        with patch(
            "fastmcp.auth.mcp_keycloak_validator.get_mcp_validator"
...
class TestErrorScenarios:
    ...
    @pytest.mark.asyncio
    async def test_validate_token_with_expired_claims(
        self, validator, valid_jwt_claims
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_missing_required_claims(
        self, validator, valid_jwt_claims
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_audience_list_without_expected(
        self, validator, valid_jwt_claims
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_token_audience_list_with_expected(
        self, validator, valid_jwt_claims
    ):
    ...
    @pytest.mark.asyncio
    async def test_decode_token_with_missing_kid(
        self, validator, mock_jwks, create_test_token
    ):
        ...
        with patch(
            "jose.jwt.get_unverified_header", return_value={"kid": "non-existent-key"}
        ):
    ...
    async def test_decode_token_jwt_error(
        ...
        with patch(
            ...
            with patch(
                "jose.jwt.decode", side_effect=jwt.JWTError("Invalid signature")
            ):
                result = await validator._decode_token(token, mock_jwks)
    ...
    @pytest.mark.asyncio
    async def test_validate_mcp_token_general_exception(
        self, validator, create_test_token
    ):
        """Test handling of unexpected exceptions during validation."""
        token = create_test_token()

        with patch.object(
            validator, "_get_jwks", side_effect=Exception("Unexpected error")
        ):
            result = await validator.validate_mcp_token(token)
...
class TestCachingScenarios:
    ...
    @pytest.mark.asyncio
    async def test_cache_cleanup_removes_expired_entries(
        self, validator, valid_jwt_claims
    ):
...
class TestEdgeCases:
    ...
    async def test_missing_realm_access(self, validator):
        ...
        claims = {"sub": "user-123", "resource_access": {}}
    ...
    async def test_missing_resource_access(self, validator):
        ...
        claims = {"sub": "user-123", "realm_access": {"roles": ["user"]}}
