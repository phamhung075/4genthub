...
class TestSubtaskUserIdFix:
    ...
    def setup_method(self, monkeypatch):
        ...
        try:
            self.session.query(SubtaskModel).filter(
                SubtaskModel.task_id == self.task_id
            ).delete()
            self.session.query(Task).filter(Task.id == self.task_id).delete()
            self.session.query(ProjectGitBranch).filter(
                ProjectGitBranch.id == self.branch_id
            ).delete()
    ...
    def test_create_subtask_with_user_id(self):
        ...
        db_subtask = (
            self.session.query(SubtaskModel)
            .filter(SubtaskModel.id == subtask_id.value)
            .first()
        )
    ...
    def test_create_subtask_without_user_id_uses_mvp_fallback(self):
        ...
        db_subtask = (
            self.session.query(SubtaskModel).filter_by(id=str(subtask_id)).first()
        )
    ...
    def test_subtask_repository_from_factory(self):
        ...
        if (
            hasattr(subtask_repo, "_is_mock")
            or subtask_repo.__class__.__name__ == "MockSubtaskRepository"
        ):
