...
class TestGitBranchZeroTasksDeletionIntegration:
    ...
    def _cleanup_test_data(self):
        ...
        try:
            ...
            session.query(Task).filter(
                Task.git_branch_id.in_(
                    [self.empty_branch_id, self.branch_with_tasks_id]
                )
            ).delete(synchronize_session=False)

            # Delete branches
            session.query(ProjectGitBranch).filter(
                ProjectGitBranch.project_id == self.project_id
            ).delete()
    ...
    def test_delete_branch_with_tasks_for_comparison(self):
        ...
        logger.info(
            f"🧪 Attempting to delete branch with tasks {self.branch_with_tasks_id}"
        )
        ...
        if result["success"]:
            ...
            logger.info(
                f"ℹ️ Branch with tasks deletion failed (might be expected): {result.get('error')}"
            )
    ...
    def test_list_branches_shows_task_counts_correctly(self):
        ...
        logger.info(
            f"Empty branch task count in list: {empty_branch_data.get('task_count', 'MISSING')}"
        )
        logger.info(
            f"Tasks branch task count in list: {tasks_branch_data.get('task_count', 'MISSING')}"
        )

        # The task count might be in different fields depending on implementation
        empty_count = (
            empty_branch_data.get("task_count", 0)
            or empty_branch_data.get("total_tasks", 0)
            or 0
        )
        tasks_count = (
            tasks_branch_data.get("task_count", 0)
            or tasks_branch_data.get("total_tasks", 0)
            or 0
        )

        # Note: The counts might not match database exactly due to caching or calculation differences
        logger.info(
            f"Reported task counts - Empty: {empty_count}, With tasks: {tasks_count}"
        )
    ...
    def test_edge_case_delete_nonexistent_branch(self):
        ...
        # Debug: print the actual result to understand its structure
        logger.info(f"Result from delete non-existent branch: {result}")
        ...
        error_found = (
            ...
            or "NOT_FOUND" in str(result)
            or (
                result.get("error")
                and "not found" in result.get("error", {}).get("message", "").lower()
            )
            or result.get("error_code") == "NOT_FOUND"
        )
        assert error_found, f"Expected 'not found' error indication in result: {result}"
    ...
    def test_comprehensive_branch_lifecycle(self):
        ...
        created_branch_id = (
            create_result.get("data", {}).get("git_branch", {}).get("id")
        )
