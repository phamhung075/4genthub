...
class TestGitBranchFilteringIntegration:
    ...
    def _create_test_data(self):
        ...
        if not self.task_a1.get("success", False):
            raise RuntimeError(
                f"Failed to create task A1: {self.task_a1.get('error', 'Unknown error')}"
            )
        if not self.task_a2.get("success", False):
            raise RuntimeError(
                f"Failed to create task A2: {self.task_a2.get('error', 'Unknown error')}"
            )
        if not self.task_b1.get("success", False):
            raise RuntimeError(
                f"Failed to create task B1: {self.task_b1.get('error', 'Unknown error')}"
            )
        if not self.task_b2.get("success", False):
            raise RuntimeError(
                f"Failed to create task B2: {self.task_b2.get('error', 'Unknown error')}"
            )
        ...
        if "task" in self.task_a1:
            logger.info(
                f"  Task A1 git_branch_id: {self.task_a1['task'].get('git_branch_id', 'MISSING')}"
            )
            logger.info(
                f"  Task A2 git_branch_id: {self.task_a2['task'].get('git_branch_id', 'MISSING')}"
            )
            logger.info(
                f"  Task B1 git_branch_id: {self.task_b1['task'].get('git_branch_id', 'MISSING')}"
            )
            logger.info(
                f"  Task B2 git_branch_id: {self.task_b2['task'].get('git_branch_id', 'MISSING')}"
            )
    ...
    def test_list_tasks_filters_by_branch_a(self):
        ...
        request = ListTasksRequest(git_branch_id=self.branch_a_id, limit=50)
        ...
        for i, task in enumerate(returned_tasks):
            logger.info(
                f"  Task {i + 1}: {task.get('title', 'NO TITLE')} | git_branch_id: {task.get('git_branch_id', 'MISSING')}"
            )
        ...
        for i, task in enumerate(no_filter_tasks):
            logger.info(
                f"  All Task {i + 1}: {task.get('title', 'NO TITLE')} | git_branch_id: {task.get('git_branch_id', 'MISSING')}"
            )
    ...
    def test_list_tasks_filters_by_branch_b(self):
        ...
        request = ListTasksRequest(git_branch_id=self.branch_b_id, limit=50)
    ...
    def test_list_tasks_without_filter_returns_all_tasks(self):
        ...
        request = ListTasksRequest(limit=50)
    ...
    def test_optimized_repository_direct_filtering(self):
        ...
        repo_a = ORMTaskRepository(
            git_branch_id=self.branch_a_id, performance_mode=True
        )
        repo_b = ORMTaskRepository(
            git_branch_id=self.branch_b_id, performance_mode=True
        )
    ...
    def teardown_method(self):
        ...
        try:
            ...
            logger.info(
                "Integration test completed - test data cleanup handled by test framework"
            )
        except Exception as e:
...
if __name__ == "__main__":
    ...
    pytest.main(
        [
            ...
            "--tb=short",
            "-k",
            "test_list_tasks_filters_by_branch_a",  # Run specific test first
        ]
    )
