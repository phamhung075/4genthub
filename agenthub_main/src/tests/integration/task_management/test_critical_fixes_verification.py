...
class TestCriticalFixesVerification:
    ...
    def test_subtask_requires_authentication(self):
        ...
        with patch(
            "fastmcp.task_management.interface.mcp_controllers.auth_helper.auth_helper.get_authenticated_user_id"
        ) as mock_auth:
            mock_auth.side_effect = ValueError(
                "User authentication required for task lookup"
            )
    ...
    def test_context_creation_with_none_data(self):
        ...
        with patch.object(
            service, "_get_user_scoped_repository", return_value=mock_repo
        ):
            with patch.object(
                ...
                with patch.object(
                    service, "_create_context_entity", return_value=mock_entity
                ):
                    with patch.object(
                        service,
                        "_ensure_parent_contexts_exist",
                        return_value={"success": True},
                    ):
                        with patch.object(
                            service.hierarchy_validator,
                            "validate_hierarchy_requirements",
                            return_value=(True, None, None),
                        ):
                            with patch.object(
                                service,
                                "_entity_to_dict",
                                return_value={"id": "test-id", "data": {}},
                            ):
                                ...
                                logger.info(
                                    "✅ Context creation with None data works correctly"
                                )
