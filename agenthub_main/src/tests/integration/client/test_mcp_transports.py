...
from fastmcp.client.transports import (  # noqa: E402, I001
    ClientTransport,
...
from fastmcp.server.server import FastMCP  # noqa: E402, I001
from fastmcp.utilities.mcp_config import MCPConfig  # noqa: E402, I001
# fmt: on
...
class TestStdioTransport:
    ...
    async def test_stdio_connect_session_with_keep_alive(self):
        ...
        transport = StdioTransport(
            command="python", args=["-c", "import sys"], keep_alive=True
        )
    ...
    async def test_stdio_connect_session_without_keep_alive(self):
        ...
        transport = StdioTransport(
            command="python", args=["-c", "import sys"], keep_alive=False
        )
    ...
    async def test_python_stdio_transport_valid_script(self, temp_python_script):
        ...
        transport = PythonStdioTransport(
            script_path=temp_python_script, args=["--test"], python_cmd=sys.executable
        )
    ...
    async def test_node_stdio_transport_valid_script(self, temp_js_script):
        ...
        transport = NodeStdioTransport(
            script_path=temp_js_script, args=["--test"], node_cmd="node"
        )
...
class TestSSETransport:
    ...
    async def test_sse_transport_bearer_auth(self):
        ...
        transport = SSETransport(url="http://example.com/mcp", auth="test-token-123")
    ...
    async def test_sse_transport_oauth_auth(self, mock_oauth_cls):
        ...
        transport = SSETransport(url="http://example.com/mcp", auth="oauth")
    ...
    async def test_sse_transport_custom_httpx_auth(self):
        ...
        transport = SSETransport(url="http://example.com/mcp", auth=custom_auth)
    ...
    async def test_sse_transport_timedelta_timeout(self):
        ...
        transport = SSETransport(
            url="http://example.com/mcp", sse_read_timeout=timedelta(seconds=45)
        )
...
class TestStreamableHttpTransport:
    ...
    async def test_streamable_http_bearer_auth(self):
        ...
        transport = StreamableHttpTransport(
            url="https://api.example.com/mcp", auth="secret-token"
        )
...
class TestFastMCPTransport:
    ...
    async def test_fastmcp_transport_with_raise_exceptions(self, mock_fastmcp_server):
        ...
        transport = FastMCPTransport(mcp=mock_fastmcp_server, raise_exceptions=True)
...
class TestUvxTransport:
    ...
    async def test_uvx_transport_initialization(self):
        ...
        transport = UvxStdioTransport(
            tool_name="my-tool", tool_args=["--arg1", "value1"]
        )
    ...
    async def test_uvx_transport_with_python_version(self):
        ...
        transport = UvxStdioTransport(tool_name="my-tool", python_version="3.11")
    ...
    async def test_uvx_transport_with_from_package(self):
        ...
        transport = UvxStdioTransport(tool_name="my-tool", from_package="my-package")
    ...
    async def test_uvx_transport_with_packages(self):
        ...
        transport = UvxStdioTransport(
            tool_name="my-tool", with_packages=["pkg1", "pkg2"]
        )
    ...
    async def test_uvx_transport_invalid_directory(self):
        ...
        with pytest.raises(NotADirectoryError, match="Project directory not found"):
            UvxStdioTransport(
                tool_name="my-tool", project_directory="/nonexistent/directory"
            )
...
class TestNpxTransport:
    ...
    async def test_npx_transport_initialization(self, mock_which):
        ...
        transport = NpxStdioTransport(package="my-package", args=["--arg1", "value1"])
    ...
    async def test_npx_transport_with_package_lock(self, mock_which):
        ...
        transport = NpxStdioTransport(package="my-package", use_package_lock=True)
    ...
    async def test_npx_transport_without_package_lock(self, mock_which):
        ...
        transport = NpxStdioTransport(package="my-package", use_package_lock=False)
    ...
    async def test_npx_transport_invalid_directory(self, mock_which):
        ...
        with pytest.raises(NotADirectoryError, match="Project directory not found"):
            NpxStdioTransport(
                package="my-package", project_directory="/nonexistent/directory"
            )
...
class TestTransportErrorHandling:
    ...
    async def test_stdio_transport_close(self):
        ...
        with patch.object(
            transport, "disconnect", new_callable=AsyncMock
        ) as mock_disconnect:
            await transport.close()
    ...
    async def test_sse_transport_timeout_as_int(self):
        ...
        transport = SSETransport(url="http://example.com/mcp", sse_read_timeout=30)
    ...
    async def test_sse_transport_timeout_as_float(self):
        ...
        transport = SSETransport(url="http://example.com/mcp", sse_read_timeout=45.5)
    ...
    async def test_streamable_http_timeout_as_int(self):
        ...
        transport = StreamableHttpTransport(
            url="http://example.com/mcp", sse_read_timeout=60
        )
...
class TestTransportConnectionExecution:
    ...
    async def test_stdio_actual_connect_execution(self, mock_stdio):
        ...
        with patch("mcp.ClientSession", side_effect=lambda *a, **k: mock_session_ctx()):
            transport = StdioTransport(
                command="python", args=["-c", "print('test')"], keep_alive=False
            )
            ...
            try:
                ...
                assert (
                    transport._session is not None
                    or transport._connect_task is not None
                )
            except TimeoutError:
    ...
    async def test_stdio_disconnect_flow(self):
        ...
        transport = StdioTransport(command="python", args=["-c", "pass"])
    ...
    async def test_sse_with_get_http_headers(self, mock_sse):
        ...
        with patch(
            "fastmcp.client.transports.get_http_headers",
            return_value={"X-Request-ID": "123"},
        ):
            with patch("mcp.ClientSession"):
                transport = SSETransport(
                    url="http://example.com/mcp", headers={"Custom": "value"}
                )
    ...
    async def test_uvx_with_env_vars(self):
        ...
        transport = UvxStdioTransport(
            tool_name="test-tool", env_vars={"CUSTOM_VAR": "value"}
        )
    ...
    async def test_npx_with_env_vars(self, mock_which):
        ...
        transport = NpxStdioTransport(
            package="test-pkg", env_vars={"NPM_CONFIG": "value"}
        )
