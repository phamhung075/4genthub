...
def truncate_all_tables(exclude_defaults=True):
    ...
    with db_config.get_session() as session:
        ...
        for table in table_order:
            if table in tables:
                if exclude_defaults and table in [
                    "projects",
                    "project_git_branchs",
                    "global_contexts",
                ]:
                    ...
                    if table == "projects":
                        session.execute(
                            text(f"DELETE FROM {table} WHERE id != 'default_project'")
                        )
                    elif table == "project_git_branchs":
                        session.execute(
                            text(
                                f"DELETE FROM {table} WHERE project_id != 'default_project'"
                            )
                        )
                    elif table == "global_contexts":
                        session.execute(
                            text(f"DELETE FROM {table} WHERE id != 'global_singleton'")
                        )
                else:
