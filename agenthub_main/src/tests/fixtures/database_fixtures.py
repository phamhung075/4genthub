...
def test_project_data(request):
    ...
    test_data = {"project_id": project_id, "git_branch_id": branch_id}
    ...
    with db_config.get_session() as session:
        try:
            ...
            session.execute(
                text("DELETE FROM tasks WHERE git_branch_id = :branch_id"),
                {"branch_id": branch_id},
            )
            # Delete branch
            session.execute(
                text("DELETE FROM project_git_branchs WHERE id = :id"),
                {"id": branch_id},
            )
            # Delete project
            session.execute(
