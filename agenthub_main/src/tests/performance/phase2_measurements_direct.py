...
def measure_metadata_flattening():
    ...
    total_chars = base_chars + structure_overhead + len(duplicate_fields['metadata_wrapper']) + len(duplicate_fields['objective_wrapper']) + len(duplicate_fields['progress_wrapper'])
...
def calculate_aggregate_savings():
    ...
    with_subtasks_chars = base_savings_chars + (parent_id['chars_saved_per_subtask'] * 5)
...
def generate_report():
    ...
    print(f"   Characters saved per response: {context_data['chars_saved_per_response']}")
    ...
    print(f"   Characters saved per response: {metadata_data['chars_saved_per_response']}")
    print(f"   Tokens saved per response: ~{metadata_data['tokens_saved_per_response']}")
    ...
    print(f"   Characters saved per response: {timestamp_data['chars_saved_per_response']}")
    print(f"   Tokens saved per response: ~{timestamp_data['tokens_saved_per_response']}")
    ...
    print(f"   Base savings (no subtasks): {agg['base_savings_chars']} chars = ~{agg['base_savings_tokens']} tokens")
    print(f"   With 5 subtasks: {agg['with_5_subtasks_chars']} chars = ~{agg['with_5_subtasks_tokens']} tokens")
    print()
    ...
    print(f"   With subtasks vs. target: {agg['with_subtasks_vs_target_percentage']:.1f}%")
    print(f"   Target met: {'✅ YES' if agg['target_met'] else '❌ NO'}")
    print()

    # Success criteria
    print("7. SUCCESS CRITERIA:")
    print(f"   ✓ context_data.id removal: {context_data['tokens_saved_per_response']} tokens")
    ...
    print()
    print(f"With typical 5-subtask scenarios, total savings reach {agg['with_5_subtasks_tokens']}")
