...
def project_with_branch(mcp_tools, mock_auth, test_user_id):
    ...
    try:
        mcp_tools._project_controller.manage_project(
            action="delete", project_id=project_id, force="true", user_id=test_user_id
        )
    except Exception:
        pass
...
class TestPhase1TokenSavings:
    ...
    def test_task_get_response_size(
        self, mcp_tools, mock_auth, project_with_branch, test_user_id
    ):
        """
        ...
        response_without_context = mcp_tools._task_controller.manage_task(
            action="get", task_id=task_id, include_context=False, user_id=test_user_id
        )

        # Measure WITH context (opt-in)
        response_with_context = mcp_tools._task_controller.manage_task(
            action="get", task_id=task_id, include_context=True, user_id=test_user_id
        )
        ...
        print(
            f"Without context (default): {size_without_context} chars = ~{tokens_without_context} tokens"
        )
        print(
            f"With context (opt-in): {size_with_context} chars = ~{tokens_with_context} tokens"
        )
        print(
            f"Context savings: {context_savings} chars = ~{context_token_savings} tokens"
        )
    ...
    def test_task_list_response_size(
        self, mcp_tools, mock_auth, project_with_branch, test_user_id
    ):
        """
    ...
    def test_subtask_list_response_size(
        self, mcp_tools, mock_auth, project_with_branch, test_user_id
    ):
        """
        ...
        subtask_list_response = mcp_tools._subtask_controller.manage_subtask(
            action="list", task_id=task_id, user_id=test_user_id
        )
        ...
        print(
            f"Total list size: {subtask_list_size} chars = ~{subtask_list_tokens} tokens"
        )
        print(f"Number of subtasks: {len(subtasks)}")
        print(
            f"Per-subtask average: {per_subtask_chars} chars = ~{per_subtask_tokens} tokens"
        )
    ...
    def test_emoji_field_removal_impact(
        self, mcp_tools, mock_auth, project_with_branch, test_user_id
    ):
        """
        ...
        # Get task response
        mcp_tools._task_controller.manage_task(
            action="get", task_id=result["data"]["task"]["id"], user_id=test_user_id
        )
    ...
    def test_count_field_removal_impact(
        self, mcp_tools, mock_auth, project_with_branch, test_user_id
    ):
        """
        ...
        # Get response
        mcp_tools._task_controller.manage_task(
            action="get", task_id=task_id, user_id=test_user_id
        )

        # Calculate what count fields would have added
        # Format: "subtask_count":3,"assignees_count":2,"dependency_count":1
        count_field_size = len(
            '"subtask_count":3,"assignees_count":2,"dependency_count":1'
        )
    ...
    def test_aggregate_token_savings(
        self, mcp_tools, mock_auth, project_with_branch, test_user_id
    ):
        """
        ...
        accuracy = (
            abs(percentage_reduction - predicted_percentage)
        ...
        print(
            f"Estimated baseline (before Phase 1): {estimated_baseline_size} chars = ~{estimated_baseline_tokens} tokens"
        )
        print(
            f"Current response (after Phase 1): {current_size} chars = ~{current_tokens} tokens"
        )
        print(
            f"Total savings: {total_chars_saved} chars = ~{total_tokens_saved} tokens"
        )
        ...
        print(
            f"Predicted reduction: ~{predicted_reduction} tokens ({predicted_percentage}%)"
        )
