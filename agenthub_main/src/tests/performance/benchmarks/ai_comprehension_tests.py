...
class AITaskProcessor:
    ...
    def process_task_response(
        ...
        try:
            ...
            for pattern_name, pattern in self.processing_patterns.items():
                matches = re.findall(
                    pattern, response_json, re.IGNORECASE | re.MULTILINE
                )
            ...
            accuracy = self._calculate_comprehension_accuracy(
                response_data, extracted_items
            )
    ...
    def _calculate_comprehension_accuracy(
        self, response_data: dict[str, Any], extracted_items: dict[str, int]
    ) -> float:
        """Calculate comprehension accuracy based on successful extractions."""
        weights = {
        ...
        for item_type, weight in weights.items():
            if item_type in extracted_items:
                ...
                score = min(
                    extracted_items[item_type]
                    / max(self._get_expected_count(response_data, item_type), 1),
                    1.0,
                )
...
class AIComprehensionBenchmark(PerformanceBenchmark):
    ...
    def _simulate_ai_analysis_delay(
        self, response_data: dict[str, Any], is_optimized: bool = False
    ) -> float:
    ...
    async def run_benchmark(self) -> BenchmarkResult:
        ...
        try:
            for scenario in self.test_scenarios:
                ...
                baseline_response = self.response_benchmark._create_baseline_response(
                    scenario
                )
                optimized_response = self.response_benchmark._create_optimized_response(
                    scenario
                )

                # Process baseline response
                baseline_result = self.ai_processor.process_task_response(
                    baseline_response
                )
                ...
                analysis_delay = self._simulate_ai_analysis_delay(
                    baseline_response, is_optimized=False
                )
                ...
                optimized_result = self.ai_processor.process_task_response(
                    optimized_response
                )
                ...
                analysis_delay = self._simulate_ai_analysis_delay(
                    optimized_response, is_optimized=True
                )
                ...
                if baseline_result.processing_time_ms > 0:
                    speed_improvement = (
                        ...
                        - optimized_result.processing_time_ms
                    ) / baseline_result.processing_time_ms
                    meets_speed_target = (
                ...
                metrics.extend(
                    [
                        self.create_metric(
                            f"{scenario}_baseline_processing_time",
                            baseline_result.processing_time_ms,
                            "ms",
                            None,
                            "processing",
                        ...
                        self.create_metric(
                            f"{scenario}_optimized_processing_time",
                            optimized_result.processing_time_ms,
                            "ms",
                            None,
                            "processing",
                        ...
                        self.create_metric(
                            f"{scenario}_baseline_accuracy",
                            baseline_result.comprehension_accuracy * 100,
                            "percent",
                            90.0,
                            "accuracy",
                        ),
                        self.create_metric(
                            f"{scenario}_optimized_accuracy",
                            optimized_result.comprehension_accuracy * 100,
                            "percent",
                            85.0,
                            "accuracy",
                        ),
                ...
                logger.info(
                    f"  Baseline processing: {baseline_result.processing_time_ms:.1f}ms (accuracy: {baseline_result.comprehension_accuracy:.2%})"
                )
                logger.info(
                    f"  Optimized processing: {optimized_result.processing_time_ms:.1f}ms (accuracy: {optimized_result.comprehension_accuracy:.2%})"
                )
                logger.info(
                    f"  Speed improvement: {speed_improvement:.1%} ({'✅' if meets_speed_target else '❌'})"
                )

            # Calculate overall statistics
            baseline_results = [
                r for r in comprehension_results if r.response_type == "baseline"
            ]
            optimized_results = [
                r for r in comprehension_results if r.response_type == "optimized"
            ]

            if baseline_results and optimized_results:
                avg_baseline_time = statistics.mean(
                    [r.processing_time_ms for r in baseline_results]
                )
                avg_optimized_time = statistics.mean(
                    [r.processing_time_ms for r in optimized_results]
                )
                avg_baseline_accuracy = statistics.mean(
                    [r.comprehension_accuracy for r in baseline_results]
                )
                avg_optimized_accuracy = statistics.mean(
                    [r.comprehension_accuracy for r in optimized_results]
                )

                overall_speed_improvement = (
                    avg_baseline_time - avg_optimized_time
                ) / avg_baseline_time
                accuracy_retention = (
                    avg_optimized_accuracy / avg_baseline_accuracy
                    if avg_baseline_accuracy > 0
                    else 1.0
                )

                # Parsing success rates
                baseline_success_rate = sum(
                    1 for r in baseline_results if r.parsing_success
                ) / len(baseline_results)
                optimized_success_rate = sum(
                    1 for r in optimized_results if r.parsing_success
                ...
                metrics.extend(
                    [
                        ...
                        self.create_metric(
                            ...
                            avg_baseline_accuracy * 100,
                            "percent",
                            None,
                            "overall",
                        ),
                        self.create_metric(
                            "avg_optimized_accuracy",
                            avg_optimized_accuracy * 100,
                            "percent",
                            None,
                            "overall",
                        ),
                        ...
                        self.create_metric(
                            "optimized_parsing_success_rate",
                            optimized_success_rate * 100,
                            "percent",
                            95.0,
                            "reliability",
                        ),
                ...
                logger.info(
                    f"  Speed improvement: {overall_speed_improvement:.1%} (target: 40%)"
                )
                logger.info(f"  Accuracy retention: {accuracy_retention:.1%}")
                logger.info(
                    f"  Parsing success: baseline {baseline_success_rate:.1%}, optimized {optimized_success_rate:.1%}"
                )
...
class ComprehensionQualityValidator:
    ...
    @staticmethod
    def validate_essential_information_preserved(
        baseline_response: dict[str, Any], optimized_response: dict[str, Any]
    ) -> dict[str, bool]:
        ...
        baseline_ids = set(
            re.findall(r'"id":\s*"([^"]+)"', json.dumps(baseline_response))
        )
        optimized_ids = set(
            re.findall(r'"id":\s*"([^"]+)"', json.dumps(optimized_response))
        )
        validation_results["task_ids_preserved"] = (
            baseline_ids.issubset(optimized_ids) if baseline_ids else True
        )

        # Check status information
        baseline_statuses = set(
            re.findall(r'"status":\s*"([^"]+)"', json.dumps(baseline_response))
        )
        optimized_statuses = set(
            re.findall(r'"status":\s*"([^"]+)"', json.dumps(optimized_response))
        )
        validation_results["statuses_preserved"] = len(optimized_statuses) >= len(
            baseline_statuses
        )

        # Check priority information
        baseline_priorities = len(
            re.findall(r'"priority":\s*"([^"]+)"', json.dumps(baseline_response))
        )
        optimized_priorities = len(
            re.findall(r'"priority":\s*"([^"]+)"', json.dumps(optimized_response))
        )
        validation_results["priorities_preserved"] = (
            optimized_priorities >= baseline_priorities
        )

        # Check assignee information
        baseline_assignees = len(
            re.findall(r"@([a-z-]+agent)", json.dumps(baseline_response))
        )
        optimized_assignees = len(
            re.findall(r"@([a-z-]+agent)", json.dumps(optimized_response))
        )
        validation_results["assignees_preserved"] = (
            optimized_assignees >= baseline_assignees
        )
    ...
    try:
        ...
        for metric in result.metrics:
            if metric.category in ["improvement", "overall", "quality", "reliability"]:
                ...
                print(
                    f"  {metric.name}: {metric.value:.1f}{metric.unit}{target_text} {status}"
                )
        ...
        if accuracy_retention_metric:
            ...
            print(
                f"95% Accuracy Retention Target: {'✅ MET' if retention_met else '❌ NOT MET'} ({accuracy_retention_metric.value:.1f}%)"
            )
