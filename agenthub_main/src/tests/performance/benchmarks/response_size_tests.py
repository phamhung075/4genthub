...
class ResponseSizeBenchmark(PerformanceBenchmark):
    ...
    async def setup(self):
        ...
        self.mock_server = create_performance_test_server(
            response_delay=0.01, error_rate=0.0
        )
    ...
    def _create_baseline_response(self, scenario: str) -> dict[str, Any]:
        ...
        return {
            "success": True,
            "data": {"message": f"Baseline response for {scenario}"},
        }
        ...
        try:
            for scenario in self.scenarios:
                ...
                reduction_percentage = (
                    (size_reduction / baseline_size) * 100 if baseline_size > 0 else 0
                )
                ...
                logger.info(
                    f"  Reduction: {reduction_percentage:.1f}% ({'✅' if meets_target else '❌'})"
                )
            ...
            metrics.extend(
                [
                    self.create_metric(
                        "avg_size_reduction", avg_reduction, "percent", 60.0, "overall"
                    ),
                    self.create_metric(
                        "min_size_reduction", min_reduction, "percent", 50.0, "overall"
                    ),
                    self.create_metric(
                        "max_size_reduction", max_reduction, "percent", None, "overall"
                    ),
                    self.create_metric(
                        "scenarios_meeting_target",
                        targets_met,
                        "count",
                        len(self.scenarios),
                        "overall",
                    ...
                    self.create_metric(
                        ...
                        (targets_met / len(self.scenarios)) * 100,
                        "percent",
                        80.0,
                        "overall",
                    ),
            ...
            logger.info(
                f"  Scenarios meeting target: {targets_met}/{len(self.scenarios)}"
            )
