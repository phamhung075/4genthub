...
async def run_individual_benchmark(benchmark_type: str, output_dir: Path = None):
    ...
    try:
        ...
        targets_met = (
            result.all_targets_met if hasattr(result, "all_targets_met") else True
        )
...
async def run_comprehensive_benchmarks(output_dir: Path = None, ci_mode: bool = False):
    ...
    if ci_mode:
        ...
        original_requests = PERFORMANCE_CONFIG["load_testing"][
            "max_concurrent_requests"
        ]

        PERFORMANCE_CONFIG["load_testing"]["test_duration_seconds"] = (
            60  # 1 minute instead of 5
        )
        PERFORMANCE_CONFIG["load_testing"]["max_concurrent_requests"] = (
            20  # Reduced concurrency
        )
    ...
    try:
        ...
        logger.info(
            f"Phase 4 targets met: {'✅ YES' if phase4_targets_met else '❌ NO'}"
        )
...
async def main():
    ...
    logger.info(
        f"Phase 4 Performance Benchmarks - Started at {datetime.now(UTC).isoformat()}"
    )
    ...
    if args.all or args.benchmark == "comprehensive":
        targets_met, results = await run_comprehensive_benchmarks(
            output_dir, args.ci_mode
        )
    elif args.benchmark:
        targets_met, results = await run_individual_benchmark(
