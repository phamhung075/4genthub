...
class DualModeConfig:
    ...
    def _get_project_root(self) -> Path:
        ...
        if self.runtime_mode == "http":
            ...
            fallback_indicators = ["pyproject.toml", "src", "agenthub_main"]

            for path in [current] + list(current.parents):
                if any(
                    (path / indicator).exists() for indicator in fallback_indicators
                ):
                    return path
    ...
    def get_environment_config(self) -> dict[str, Any]:
        ...
        if self.runtime_mode == "http":
            ...
            base_config.update(
                {"transport": "stdio", "container_mode": False, "auth_enabled": False}
            )
