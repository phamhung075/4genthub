...
class RedisEventStore(EventStore):
    ...
    async def _store_event_redis(self, event: SessionEvent) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Stored event {event.event_type} for session {event.session_id}, stream {event.stream_id}"
            )
            return True
    ...
    async def _store_event_memory(self, event: SessionEvent) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Stored event {event.event_type} for session {event.session_id}, stream {event.stream_id} in memory"
            )
            return True
    ...
    async def get_events(
        ...
        if self._using_fallback:
            return await self._get_events_memory(
                session_id, stream_id, event_type, limit
            )
        else:
            return await self._get_events_redis(
                session_id, stream_id, event_type, limit
            )
    ...
    async def _get_events_redis(
        ...
        try:
            ...
            raw_events = await self._redis.zrange(
                session_key, 0, limit - 1, withscores=False
            )
            ...
            logger.debug(
                f"Retrieved {len(events)} events for session {session_id}, stream {stream_id} from Redis"
            )
            return events
    ...
    async def _get_events_memory(
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {len(filtered_events)} events for session {session_id}, stream {stream_id} from memory"
            )
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        try:
            keys_to_delete = [
                ...
                if key.startswith(f"{self.key_prefix}{session_id}")
            ]
    ...
    async def replay_events_after(
        ...
        try:
            ...
            logger.debug(
                f"Replayed {len(events_to_replay)} events for session {session_id}, stream {stream_id}"
            )
            return last_sent_id
...
class MemoryEventStore(EventStore):
    ...
    async def get_events(
        ...
        """Get events for a session/stream"""
        key = (
            f"mcp:session:{session_id}:stream:{stream_id}"
            if stream_id
            else f"mcp:session:{session_id}"
        )
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        keys_to_delete = [
            ...
            if key.startswith(f"mcp:session:{session_id}")
        ]
        for key in keys_to_delete:
            del self._store[key]
        return True
    ...
    async def replay_events_after(
        ...
        try:
            ...
            logger.debug(
                f"Replayed {len(events_to_replay)} events for session {session_id}, stream {stream_id} (memory)"
            )
            return last_sent_id
...
def create_event_store(
    redis_url: str | None = None, fallback_to_memory: bool = True, **kwargs
) -> EventStore:
    """
    ...
    if REDIS_AVAILABLE and redis_url:
        ...
        return RedisEventStore(
            redis_url=redis_url, fallback_to_memory=fallback_to_memory, **kwargs
        )
    else:
