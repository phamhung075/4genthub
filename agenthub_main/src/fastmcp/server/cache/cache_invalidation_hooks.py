...
class CacheInvalidationHooks:
    ...
    async def on_subtask_created(subtask_id: str, parent_task_id: str):
        ...
        try:
            logger.info(
                f"Invalidating cache for new subtask {subtask_id} of task {parent_task_id}"
            )
    ...
    @staticmethod
    async def on_subtask_updated(
        subtask_id: str, parent_task_id: str, updates: dict[str, Any]
    ):
    ...
    async def on_bulk_operation(operation: str, affected_count: int):
        ...
        try:
            logger.info(
                f"Invalidating cache for bulk {operation} affecting {affected_count} items"
            )
            ...
            if result and isinstance(result, dict) and result.get("success"):
                try:
                    if invalidation_type == "task":
                        ...
                        parent_task_id = kwargs.get("parent_task_id") or kwargs.get(
                            "task_id"
                        )
            ...
            if result and isinstance(result, dict) and result.get("success"):
                try:
                    ...
                    if invalidation_type == "task":
                        ...
                        if subtask_id and parent_task_id:
                            loop.run_until_complete(
                                CacheInvalidationHooks.on_subtask_updated(
                                    subtask_id, parent_task_id, {}
                                )
                            )
                    elif invalidation_type == "context":
