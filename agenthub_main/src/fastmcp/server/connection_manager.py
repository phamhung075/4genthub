...
class ConnectionManager:
    ...
    async def register_connection(
        ...
        logger.info(
            f"Registered new MCP connection: {session_id} from {client_info.get('name', 'unknown')}"
        )
    ...
    async def get_connection_stats(self) -> dict[str, Any]:
        ...
        stats = {
            "server_info": {
                ...
                "uptime_seconds": (
                    datetime.now() - self.server_start_time
                ).total_seconds(),
        ...
        for session_id, conn in active_connections.items():
            client_stats = {
                ...
                "capabilities": list(conn.client_capabilities.keys())
                if conn.client_capabilities
                else [],
            }
        ...
        while self._running:
            try:
                ...
                if len(active_connections) > 0:
                    logger.debug(
                        f"Health check: {len(active_connections)} active connections"
                    )
    ...
    async def handle_server_restart(self):
        ...
        logger.info(
            f"Server restart detected (#{self.server_restart_count}). Existing connections marked for validation."
        )
    ...
    async def get_reconnection_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            if self.server_restart_count > 0
            else "continue",
