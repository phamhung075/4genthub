...
async def session_health_check(ctx: Context) -> dict[str, Any]:
    ...
    try:
        ...
        health_info = {
            ...
            "timestamp": ctx.request_context.session.created_at.isoformat()
            if hasattr(ctx.request_context.session, "created_at")
            else "unknown",
        }
        ...
        if ctx.session_id:
                    ...
                    health_info["recent_events"] = [
                        {
                            ...
                            "age_seconds": event.timestamp - event.timestamp
            ...
            health_info["session_warning"] = (
                "No session ID available - this may indicate session persistence issues"
            )
        ...
        if hasattr(event_store, "store_event") and ctx.session_id:
            try:
                test_success = await event_store.store_event(
                    ...
                    event_data={
                        "test": True,
                        "timestamp": health_info.get("timestamp", "unknown"),
                    },
        ...
        if warnings:
            ...
            health_info["overall_status"] = (
                "degraded" if len(warnings) <= 2 else "unhealthy"
            )
...
def register_session_health_tool(server):
    ...
    async def session_health_tool(ctx: Context) -> str:
        ...
        response += (
            f"- Session Store: {health_info.get('session_store_type', 'unknown')}\n"
        )
        response += f"- Session Active: {health_info.get('session_active', False)}\n"
        response += (
            f"- Current Session ID: {health_info.get('current_session_id', 'none')}\n"
        )
        ...
        if "redis_available" in health_info:
            response += "\n**Redis Information:**\n"
            response += (
                f"- Redis Available: {health_info.get('redis_available', False)}\n"
            )
            response += (
                f"- Redis Connected: {health_info.get('redis_connected', False)}\n"
            )
            response += (
                f"- Using Fallback: {health_info.get('using_fallback', False)}\n"
            )
            ...
            if health_info.get("redis_info"):
                ...
                response += (
                    f"- Memory Used: {redis_info.get('used_memory', 'unknown')}\n"
                )
