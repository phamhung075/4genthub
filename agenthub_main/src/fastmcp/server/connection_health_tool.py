    ...
    try:
        ...
        if restart_count > 0 and active_connections == 0:
            ...
            health_info["recommendation"] = (
                "Server was restarted. MCP clients should reconnect."
            )
        elif restart_count > 0:
            health_info["server_status"] = "restarted_with_clients"
            health_info["recommendation"] = (
                "Server was restarted but has active connections."
            )
        ...
        if restart_count > 0:
            warnings.append(
                f"Server has been restarted {restart_count} time(s). MCP clients may need to reconnect."
            )

        if connection_stats["connections"]["stale_connections"] > 0:
            warnings.append(
                f"{connection_stats['connections']['stale_connections']} stale connections detected."
            )

        if active_connections == 0 and ctx.session_id:
            warnings.append(
                "No active connections detected, but current session exists. Connection state may be inconsistent."
            )
...
def register_connection_health_tool(server):
    ...
    async def connection_health_tool(ctx: Context) -> str:
        ...
        # Active clients
        active_clients = health_info.get("connection_manager", {}).get(
            "active_clients", []
        )
        if active_clients:
