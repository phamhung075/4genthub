    ...
    try:
        ...
        try:
            ...
            status["connection_info"] = {
                ...
                "stale_connections": connection_stats["connections"][
                    "stale_connections"
                ],
            ...
            if restart_count > 0 and uptime < 60:
                ...
                status["server_info"]["message"] = (
                    "Server recently restarted, reconnection recommended"
                )
            elif active_connections == 0 and uptime > 60:
                status["server_info"]["status"] = "no_clients"
                status["server_info"]["message"] = (
                    "Server healthy but no active client connections"
                )
            else:
        ...
        try:
            ...
            status["broadcast_info"] = {"error": str(e), "broadcasting_active": False}
        ...
        try:
            ...
            status["auth_info"] = {
                "enabled": True,
                "production_mode": os.environ.get("PRODUCTION", "false").lower()
                == "true",
            }
        except Exception:
            status["auth_info"] = {"enabled": True, "production_mode": False}
        ...
        if status["server_info"]["status"] == "restarted":
            recommendations.append(
                "Use Cursor MCP toggle: Settings → Extensions → MCP → Toggle agenthub_http OFF/ON"
            )
        ...
        logger.info(
            f"MCP status check completed for session {ctx.session_id if ctx else 'unknown'}"
        )
        return status
        ...
        return {
            "timestamp": time.time(),
            "session_id": ctx.session_id if ctx else "unknown",
            "server_info": {"status": "error", "message": f"Status check failed: {e}"},
            "error": str(e),
        }
...
async def register_for_status_updates(ctx: Context) -> dict[str, Any]:
    ...
    try:
        if not ctx or not ctx.session_id:
            return {"success": False, "error": "No valid session ID for registration"}
...
def register_mcp_status_tools(server):
    ...
    async def mcp_status_tool(ctx: Context, include_details: bool = True) -> str:
        ...
        if "error" not in conn_info:
            ...
            response += (
                f"- Active Connections: {conn_info.get('active_connections', 0)}\n"
            )
            response += f"- Total Registered: {conn_info.get('total_registered', 0)}\n"
            response += (
                f"- Stale Connections: {conn_info.get('stale_connections', 0)}\n"
            )
            response += (
                f"- Server Restarts: {conn_info.get('server_restart_count', 0)}\n"
            )
        ...
        if broadcast_info.get("broadcasting_active"):
            ...
            response += (
                f"- Registered Clients: {broadcast_info.get('registered_clients', 0)}\n"
            )
            if broadcast_info.get("last_broadcast"):
                last_broadcast = broadcast_info["last_broadcast"]
                response += (
                    f"- Last Broadcast: {last_broadcast.get('event_type', 'unknown')} "
                )
        ...
        response += (
            f"- Container: {'Docker' if container_info.get('is_docker') else 'Local'}\n"
        )
        ...
        if include_details and status_info.get("active_clients"):
            ...
            for client in status_info["active_clients"]:
                ...
                response += (
                    f"  Connected: {client.get('connection_age_seconds', 0):.1f}s ago\n"
                )
        ...
        response += "- Server health: Visit your backend service URL at /health\n"
    ...
    async def register_status_updates_tool(ctx: Context) -> str:
        ...
        if result.get("success"):
            ...
            response += (
                f"- Update Interval: {result.get('update_interval', 30)} seconds\n"
            )
            ...
            response += (
                "- Use the `get_mcp_status` tool to check current status anytime\n"
            )
        else:
