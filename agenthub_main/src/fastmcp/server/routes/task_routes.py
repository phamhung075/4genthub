...
async def get_task_summaries(
    ...
    try:
        ...
        if not git_branch_id:
            return JSONResponse({"error": "git_branch_id is required"}, status_code=400)
        ...
        task_result = task_controller.list_tasks_summary(
            filters=filters, offset=offset, limit=limit, user_id=user_id, session=db
        )

        if not task_result.success:
            return JSONResponse(
                {"error": task_result.error or "Failed to fetch tasks"}, status_code=500
            )
        ...
        for task_dto in tasks_data:
            ...
            task_dict = (
                task_dto.model_dump()
                if hasattr(task_dto, "model_dump")
                else task_dto.dict()
            )

            # 🔍 DEBUG: Log task_data from DTO
            logger.info(
                f"🔍 REST API DEBUG - Task {task_dict.get('id', 'unknown')[:8] if task_dict.get('id') else 'no-id'}..."
            )
            logger.info(f"  - title: {task_dict.get('title')}")
            logger.info(
                f"  - subtask_count value: {task_dict.get('subtask_count', 'KEY NOT FOUND')}"
            )

            # Check if task has context - use API controller
            context_result = context_controller.get_context(
                "task", task_dict.get("id"), False, user_id, db
            )
            ...
            summary = {
                ...
                "subtask_count": task_dict.get(
                    "subtask_count", 0
                ),  # Use denormalized count from database
        ...
        logger.info(
            f"Returned {len(task_summaries)} task summaries, total: {total_count}"
        )
...
async def get_subtask_summaries(request: Request, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        user_id = current_user.id
        logger.info(
            f"Loading subtask summaries for user: {current_user.email if hasattr(current_user, 'email') else user_id}"
        )
        ...
        logger.info(
            f"Returned {len(subtask_summaries)} subtask summaries for task {parent_task_id}"
        )
        return response
...
async def get_task_context_summary(
    ...
    try:
        ...
        if not result.success:
            return {"has_context": False, "error": result.error}
...
async def get_performance_metrics(
    ...
    if REDIS_CACHE_ENABLED and cache_metrics:
        ...
        hit_rate = actual_metrics.get(
            "hit_rate", "0.00%"
        )  # stats is a dict, keep .get()
    else:
        actual_metrics = {}
