...
async def create_alert_rule(
    rule_data: dict[str, Any], user: dict[str, Any] = Depends(get_authenticated_user)
) -> dict[str, Any]:
    """
...
async def update_alert_rule(
    ...
    try:
        ...
        return {"message": "Alert rule updated successfully", "rule": asdict(rule)}
...
async def delete_alert_rule(
    rule_id: str, user: dict[str, Any] = Depends(get_authenticated_user)
) -> dict[str, Any]:
...
async def acknowledge_alert(
    event_index: int, user: dict[str, Any] = Depends(get_authenticated_user)
) -> dict[str, Any]:
...
async def test_webhook(
    webhook_data: dict[str, Any], user: dict[str, Any] = Depends(get_authenticated_user)
) -> dict[str, Any]:
    ...
    test_payload = {
        ...
        "threshold": 0.8,
        "severity": "info",
        "message": "This is a test alert from the agenthub system",
...
async def _check_and_trigger_alerts():
    ...
    try:
        ...
        # Get current metrics
        current_metrics = await get_performance_overview(
            mock_user, include_details=False
        )
        ...
        for rule in _alert_rules.values():
            ...
            if current_value is None:
                logger.warning(
                    f"Could not extract metric '{rule.metric}' for rule '{rule.name}'"
                )
                continue
            ...
            if rule.condition == "greater_than" and current_value > rule.threshold:
                ...
                and abs(current_value - rule.threshold) < 0.001
            ):
                triggered = True
...
def _extract_metric_value(metrics_data: dict, metric_name: str) -> float | None:
    ...
    try:
        ...
        if metric_name == "connection_wait_time":
            sqlite_data = (
                metrics_data.get("metrics", {}).get("connections", {}).get("sqlite", {})
            )
