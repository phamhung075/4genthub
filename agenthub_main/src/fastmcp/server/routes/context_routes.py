...
async def create_context(
    ...
    For global context, if context_id is 'me' or not provided, it will use the current user's ID.
    ...
    try:
        ...
        # For global context, if context_id is 'me' or not provided, use the current user's ID
        if level == "global" and (request.context_id == "me" or not request.context_id):
            request.context_id = str(current_user.id)

        # Log the access for audit
        logger.info(
            f"User {current_user.email} creating {level} context: {request.context_id}"
        )
...
async def get_context(
    ...
    For global context, context_id is ignored and the current user's ID is used automatically.
    Returns 404 if the context doesn't exist or doesn't belong to the user.
    """
    try:
        # For global context, always use the current user's ID (ignore provided context_id)
        if level == "global":
        ...
        if result.success:
            logger.info(
                f"User {current_user.email} accessed {level} context {context_id}"
            )
            return result.model_dump(by_alias=True)
        else:
            logger.warning(
                f"User {current_user.email} attempted to access non-existent or unauthorized {level} context {context_id}"
            )
            raise HTTPException(
        ...
        logger.error(
            f"Error getting {level} context {context_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_context(
    ...
    try:
        ...
        existing_result = context_controller.get_context(
            level=level, context_id=context_id, user_id=current_user.id, session=db
        )
        if not existing_result.success:
            logger.warning(
                f"User {current_user.email} attempted to update non-existent or unauthorized {level} context {context_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Context not found"
            )
        ...
        if result.success:
            logger.info(
                f"User {current_user.email} updated {level} context {context_id}"
            )
            return result.model_dump(by_alias=True)
        else:
            raise HTTPException(
        ...
        logger.error(
            f"Error updating {level} context {context_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delete_context(
    ...
    try:
        ...
        if not existing_result.success:
            logger.warning(
                f"User {current_user.email} attempted to delete non-existent or unauthorized {level} context {context_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Context not found"
            )
        ...
        if result.success:
            logger.info(
                f"User {current_user.email} deleted {level} context {context_id}"
            )
            return result.model_dump(by_alias=True)
        else:
            raise HTTPException(
        ...
        logger.error(
            f"Error deleting {level} context {context_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def resolve_context(
    ...
    try:
        ...
        if result.success:
            logger.info(
                f"User {current_user.email} resolved {level} context {context_id}"
            )
            return result.model_dump(by_alias=True)
        else:
            logger.warning(
                f"User {current_user.email} attempted to resolve non-existent or unauthorized {level} context {context_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Context not found"
            )
        ...
        logger.error(
            f"Error resolving {level} context {context_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def delegate_context(
    ...
    try:
        ...
        if not existing_result.success:
            logger.warning(
                f"User {current_user.email} attempted to delegate from non-existent or unauthorized {level} context {context_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Context not found"
            )
        ...
        logger.error(
            f"Error delegating {level} context {context_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def add_insight(
    ...
    try:
        ...
        if not existing_result.success:
            logger.warning(
                f"User {current_user.email} attempted to add insight to non-existent or unauthorized {level} context {context_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Context not found"
            )
        ...
        logger.error(
            f"Error adding insight to {level} context {context_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def add_progress(
    ...
    try:
        ...
        if not existing_result.success:
            logger.warning(
                f"User {current_user.email} attempted to add progress to non-existent or unauthorized {level} context {context_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Context not found"
            )
        ...
        logger.error(
            f"Error adding progress to {level} context {context_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_context_summary(
    ...
    try:
        ...
        logger.error(
            f"Error getting {level} context summary {context_id} for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
