...
class TokenCreateRequest(BaseModel):
    ...
    expires_in_days: int = Field(
        default=30, description="Token expiration in days", ge=1, le=365
    )
...
async def generate_token_handler(
    request: TokenCreateRequest, current_user: User, db: Session
) -> TokenResponse:
...
async def list_tokens_handler(
    ...
    if result["success"]:
        ...
        tokens = (
            result["tokens"][skip : skip + limit]
            if skip or limit != 100
            else result["tokens"]
        )
...
async def list_tokens(
    ...
    if result["success"]:
        ...
        token_responses = [TokenResponse(**token) for token in result["tokens"]]
        return TokenListResponse(data=token_responses, total=result["total"])
    else:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_token_details(
    ...
    result = await token_controller.get_token_details(
        token_id=token_id, user_id=current_user.id, session=db
    )
...
async def validate_token_endpoint(token: str, db: Session = Depends(get_db)):
    """Validate a token and return its claims"""

    # Delegate to controller following DDD pattern
    result = await token_controller.validate_token(token=token, session=db)
