    ...
    try:
        ...
        try:
            ...
            connection_metrics["sqlite"] = {
                ...
                "hit_rate": 1.0
                - (
                    sqlite_stats.get("pool_exhausted_count", 0)
                    / max(sqlite_stats.get("get_count", 1), 1)
                ),
            }
        ...
        try:
            ...
            overview["metrics"]["server_health"] = {
                ...
                "database_configured": environment_info.get(
        ...
        overview["system_health"] = (
            "healthy" if overview["performance_score"] > 70 else "degraded"
        )
        ...
        if include_details:
            overview["details"] = {
                "cache_distribution": {
                    name: data.get("size", 0)
                    for name, data in cache_metrics.items()
                    if isinstance(data, dict) and "size" in data
                },
                "connection_efficiency": {
                    "sqlite_avg_wait_ms": connection_metrics.get("sqlite", {}).get(
                        "avg_wait_time", 0
                    )
                    * 1000,
                    "total_connections_created": connection_metrics.get(
                        "sqlite", {}
                    ).get("connections_created", 0),
                },
...
async def get_performance_timeseries(
    ...
    interval: str = Query(
        "1h", regex="^(5m|15m|1h|6h|24h)$", description="Data interval"
    ),
) -> dict[str, Any]:
    """
    ...
    interval_minutes = {"5m": 5, "15m": 15, "1h": 60, "6h": 360, "24h": 1440}[interval]
...
def _generate_performance_recommendations(overview: dict[str, Any]) -> list[str]:
    ...
    if low_hit_rate_caches:
        recommendations.append(
            f"Consider increasing TTL or cache size for: {', '.join(low_hit_rate_caches)}"
        )
    ...
    if sqlite_data.get("avg_wait_time", 0) > 0.1:  # 100ms
        recommendations.append(
            "Consider increasing SQLite connection pool size due to high wait times"
        )

    # Performance score recommendations
    if overview["performance_score"] < 80:
        recommendations.append(
            "Overall performance below optimal - review caching strategy and connection pooling"
        )
...
async def clear_performance_cache(
    user: dict[str, Any] = Depends(get_authenticated_user),
    cache_name: str | None = Query(
        None, description="Specific cache to clear (default: all)"
    ),
) -> dict[str, Any]:
    """
    ...
    if cache_name:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Could not clear cache '{cache_name}': {e}"
            )
    else:
        # Clear all caches
        cache_names = ["default", "task_cache", "context_cache", "agent_cache"]
