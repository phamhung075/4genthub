    ...
    try:
        with get_database_session() as session:
            ...
            result = session.execute(
                context_query, {"user_id": user_id, "since_date": since_date}
            )

            # Process results
            daily_usage = defaultdict(
                lambda: {"task_contexts": 0, "global_contexts": 0, "total_accesses": 0}
            )
            ...
            hierarchy_result = session.execute(
                hierarchy_query, {"user_id": user_id, "since_date": since_date}
            ).fetchone()
            ...
            analytics_data = {
                ...
                "context_summary": {
                    ...
                    "context_utilization_rate": float(
                        hierarchy_result.context_utilization_rate or 0
                    ),
                },
                "daily_usage": dict(daily_usage),
                "hierarchy_health": {
                    "project_branch_ratio": (
                        (hierarchy_result.branches_used or 0)
                        / max(hierarchy_result.projects_used or 1, 1)
                    ),
                    "task_branch_ratio": (
                        (hierarchy_result.tasks_created or 0)
                        / max(hierarchy_result.branches_used or 1, 1)
                    ),
                },
            }
            ...
            if include_patterns:
                ...
                active_contexts = session.execute(
                    active_contexts_query,
                ...
                analytics_data["usage_patterns"] = {
                    ...
                    "context_lifecycle_insights": _analyze_context_lifecycle(
        ...
        raise HTTPException(
            status_code=500, detail="Failed to analyze context usage patterns"
        )
...
async def get_agent_performance_analytics(
    ...
    try:
        with get_database_session() as session:
            ...
            since_date = datetime.now() - timedelta(days=days)
            agent_results = session.execute(
                agent_performance_query, {"user_id": user_id, "since_date": since_date}
            ).fetchall()
            ...
            collaboration_results = session.execute(
                collaboration_query, {"user_id": user_id, "since_date": since_date}
            ).fetchall()
            ...
            for collab in collaboration_results:
                ...
                agent_team = (
                    collab.agent_team if isinstance(collab.agent_team, list) else []
                )
            ...
            for team, tasks in team_effectiveness.items():
                ...
                avg_duration = sum(t["duration"] for t in completed_tasks) / max(
                ...
                effective_teams.append(
                    {
                        ...
                        "effectiveness_score": (completion_rate / max(avg_duration, 1))
                        * 10,  # Normalized score
                    }
                )
            ...
            return {
                ...
                "performance_insights": {
                    ...
                    "avg_completion_rate": sum(
                        a["completion_rate"] for a in agent_metrics
                    )
                    / max(len(agent_metrics), 1),
                    "most_collaborative_agent": max(
                        agent_metrics, key=lambda x: x["total_tasks"]
                    )["agent_name"]
                    if agent_metrics
                    else None,
                    "specialization_recommendations": _generate_specialization_recommendations(
                        agent_metrics
                    ),
                },
            }
        ...
        raise HTTPException(
            status_code=500, detail="Failed to analyze agent performance"
        )
...
async def get_system_insights(
    ...
    try:
        with get_database_session() as session:
            ...
            since_date = datetime.now() - timedelta(days=days)
            activity_results = session.execute(
                activity_query, {"user_id": user_id, "since_date": since_date}
            ).fetchall()
                ...
                daily_metrics.append(
                    {
                        ...
                        "avg_completion_hours": float(
                            day.avg_task_completion_hours or 0
                        ),
            ...
            avg_productivity = sum(productivity_scores) / max(
                len(productivity_scores), 1
            )
            completion_rate = (
                total_tasks_completed / max(total_tasks_created, 1)
            ) * 100
            ...
            avg_branches_per_day = sum(
                d["branches_active"] for d in daily_metrics
            ) / max(len(daily_metrics), 1)
                ...
                "insights": {
                    ...
                    "key_metrics": {
                        ...
                        "most_productive_day": max(
                            daily_metrics, key=lambda x: x["productivity_score"]
                        )["date"]
                        if daily_metrics
                        else None,
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate system insights"
        )


def _analyze_context_lifecycle(
    session: Session, user_id: str, since_date: datetime
) -> dict[str, Any]:
    ...
    result = session.execute(
        lifecycle_query, {"user_id": user_id, "since_date": since_date}
    ).fetchone()
    ...
    return {
        ...
        "health_score": (
            (
                ...
                / max(
                    result.completed_contexts
                    + result.pending_contexts
                    + result.active_contexts
                    + result.blocked_contexts,
                    1,
                )
            )
            * 100
...
def _calculate_agent_efficiency(
    completion_rate: float, avg_hours: float, blocked_ratio: float
) -> float:
    ...
    speed_score = max(
        0, 30 - (avg_hours / 24) * 10
    )  # Penalty for tasks taking more than 24 hours
...
def _generate_specialization_recommendations(agent_metrics: list[dict]) -> list[str]:
    ...
    if top_performer["efficiency_score"] > 70:
        recommendations.append(
            f"Consider having {top_performer['agent_name']} mentor other agents"
        )
    ...
    if struggling_agents:
        recommendations.append(
            "Some agents have low completion rates - consider task reassignment or additional support"
        )
    ...
    if len(high_volume_agents) < len(agent_metrics) * 0.5:
        recommendations.append(
            "Consider specializing agents for specific task types to improve efficiency"
        )
...
def _calculate_completion_trend(daily_metrics: list[dict]) -> str:
    ...
    completion_rates = [
        (d["tasks_completed"] / max(d["tasks_created"], 1)) * 100 for d in daily_metrics
    ]
