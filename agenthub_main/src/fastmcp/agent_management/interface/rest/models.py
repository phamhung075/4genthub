...
class AgentTemplateResponse(BaseModel):
    ...
    rules: Any | None = Field(
        None, description="Agent rules (can be list of strings or list of rule objects)"
    )
    output_format: Any | None = Field(
        None, description="Output format preference (can be string or format object)"
    )
...
class UserAgentInstanceResponse(BaseModel):
    ...
    is_imported: bool = Field(
        default=False,
        description="Whether this is an imported agent (read-only for non-owners)",
    )
    original_creator_id: str | None = Field(
        None, description="Original creator's user ID (if imported)"
    )
    is_read_only: bool = Field(
        default=False, description="Whether current user can edit this agent"
    )
    ...
    capabilities: dict[str, Any] = Field(
        default_factory=dict, description="Active capabilities"
    )
    rules: Any | None = Field(
        None,
        description="Active rules (can be list of strings or list of rule objects)",
    )
    output_format: Any | None = Field(
        None, description="Active output format (can be string or format object)"
    )
...
class UserUsageStats(BaseModel):
    ...
    usage_by_agent: dict[str, int] = Field(
        default_factory=dict, description="Call count per agent"
    )
...
class ImportAgentRequest(BaseModel):
    ...
    share_token: str = Field(
        ..., description="64-character share token from shared agent"
    )
...
class SharedAgentPreviewResponse(BaseModel):
    ...
    template_slug: str = Field(..., description="Template slug")
    creator_display_name: str = Field(..., description="Creator attribution")
    configuration_preview: dict[str, Any] = Field(
        ..., description="Configuration details"
    )
    customizations_summary: str = Field(
        ..., description="Summary of changes from template"
    )
    can_import: bool = Field(
        default=True, description="Whether user can import this agent"
    )
    message: str | None = None
