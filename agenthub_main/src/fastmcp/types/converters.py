...
def task_to_dto(task: Any, include_subtasks: bool = False) -> TaskDTO:
    ...
    if isinstance(task, dict):
        ...
        return TaskDTO(
            ...
            subtasks=[subtask_to_dto(st) for st in subtasks]
    ...
    return TaskDTO(
        ...
        context_id=str(getattr(task, "context_id", None))
...
def subtask_to_dto(subtask: Any) -> SubtaskDTO:
    ...
    if isinstance(subtask, dict):
        ...
        return SubtaskDTO(
            id=str(subtask["id"]),
            task_id=str(
                subtask.get("parent_task_id", subtask.get("task_id", ""))
            ),  # Frontend expects task_id
            title=subtask["title"],
            ...
            progress_history=subtask.get("progress_history"),
            progress_count=subtask.get("progress_count"),
    ...
    parent_task_id = getattr(subtask, "parent_task_id", None) or getattr(
        subtask, "task_id", ""
    )
    ...
    return SubtaskDTO(
        ...
        progress_history=getattr(subtask, "progress_history", None),
        progress_count=getattr(subtask, "progress_count", None),
...
def task_summary_to_dto(task: Any) -> TaskSummaryDTO:
    ...
    if isinstance(task, dict):
        ...
        logger.info(
            f"  - subtask_count in dict: {task.get('subtask_count', 'MISSING')}"
        )
        logger.info(f"  - project_id in dict: {task.get('project_id', 'MISSING')}")
        logger.info(
            f"  - git_branch_id in dict: {task.get('git_branch_id', 'MISSING')}"
        )
    ...
    dto = TaskSummaryDTO(
        ...
        dependency_count=len(dependencies),
        has_context=bool(getattr(task, "context_id", None)),
        git_branch_id=str(getattr(task, "git_branch_id", None))
        if getattr(task, "git_branch_id", None)
        else None,  # Required by frontend validation
        project_id=str(getattr(task, "project_id", None))
    ...
    logger.info(
        f"  ✅ DTO created with subtask_count={dto.subtask_count}, project_id={dto.project_id}"
    )
    return dto
