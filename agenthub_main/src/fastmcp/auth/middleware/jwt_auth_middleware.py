...
class JWTAuthMiddleware:
    ...
    def __init__(self, secret_key: str, algorithm: str = "HS256", enabled: bool = True):
        ...
        logger.info(
            f"🔐 JWTAuthMiddleware initialized with secret length: {len(secret_key)} chars"
        )
        logger.debug(
            f"🔐 JWTAuthMiddleware secret (first 10 chars): {secret_key[:10]}..."
        )
    ...
    def extract_user_from_token(self, token: str) -> str | None:
        ...
        try:
            ...
            logger.debug(
                f"🔍 JWTAuthMiddleware attempting token decode with secret length: {len(self.secret_key)}"
            )
            ...
            try:
                ...
                logger.debug(
                    "✅ Token validated with audience='authenticated' (Supabase)"
                )
            except jwt.InvalidAudienceError:
                logger.debug(
                    "Token doesn't have 'authenticated' audience, trying without audience"
                )
            ...
            if not payload:
                try:
                    ...
                    logger.debug(
                        "✅ Token validated without audience check (local token)"
                    )
                except Exception as e:
            ...
            logger.error(
                f"❌ Using secret length: {len(self.secret_key)}, algorithm: {self.algorithm}"
            )
            logger.error(f"❌ Token (first 50 chars): {token[:50]}...")
            return None
        except Exception as e:
            logger.error(
                f"❌ JWTAuthMiddleware - Error extracting user from token: {e}"
            )
            return None
    ...
    def create_user_scoped_repository(
        self, repository_class: type, session: Session, user_id: str, **kwargs
    ) -> Any:
        """
        ...
        if hasattr(repo, "with_user"):
            ...
            logger.warning(
                f"Repository {repository_class.__name__} doesn't support user scoping"
            )
            return repo

    def create_user_scoped_service(
        self, service_class: type, user_id: str, **dependencies
    ) -> Any:
        """
        ...
        if hasattr(service, "with_user"):
            ...
            logger.warning(
                f"Service {service_class.__name__} doesn't support user scoping"
            )
            return service
...
class UserContextManager:
    ...
    def get_repository(self, repository_class: type, session: Session, **kwargs) -> Any:
        ...
        if cache_key not in self._scoped_repositories:
            middleware = JWTAuthMiddleware(
                "dummy_key"
            )  # Key not needed for this operation
            repo = middleware.create_user_scoped_repository(
                repository_class, session, self.user_id, **kwargs
            )
    ...
    def get_service(self, service_class: type, **dependencies) -> Any:
        ...
        if cache_key not in self._scoped_services:
            ...
            service = middleware.create_user_scoped_service(
                service_class, self.user_id, **dependencies
            )
