...
from ..token_validator import RateLimitError, TokenValidationError, TokenValidator
...
class DualAuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp):
        ...
        # Only initialize Supabase if it's the auth provider
        if auth_provider == "supabase":
            ...
            self.supabase_auth = None
            logger.info(f"Using {auth_provider} authentication - Supabase disabled")
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if not auth_enabled:
            ...
            logger.warning(
                "🔓 AUTH_ENABLED=false - Bypassing authentication (development mode)"
            )
            request.state.user_id = os.getenv(
            ...
            logger.info(
                f"🔓 Injected default user: {request.state.user_id} ({request.state.auth_info['email']})"
            )
        ...
        logger.debug(
            f"🔍 UNIFIED AUTH: Request type: {request_type} (for error formatting)"
        )
        ...
        try:
            ...
            if auth_result:
                ...
                logger.info(
                    f"✅ UNIFIED AUTH: Authenticated user {auth_result.get('user_id')} with {auth_result.get('auth_method')} method"
                )
            else:
            ...
            return self._create_auth_error_response(
                request_type, "Authentication system error"
            )
    ...
    def _detect_request_type(self, request: Request) -> str:
        ...
        if "application/json" in content_type and "jsonrpc" in str(
    ...
    async def _authenticate_request(
        self, request: Request, request_type: str
    ) -> dict[str, Any] | None:
        """
        ...
        if token.startswith("eyJ"):  # JWT tokens start with this
            try:
                logger.debug(
                    "🔍 UNIFIED AUTH: Detected JWT token, trying API token validation first"
                )
                ...
                try:
                    ...
                    if (
                        ...
                        logger.debug(
                            "🔍 UNIFIED AUTH: Token appears to be a Keycloak token, delegating to Keycloak handler"
                        )
                        # Import Keycloak validation
                        from ..keycloak_dependencies import validate_keycloak_token

                        try:
                            user = await validate_keycloak_token(token)
                            logger.info(
                                f"✅ UNIFIED AUTH: Keycloak token validated for user {user.id}"
                            )
                            return {
                                "user_id": user.id,
                                "email": user.email,
                            ...
                            logger.debug(
                                f"🔍 UNIFIED AUTH: Keycloak validation failed: {keycloak_error}"
                            )
                    ...
                    # Check if it's an API token (has token_id claim)
                        ...
                        logger.debug(
                            "🔍 UNIFIED AUTH: Token appears to be an API token, validating with JWT_SECRET_KEY"
                        )
                        auth_result = await self._validate_api_token(token)
                        if auth_result:
                            logger.info(
                                f"✅ UNIFIED AUTH: API token validated for user {auth_result.get('user_id')}"
                            )
                            return auth_result
                except Exception as decode_error:
                    logger.debug(
                        f"🔍 UNIFIED AUTH: Could not decode token for inspection: {decode_error}"
                    )
                ...
                if auth_result:
                    logger.info(
                        f"✅ UNIFIED AUTH: Local JWT validated for user {auth_result.get('user_id')}"
                    )
                    return auth_result
            except Exception as e:
            ...
            try:
                ...
                if result.success and result.user:
                    user_id = (
                        ...
                        else result.user.get("id")
                    )
                    email = (
                        result.user.email
                        if hasattr(result.user, "email")
                        else result.user.get("email")
                    )

                    # CRITICAL: Check that we actually got a valid user_id
                    if user_id:
                        logger.info(
                            f"✅ UNIFIED AUTH: Supabase token validated for user {user_id}"
                        )
                        return {
                            "user_id": user_id,
                            "email": email,
                        ...
                        logger.debug(
                            "🔍 UNIFIED AUTH: Supabase returned success but no user_id"
                        )
            except Exception as e:
                logger.debug(f"🔍 UNIFIED AUTH: Supabase validation failed: {e}")
        else:
            logger.debug(
                "🔍 UNIFIED AUTH: Supabase auth not available (using different provider)"
            )
        ...
        try:
            ...
            logger.info(
                f"✅ UNIFIED AUTH: MCP token validated for user {token_info.user_id}"
            )
            return {
    ...
    async def _authenticate_frontend_request(
        ...
        if not self.supabase_auth:
            logger.debug(
                "🔍 FRONTEND AUTH: Supabase not available, skipping frontend-specific auth"
            )
            return None
    ...
    async def _authenticate_mcp_request(
        self, request: Request
    ) -> dict[str, Any] | None:
        """
        ...
        if not token:
            token = request.headers.get("x-mcp-token") or request.headers.get(
                "mcp-token"
            )
        ...
        logger.debug(
            f"🔍 MCP AUTH: Validating MCP token (first 20 chars): {token[:20]}..."
        )

        # Special handling for JWT tokens (local development)
        if token.startswith("eyJ"):  # JWT tokens start with this
            logger.info(
                "🔍 MCP AUTH: Detected JWT token, trying dual secret validation"
            )
            ...
            for secret_name, secret_value in secrets_to_try:
                try:
                    logger.debug(
                        f"🔍 MCP AUTH: Trying JWT validation with {secret_name} (length: {len(secret_value)})"
                    )
                    ...
                    for token_type in ["api_token", "access"]:
                        ...
                        if secret_name == "SUPABASE_JWT_SECRET":
                            try:
                                ...
                                payload = pyjwt.decode(
                                    ...
                                    options={
                                        "verify_iss": False
                                    },  # Skip issuer check for Supabase
                                )
                                if payload:
                                    logger.info(
                                        "✅ MCP AUTH: Supabase JWT token validated with audience 'authenticated'"
                                    )
                                    break
                            except Exception as supabase_error:
                                logger.debug(
                                    f"🔍 MCP AUTH: Supabase token validation failed: {supabase_error}"
                                )
                        ...
                        try:
                            ...
                            if payload:
                                logger.info(
                                    f"✅ MCP AUTH: JWT token validated with {secret_name} as {token_type} type"
                                )
                                break
                        except Exception as type_error:
                            logger.debug(
                                f"🔍 MCP AUTH: Token type {token_type} failed with {secret_name}: {type_error}"
                            )
                            continue
                    ...
                    logger.debug(
                        f"🔍 MCP AUTH: JWT validation with {secret_name} failed: {jwt_error}"
                    )
                    continue

            logger.warning(
                "🔍 MCP AUTH: All JWT secret validation attempts failed, will try other methods"
            )
    ...
    def _extract_token(self, request: Request) -> str | None:
        ...
        if "token" in request.query_params:
            logger.warning(
                "⚠️ UNIFIED AUTH: Using token from query parameter (insecure)"
            )
            return request.query_params["token"]

        return None

    async def _validate_api_token(self, token: str) -> dict[str, Any] | None:
        """
                ...
                if token_id:
                    ...
                    async def track_usage():
                        try:
                            ...
                            from fastmcp.task_management.infrastructure.repositories.token_repository import (
                                TokenRepository,
                            )
                            ...
                            try:
                                ...
                                await repository.update_token_usage(
                                    token_id, operation=None
                                )
                                logger.info(
                                    f"✅ Tracked token usage for token_id: {token_id}"
                                )
                            finally:
                                db.close()
                        except Exception as track_error:
                            logger.error(
                                f"❌ Failed to track token usage: {track_error}",
                                exc_info=True,
                            )
    ...
    async def _validate_local_jwt(self, token: str) -> dict[str, Any] | None:
        """
    ...
    def _create_auth_error_response(
        self, request_type: str, error_message: str
    ) -> Response:
        """
