...
class DualAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def _authenticate_request(self, request: Request, request_type: str) -> dict[str, Any] | None:
        """
            ...
            try:
                ...
                try:
                    ...
                    if auth_provider == "keycloak" and keycloak_url and issuer.startswith(keycloak_url):
                        logger.debug("🔍 UNIFIED AUTH: Token appears to be a Keycloak token, delegating to Keycloak handler")
                        ...
                        try:
                            ...
                            logger.info(f"✅ UNIFIED AUTH: Keycloak token validated for user {user.id}")
                            return {
                                'user_id': user.id,
                                'email': user.email,
                            ...
                            logger.debug(f"🔍 UNIFIED AUTH: Keycloak validation failed: {keycloak_error}")
        ...
        if self.supabase_auth:
            ...
            logger.debug("🔍 UNIFIED AUTH: Supabase auth not available (using different provider)")
    ...
    async def _authenticate_frontend_request(self, request: Request) -> dict[str, Any] | None:
        """
        ...
        if not self.supabase_auth:
            logger.debug("🔍 FRONTEND AUTH: Supabase not available, skipping frontend-specific auth")
            return None
    ...
    async def _authenticate_mcp_request(self, request: Request) -> dict[str, Any] | None:
        """
                    ...
                    for token_type in ["api_token", "access"]:
                        ...
                        if secret_name == "SUPABASE_JWT_SECRET":
                            try:
                                ...
                                if payload:
                                    logger.info("✅ MCP AUTH: Supabase JWT token validated with audience 'authenticated'")
                                    break
