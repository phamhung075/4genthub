...
class AuthService:
    ...
    async def register_user(
        self, email: str, username: str, password: str, full_name: str | None = None
    ) -> RegistrationResult:
        """
        Register a new user
        ...
        try:
            ...
            if existing_user:
                return RegistrationResult(
                    success=False, error_message="Username already taken"
                )
            ...
            if strength["strength"] == "weak":
                ...
                return RegistrationResult(
                    success=False, error_message=f"Password too weak. {suggestions}"
                )
            ...
            return RegistrationResult(
                success=True, user=saved_user, verification_token=verification_token
            )
    ...
    async def login(
        self, email_or_username: str, password: str, ip_address: str | None = None
    ) -> LoginResult:
        """
        ...
        try:
            ...
            if not user:
                return LoginResult(success=False, error_message="Invalid credentials")
            ...
            if not user.can_login():
                if user.is_locked():
                    ...
                    return LoginResult(success=False, error_message="Account suspended")
                else:
                    return LoginResult(
                        success=False, error_message="Account not active"
                    )
            ...
            return LoginResult(success=False, error_message="Login failed")
    ...
    async def refresh_tokens(self, refresh_token: str) -> tuple[str, str] | None:
        ...
        try:
            ...
            if (
                user.refresh_token_family != token_family
                or user.refresh_token_version > token_version
            ):
                # Token has been rotated or revoked
                logger.warning(
                    f"Invalid refresh token version for user: {user.username}"
                )
                return None
