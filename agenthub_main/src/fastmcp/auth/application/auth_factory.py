...
class SupabaseAuthAdapter(AuthServiceInterface):
    ...
    async def reset_password_request(self, email: str) -> AuthResult:
        ...
        try:
            ...
            return AuthResult(
                success=False, error_message="Password reset request failed"
            )
...
class KeycloakAuthAdapter(AuthServiceInterface):
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(
                "Keycloak authentication adapter initialized with KeycloakAuth service"
            )
        except Exception as e:
    ...
    async def sign_up(
        ...
        **kwargs,
        ...
        return AuthResult(
            success=False, error_message="Keycloak signup not implemented"
        )
    ...
    async def sign_in(self, email: str, password: str) -> AuthResult:
        ...
        if not self.keycloak:
            return AuthResult(
                success=False, error_message="Keycloak service not available"
            )
    ...
    async def refresh_token(self, refresh_token: str) -> AuthResult:
        ...
        return AuthResult(
            success=False, error_message="Keycloak token refresh not implemented"
        )
    ...
    async def verify_token(self, access_token: str) -> AuthResult:
        ...
        return AuthResult(
            success=False, error_message="Keycloak token verification not implemented"
        )
    ...
    async def reset_password_request(self, email: str) -> AuthResult:
        ...
        return AuthResult(
            success=False, error_message="Keycloak password reset not implemented"
        )
    ...
    async def reset_password_confirm(self, token: str, new_password: str) -> AuthResult:
        ...
        return AuthResult(
            success=False,
            error_message="Keycloak password reset confirm not implemented",
        )
        ...
        try:
            ...
            try:
                result = await auth_service.register_user(
                    email, username or email.split("@")[0], password, full_name
                )
                ...
                if payload:
                    ...
                    if user_id:
                        return await auth_service.logout(
                            user_id, revoke_all_tokens=True
                        )
                return False
            finally:
                db.close()
    ...
    async def reset_password_request(self, email: str) -> AuthResult:
        ...
        try:
            ...
            try:
                ...
                return AuthResult(success=success, error_message=error)
            finally:
                db.close()
        except Exception as e:
    ...
    async def reset_password_confirm(self, token: str, new_password: str) -> AuthResult:
        ...
        try:
            ...
            try:
                ...
                return AuthResult(success=success, error_message=error_message)
            finally:
                db.close()
        except Exception as e:
...
class AuthFactory:
    ...
    def create_auth_service(
        cls, provider: AuthProvider | None = None
        ...
        if provider is None:
            ...
            try:
                ...
                logger.warning(
                    f"Unknown AUTH_PROVIDER '{provider_str}', defaulting to local"
                )
    ...
    def is_provider_available(cls, provider: AuthProvider) -> bool:
        ...
        if provider == AuthProvider.SUPABASE:
            ...
            return bool(
                ...
                and os.getenv("KEYCLOAK_CLIENT_ID")
                and os.getenv("KEYCLOAK_CLIENT_SECRET")
            )
        else:  # LOCAL
