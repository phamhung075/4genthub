...
def configure_jwt_auth_for_mcp(
    required_scopes: list[str] | None = None, database_session_factory=None
) -> JWTAuthBackend:
    """
...
def get_jwt_middleware(jwt_backend: JWTAuthBackend | None = None) -> list[Middleware]:
    """
...
def setup_mcp_with_jwt_auth(
    mcp_server, required_scopes: list[str] | None = None, database_session_factory=None
) -> None:
    """
...
def integrate_jwt_with_http_server(
    http_server_factory_kwargs: dict, database_session_factory=None
) -> dict:
    """
...
def configure_jwt_from_env() -> dict:
    ...
    config = {
        ...
        "issuer": os.getenv("JWT_ISSUER", "agenthub"),
