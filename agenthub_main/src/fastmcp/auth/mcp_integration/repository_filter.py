        ...
        try:
            ...
            if task:
                ...
                if task_user_id == user_id:
                    ...
                    logger.warning(
                        f"User {user_id} attempted to access task {task_id} belonging to {task_user_id}"
                    )
                    return None
            return None
        ...
        try:
            ...
            if project:
                ...
                if project_user_id == user_id:
                    ...
                    logger.warning(
                        f"User {user_id} attempted to access project {project_id} belonging to {project_user_id}"
                    )
                    return None
            return None
        except Exception as e:
...
class UserFilteredContextRepository(UserFilteredRepository):
        ...
        try:
            ...
            if context:
                ...
                if context_user_id == user_id:
                    ...
                    logger.warning(
                        f"User {user_id} attempted to access context {context_id} belonging to {context_user_id}"
                    )
                    return None
            return None
        except Exception as e:
    ...
    def find_all(self, **filters) -> list[Any]:
        ...
        try:
            ...
            global_filters[self._user_id_field] = (
                None  # Global contexts have no user_id
            )
            ...
            if context_level == "global":
                ...
                if not hasattr(context, "id") or context.id is None:
                    ...
                    if context_user_id is not None and context_user_id != user_id:
                        raise RuntimeError(
                            "Cannot save context belonging to another user"
                        )
    ...
    def delete(self, context_id: Any) -> bool:
        ...
        try:
            ...
            if context_level == "global":
                logger.warning(
                    f"User {self._get_current_user_id()} attempted to delete global context"
                )
                return False
...
def create_user_filtered_repository(
    repository_type: str, base_repository: Any, user_id_field: str = "user_id"
