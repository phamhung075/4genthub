...
class JWTAuthBackend(TokenVerifier):
    ...
    async def _validate_token_dual_auth(self, token: str) -> dict[str, Any] | None:
        ...
        for token_type in ["access", "api_token"]:
            try:
                payload = self._jwt_service.verify_token(
                    token, expected_type=token_type, expected_audience="mcp-server"
                )
                if payload:
                    logger.info(
                        f"✅ Token validated with local JWT secret as '{token_type}' type with audience 'mcp-server'"
                    )
                    return payload
                else:
                    logger.debug(
                        f"❌ Local JWT validation failed for type '{token_type}'"
                    )
            except Exception as e:
                logger.debug(
                    f"❌ Local JWT validation exception for type '{token_type}': {e}"
                )
        ...
        if supabase_jwt_secret:
            logger.info(
                f"✅ SUPABASE_JWT_SECRET found, length: {len(supabase_jwt_secret)}"
            )
            try:
                logger.info(
                    "🔍 Attempting to decode token with Supabase secret and 'authenticated' audience..."
                )
                ...
                if payload:
                    ...
                    logger.debug(
                        f"✅ User from Supabase token: {payload.get('sub')} ({payload.get('email')})"
                    )
            ...
            logger.debug(
                "❌ SUPABASE_JWT_SECRET not configured, skipping Supabase validation"
            )
    ...
    async def verify_token(self, token: str) -> AccessToken | None:
        ...
        try:
            ...
            if not payload:
                logger.error(
                    "❌ Token validation failed with both local and Supabase JWT secrets"
                )
                return None
    ...
    async def _get_user_context(
        self, user_id: str, payload: dict[str, Any] = None
    ) -> MCPUserContext | None:
        """
...
def create_jwt_auth_backend(
    database_session_factory=None, required_scopes: list[str] | None = None
) -> JWTAuthBackend:
    """
