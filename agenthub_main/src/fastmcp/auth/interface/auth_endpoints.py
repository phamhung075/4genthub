    ...
    def validate_password(cls, v):
        ...
        if errors:
            ...
            if len(errors) == 1 and "special character" in errors[0]:
                error_msg += (
                    " Try adding a special character like ! or @ to your password."
                )
...
async def register(request: RegisterRequest):
    ...
    logger.info(
        f"Registration attempt for email: {request.email}, username: {request.username or 'not provided'}"
    )
        ...
        try:
            ...
            async with httpx.AsyncClient(verify=False) as client:
                ...
                if token_response.status_code != 200:
                    ...
                    logger.warning(
                        f"Client credentials failed: {token_response.status_code} - {token_response.text}"
                    )
                    ...
                    if KEYCLOAK_CLIENT_SECRET:
                        ...
                        if admin_token_response.status_code == 200:
                            admin_token = admin_token_response.json().get(
                                "access_token"
                            )
                        else:
                            logger.error(
                                f"Admin login failed: {admin_token_response.status_code}"
                            )
                            raise HTTPException(
                                status_code=503,
                ...
                if not EMAIL_VERIFIED_AUTO:
                    ...
                    logger.info(
                        f"Email verification will be required for user: {request.email}"
                    )
                ...
                if create_response.status_code == 201:
                    ...
                    logger.info(
                        f"✅ User successfully registered in Keycloak: {request.email}"
                    )
                    ...
                    if user_id and user_id != "new-user":
                        try:
                            await setup_user_roles(
                                client, admin_token, user_id, request.email
                            )
                            # Add a small delay to ensure Keycloak processes the changes
                            await asyncio.sleep(0.5)
                        except Exception as e:
                            logger.warning(
                                f"Role setup failed for {request.email}: {e} - proceeding with login attempt"
                            )
                    ...
                    while login_attempt < max_login_attempts and not auto_login_token:
                        try:
                            ...
                            if login_response.status_code == 200:
                                ...
                                logger.info(
                                    f"Auto-login successful for newly registered user: {request.email} (attempt {login_attempt + 1})"
                                )
                                break
                            elif login_response.status_code == 400:
                                error_detail = login_response.json().get(
                                    "error_description", ""
                                )
                                if "Account is not fully set up" in error_detail:
                                    logger.info(
                                        f"Account not ready yet, attempt {login_attempt + 1}/{max_login_attempts}"
                                    )
                                    login_attempt += 1
                                else:
                                    break
                            ...
                            logger.warning(
                                f"Auto-login attempt {login_attempt + 1} failed: {e}"
                            )
                            login_attempt += 1

                    if not auto_login_token:
                        logger.warning(
                            f"Auto-login failed after {max_login_attempts} attempts for {request.email}"
                        )
                    ...
                    if not EMAIL_VERIFIED_AUTO:
                        ...
                        response = RegisterResponse(
                            ...
                            next_steps=[
                                ...
                                if not auto_login_token
                                else "You have been automatically logged in",
                    ...
                    if auto_login_token:
                        ...
                        response_dict["message"] = (
                            "🎉 SUCCESS: Account created and you have been automatically logged in!"
                        )
                    ...
                    logger.info(
                        f"User already exists, attempting cleanup for: {request.email}"
                    )

                    cleanup_result = await cleanup_incomplete_account_internal(
                        request.email
                    )

                    if cleanup_result.get("success") and cleanup_result.get(
                        "can_register"
                    ):
                        # Cleanup successful, retry registration
                        logger.info(
                            f"Cleanup successful, retrying registration for: {request.email}"
                        )
                        ...
                        if retry_response.status_code == 201:
                            logger.info(
                                f"✅ Registration successful after cleanup: {request.email}"
                            )

                            # Extract user ID from Location header
                            location = retry_response.headers.get("Location", "")
                            user_id = (
                                location.split("/")[-1] if location else "new-user"
                            )
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"Auto-login failed after cleanup registration: {e}"
                                )
                            ...
                            if auto_login_token:
                                ...
                                response_dict["auto_login_token"] = auto_login_token
                                response_dict["message"] = (
                                    "🎉 SUCCESS: Account fixed and you're now logged in!"
                                )
                                return response_dict
                            ...
                            # Retry also failed
                            logger.error(
                                f"Registration retry failed: {retry_response.status_code} - {retry_response.text}"
                            )
                            raise HTTPException(
                                status_code=409,
                    ...
                    try:
                        ...
                        error_msg = error_data.get(
                            "error_description",
                            error_data.get("error", "Invalid input"),
                        )
                    except Exception:
                        error_msg = create_response.text

                    logger.error(
                        f"Failed to create user: {create_response.status_code} - {error_msg}"
                    )
...
async def login(request: LoginRequest):
    ...
    if AUTH_PROVIDER == "keycloak":
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                if response.status_code == 200:
                    ...
                    decoded = jwt.decode(
                        token_data["access_token"], options={"verify_signature": False}
                    )
                    ...
                    logger.error(
                        f"Keycloak authentication failed for {request.email}: {error_details}"
                    )
                    ...
                    if (
                        "invalid_grant" in str(error_msg).lower()
                        and "not fully set up" in str(error_msg).lower()
                    ):
                        # Account exists but needs completion - provide clear instructions
                        logger.warning(
                            f"Account not fully set up for user: {request.email}"
                        )
                        raise HTTPException(
                            status_code=400,
                        ...
                        "invalid_scope" in str(error_msg).lower()
                        or "scope" in str(error_msg).lower()
                    ):
            ...
            if env in ["local", "development", "dev"]:
                logger.warning(
                    "Keycloak unavailable in development mode, issuing local JWT token"
                )
                ...
                if jwt_secret:
                    ...
                    payload = {
                        ...
                        + timedelta(hours=24),  # 24-hour expiry for development
                        "type": "local_dev",
                    }
                    ...
                    logger.error(
                        "JWT_SECRET_KEY not configured for development fallback"
                    )
        ...
        raise HTTPException(
            status_code=501, detail="Supabase authentication not implemented"
        )
    ...
    if AUTH_PROVIDER == "keycloak":
        ...
        try:
            ...
            async with httpx.AsyncClient(verify=False) as client:
                ...
                logger.debug(
                    f"Keycloak refresh response status: {response.status_code}"
                )
                ...
                if response.status_code == 200:
                    ...
                    return {
                        ...
                        "refresh_token": token_data.get(
                            "refresh_token", refresh_token
                        ),  # Return same refresh token if not provided
                        "expires_in": token_data.get("expires_in"),
                        "user_id": decoded.get("sub"),
                    ...
                    try:
                        ...
                        if "invalid_grant" in str(error_msg).lower():
                            if (
                                "token is not active" in str(error_msg).lower()
                        ...
                        logger.error(
                            f"Keycloak refresh token failed with status {response.status_code}: {response.text}"
                        )
                        raise HTTPException(
                    ...
                    logger.error(
                        f"Unexpected response from Keycloak: {response.status_code} - {response.text}"
                    )
        ...
        raise HTTPException(
            status_code=501, detail="Supabase token refresh is not yet implemented"
        )
...
async def cleanup_incomplete_account_internal(email: str) -> dict:
    ...
    if AUTH_PROVIDER == "keycloak":
            ...
            async with httpx.AsyncClient(verify=False) as client:
                ...
                if search_response.status_code == 200:
                    ...
                    if users and len(users) > 0:
                        ...
                        has_credentials = user.get(
                            "credentials", []
                        )  # Check if user has set password

                        # Log current account state for debugging
                        logger.info(
                            f"Account analysis for {email}: verified={email_verified}, enabled={account_enabled}, required_actions={required_actions}, has_credentials={bool(has_credentials)}"
                        )
                        ...
                        should_cleanup = (
                            not email_verified  # Email not verified
                            and required_actions  # Has pending required actions
                            and (
                                not account_enabled or not has_credentials
                            )  # And either disabled or no password set
                        )

                        if should_cleanup:
                            logger.warning(
                                f"🗑️ CLEANUP JUSTIFIED: Account {email} is incomplete (unverified={not email_verified}, required_actions={required_actions}, enabled={account_enabled}, has_credentials={bool(has_credentials)})"
                            )
                            ...
                            if delete_response.status_code == 204:
                                logger.info(
                                    f"Successfully cleaned up incomplete account: {email}"
                                )
                                return {
                                    "success": True,
                                ...
                                logger.error(
                                    f"Failed to delete user: {delete_response.status_code}"
                                )
                                return {
                                    "success": False,
                                    "message": "Failed to clean up account",
                                }
                        else:
                            # Account is verified and complete - DO NOT DELETE
                            logger.info(
                                f"✅ ACCOUNT PROTECTED: {email} is verified and complete (verified={email_verified}, enabled={account_enabled}, no_required_actions={not bool(required_actions)}, has_credentials={bool(has_credentials)})"
                            )
                            return {
                                "success": False,
        ...
        return {
            "success": False,
            "message": "Account cleanup not supported for this provider",
        }
...
async def setup_user_roles(client, admin_token: str, user_id: str, user_email: str):
    ...
    try:
        ...
        if user_response.status_code == 200:
            ...
            if update_response.status_code == 204:
                logger.info(
                    f"✅ User setup completed: email_verified={EMAIL_VERIFIED_AUTO}, required_actions=cleared"
                )
            else:
                logger.warning(
                    f"Failed to update user setup: {update_response.status_code} - {update_response.text}"
                )
    ...
    try:
        ...
        if clients_response.status_code == 200:
            ...
            if clients and len(clients) > 0:
                ...
                logger.info(
                    f"Found client {KEYCLOAK_CLIENT_ID} with UUID: {client_uuid}"
                )
                ...
                client_roles_response = await client.get(
                    client_roles_url, headers={"Authorization": f"Bearer {admin_token}"}
                )
                ...
                if client_roles_response.status_code == 200:
                    ...
                    if client_roles:
                        ...
                        if assign_client_roles_response.status_code == 204:
                            logger.info(
                                f"✅ Successfully assigned client roles to user {user_email}"
                            )
                        else:
                            logger.warning(
                                f"Could not assign client roles: {assign_client_roles_response.status_code}"
                            )
                    else:
                        logger.info("No client-specific roles found to assign")
                else:
                    logger.warning(
                        f"Could not fetch client roles: {client_roles_response.status_code}"
                    )
            else:
    ...
    try:
        ...
        roles_response = await client.get(
            realm_roles_url, headers={"Authorization": f"Bearer {admin_token}"}
        )
        ...
        if roles_to_assign:
            ...
            if assign_response.status_code == 204:
                logger.info(
                    f"✅ Successfully assigned {len(roles_to_assign)} roles to user {user_email}"
                )
                for role in roles_to_assign:
                    logger.info(f"   - Assigned role: {role['name']}")
            else:
                logger.error(
                    f"❌ Failed to assign roles: {assign_response.status_code} - {assign_response.text}"
                )
        else:
...
async def handle_registration_success(user_id: str, email: str):
    ...
    logger.info(
        f"✅ Registration complete for {email}. Onboarding information prepared."
    )
    ...
    return {
        "requirements": [
            {"rule": "min_length", "value": 8, "description": "At least 8 characters"},
            {
                "rule": "uppercase",
                "value": 1,
                "description": "At least 1 uppercase letter (A-Z)",
            },
            {
                "rule": "lowercase",
                "value": 1,
                "description": "At least 1 lowercase letter (a-z)",
            },
        ...
        "example_passwords": ["Password123!", "SecurePass@2024", "MyP@ssw0rd"],
        "tips": [
