        ...
        if hasattr(domain_user.status, "value"):
            ...
            if domain_user.status in [
                ...
                status_value = status_mapping.get(
                    domain_user.status, domain_user.status.lower()
                )
            else:
...
class UserTokenBalance(Base):
    ...
    # Usage statistics
    tokens_consumed_today: Mapped[int] = mapped_column(
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "available_tokens >= 0", name="check_available_tokens_non_negative"
        ),
