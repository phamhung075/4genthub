...
class EnhancedAuthService:
        ...
        try:
            ...
            if send_custom_email:
                try:
                    ...
                    token_data = token_manager.generate_verification_token(
                        email, "verification"
                    )
                    ...
                    email_token = EmailToken(
                        ...
                        user_id=supabase_result.user.get("id")
                        if supabase_result.user
                        else None,
                        metadata={
                            "registration": True,
                    ...
                    user_name = None
                    if metadata:
                        user_name = metadata.get("username") or metadata.get(
                            "full_name"
                        )
            ...
            return EnhancedAuthResult(
                ...
                error_message="Registration successful. Please check your email to verify your account."
                if email_sent
                else supabase_result.error_message,
            )

        except Exception as e:
            logger.error(f"Registration error: {e}")
            return EnhancedAuthResult(
                success=False, error_message=f"Registration failed: {str(e)}"
            )

    async def request_password_reset(
        self, email: str, send_custom_email: bool = True
    ) -> EnhancedAuthResult:
        """
        ...
        try:
            ...
            if send_custom_email:
                try:
                    ...
                    if not saved:
                        logger.warning(
                            f"Failed to save password reset token for {email}"
                        )
                    ...
                    if not email_result.success:
                        email_error = email_result.error_message
                        logger.error(
                            f"Failed to send password reset email: {email_error}"
                        )
                    else:
            ...
            return EnhancedAuthResult(
                success=False, error_message=f"Password reset request failed: {str(e)}"
            )
    ...
    async def verify_email_token(self, token: str, email: str) -> EnhancedAuthResult:
        ...
        try:
            ...
            email_token = self.token_repository.validate_token(
                token=token, email=email, token_type="verification", mark_used=True
            )
            ...
            try:
                ...
                if email_token.metadata and "user_metadata" in email_token.metadata:
                    ...
                    user_name = user_metadata.get("username") or user_metadata.get(
                        "full_name"
                    )
            ...
            return EnhancedAuthResult(
                success=False, error_message=f"Email verification failed: {str(e)}"
            )
    ...
    async def reset_password_with_token(
        ...
        try:
            # Validate token with repository
            email_token = self.token_repository.validate_token(
                token=token, email=email, token_type="password_reset", mark_used=True
            )

            if not email_token:
                return EnhancedAuthResult(
                    success=False, error_message="Invalid or expired reset token"
                )
            ...
            try:
                ...
                if not email_result.success:
                    email_error = email_result.error_message
                    logger.error(
                        f"Failed to send password changed email: {email_error}"
                    )
                else:
            ...
            return EnhancedAuthResult(
                success=False, error_message=f"Password reset failed: {str(e)}"
            )
    ...
    async def resend_verification_email(self, email: str) -> EnhancedAuthResult:
        ...
        try:
            ...
            if not saved:
                logger.warning(f"Failed to save verification token for {email}")
                return EnhancedAuthResult(
                    success=False, error_message="Failed to generate verification token"
                )
            ...
            if email_result.success:
                ...
                logger.error(
                    f"Failed to resend verification email: {email_result.error_message}"
                )
                return EnhancedAuthResult(
                    success=False,
                    email_sent=False,
    ...
    async def cleanup_expired_tokens(self, older_than_days: int = 7) -> dict[str, Any]:
        ...
        try:
            deleted_count = self.token_repository.cleanup_expired_tokens(
                older_than_days
            )
            return {
                "success": True,
