...
class UserRepository:
    ...
    def find_by_id(self, user_id: str) -> DomainUser | None:
        ...
        try:
            ...
            return DomainUser(
                ...
                if isinstance(db_user.status, str)
                else db_user.status,
                roles=[
                    UserRole(role) if isinstance(role, str) else role
                    for role in (db_user.roles or [])
                ],
    ...
    async def get_by_email(self, email: str) -> DomainUser | None:
        ...
        try:
            db_user = (
                self.session.query(UserModel).filter_by(email=email.lower()).first()
            )
            if not db_user:
                return None
    ...
    async def exists_by_email(self, email: str) -> bool:
        ...
        try:
            return (
                self.session.query(UserModel).filter_by(email=email.lower()).count() > 0
            )
        except Exception as e:
    ...
    async def exists_by_username(self, username: str) -> bool:
        ...
        try:
            return (
                self.session.query(UserModel).filter_by(username=username).count() > 0
            )
        except Exception as e:
