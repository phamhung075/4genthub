...
if os.getenv("SUPABASE_URL") and re.search(
    r"\d+\.\d+\.\d+\.\d+", os.getenv("SUPABASE_URL", "")
):
...
class SupabaseAuthService:
    ...
    def __init__(self):
        ...
        if is_self_hosted and not use_mock:
            ...
            try:
                ...
                if ip_match:
                    ...
                    if result != 0:
                        logger.warning(
                            f"⚠️  Self-hosted Supabase at {ip}:443 is not reachable"
                        )
                        logger.warning(
                            "⚠️  To use mock authentication, set USE_MOCK_AUTH=true in .env"
                        )
        ...
        if use_mock:
            ...
            logger.warning(
                "⚠️  Set USE_MOCK_AUTH=false when self-hosted instance is ready"
            )
        ...
        if is_self_hosted:
            ...
            self.admin_client: Client = create_client(
                ...
                or self.supabase_anon_key,  # Fallback to anon key if no service key
                options=ClientOptions(
    ...
    async def sign_up(
        ...
        try:
            ...
            if response.user:
                ...
                return SupabaseAuthResult(
                    success=True, user=response.user, session=response.session
                )
            else:
                return SupabaseAuthResult(
            ...
            if "SSL" in error_msg or "CERTIFICATE_VERIFY_FAILED" in error_msg:
                logger.warning(
                    "SSL certificate verification failed during signup - likely self-hosted instance"
                )
            ...
            # Parse common Supabase error messages
                ...
                return SupabaseAuthResult(
                    success=False, error_message="Email already registered"
                )
    ...
    async def sign_in(self, email: str, password: str) -> SupabaseAuthResult:
        ...
        try:
            ...
            if "SSL" in error_msg or "CERTIFICATE_VERIFY_FAILED" in error_msg:
                logger.warning(
                    "SSL certificate verification failed - likely self-hosted instance"
                )
    ...
    async def reset_password_request(self, email: str) -> SupabaseAuthResult:
        ...
        try:
            ...
            return SupabaseAuthResult(
                success=False, error_message="Failed to send password reset email"
            )
    ...
    async def update_password(
        ...
        try:
            ...
            if response.user:
                ...
                return SupabaseAuthResult(success=True, user=response.user)
            else:
                return SupabaseAuthResult(
                    success=False, error_message="Failed to update password"
                )

        except Exception as e:
            logger.error(f"Password update error: {e}")
            return SupabaseAuthResult(success=False, error_message=str(e))
        ...
        try:
            ...
            if hasattr(self, "admin_client") and self.admin_client:
                try:
                    ...
                    logger.debug(
                        f"Supabase native client failed, trying manual JWT validation: {supabase_error}"
                    )
            ...
            if self.supabase_jwt_secret:  # Use JWT secret for JWT verification
                try:
                    ...
                    logger.info(
                        f"Manual JWT validation successful for user: {user_data['email']}"
                    )

                    return SupabaseAuthResult(success=True, user=user_data)
    ...
    async def resend_verification_email(self, email: str) -> SupabaseAuthResult:
        ...
        try:
            ...
            response = self.client.auth.sign_up(
                {
                    "email": email,
                    "password": "temporary_resend_"
                    + os.urandom(16).hex(),  # Dummy password for resend
                    "options": {
                        "email_redirect_to": f"{os.getenv('FRONTEND_URL', 'http://localhost:3800')}/auth/verify"
                    },
                }
    ...
    async def refresh_session(self, refresh_token: str) -> SupabaseAuthResult:
        ...
        try:
            ...
            if response.user and response.session:
                logger.info(
                    f"Session refresh successful for user: {response.user.email}"
                )
                return SupabaseAuthResult(
                    success=True, user=response.user, session=response.session
                )
                ...
                logger.warning(
                    "Session refresh failed - no user or session in response"
                )
                return SupabaseAuthResult(
                    success=False, error_message="Invalid or expired refresh token"
                )
            ...
            if "Invalid Refresh Token" in error_msg or "refresh_token" in error_msg:
                return SupabaseAuthResult(
                    success=False, error_message="Refresh token expired or invalid"
                )
            elif "Network" in error_msg or "Connection" in error_msg:
                return SupabaseAuthResult(
                    success=False, error_message="Network error during token refresh"
                )
