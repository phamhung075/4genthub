...
class SupabaseTokenClient:
    ...
    def __init__(self):
        ...
        if not self.supabase_url or not self.api_key:
            logger.warning(
                "Supabase configuration not found. Token validation will be disabled."
            )
            self.enabled = False
        else:
            self.enabled = True
            if self.is_self_hosted:
                logger.info(
                    "Supabase token client initialized (self-hosted mode with relaxed SSL)"
                )
            else:
    ...
    async def validate_token(self, token: str) -> TokenInfo | None:
        ...
        if not self.enabled:
            ...
            logger.debug(
                "Supabase disabled, token validation not available via Supabase"
            )
            return None
        ...
        try:
            ...
            async with self._get_http_client() as client:
                response = await client.get(
                    ...
                    params={"token_hash": f"eq.{token_hash}", "is_active": "eq.true"},
                )
                ...
                return TokenInfo(
                    ...
                    last_used=datetime.fromisoformat(
                        token_data["last_used"].replace("Z", "+00:00")
                    )
    ...
    async def log_security_event(
        self, event_type: str, token_hash: str, details: dict[str, Any]
    ) -> None:
        """
