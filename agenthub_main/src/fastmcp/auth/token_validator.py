"""
Token validation and rate limiting for agenthub server.
...
class TokenValidator:
    ...
    def __init__(self, rate_limit_config: RateLimitConfig | None = None):
        ...
        self._rate_limits: dict[str, deque] = defaultdict(
            lambda: deque(maxlen=self.rate_config.requests_per_hour)
        )
        ...
        logger.info(
            f"Token validator initialized with rate limit: {self.rate_config.requests_per_minute}/min"
        )

    async def validate_token(
        self, token: str, client_info: dict | None = None
    ) -> TokenInfo:
        """
        ...
        try:
            ...
            await self._log_failed_attempt(
                token_hash, "rate_limit_exceeded", client_info
            )
            raise
    ...
    async def _check_rate_limit(self, token_hash: str) -> None:
        ...
        if recent_requests >= self.rate_config.requests_per_minute:
            ...
            raise RateLimitError(
                f"Rate limit exceeded: {recent_requests}/{self.rate_config.requests_per_minute} requests per minute"
            )
        ...
        if burst_requests >= self.rate_config.burst_limit:
            ...
            raise RateLimitError(
                f"Burst limit exceeded: {burst_requests}/{self.rate_config.burst_limit} requests per 10 seconds"
            )
    ...
    async def _log_failed_attempt(
        self, token_hash: str, reason: str, client_info: dict | None = None
    ) -> None:
        """
        ...
        if len(failures) >= 5:
            logger.warning(
                f"Multiple failed attempts for token: {len(failures)} failures in last hour"
            )
    ...
    def get_rate_limit_status(self, token: str) -> dict[str, int]:
        ...
        return {
            ...
            "remaining_minute": max(
                0, self.rate_config.requests_per_minute - minute_requests
            ),
            "remaining_hour": max(
                0, self.rate_config.requests_per_hour - hour_requests
            ),
        }
