...
class AITaskPlanningService:
    ...
    async def create_intelligent_plan(
        self, planning_request: PlanningRequest
    ) -> TaskPlan:
        """
        ...
        insights = self.requirement_analyzer.generate_planning_insights(
        ...
        await self._generate_tasks_from_requirements(
            plan, analyzed_requirements, planning_request
        )
        ...
        plan.confidence_score = self._calculate_confidence_score(
            plan, analyzed_requirements, insights
        )
        ...
        plan.execution_phases = [
            phase for phase in phase_order if phase in plan.execution_phases
        ]

        return plan

    async def _generate_tasks_from_requirements(
        self,
        plan: TaskPlan,
    ...
    async def _generate_subtasks_for_patterns(
        self, plan: TaskPlan, main_task: PlannedTask, analysis: AnalyzedRequirement
    ):
    ...
    async def _optimize_agent_assignments(self, plan: TaskPlan):
        ...
        for task in unassigned_tasks:
            ...
            if best_agent:
                ...
                plan.agent_workload[best_agent] = (
                    plan.agent_workload.get(best_agent, 0) + task.estimated_hours
                )
                plan.required_agents.add(best_agent)

    async def _find_optimal_agent(
        self, task: PlannedTask, plan: TaskPlan
    ) -> str | None:
        ...
        for agent, capabilities in self.agent_capabilities.items():
            ...
            if (
                ...
                and "testing" in capabilities["specializations"]
            ):
                score += 4
            elif (
                task.task_type in [TaskType.FEATURE, TaskType.TASK]
                and "backend" in capabilities["specializations"]
            ):
                score += 2
            elif (
                task.phase == ExecutionPhase.ARCHITECTURE
                and "architecture" in capabilities["specializations"]
            ):
                score += 4
    ...
    async def _generate_intelligent_dependencies(
        self, plan: TaskPlan, analyzed_requirements: list[AnalyzedRequirement]
    ):
        ...
        for parent_id, child_ids in parent_child_map.items():
            for child_id in child_ids:
                ...
                existing_dep = any(
                    (
                        dep.dependent_task_id == parent_id
                        and dep.prerequisite_task_id == child_id
                    )
                    or (
                        dep.dependent_task_id == child_id
                        and dep.prerequisite_task_id == parent_id
                    )
    ...
    def _tasks_are_related(self, task1: PlannedTask, task2: PlannedTask) -> bool:
        ...
        common_words = set(task1.title.lower().split()).intersection(
            set(task2.title.lower().split())
        )
    ...
    def _calculate_confidence_score(
        ...
        if analyzed_requirements:
            avg_effort = sum(
                ar.estimated_effort_hours for ar in analyzed_requirements
    ...
    def _determine_execution_phase(
        self, analysis: AnalyzedRequirement
    ) -> ExecutionPhase:
    ...
    def _map_complexity_level(self, analysis: AnalyzedRequirement) -> str:
        ...
        keyword_complexity = analysis.complexity_indicators.get(
            "keyword_complexity", "medium"
        )
        pattern_complexity = analysis.complexity_indicators.get(
            "pattern_complexity", 2.0
        )
    ...
    async def execute_plan_with_mcp(
        self, plan: TaskPlan, git_branch_id: str
    ) -> dict[str, Any]:
        """
        ...
        for root_task in root_tasks:
            try:
                ...
                failed_tasks.append({"planned_task_id": root_task.id, "error": str(e)})
