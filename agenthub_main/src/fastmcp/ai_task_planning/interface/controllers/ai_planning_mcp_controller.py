...
class AITaskPlanningMCPController:
    ...
    async def create_ai_plan(self, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(requirements_data, str):
                if requirements_data.startswith("[") or requirements_data.startswith(
                    ...
                    requirements_list = [
                        ...
                        for req in requirements_data.split(",")
                        if req.strip()
                    ]
            ...
            # Generate AI plan
            task_plan = await self.planning_service.create_intelligent_plan(
                planning_request
            )
            ...
            return {
                ...
                "task_plan": {
                    ...
                    "execution_phases": [
                        phase.value for phase in task_plan.execution_phases
                    ],
    ...
    async def analyze_requirements(self, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            analyzed_requirements = (
                self.planning_service.requirement_analyzer.analyze_requirements_batch(
                    requirement_items
                )
            )
            ...
            # Generate insights
            insights = (
                self.planning_service.requirement_analyzer.generate_planning_insights(
                    analyzed_requirements
                )
            )
                ...
                "detailed_analysis": [
                    {
                        ...
                        "detected_patterns": [
                            p.value for p in analysis.detected_patterns
                        ],
    ...
    async def estimate_effort(self, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Effort estimation failed: {str(e)}"}
    ...
    async def suggest_agents(self, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            if kwargs.get("available_agents"):
                available_agents = [
                    agent.strip() for agent in kwargs["available_agents"].split(",")
                ]
                if available_agents:
            ...
            return {
                ...
                "agent_suggestions": {
                    ...
                    "workload_distribution": self._estimate_agent_workload(
                        agent_recommendations,
                        analysis_result["analysis"]["total_estimated_hours"],
                    ),
            ...
            return {"success": False, "error": f"Agent suggestion failed: {str(e)}"}
    ...
    def _generate_recommendations(self, task_plan) -> list[str]:
        ...
        agent_workload = task_plan.agent_workload
        overloaded_agents = [
            agent for agent, hours in agent_workload.items() if hours > 40
        ]
        if overloaded_agents:
            recommendations.append(
                f"Agents may be overloaded: {', '.join(overloaded_agents)}"
            )
    ...
    def _estimate_agent_workload(
        self, agent_recommendations: dict[str, int], total_hours: float
    ) -> dict[str, float]:
    ...
    def _identify_specialization_needs(
