...
class PlannedTask:
    ...
    def can_run_in_parallel(self, other_task: PlannedTask) -> bool:
        ...
        if (
            ...
            and other_task.agent_assignment
            and self.agent_assignment.primary_agent
            == other_task.agent_assignment.primary_agent
        ):
            return False
    ...
    def to_mcp_task_request(self) -> dict[str, Any]:
        ...
        assignees = (
            [self.agent_assignment.primary_agent] if self.agent_assignment else []
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "agent_assignment": self.agent_assignment.to_dict()
...
class TaskPlan:
    ...
    # Agent allocation
    agent_workload: dict[str, float] = field(
        ...
        if task.id not in [t.id for t in self.tasks]:
            ...
            if task.agent_assignment:
                ...
                self.agent_workload[agent] = (
                    self.agent_workload.get(agent, 0) + task.estimated_hours
                )
                ...
                for support_agent in task.agent_assignment.supporting_agents:
                    support_hours = (
                        ...
                        * task.agent_assignment.effort_percentage.get(support_agent, 10)
                        / 100
                    )
                    self.agent_workload[support_agent] = (
                        self.agent_workload.get(support_agent, 0) + support_hours
                    )
    ...
    def validate_plan(self) -> tuple[bool, list[str]]:
        ...
        for dep in self.dependencies:
            if dep.dependent_task_id not in task_ids:
                errors.append(
                    f"Dependency references non-existent task: {dep.dependent_task_id}"
                )
            if dep.prerequisite_task_id not in task_ids:
                errors.append(
                    f"Dependency references non-existent task: {dep.prerequisite_task_id}"
                )
        ...
        if unassigned_tasks:
            errors.append(
                f"Tasks without agent assignments: {[task.id for task in unassigned_tasks]}"
            )
