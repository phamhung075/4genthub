        ...
        try:
            ...
            if action == "create":
                guidance["hints"].append(
                    "Subtask created successfully. Remember to update progress regularly."
                )
                guidance["next_actions"].append(
                    {"action": "update", "description": "Update progress as you work"}
                )
                guidance["rules"].append(
                ...
                if response.get("subtask", {}).get("progress_percentage", 0) >= 75:
                    guidance["hints"].append(
                        "Subtask is nearing completion. Prepare completion summary."
                    )
                guidance["next_actions"].append(
                    {"action": "complete", "description": "Complete when finished"}
                )

            elif action == "complete":
                guidance["hints"].append(
                    "Subtask completed! Parent task progress has been updated."
                )
                guidance["next_actions"].append(
                    {"action": "list", "description": "Check overall progress"}
                )
                guidance["recommendations"].append(
                    "Review parent task to see if it's ready for completion"
                )
                ...
                if completed == total and total > 0:
                    guidance["hints"].append(
                        f"All {total} subtasks complete! Parent task ready for completion."
                    )
                    guidance["recommendations"].append(
                        "Consider completing the parent task"
                    )
                elif completed > 0:
                    guidance["hints"].append(
                ...
                guidance["hints"].append(
                    "Subtask deleted. Parent task progress recalculated."
                )
                guidance["next_actions"].append(
                    {"action": "list", "description": "View remaining subtasks"}
                )
