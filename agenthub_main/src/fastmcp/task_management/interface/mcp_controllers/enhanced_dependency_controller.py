...
from ...application.services.dependency_resolver_service import (
...
class EnhancedDependencyController:
    ...
    def __init__(
        ...
        self.dependency_engine = DependencyManagementEngine(
            dependency_resolver, task_repository
        )
    ...
    def register_tools(self, mcp: "FastMCP"):
        ...
        def analyze_dependencies_ai(
            task_id: Annotated[
                str, Field(description="Task ID to analyze dependencies for")
            ],
            user_id: Annotated[
                str, Field(description="User ID for authentication")
            ] = None,
            include_suggestions: Annotated[
                bool, Field(description="Include AI dependency suggestions")
            ] = True,
            confidence_threshold: Annotated[
                float, Field(description="Minimum confidence for suggestions (0.0-1.0)")
            ] = 0.5,
        ) -> dict[str, Any]:
            """
        ...
        def suggest_dependencies(
            task_id: Annotated[
                str, Field(description="Task ID to generate suggestions for")
            ],
            user_id: Annotated[
                str, Field(description="User ID for authentication")
            ] = None,
            suggestion_types: Annotated[
                str,
                Field(
                    description="Comma-separated list of suggestion types: content,pattern,semantic,resource"
                ),
            ] = "content,pattern,resource",
            max_suggestions: Annotated[
                int, Field(description="Maximum number of suggestions to return")
            ] = 10,
        ) -> dict[str, Any]:
            """
        ...
        def manage_dependency_model(
            action: Annotated[
                str, Field(description="Action: train, retrain, status, info")
            ],
            user_id: Annotated[
                str, Field(description="User ID for authentication")
            ] = None,
            project_limit: Annotated[
                int, Field(description="Limit projects for training (optional)")
            ] = None,
        ) -> dict[str, Any]:
            """
            ...
            return self.handle_model_management(
                action=action, user_id=user_id, project_limit=project_limit
            )
        ...
        def optimize_project_dependencies(
            project_id: Annotated[
                str, Field(description="Project ID to optimize")
            ] = None,
            git_branch_id: Annotated[
                str, Field(description="Git branch ID to optimize")
            ] = None,
            user_id: Annotated[
                str, Field(description="User ID for authentication")
            ] = None,
            auto_apply: Annotated[
                bool,
                Field(description="Automatically apply high-confidence suggestions"),
            ] = False,
            confidence_threshold: Annotated[
                float, Field(description="Confidence threshold for auto-application")
            ] = 0.8,
        ) -> dict[str, Any]:
            """
        ...
        try:
            ...
            response = {
                ...
                "analysis": {
                    ...
                    "existing_dependencies": {
                        "depends_on": [
                            self._format_dependency_info(dep)
                            for dep in enhanced_relationships.basic_relationships.depends_on
                        ],
                        "blocks": [
                            self._format_dependency_info(dep)
                            for dep in enhanced_relationships.basic_relationships.blocks
                        ],
                    ...
                    "ai_analysis": {
                        "suggestions_generated": len(
                            enhanced_relationships.ai_suggestions
                        ),
            ...
            if include_suggestions:
                filtered_suggestions = [
                    s
                    for s in enhanced_relationships.ai_suggestions
                    if s.hint.confidence_score >= confidence_threshold
                ]

                response["analysis"]["ai_suggestions"] = [
                    self._format_suggestion(suggestion)
                    for suggestion in filtered_suggestions
                ]
                response["analysis"]["ai_analysis"]["high_confidence_suggestions"] = (
    ...
    def handle_dependency_suggestions(
        ...
        try:
            ...
            response = {
                ...
                "statistics": {
                    "avg_confidence": sum(
                        s.hint.confidence_score for s in limited_suggestions
                    )
                    ...
                    "high_confidence_count": len(
                        [
                            ...
                            if s.hint.confidence_score > 0.7
                        ]
                    ),
                    "medium_confidence_count": len(
                        [
                            s
                            for s in limited_suggestions
                            if 0.4 < s.hint.confidence_score <= 0.7
                        ]
                    ),
            ...
            logger.error(
                f"Error generating dependency suggestions for task {task_id}: {e}"
            )
            return {
                "success": False,
    ...
    def handle_model_management(
        self, action: str, user_id: str | None = None, project_limit: int | None = None
    ) -> dict[str, Any]:
        ...
        try:
            if action == "train":
                ...
                training_results = self.ml_predictor.train_model(
                    project_limit=project_limit, save_model=True
                )
    ...
    def handle_batch_optimization(
        ...
        try:
            ...
            for task in tasks:
                try:
                    ...
                    task_result = {
                        ...
                        "suggestions": [
                            self._format_suggestion(s) for s in suggestions[:5]
                        ],  # Top 5
                    }
                    ...
                    logger.error(
                        f"Error analyzing task {task.id} in batch optimization: {e}"
                    )
            ...
            total_suggestions = sum(
                r.get("suggestions_count", 0) for r in optimization_results
            )
            total_high_confidence = sum(
                r.get("high_confidence_count", 0) for r in optimization_results
            )
            ...
            return {
                ...
                "optimization_results": {
                    ...
                    "tasks_with_suggestions": len(
                        [
                            r
                            for r in optimization_results
                            if r.get("suggestions_count", 0) > 0
                        ]
                    ),
    ...
    def _get_tasks_by_branch(self, git_branch_id: str) -> list:
        ...
        try:
            ...
            return [
                ...
                if getattr(task, "git_branch_id", None) == git_branch_id
            ]
        except Exception as e:
    ...
    def _format_dependency_info(self, dep_info) -> dict[str, Any]:
        ...
        return {
            ...
            "updated_at": dep_info.updated_at.isoformat()
            if dep_info.updated_at
            else None,
        }
    ...
    def _format_suggestion(self, suggestion: DependencySuggestion) -> dict[str, Any]:
        ...
        return {
            ...
            "target_task": self._format_dependency_info(suggestion.target_task_info)
            if suggestion.target_task_info
            else None,
    ...
    def _get_type_breakdown(
        self, suggestions: list[DependencySuggestion]
    ) -> dict[str, int]:
