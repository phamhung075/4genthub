...
class AgentMCPController:
    ...
    def _get_facade_for_request(
        ...
        if user_id:
            ...
            logger.info(
                f"🎯 Agent Controller: get_current_user_id() returned: {context_user_obj} (type: {type(context_user_obj)})"
            )
            ...
            if context_user_obj:
                if isinstance(context_user_obj, str):
                    ...
                    logger.info(
                        f"🔧 Agent Controller: Extracted user_id from context object: {current_user_id}"
                    )
                else:
                    ...
                    logger.warning(
                        f"⚠️ Agent Controller: Fallback string conversion: {current_user_id}"
                    )

            # Let validate_user_id handle MVP mode fallbacks
            validated_user_id = validate_user_id(
                current_user_id, "Agent facade creation"
            )

        # Get facade from service for proper data isolation
        return self._facade_service.get_agent_facade(
            project_id=project_id, user_id=validated_user_id
        )
    ...
    def manage_agent(
        ...
        try:
            ...
            validation_result = self._validate_required_fields(
                action, agent_id, name, git_branch_id
            )
            ...
            return self._enhance_response_with_workflow_guidance(
                response, action, project_id, agent_id
            )
    ...
    def _validate_required_fields(
        ...
        if (
            action in ["get", "update", "unregister", "assign", "unassign"]
            and not agent_id
        ):
            return {
                "valid": False,
    ...
    def handle_rebalance_operation(
        self, project_id: str, user_id: str | None = None
    ) -> dict[str, Any]:
        """Backward compatibility method for tests - delegates to factory."""
        facade = self._get_facade_for_request(project_id, user_id)
    ...
    def _enhance_response_with_workflow_guidance(
        ...
        if response.get("success", False):
            ...
            workflow_guidance = self._workflow_guidance.generate_guidance(
                action, guidance_context
            )
