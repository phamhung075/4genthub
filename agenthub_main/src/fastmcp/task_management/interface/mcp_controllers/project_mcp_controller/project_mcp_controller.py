...
from .manage_project_description import (
...
class ProjectMCPController(ContextPropagationMixin):
    ...
    def __init__(
        self, facade_factory=None, facade_service: FacadeService | None = None
    ):
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run,
                        self._manage_project_async(action, user_id, **kwargs),
                    )
    ...
    async def _manage_project_async(
        self, action: str, user_id: str | None = None, **kwargs
    ) -> dict[str, Any]:
        ...
        try:
            ...
            user_id = get_authenticated_user_id(
                provided_user_id=user_id, operation_name=f"manage_project:{action}"
            )
            ...
            permission_result = self._check_project_permissions(
                action, user_id, kwargs.get("project_id")
            )
    ...
    def _check_project_permissions(
        ...
        try:
            ...
            if not required_permission:
                ...
                logger.warning(
                    f"Unknown project action '{action}' - allowing by default"
                )
                return True, None
            ...
            try:
                ...
                if not auth_info:
                    logger.error(
                        f"No authentication context found for permission check - user_id: {user_id}"
                    )
                ...
                has_permission = checker.has_permission(
                    ResourceType.PROJECTS, required_permission
                )

                if not has_permission:
                    logger.warning(
                        f"User {user_id} lacks permission for projects:{required_permission.value}"
                    )
                    return False, self._response_formatter.create_error_response(
                        operation=action,
                ...
                logger.debug(
                    f"User {user_id} has permission for projects:{required_permission.value}"
                )
                return True, None
                ...
                logger.warning(
                    f"Request context middleware not available - allowing project {action} for user {user_id}"
                )
                return True, None

        except Exception as e:
            logger.error(
                f"Error checking project permissions for user {user_id}, action {action}: {e}"
            )
    ...
    def handle_crud_operations(
        ...
        try:
            ...
            validation_error = self._validate_operation_parameters(
                action, project_id=project_id, name=name, description=description
            )
            if validation_error:
                return validation_error
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future = executor.submit(
                            asyncio.run,
                            self._operation_factory.handle_operation(
                                operation=action,
                                facade=facade,
                                user_id=user_id,
                                project_id=project_id,
    ...
    def handle_maintenance_operations(
        ...
        try:
            ...
            return self._handle_maintenance_action(
                action=action, project_id=project_id, force=force, user_id=user_id
            )
    ...
    def _handle_maintenance_action(
        self,
        action: str,
        project_id: str,
        force: bool | None = False,
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future = executor.submit(
                            asyncio.run,
                            maintenance_handler.handle_maintenance_action(
                                facade=facade,
                                action=action,
                                project_id=project_id,
                                force=force,
    ...
    def _include_project_context(self, response: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            from .....task_management.infrastructure.factories.unified_context_facade_factory import (
            ...
            context_facade = context_factory.create_facade(
                user_id=None
            )  # Tests don't provide user_id
            context_result = context_facade.get_context(
                "project", project_id, include_inherited=True
            )
    ...
    def _handle_list_projects(self) -> dict[str, Any]:
        ...
        try:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run, crud_handler.list_projects(facade=facade)
                    )
                    return future.result()
            else:
                return loop.run_until_complete(
                    crud_handler.list_projects(facade=facade)
                )
        except RuntimeError:
