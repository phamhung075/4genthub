...
class SubtaskOperationFactory:
    ...
    def __init__(
        ...
        self._progress_handler = ProgressHandler(
            context_facade=context_facade, task_facade=task_facade
        )
    ...
    def _handle_crud_operation(
        self, operation: str, facade, **kwargs
    ) -> dict[str, Any]:
        ...
        if (
            "assignees" in kwargs
            and kwargs["assignees"] is not None
            and isinstance(kwargs["assignees"], str)
        ):
        ...
        if operation == "create":
            # create_subtask only accepts: task_id, title, description, priority, assignees, progress_notes, user_id
            allowed_params = {
                "task_id",
                "title",
                "description",
            ...
            if "subtask_id" in kwargs:
                logger.debug(
                    f"Warning: subtask_id found in kwargs for list operation and will be filtered out: {kwargs.get('subtask_id')}"
                )
            logger.debug(
                f"List operation - Original kwargs: {list(kwargs.keys())}, Filtered kwargs: {list(filtered_kwargs.keys())}"
            )
        elif operation == "get":
            # get_subtask only accepts: task_id, subtask_id
            allowed_params = {"task_id", "subtask_id"}
            filtered_kwargs = {k: v for k, v in kwargs.items() if k in allowed_params}
        ...
        if operation == "create":
            ...
            if "subtask_id" in filtered_kwargs:
                logger.error(
                    f"CRITICAL: subtask_id still in filtered_kwargs for list operation: {filtered_kwargs}"
                )
        ...
        if result.get("success") and operation in [
            "create",
            "update",
            "delete",
            "complete",
            ...
            if task_id and result.get("subtasks"):
                ...
                progress_summary = self._progress_handler.get_progress_summary(
                    task_id=task_id, subtasks=result.get("subtasks", [])
                )
                result["progress_summary"] = progress_summary

        return result

    def _handle_progress_operation(
        self, operation: str, facade, **kwargs
    ) -> dict[str, Any]:
        ...
        try:
            if operation == "progress":
                ...
                return self._response_formatter.create_success_response(
                    operation="get_progress", data={"progress": progress}
                )
                ...
                return self._response_formatter.create_success_response(
                    operation="get_progress_summary", data={"summary": summary}
                )
