...
class AgentDiscoveryService:
    ...
    def get_available_agents(self) -> list[str]:
        ...
        # then into agenthub_main/agent-library/agents
        ...
        AGENT_DIR = os.path.join(project_root, "agenthub_main/agent-library/agents")
        available_agents = []
        ...
        try:
            ...
            if os.path.exists(abs_agent_dir):
                for entry in os.listdir(abs_agent_dir):
                    if entry.endswith("_agent") and os.path.isdir(
                        os.path.join(abs_agent_dir, entry)
                    ):
