...
class CRUDHandler:
    ...
    def create_task(
        ...
        if assignees:
            try:
                ...
                for assignee in assignees:
                    if assignee and assignee.strip():
                        ...
                        if resolved_assignee:
                            ...
                            logger.info(
                                f"Resolved legacy assignee '{assignee}' to '{resolved_assignee}'"
                            )
                        ...
                        elif clean_assignee.startswith("@") and AgentRole.is_valid_role(
                            clean_assignee[1:]
                        ):
                            # Already has @ prefix and is valid
                            validated_assignees.append(clean_assignee)
                            logger.info(
                                f"Validated assignee with @ prefix: '{clean_assignee}'"
                            )
                        else:
                            # Invalid assignee
                ...
                logger.info(
                    f"Validated {len(assignees)} assignees for task creation: {assignees}"
                )
    ...
    def update_task(
        ...
        # CONDITIONAL: details (progress_notes) required only for status or progress changes
        ...
        if progress_percentage is not None:
            try:
                ...
                logger.warning(
                    f"Ignoring invalid progress_percentage value: {progress_percentage}"
                )
    ...
    def get_task(
        ...
        logger.info(
            f"DEBUG: CRUDHandler.get_task called - task_id={task_id}, include_context={include_context}"
        )
        print(
            f"[CRUD DEBUG] get_task called with task_id={task_id}, include_context={include_context}"
        )
        if not task_id:
            return self._create_standardized_error(
                operation="get_task",
        ...
        if result.get("success") and include_context and result.get("task"):
            ...
            logger.info(
                f"DEBUG: get_task - include_context={include_context}, task_context_id={task_context_id}"
            )
            ...
            if task_context_id:
                try:
                    ...
                    inherited_context_result = context_facade.resolve_context(
                        level="task", context_id=task_context_id, include_inherited=True
                    )
                    ...
                    if inherited_context.get("success") and inherited_context.get(
                        "data"
                    ):
                    ...
                    logger.error(
                        f"Failed to fetch inherited context for task {task_id}: {e}"
                    )
                    result["task"]["context_available"] = True
                    result["task"]["inherited_context_available"] = False
            else:
    ...
    def complete_task(
        ...
        user_id = None
        if hasattr(facade, "_task_repository") and hasattr(
            facade._task_repository, "_user_id"
        ):
