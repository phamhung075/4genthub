...
class SearchHandler:
            ...
            if result.get("success") and "tasks" in result:
                ...
                for task in tasks:
                    minimal_task = {
                        ...
                        "git_branch_id": task.get(
                            "git_branch_id"
                        ),  # Include git_branch_id for filtering tests
    ...
    async def get_next_task(
        ...
        logger.info(
            f"DEBUG: SearchHandler get_next_task called with include_context={include_context}"
        )
        try:
            ...
            if (
                ...
                full_task_result = crud_handler.get_task(
                    facade, task_id, include_context=include_context
                )

                logger.info(
                    f"DEBUG: CRUDHandler get_task result - success={full_task_result.get('success')}, has_task={bool(full_task_result.get('task'))}"
                )
                ...
                if full_task_result.get("success") and full_task_result.get("task"):
                    ...
                    if include_context:
                        try:
                            ...
                            if parent_context:
                                ...
                                logger.info(
                                    f"DEBUG: Added parent contexts to task {task_id}"
                                )
                            else:
                            ...
                            logger.warning(
                                f"Failed to resolve parent contexts for task {task_id}: {e}"
                            )
                            task_data["parent_context_available"] = False
                    ...
                    logger.error(
                        f"Failed to get full task details for task {task_id}: {full_task_result}"
                    )
                    return self._response_formatter.create_error_response(
                        operation="next_task",
    ...
    def _resolve_parent_context_hierarchy(
        self, task_data: dict[str, Any], git_branch_id: str | None
    ) -> dict[str, Any] | None:
            ...
            if git_branch_id:
                try:
                    branch_context_result = context_facade.resolve_context(
                        level="branch", context_id=git_branch_id, force_refresh=False
                    )

                    if branch_context_result.get(
                        "success"
                    ) and branch_context_result.get("resolved_context"):
                        parent_contexts["branch"] = branch_context_result[
                            "resolved_context"
                        ]
                        logger.info(
                            f"DEBUG: Retrieved branch context for {git_branch_id}"
                        )
                        ...
                        if project_id:
                            try:
                                ...
                                if project_context_result.get(
                                    "success"
                                ) and project_context_result.get("resolved_context"):
                                    parent_contexts["project"] = project_context_result[
                                        "resolved_context"
                                    ]
                                    logger.info(
                                        f"DEBUG: Retrieved project context for {project_id}"
                                    )

                                    # Extract user_id for global context lookup
                                    user_id = None
                                    project_data = project_context_result.get(
                                        "resolved_context", {}
                                    )
                                    ...
                                    if user_id:
                                        try:
                                            global_context_result = (
                                                context_facade.resolve_context(
                                                    level="global",
                                                    context_id=user_id,
                                                    force_refresh=False,
                                                )
                                            ...
                                            if global_context_result.get(
                                                "success"
                                            ) and global_context_result.get(
                                                "resolved_context"
                                            ):
                                                parent_contexts["global"] = (
                                                    global_context_result[
                                                        "resolved_context"
                                                    ]
                                                )
                                                logger.info(
                                                    f"DEBUG: Retrieved global context for user {user_id}"
                                                )

                                        except Exception as e:
                                            logger.debug(
                                                f"Could not retrieve global context: {e}"
                                            )
    ...
    def _build_inheritance_chain(
        self, parent_contexts: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def count_tasks(
        ...
        try:
            ...
            # Use count_tasks facade method directly
