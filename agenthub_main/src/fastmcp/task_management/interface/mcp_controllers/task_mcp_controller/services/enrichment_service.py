...
class EnrichmentService:
    ...
    def enrich_task_data(
        self,
        task_data: dict[str, Any],
        include_visual_indicators: bool = True,
        include_workflow_hints: bool = True,
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            # Add computed fields
            enriched_data["computed_fields"] = self._compute_additional_fields(
                task_data
            )
            ...
            if include_visual_indicators:
                enriched_data["enrichment_metadata"]["features_applied"].append(
                    "visual_indicators"
                )
            if include_workflow_hints:
                enriched_data["enrichment_metadata"]["features_applied"].append(
    ...
    def _generate_visual_indicators(self, task_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "status_indicator": status_indicators.get(
                status.lower(), status_indicators["pending"]
            ),
            "priority_indicator": priority_indicators.get(
                priority.lower(), priority_indicators["medium"]
            ),
            "completion_percentage": self._calculate_completion_percentage(task_data),
        }

    def _generate_workflow_hints(
        ...
        if status == "pending":
            hints.append(
                "Task is ready to start. Consider updating status to 'in_progress'."
            )
        elif status == "in_progress":
            hints.append(
                "Task is active. Update progress regularly and complete when done."
            )
        elif status == "blocked":
