...
class ValidationFactory:
    ...
    def validate_create_request(
        ...
        param_valid, param_error = (
            self._parameter_validator.validate_create_task_params(
                title=title,
        ...
        context_valid, context_error = (
            self._context_validator.validate_context_requirements(
                operation="create", git_branch_id=git_branch_id
            )
        )
        ...
        if context_data:
            context_data_valid, context_data_error = (
                self._context_validator.validate_context_data(context_data)
            )
            if not context_data_valid:
                return False, context_data_error

        # Step 3: Business rules validation
        business_valid, business_error = (
            self._business_validator.validate_task_creation_rules(
                title=title,
                git_branch_id=git_branch_id,
                priority=priority,
                due_date=due_date,
    ...
    def validate_update_request(
        ...
        **update_params,
        ...
        update_params_filtered = {
            k: v for k, v in update_params.items() if k != "task_id"
        }
        param_valid, param_error = (
            self._parameter_validator.validate_update_task_params(
                task_id=task_id, **update_params_filtered
            )
        )
        ...
        if (
            update_params.get("include_context") is True
            or "context_data" in update_params
        ):
            context_valid, context_error = (
                self._context_validator.validate_context_requirements(
                    operation="update",
        ...
        # Step 3: Business rules validation
        business_valid, business_error = (
            self._business_validator.validate_task_update_rules(
                task_id=task_id,
    ...
    def validate_search_request(
        self, operation: str, query: str | None = None, **search_params
    ) -> tuple[bool, dict[str, Any] | None]:
        """
        ...
        param_valid, param_error = self._parameter_validator.validate_search_params(
            query=query if operation == "search" else None, **search_params
        )
    ...
    def validate_completion_request(
        ...
        completion_valid, completion_error = (
            self._business_validator.validate_completion_requirements(
                task_data=task_data,
    ...
    def validate_deletion_request(
        self, task_id: str, current_task_data: dict[str, Any] | None = None
    ) -> tuple[bool, dict[str, Any] | None]:
        """
        ...
        deletion_valid, deletion_error = (
            self._business_validator.validate_task_deletion_rules(
                task_id=task_id, current_task_data=current_task_data
            )
        )
