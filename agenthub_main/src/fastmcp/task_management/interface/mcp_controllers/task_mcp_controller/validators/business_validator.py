...
class BusinessValidator:
    ...
    def validate_task_creation_rules(
        self,
        title: str,
        git_branch_id: str,
    ...
    def validate_task_update_rules(
        self,
        task_id: str,
        ...
        if status and status.lower() == "completed" and current_task_data:
            ...
            if (
                current_priority.lower() in ["high", "critical"]
                and not completion_summary
            ):
                logger.warning(
                    f"High/critical priority task {task_id} completed without summary"
                )
        ...
        if (
            due_date
            and current_task_data
            and current_task_data.get("status") == "completed"
        ):
            return False, self._create_business_error(
    ...
    def validate_task_deletion_rules(
    ...
    def validate_completion_requirements(
        ...
        if any(keyword in title.lower() for keyword in ["test", "bug", "fix", "issue"]):
            if not testing_notes:
                logger.warning(
                    f"Task '{title}' appears to be test/bug-related but has no testing notes"
                )
                # This is a warning, not a blocking error
    ...
    def _create_business_error(
        self, rule_name: str, message: str, hint: str
    ) -> dict[str, Any]:
