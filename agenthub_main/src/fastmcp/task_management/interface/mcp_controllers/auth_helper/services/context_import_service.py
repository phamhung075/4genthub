...
class ContextImportService:
    ...
    def _import_request_context_middleware(self):
        ...
        try:
            ...
            logger.info(
                "✅ RequestContextMiddleware context functions imported successfully"
            )
        except ImportError as e:
            logger.warning(
                f"RequestContextMiddleware context functions not available: {e}"
            )

    def _import_user_context_middleware(self):
        """Import custom user context middleware"""
        try:
            from fastmcp.auth.middleware.user_context_middleware import (
                get_current_user_id,
            )
