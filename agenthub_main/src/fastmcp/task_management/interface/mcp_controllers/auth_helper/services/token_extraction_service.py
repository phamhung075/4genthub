...
class TokenExtractionService:
    ...
    def extract_user_id_from_token(
        self, token: str, verify_exp: bool = True, verify_signature: bool = False
    ) -> str | None:
        """
        ...
        try:
            ...
            if user_id:
                self.logger.info(
                    f"Successfully extracted user_id from token: {user_id[:8]}..."
                )
                ...
                if self.logger.isEnabledFor(logging.DEBUG):
                    ...
                    self.logger.debug(
                        f"Token user info - Email: {email}, Username: {username}"
                    )
            else:
            ...
            self.logger.error(
                f"Unexpected error extracting user_id from token: {str(e)}"
            )
            return None
    ...
    def extract_user_info_from_token(
        ...
        try:
            ...
            decoded_token = jwt.decode(
                token, options=decode_options, algorithms=["HS256", "RS256"]
            )
            ...
            self.logger.info(
                f"Successfully extracted user info for user: {user_info.get('user_id', 'unknown')[:8]}..."
            )
            return user_info
    ...
    def extract_token_from_header(self, authorization_header: str) -> str | None:
        ...
        self.logger.warning(
            f"Authorization header does not start with 'Bearer ': {authorization_header[:20]}..."
        )
        return None

    def extract_user_id_from_request(self, request) -> str | None:
        """
...
def extract_user_id_from_token(token: str, **kwargs) -> str | None:
