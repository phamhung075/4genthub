    ...
    def extract_user_id() -> str | None:
        ...
        try:
            ...
            if isinstance(mcp_auth_context, AuthenticatedUser):
                ...
                if (
                    hasattr(mcp_auth_context, "access_token")
                    and mcp_auth_context.access_token
                ):
                    user_id = mcp_auth_context.access_token.client_id
                    logger.info(
                        f"✅ Got user_id from MCP AuthenticatedUser.access_token: {user_id}"
                    )
                    return user_id
                elif hasattr(mcp_auth_context, "identity"):
                    user_id = mcp_auth_context.identity
                    logger.info(
                        f"✅ Got user_id from MCP AuthenticatedUser.identity: {user_id}"
                    )
                    return user_id
                ...
                logger.warning(
                    f"⚠️ MCP auth context available but no user_id/client_id: {dir(mcp_auth_context)}"
                )
            else:
            ...
            logger.info(
                "⚠️ This is expected if using DualAuthMiddleware instead of MCP auth system"
            )
