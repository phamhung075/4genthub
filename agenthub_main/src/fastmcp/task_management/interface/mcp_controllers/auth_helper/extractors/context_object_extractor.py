...
class ContextObjectExtractor:
    ...
    @staticmethod
    def extract_user_id(context_obj) -> str | None:
        """
        ...
        logger.warning(
            f"⚠️ Could not extract user_id from context object type: {type(context_obj)}"
        )
        if hasattr(context_obj, "__dict__"):
            logger.debug(
                f"🔍 Context object attributes: {list(context_obj.__dict__.keys())}"
            )
