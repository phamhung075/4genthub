...
class ContextOperationHandler:
    ...
    def handle_context_operation(
        self, facade: UnifiedContextFacade, action: str, **kwargs
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if action == "create":
                ...
                result = facade.create_context(
                    level=level, context_id=context_id, data=data, user_id=user_id
                )
            elif action == "get":
                ...
                result = facade.resolve_context(
                    level=level, context_id=context_id, force_refresh=force_refresh
                )
                ...
                result = facade.add_progress(
                    level=level, context_id=context_id, content=content, agent=agent
                )
            elif action == "list":
                result = facade.list_contexts(level=level, filters=filters)
            else:
                valid_actions = [
                    "create",
                    "get",
            ...
            logger.error(f"Validation error in context operation '{action}': {e}")
            return self._response_formatter.create_error_response(
                operation=f"manage_context.{action}",
                error=str(e),
    ...
    def _standardize_facade_response(
        self, facade_response: dict[str, Any], operation: str
    ) -> dict[str, Any]:
        """Convert facade response to standardized format."""
        logger.info(
            f"DEBUG: _standardize_facade_response - operation: {operation}, facade_response keys: {facade_response.keys()}"
        )
        if "context" in facade_response:
            logger.info(
                f"DEBUG: facade_response has context with keys: {facade_response['context'].keys() if isinstance(facade_response['context'], dict) else 'not a dict'}"
            )

        return self._response_formatter.format_context_response(
            facade_response, operation=operation, standardize_field_names=True
        )
