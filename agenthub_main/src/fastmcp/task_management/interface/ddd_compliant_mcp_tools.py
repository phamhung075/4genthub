...
from .mcp_controllers.project_mcp_controller.project_mcp_controller import (
    ProjectMCPController,
)
from .mcp_controllers.subtask_mcp_controller.subtask_mcp_controller import (
...
from .mcp_controllers.unified_context_controller.unified_context_controller import (
...
class DDDCompliantMCPTools:
    ...
    def __init__(
        ...
        | None = None,  # Kept for backward compatibility, ignored
        ...
        if self._session_factory:
            try:
                ...
                if not system_user_id:
                    raise ValueError(
                        "SYSTEM_USER_ID environment variable is required for system initialization"
                    )
            ...
            if not self._session_factory:
                logger.warning(
                    "Database not available - context operations will be limited"
                )
        ...
        self._cursor_rules_tools = (
            None  # Temporarily disabled until module is available
        )

        # Vision System Disabled (components removed as requested)
        self._enable_vision_system = False
        logger.info(
            "Vision System disabled - using standard workflow without enhanced features"
        )
    ...
    def register_tools(self, mcp: "FastMCP"):
        ...
        logger.info(
            "Using clean parameter patterns instead of schema monkey patches for MCP compatibility"
        )
    ...
    @property
    def enhanced_task_controller(self) -> TaskMCPController | None:
    ...
    @property
    def context_enforcing_controller(self) -> TaskMCPController | None:
    ...
    @property
    def subtask_progress_controller(self) -> SubtaskMCPController | None:
        """Get the subtask controller (includes progress tracking if Vision System is enabled)"""
        return self._subtask_controller

    @property
    def workflow_hint_enhancer(self) -> WorkflowHintEnhancer | None:
