...
class BranchAPIController:
    ...
    def get_bulk_summaries(self, project_ids: Optional[list] = None, user_id: str = None,
        ...
        try:
            ...
            logger.info(f"✅ FIXED: Bulk summaries retrieved using domain layer counts from actual tasks: {len(summaries)} branches, {len(projects)} projects in {query_time:.2f}ms")
            ...
            response = {
                ...
                'metadata': {
                    ...
                    'from_cache': False,
                    'data_source': 'domain_layer_real_time_counts'  # Counting actual tasks, not DB fields
                },
                'timestamp': time.time()
            }
