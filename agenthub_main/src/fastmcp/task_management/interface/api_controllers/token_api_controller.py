...
class TokenAPIController:
    ...
    async def get_token_details(
        ...
        return await self.token_facade.get_token_details(
            token_id=token_id, user_id=user_id, session=session
        )
    ...
    async def generate_api_token(
        ...
        if result["success"]:
            return {"success": True, "token_data": result["token"]}
        return result
    ...
    async def validate_token(self, token: str, session: Session) -> dict[str, Any]:
        ...
        return await self.token_facade.validate_token(token=token, session=session)
