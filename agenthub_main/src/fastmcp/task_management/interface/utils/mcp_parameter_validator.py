...
class MCPParameterValidator:
    ...
    @staticmethod
    def validate_and_coerce_mcp_parameters(
        ...
        try:
            ...
            if validation_result["success"]:
                ...
                logger.warning(
                    f"Parameter validation failed for action '{action}': {validation_result.get('error')}"
                )
                return validation_result

        except Exception as e:
            logger.error(
                f"Unexpected error during parameter validation for action '{action}': {e}"
            )
            return {
                "success": False,
    ...
    def create_parameter_validation_decorator():
        ...
        def decorator(func):
            def wrapper(self, action: str, **params):
                # Validate parameters
                validation_result = (
                    MCPParameterValidator.validate_and_coerce_mcp_parameters(
                        action, params
                    )
                )
...
def example_mcp_controller_integration():
    ...
    if validation_result["success"]:
        ...
        include_context = coerced_params[
            "include_context"
        ...
        print(
            f"   include_context: {include_context} (type: {type(include_context).__name__})"
        )
