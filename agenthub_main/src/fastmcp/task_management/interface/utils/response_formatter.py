...
class StandardResponseFormatter:
    ...
    def __init__(self):
        ...
        self.optimization_enabled = OPTIMIZER_AVAILABLE and os.getenv(
    ...
    def create_response(
        ...
        response = {
            ...
            "status": status.value,
            "success": status
            != ResponseStatus.FAILURE,  # True for success and partial_success
            "operation": operation,
        ...
        # Log the operation
        logger.info(
            f"Operation {operation} completed with status {status.value} (ID: {operation_id})"
        )
        ...
        if self.optimization_enabled and self.optimizer:
            ...
            if (
                request_context
                and request_context.get("headers", {}).get("X-Response-Format")
                == "legacy"
            ):
            ...
            try:
                optimized = self.optimizer.optimize_response(
                    response, profile=profile, request_context=request_context
                )
                logger.debug(
                    f"Response optimized: {len(str(response))} -> {len(str(optimized))} bytes"
                )
                return optimized
            except Exception as e:
                logger.warning(
                    f"Response optimization failed, returning unoptimized: {e}"
                )
                return response
    ...
    def verify_success(response: dict[str, Any]) -> bool:
        ...
        """
        # Check multiple indicators as recommended in agenthub.mdc
        return (
            response.get("status") == ResponseStatus.SUCCESS.value
            and response.get("success", False)
            and "error" not in response
            and response.get("confirmation", {}).get("operation_completed", False)
            and response.get("confirmation", {}).get("data_persisted", False)
            and len(response.get("confirmation", {}).get("partial_failures", [])) == 0
        )

    @staticmethod
    def extract_data(response: dict[str, Any]) -> dict[str, Any] | None:
        """
    ...
    @staticmethod
    def get_operation_id(response: dict[str, Any]) -> str | None:
    ...
    def has_partial_failures(response: dict[str, Any]) -> bool:
        ...
        return (
            response.get("status") == ResponseStatus.PARTIAL_SUCCESS.value
            or len(response.get("confirmation", {}).get("partial_failures", [])) > 0
        )
    ...
    def format_context_response(
        ...
        if not standardize_field_names:
            return self.format_success(
                data, operation, request_context=request_context, profile=profile
            )
        ...
        if data.get("success") is True:
            logger.info(
                f"DEBUG format_context_response: operation={operation}, data keys={data.keys()}"
            )
            # Handle different context operations
            if (
                operation in ["create", "get"]
                or operation.endswith(".create")
                or operation.endswith(".get")
            ):
                ...
                if "context" in data and data["context"]:
                    ...
                    logger.info(
                        f"DEBUG format_context_response: found context, standardized_data={standardized_data}"
                    )
                else:
                    # If no context, return empty dict
                    standardized_data = {}
                    logger.info(
                        "DEBUG format_context_response: no context found, returning empty dict"
                    )
                ...
                if "context" in data and data["context"]:
                    ...
                    logger.debug("Update operation formatted with updated_data field")
                else:
                    ...
                    logger.debug(
                        "Update operation formatted with empty updated_data field"
                    )
            ...
            metadata["context_operation"] = {
                k: v for k, v in metadata["context_operation"].items() if v is not None
            }
