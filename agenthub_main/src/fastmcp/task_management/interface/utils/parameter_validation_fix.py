...
class ParameterTypeCoercer:
    ...
    TRUE_VALUES: set[str] = {"true", "1", "yes", "on", "enabled", "active", "y", "t"}
    ...
    def coerce_parameters(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        for key, value in params.items():
            try:
                ...
                coerced[key] = (
                    value  # Keep original value if coercion fails unexpectedly
                )
                    ...
                    flexible_schema["properties"][prop_name] = {
                        "anyOf": [
                            {"type": "boolean"},
                            {
                                "type": "string",
                                "enum": list(
                                    self.coercer.TRUE_VALUES | self.coercer.FALSE_VALUES
                                ),
                            },
                        ]
                    }
