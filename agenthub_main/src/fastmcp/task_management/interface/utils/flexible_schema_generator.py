...
class FlexibleSchemaGenerator:
    ...
    @classmethod
    def create_flexible_schema_for_parameter(
        cls, param_name: str, original_schema: dict[str, Any]
    ) -> dict[str, Any]:
        """
    ...
    @classmethod
    def _create_flexible_array_schema(
        cls, param_name: str, original_schema: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        description = original_schema.get(
            "description", f"Array parameter for {param_name}"
        )
    ...
    @classmethod
    def apply_flexible_schemas_to_tool_schema(
        cls, tool_schema: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        for param_name, param_schema in tool_schema["properties"].items():
            if param_name in cls.FLEXIBLE_ARRAY_PARAMETERS:
                enhanced_properties[param_name] = (
                    cls.create_flexible_schema_for_parameter(param_name, param_schema)
                )
            else:
...
class FlexibleToolDecorator:
    ...
    def enhance_tool_schema(tool_instance):
        ...
        if hasattr(tool_instance, "parameters"):
            ...
            flexible_schema = (
                FlexibleSchemaGenerator.apply_flexible_schemas_to_tool_schema(
                    original_schema
                )
            )
            ...
            logger.info(
                f"Enhanced tool schema for {getattr(tool_instance, 'name', 'unknown tool')} with flexible parameter support"
            )
...
def test_flexible_schema_generation():
    ...
    flexible_schema = generator.create_flexible_schema_for_parameter(
        "insights_found", original_schema
    )
