...
class DocumentValidator:
    ...
    def validate_document_creation(
        ...
        try:
            ...
            index_update = self._update_index_if_needed(
                correction_result["corrected_path"], content, doc_type
            )

            # Calculate compliance score
            compliance_score = self._calculate_compliance_score(
                correction_result, metadata_validation
            )
            ...
            return {"success": False, "error": str(e), "compliance_score": 0.0}
    ...
    def _check_path_correction(
        self, path_obj: Path, doc_type: DocumentType
    ) -> dict[str, Any]:
        """Check if path correction is needed"""
    ...
    def _update_index_if_needed(
        self, file_path: str, content: str, doc_type: DocumentType
    ) -> dict[str, Any]:
        """Update index.json if needed"""
        if doc_type != DocumentType.AI_GENERATED:
            return {
                "updated": False,
                "reason": "Non-AI document, index update not needed",
            }
    ...
    def _calculate_compliance_score(
        self, correction_result: dict[str, Any], metadata_validation: dict[str, Any]
    ) -> float:
