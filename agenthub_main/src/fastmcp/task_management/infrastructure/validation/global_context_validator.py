...
class GlobalContextValidator:
    ...
    def _create_nested_json_schema(self) -> dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "_schema_version": {"type": "string", "enum": ["2.0"]},
                "_migrated_from": {"type": ["string", "null"]},
                "_migration_timestamp": {
                    "type": ["string", "null"],
                    "format": "date-time",
                },
                "_custom_categories": {"type": "object"},
            },
    ...
    def _validate_nested_semantics(self, nested_data: dict[str, Any]) -> list[str]:
                ...
                for subcategory in subcategories.keys():
                    if subcategory in category_data:
                        ...
                        if not isinstance(subcategory_data, dict):
                            errors.append(
                                f"Subcategory {category}.{subcategory} must be an object"
                            )
        ...
        if migration_timestamp:
            try:
                ...
                errors.append(
                    f"Invalid migration timestamp format: {migration_timestamp}"
                )
        ...
        for field in known_fields:
            if field in global_settings:
                ...
                if field_value is not None and not isinstance(field_value, dict):
                    errors.append(
                        f"Field {field} must be an object, got {type(field_value)}"
                    )

        return errors

    def validate_global_context_entity(
        ...
        try:
            ...
            for field in key_fields:
                ...
                if flat_value != nested_value:
                    errors.append(
                        f"Inconsistency in field {field} between flat and nested structures"
                    )
    ...
    def validate_migration_data(
        self, original_flat: dict[str, Any], migrated_nested: GlobalContextNestedData
    ) -> list[str]:
        """
        ...
        if not nested_valid:
            errors.extend(
                [f"Migration result validation: {err}" for err in nested_errors]
            )
        ...
        try:
            ...
            for key, value in original_flat.items():
                if key not in reverse_flat:
                    ...
                    if not self._is_acceptable_transformation(
                        key, value, reverse_flat[key]
                    ):
    ...
    def _is_acceptable_transformation(
        self, key: str, original: Any, transformed: Any
    ) -> bool:
    ...
    def create_validation_report(self, context: GlobalContext) -> dict[str, Any]:
        ...
        has_nested = (
            hasattr(context, "_nested_data") and context._nested_data is not None
        )
        ...
        report = {
            ...
            "structure_info": {
                ...
                "schema_version": context._nested_data._schema_version
                if has_nested
                else "1.0",
            },
            "field_counts": {
                "flat_fields": len(context.global_settings) if has_flat else 0,
                "nested_categories": len(
                    [
                        cat
                        for cat in [
                            "organization",
                        ...
                        if context._nested_data and getattr(context._nested_data, cat)
                    ]
                )
                if has_nested
                else 0,
...
def validate_global_context(
    context: GlobalContext, raise_on_error: bool = False
) -> tuple[bool, list[str]]:
    """
