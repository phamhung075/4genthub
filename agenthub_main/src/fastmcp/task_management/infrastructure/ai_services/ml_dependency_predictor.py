...
class MLModelPersistence:
    ...
    def save_patterns(
        self, patterns: dict[str, DependencyPattern], model_version: str = "latest"
    ) -> str:
    ...
    def load_patterns(
        ...
        try:
            ...
            for pattern_id, pattern_dict in pattern_data.items():
                ...
                pattern_dict["created_at"] = datetime.fromisoformat(
                    pattern_dict["created_at"]
                )
...
class TrainingDataCollector:
    ...
    def collect_project_history(self, limit: int | None = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for branch_id, branch_tasks in projects_by_branch.items():
                if (
                    len(branch_tasks) < 2
                ):  # Need at least 2 tasks for meaningful patterns
                    continue
    ...
    def _create_project_data(
        ...
        try:
            ...
            self.logger.error(
                f"Error creating project data for branch {branch_id}: {e}"
            )
            return None
...
class MLDependencyPredictor:
    ...
    def __init__(
        ...
        model_dir: str = "models/dependency_prediction",
    ):
        self.task_repository = task_repository
        ...
        try:
            ...
            if versions:
                ...
                if patterns:
                    ...
                    self.logger.info(
                        f"Loaded model version {latest_version} with {len(patterns)} patterns"
                    )
        except Exception as e:
            self.logger.error(f"Error loading latest model: {e}")

    def train_model(
        self, project_limit: int | None = None, save_model: bool = True
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            training_results = self.pattern_engine.train_from_historical_data(
                training_data
            )
            ...
            if save_model and self.pattern_engine.patterns:
                ...
                model_path = self.persistence.save_patterns(
                    self.pattern_engine.patterns, model_version
                )
    ...
    def predict_dependencies(
        self, task, candidate_tasks: list
        ...
        try:
            predictions = self.pattern_engine.predict_dependencies(
            ...
            self.logger.info(
                f"Generated {len(filtered_predictions)} predictions for task {getattr(task, 'id', 'unknown')}"
            )
    ...
    def get_model_info(self) -> dict[str, Any]:
        ...
        try:
            info = {
                ...
                "current_stats": self.pattern_engine.get_engine_stats()
                if self.pattern_engine.trained
                else None,
            ...
            if versions:
                latest_version = versions[0]
                metadata_file = (
                    self.persistence.model_dir / f"metadata_{latest_version}.json"
                )
                if metadata_file.exists():
            ...
            if pattern_id in self.pattern_engine.patterns:
                ...
                if accepted:
                    pattern.success_rate = (
                        pattern.success_rate * pattern.support_count + 1.0
                    ) / (pattern.support_count + 1)
                else:
                    pattern.success_rate = (
                        pattern.success_rate * pattern.support_count
                    ) / (pattern.support_count + 1)
                ...
                self.logger.info(
                    f"Updated pattern {pattern_id} with feedback: {'accepted' if accepted else 'rejected'}"
                )
                return True
