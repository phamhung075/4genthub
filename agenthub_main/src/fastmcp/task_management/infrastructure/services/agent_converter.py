...
class AgentConverter:
    ...
    def convert_simplified_agent_to_entity(
        self, agent_data: dict, project_id: str
    ) -> Agent:
        """Convert simplified agent data from projects.json to full Agent entity"""
        agent_id = agent_data.get("id")
        name = agent_data.get("name")
        call_agent = agent_data.get(
            "call_agent", f"@{agent_id.replace('_', '-')}-agent"
        )

        # Extract capabilities and specializations from call_agent reference
        capabilities, specializations, preferred_languages = (
            self._extract_agent_details(call_agent)
        )
    ...
    def _extract_agent_details(
        self, call_agent: str
    ) -> tuple[set[AgentCapability], list[str], list[str]]:
    ...
    def convert_project_agents_to_entities(
        ...
        for agent_id, agent_data in registered_agents_data.items():
            try:
                agent_entity = self.convert_simplified_agent_to_entity(
                ...
                agent_entities[agent_id] = self._create_fallback_agent(
                    agent_id, agent_data, project_id
                )

        return agent_entities

    def _create_fallback_agent(
        self, agent_id: str, agent_data: dict, project_id: str
    ) -> Agent:
    ...
    def update_agent_assignments(
        self, agent_entities: dict[str, Agent], agent_assignments: dict[str, str]
    ) -> None:
