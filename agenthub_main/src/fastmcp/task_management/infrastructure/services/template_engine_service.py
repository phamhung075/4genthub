...
class TemplateEngineService:
    ...
    async def render_template(self, request: TemplateRenderRequest) -> TemplateResult:
        ...
        try:
            ...
            template_data = await self.registry_service.get_template(
            ...
            compiled_template = await self._get_compiled_template(
                request.template_id.value, template_data["content"]
            )

            # Resolve variables
            resolved_context = await self._resolve_variables(
                request.template_id.value, request.variables, request.task_context
            )
            ...
            logger.info(
                f"Rendered template {request.template_id.value} in {generation_time_ms}ms"
            )
            return result

        except Exception as e:
            logger.error(
                f"Template rendering failed for {request.template_id.value}: {e}"
            )
            raise

    async def _check_cache(
        self, request: TemplateRenderRequest
    ) -> TemplateResult | None:
        """Check if template result exists in cache"""
        cache_key = self._generate_cache_key(
            request.template_id.value, request.variables
        )
    ...
    async def _cache_result(
        self, request: TemplateRenderRequest, result: TemplateResult
    ):
    ...
    async def _get_compiled_template(self, template_id: str, content: str):
        ...
        try:
            ...
            raise TemplateCompilationError(
                f"Template compilation failed: {str(e)}", template_id, [str(e)]
            )
