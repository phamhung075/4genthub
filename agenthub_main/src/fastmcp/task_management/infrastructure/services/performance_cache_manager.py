    ...
    async def put(self, key: str, entry: CacheEntry) -> bool:
        ...
        with self.lock:
            ...
            while (
                ...
                or self.size_tracker + entry.size_bytes
                > self.config.memory_max_memory_mb * 1024 * 1024
            ):
...
class EnhancedRuleCacheManager:
    ...
    def __init__(self, config: CacheConfiguration | None = None):
        ...
        self.disk_storage = (
            DiskStorage(self.config) if self.config.disk_enabled else None
        )
    ...
    async def get(
        self, key: str, lazy_load_callback: Callable | None = None
    ) -> Any | None:
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cache_levels": {
                ...
                "disk_entries": len(self.disk_storage.index)
                if self.disk_storage
                else 0,
    ...
    def _record_hit(self, response_time: float, cache_level: CacheLevel | None):
        ...
        if self.config.performance_logging:
            logger.debug(
                f"Cache hit from {cache_level} in {response_time * 1000:.2f}ms"
            )
