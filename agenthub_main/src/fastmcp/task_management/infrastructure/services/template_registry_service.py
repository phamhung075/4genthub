...
class TemplateRegistryService:
    ...
    async def _calculate_pattern_score(
        self, template_id: str, file_patterns: list[str]
    ) -> float:
    ...
    def _patterns_match(self, pattern1: str, pattern2: str) -> bool:
        ...
        return (
            pattern1 == pattern2
            or pattern1 in pattern2
            or pattern2 in pattern1
            or self._extract_pattern_components(pattern1)
            & self._extract_pattern_components(pattern2)
        )
    ...
    async def _get_suggestion_reason(
        self, template: dict[str, Any], task_context: dict[str, Any], score: float
    ) -> str:
    ...
    async def get_template_analytics(
        self, template_id: str | None = None, days: int = 30
    ) -> dict[str, Any]:
        ...
        try:
            with self._get_connection() as conn:
                ...
                cache_hit_ratio = (
                    (cache_hits / total_usage * 100) if total_usage > 0 else 0
                )
                ...
                return {
                    ...
                    "performance": {
                        "avg_generation_time_ms": round(
                            stats["avg_generation_time"] or 0, 2
                        ),
                    ...
                    "health_metrics": {
                        "performance_target_met": (stats["avg_generation_time"] or 0)
                        < 100,
            ...
            with self._get_connection() as conn:
                ...
                """,
                    (
                        ...
                        template_type,
                        priority,
                        json.dumps([agent_compatibility])
                        if agent_compatibility != "all"
                        else '["*"]',
