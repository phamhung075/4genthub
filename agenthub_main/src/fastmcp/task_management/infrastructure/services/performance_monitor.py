...
class PerformanceMonitor:
    ...
    def __init__(
        self, cache_manager, monitoring_interval: float = 1.0, history_size: int = 3600
    ):
        ...
        # Performance history
        self.performance_history: deque[PerformanceSnapshot] = deque(
            maxlen=history_size
        )
    ...
    async def _capture_snapshot(self):
        ...
        try:
            ...
            snapshot = PerformanceSnapshot(
                ...
                average_response_time_ms=metrics["performance_metrics"][
                    "average_response_time_ms"
                ],
                operations_per_second=metrics["performance_metrics"].get(
                    "operations_per_second", 0.0
                ),
                memory_usage_mb=metrics["cache_levels"].get("memory_size_bytes", 0)
                / (1024 * 1024),
    ...
    async def _check_alerts(self):
        ...
        if (
            ...
            > self.alert_thresholds["response_time_max_ms"]
        ):
            alerts.append(
                f"High response time: {latest.average_response_time_ms:.2f}ms"
            )
    ...
    def get_performance_summary(self, time_window_minutes: int = 60) -> dict[str, Any]:
        ...
        avg_response_time = sum(
            s.average_response_time_ms for s in recent_snapshots
        ) / len(recent_snapshots)
        avg_ops_per_sec = sum(s.operations_per_second for s in recent_snapshots) / len(
            recent_snapshots
        )
        avg_memory_usage = sum(s.memory_usage_mb for s in recent_snapshots) / len(
            recent_snapshots
        )
        ...
        if len(recent_snapshots) >= 2:
            hit_rate_trend = (
                recent_snapshots[-1].hit_rate - recent_snapshots[0].hit_rate
            )
            response_time_trend = (
                recent_snapshots[-1].average_response_time_ms
                - recent_snapshots[0].average_response_time_ms
            )
        else:
            hit_rate_trend = 0.0
...
class CacheBenchmark:
    ...
    async def run_basic_benchmark(self, num_operations: int = 1000) -> dict[str, Any]:
        ...
        # Test data
        test_data = {
            f"test_key_{i}": f"test_content_{i}" * 100 for i in range(num_operations)
        }
        ...
        benchmark_results = {
            ...
            "cache_metrics": {
                ...
                "memory_usage_mb": final_metrics["cache_levels"].get(
                    "memory_size_bytes", 0
                )
                / (1024 * 1024),
                "total_evictions": final_metrics["eviction_statistics"][
                    "total_evictions"
                ],
            },
        }
        ...
        if benchmark_results["performance"]["put_success_rate"] < 0.95:
            recommendations.append(
                "High put failure rate - investigate cache capacity issues"
            )

        benchmark_results["recommendations"] = recommendations

        logger.info(
            f"Benchmark completed: {benchmark_results['performance']['operations_per_second']:.2f} ops/sec"
        )
