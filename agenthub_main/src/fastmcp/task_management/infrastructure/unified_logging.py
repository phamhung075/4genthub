...
def init_logging(
    ...
    try:
        ...
        logs_dir = Path("/tmp/agenthub_logs")
        logs_dir.mkdir(exist_ok=True)
        logging.warning(
            f"Could not create logs in {project_root / 'logs'}, using {logs_dir}"
        )
    ...
    file_handler = RotatingFileHandler(
        backend_log_path, maxBytes=max_bytes, backupCount=backup_count, encoding="utf-8"
    )
