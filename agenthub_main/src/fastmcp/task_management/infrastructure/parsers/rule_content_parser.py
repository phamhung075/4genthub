...
class RuleContentParser:
    ...
    def _generate_metadata(
        self, file_path: Path, format_type: RuleFormat, content: str
    ) -> RuleMetadata:
    ...
    def _classify_rule_type(self, file_path: Path, content: str) -> RuleType:
        ...
        if "task" in path_str or "task" in content_lower:
            ...
            if (
                "config" not in path_str
                and "configuration" not in path_str
                and "config" not in content_lower
                and "configuration" not in content_lower
            ):
                return RuleType.AGENT
        ...
        if (
            "config" in path_str
            or "configuration" in path_str
            or any(
                re.search(pat, content_lower, re.MULTILINE)
                for pat in config_section_patterns
            )
        ):
