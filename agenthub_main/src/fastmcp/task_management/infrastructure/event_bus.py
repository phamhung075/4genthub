...
class EventBus:
    ...
    def subscribe(self, event_type: type, handler: Callable, priority: int = 0) -> None:
        """
        ...
        subscription = EventSubscription(
            event_type=event_type, handler=handler, priority=priority, is_async=is_async
        )
    ...
    def unsubscribe(self, event_type: type, handler: Callable) -> bool:
        ...
        self._subscriptions[event_type] = [
            sub for sub in self._subscriptions[event_type] if sub.handler != handler
        ]
    ...
    def publish_sync(self, event: Any) -> None:
        ...
        if settings.enable_async_event_queue and self._async_event_queue:
            ...
            try:
                ...
                if success:
                    logger.debug(
                        f"Event queued for async processing: {type(event).__name__}"
                    )
                    return
                else:
                    logger.warning(
                        f"Queue full, falling back to sync: {type(event).__name__}"
                    )
            except Exception as e:
    ...
    def has_subscribers(self, event_type: type) -> bool:
        ...
        return (
            ...
            and len(self._subscriptions[event_type]) > 0
        )
    ...
    async def queue_event(self, event: Any) -> None:
        ...
        if self._async_event_queue:
            ...
            logger.warning(
                "Async event queue not configured, falling back to sync publish"
            )
            self._execute_handlers_sync(event)
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"EventBus(event_types={len(self._subscriptions)}, "
