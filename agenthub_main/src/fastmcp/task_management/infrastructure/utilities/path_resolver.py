...
class PathResolver:
    ...
    def __init__(self):
        ...
        self.brain_dir = self._resolve_path(
            os.environ.get("BRAIN_DIR_PATH", ".cursor/rules/brain")
        )
        self.projects_file = self._resolve_path(
            os.environ.get("PROJECTS_FILE_PATH", str(self.brain_dir / "projects.json"))
        )
    ...
    def _ensure_projects_file(self):
        ...
        if not self.projects_file.exists():
            ...
            try:
                ...
                logger.warning(
                    f"Cannot create projects file at {self.projects_file}: {e}"
                )
                fallback_projects_file = (
                    self.project_root / ".cursor/rules/brain/projects.json"
                )
    ...
    def get_tasks_json_path(
        ...
        if project_id:
            ...
            tasks_path = self._resolve_path(
                f".cursor/rules/tasks/{user_id}/{project_id}/{git_branch_name}/tasks.json"
            )
        else:
            # Legacy fallback for backward compatibility
            tasks_path = self._resolve_path(
                os.environ.get("TASKS_JSON_PATH", ".cursor/rules/tasks/tasks.json")
            )
    ...
    def get_auto_rule_path(self) -> Path:
        ...
        return self._resolve_path(
            os.environ.get("AUTO_RULE_PATH", ".cursor/rules/auto_rule.mdc")
        )
    ...
    def get_rules_directory_from_settings(self) -> Path:
        ...
        try:
            ...
            if not is_http_mode():
                ...
                settings_path = (
                    self.project_root / "00_RULES" / "core" / "settings.json"
                )
                if settings_path.exists():
                    with open(settings_path, encoding="utf-8") as f:
                        settings = json.load(f)
                        rules_path = settings.get("runtime_constants", {}).get(
                            "DOCUMENT_RULES_PATH", "00_RULES"
                        )
