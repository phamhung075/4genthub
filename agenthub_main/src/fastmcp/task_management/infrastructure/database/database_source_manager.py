...
class DatabaseSourceManager:
    ...
    - Test mode (pytest): agenthub_test.db (for local testing, always isolated)
    - Normal mode: /data/agenthub.db (local development uses Docker database for consistency)
    - Docker mode: /data/agenthub.db (running inside Docker container)
    - Stdin mode (MCP): local agenthub.db (MCP stdin cannot access Docker database)
    ...
    def _set_database_path(self) -> None:
        ...
        if self._current_mode == DatabaseMode.TEST:
            ...
            docker_db_path = os.environ.get("DOCKER_DB_PATH", "/data/agenthub.db")
            if not os.path.exists(os.path.dirname(docker_db_path)):
                raise RuntimeError(
                    f"Docker database directory not accessible: {os.path.dirname(docker_db_path)}. Server cannot start without Docker database access."
                )
            ...
            self._database_path = str(
                project_root / "agenthub_main" / "database" / "data" / "agenthub.db"
            )
            logger.info(
                "MCP STDIN mode using local database (cannot access Docker database)"
            )
            ...
            docker_db_path = "/data/agenthub.db"
            if os.path.exists(docker_db_path):
                # Docker database exists, use it for consistency
                self._database_path = docker_db_path
                logger.info(
                    "Local development will use Docker database for consistency"
                )
            else:
                # Docker database must be accessible for local development
                raise RuntimeError(
                    f"Docker database not accessible: {docker_db_path}. Local development requires Docker database access for consistency. Please start Docker container first."
                )
    ...
    def get_database_path(self) -> str:
        ...
        if explicit_db_path and explicit_db_path != self._database_path:
            logger.info(
                f"MCP_DB_PATH changed to: {explicit_db_path}, updating database path"
            )
            self._database_path = explicit_db_path
        ...
        logger.debug(
            f"Database path requested: {self._database_path} (mode: {self._current_mode.value})"
        )
    ...
    def _validate_single_source(self) -> None:
        ...
        """
        project_root = self._find_project_root()
        main_db = project_root / "agenthub_main" / "database" / "data" / "agenthub.db"
        test_db = (
            project_root / "agenthub_main" / "database" / "data" / "agenthub_test.db"
        )
        ...
        if main_exists and test_exists:
            ...
            if main_recent and test_recent and self._current_mode != DatabaseMode.TEST:
                ...
                logger.warning(
                    f"  Main DB: {main_db} (modified {current_time - main_mtime:.0f}s ago)"
                )
                logger.warning(
                    f"  Test DB: {test_db} (modified {current_time - test_mtime:.0f}s ago)"
                )
                logger.warning(
                    "This is common during development - using mode-appropriate database"
                )
