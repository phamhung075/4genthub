...
class SQLiteConnectionPool:
    ...
    def _create_connection(self) -> sqlite3.Connection:
        ...
        logger.debug(
            f"Created new SQLite connection (total: {self._stats['connections_created']})"
        )
        return conn
...
def get_connection_pool(
    db_path: str, pool_size: int = 10, **kwargs
    ...
    with _pool_lock:
        if _pool_instance is None:
            _pool_instance = SQLiteConnectionPool(
                db_path=db_path, pool_size=pool_size, **kwargs
            )
...
class SupabaseConnectionPool:
    ...
    def __init__(self, database_url: str, **pool_config):
        ...
        self.engine = create_engine(database_url, **default_config)
        ...
        logger.info(
            f"Initialized Supabase connection pool (size={default_config['pool_size']}, overflow={default_config['max_overflow']})"
        )
    ...
    def get_pool_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            if hasattr(pool_impl, "checkedin")
            else "N/A",
            "checked_out": pool_impl.checkedout()
            if hasattr(pool_impl, "checkedout")
            else "N/A",
            "overflow": pool_impl.overflow()
            if hasattr(pool_impl, "overflow")
            else "N/A",
    ...
    with _supabase_lock:
        if _supabase_pool is None:
            ...
            if database_url is None:
                database_url = os.getenv("SUPABASE_DATABASE_URL") or os.getenv(
                    "DATABASE_URL"
                )
