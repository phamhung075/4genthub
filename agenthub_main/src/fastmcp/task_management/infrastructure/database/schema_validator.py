...
class SchemaValidator:
    ...
    async def validate_all(self) -> dict[str, Any]:
            ...
            async with self.engine.connect() as conn:
                ...
                for model_class in self.models:
                    try:
                        ...
                        error_msg = (
                            f"Failed to validate model {model_class.__name__}: {str(e)}"
                        )
        ...
        if issues:
            ...
            for issue in issues:
                logger.error(
                    f"  - {issue['model']}.{issue.get('column', '')}: {issue['message']}"
                )
        else:
        ...
        if warnings:
            ...
            for warning in warnings:
                logger.warning(
                    f"  - {warning['model']}.{warning.get('column', '')}: {warning['message']}"
                )

        return summary

    async def _validate_model(
        self, model_class: Any, metadata: MetaData, conn: Any
    ) -> dict[str, Any]:
        """
    ...
    def _validate_column_type(self, model_col: Any, db_col: Any) -> str | None:
        ...
        compatible_types = [
            ...
            # UnifiedUUID compatibility - this is the key addition for our UUID type
            (
                "UUID",
                "VARCHAR",
            ),  # Our UnifiedUUID shows as UUID in model but may be VARCHAR in SQLite
            (
                "VARCHAR",
                "UUID",
            ),  # And the reverse mapping for different database engines
        ]

        # Check if types are compatible
        for model_compat, db_compat in compatible_types:
            if model_type_normalized.startswith(
                model_compat
            ) and db_type_normalized.startswith(db_compat):
                return None
            if model_type_normalized.startswith(
                db_compat
            ) and db_type_normalized.startswith(model_compat):
                return None
...
async def validate_schema_on_startup(engine: AsyncEngine) -> bool:
    ...
    logger.info(
        f"✅ Schema validation successful for {results['validated_models']} models"
    )
    return True
