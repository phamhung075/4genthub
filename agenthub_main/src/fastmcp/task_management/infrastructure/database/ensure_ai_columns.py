    ...
    try:
        with engine.begin() as conn:
            ...
            ai_columns: list[tuple[str, str, str]] = [
                ...
                (
                    "ai_work_context",
                    "JSON" if dialect_name == "postgresql" else "TEXT",
                    "'{}'" if dialect_name == "postgresql" else "'{}'",
                ),
            ...
            for table_name in tables_to_update:
                if table_name not in inspector.get_table_names():
                    logger.warning(
                        f"Table {table_name} does not exist yet. Will be created by ORM."
                    )
                    continue
                ...
                for col_name, col_type, default_val in ai_columns:
                    if col_name not in existing_columns:
                        try:
                            ...
                            logger.info(
                                f"Adding AI column {col_name} to {table_name} table"
                            )
                            ...
                            if (
                                ...
                                logger.debug(
                                    f"Column {col_name} already exists in {table_name} table"
                                )
                            else:
                                logger.error(
                                    f"Error adding column {col_name} to {table_name}: {e}"
                                )
                                raise

            if columns_added > 0:
                logger.info(
                    f"✅ Successfully added {columns_added} AI columns to database"
                )
            else:
