...
class SupabaseConfig:
    ...
    def _get_supabase_database_url(self) -> str:
        ...
        if database_url and (
            ...
            or "PLACEHOLDER_SUPABASE_REF" in database_url
        ):
        ...
        if self.supabase_url:
            ...
            if match:
                ...
                is_self_hosted = bool(
                    re.search(r"\d+\.\d+\.\d+\.\d+", self.supabase_url or "")
                )
                ...
                if is_self_hosted:
                    ...
                    logger.info(
                        "🔒 Self-hosted Supabase detected, using sslmode=prefer for self-signed certificate"
                    )
                else:
                    # For cloud Supabase, always require SSL
                    ssl_cert_env = os.getenv(
                    ...
                    if os.path.exists(ssl_cert_path):
                        ...
                        logger.warning(
                            f"⚠️ SSL certificate not found at {ssl_cert_path}, using sslmode=require without certificate"
                        )

                logger.info(
                    f"🔧 Constructed Supabase connection for project: {project_ref}"
                )
                return database_url
    ...
    def _create_engine(self) -> Engine:
        ...
        if not self.database_url:
            raise ValueError(
                "Database URL not configured. Please set SUPABASE_URL or DATABASE_URL"
            )
...
def is_supabase_configured() -> bool:
    ...
    return bool(
        ...
        and (os.getenv("DATABASE_URL") or os.getenv("SUPABASE_DB_PASSWORD"))
    )
