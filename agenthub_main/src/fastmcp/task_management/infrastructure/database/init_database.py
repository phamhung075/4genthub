...
def init_database():
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Server will continue without migrations - some features may not work optimally"
            )
...
async def _run_migrations(db_config):
    ...
    if database_url.startswith("sqlite:///"):
        ...
        if importlib.util.find_spec("aiosqlite") is not None:
            ...
            logger.warning(
                "aiosqlite not available - skipping async migrations for SQLite"
            )
        ...
        if importlib.util.find_spec("asyncpg") is not None:
            ...
            logger.warning(
                "asyncpg not available - skipping async migrations for PostgreSQL"
            )
            logger.info("Install asyncpg with: pip install asyncpg")
            return
    else:
        logger.warning(
            f"Unsupported database type for async migrations: {database_url}"
        )
        return
