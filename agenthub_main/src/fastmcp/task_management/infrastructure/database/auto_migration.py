    ...
    def _rename_subtasks_table():
        ...
        try:
            with get_session() as session:
                ...
                if (
                    "task_subtasks" in existing_tables
                    and "subtasks" not in existing_tables
                ):
                    logger.info("Renaming task_subtasks table to subtasks...")
                    session.execute(
                        text("ALTER TABLE task_subtasks RENAME TO subtasks")
                    )
                    session.commit()
                    ...
                    logger.warning(
                        "Neither task_subtasks nor subtasks table exists - will be created by ORM"
                    )
                ...
                for table_name in tables_to_check:
                    try:
                        ...
                        if table_name not in inspector.get_table_names():
                            logger.info(
                                f"Table {table_name} doesn't exist yet - will be created by ORM"
                            )
                            continue
                        ...
                        if "progress_state" not in columns:
                            logger.info(
                                f"Adding progress_state column to {table_name} table..."
                            )
                            ...
                            try:
                                ...
                                session.commit()
                                logger.info(
                                    f"✅ Added progress_state column to {table_name} table"
                                )
                                ...
                                try:
                                    ...
                                    session.commit()
                                    logger.info(
                                        f"✅ Added progress_state column to {table_name} table (SQLite)"
                                    )
                                except SQLAlchemyError:
                                    # Column might already exist
                                    session.rollback()
                                    logger.info(
                                        f"progress_state column might already exist in {table_name}"
                                    )
                        else:
                            logger.info(
                                f"✅ progress_state column already exists in {table_name} table"
                            )
        ...
        try:
            with get_session() as session:
                ...
                if "tasks" not in inspector.get_table_names():
                    logger.info(
                        "Table tasks doesn't exist yet - will be created by ORM"
                    )
                    return
        ...
        try:
            with get_session() as session:
                ...
                if "user_agent_instances" not in inspector.get_table_names():
                    logger.info(
                        "Table user_agent_instances doesn't exist yet - will be created by ORM"
                    )
                    return

                # Check existing columns
                columns = [
                    col["name"] for col in inspector.get_columns("user_agent_instances")
                ]
                ...
                if "usage_count" not in columns:
                    logger.info(
                        "Adding usage_count column to user_agent_instances table..."
                    )
                    ...
                    session.commit()
                    logger.info(
                        "✅ Added usage_count column to user_agent_instances table"
                    )
                else:
                    logger.info(
                        "✅ usage_count column already exists in user_agent_instances table"
                    )
                ...
                if "last_used_at" not in columns:
                    logger.info(
                        "Adding last_used_at column to user_agent_instances table..."
                    )
                    ...
                    session.commit()
                    logger.info(
                        "✅ Added last_used_at column to user_agent_instances table"
                    )
                else:
                    logger.info(
                        "✅ last_used_at column already exists in user_agent_instances table"
                    )
            ...
            if (
                ...
                logger.info(
                    "Usage tracking columns already exist in user_agent_instances table"
                )
            else:
