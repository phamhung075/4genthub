...
class OptimizationMetric:
    ...
    def compression_ratio(self) -> float:
        ...
        if self.original_size and self.optimized_size:
            return (
                (self.original_size - self.optimized_size) / self.original_size
            ) * 100
        return 0.0
...
class OptimizationMetricsCollector(MetricsCollector):
    ...
    def record_response_optimization(
        self,
        original_size: int,
        optimized_size: int,
        ...
        self.record_percentage_metric(
            "response_compression_ratio", compression_ratio, tags
        )
        self.record_timing_metric(
            "response_processing_time", processing_time_ms / 1000, tags=tags
        )
        ...
        self._check_optimization_alerts(
            compression_ratio, processing_time_ms, optimization_type
        )

        logger.debug(
            f"Response optimization recorded: {compression_ratio:.1f}% compression, "
            f"{processing_time_ms:.1f}ms processing time"
        )

    def record_context_injection_metrics(
        self,
        ...
        field_reduction_ratio = (
            (fields_requested - fields_returned) / max(fields_requested, 1)
        ) * 100

        # Enhanced tags
        context_tags = {**(tags or {}), "cache_hit": str(cache_hit), "tier": tier}

        # Record metrics
        self.record_metric(
            "context_fields_requested", float(fields_requested), "count", context_tags
        )
        self.record_metric(
            "context_fields_returned", float(fields_returned), "count", context_tags
        )
        self.record_percentage_metric(
            "context_field_reduction", field_reduction_ratio, context_tags
        )
        self.record_timing_metric(
            "context_query_time", query_time_ms / 1000, tags=context_tags
        )
        self.record_metric(
            "context_cache_hit", 1.0 if cache_hit else 0.0, "boolean", context_tags
        )
        ...
        logger.debug(
            f"Context injection recorded: {field_reduction_ratio:.1f}% field reduction, "
            f"{'HIT' if cache_hit else 'MISS'} cache, {query_time_ms:.1f}ms query time"
        )

    def record_ai_performance_metrics(
        self,
        parse_success: bool,
        ...
        # Record metrics
        self.record_metric(
            "ai_parse_success", 1.0 if parse_success else 0.0, "boolean", ai_tags
        )
        self.record_timing_metric(
            "ai_extraction_time", extraction_time_ms / 1000, tags=ai_tags
        )

        if not parse_success:
            self.record_metric(
                "ai_parse_errors", 1.0, "count", ai_tags, category="errors"
            )

        self.record_metric(
            "ai_agent_operations", 1.0, "count", ai_tags, category="operations"
        )
        ...
        logger.debug(
            f"AI performance recorded: {'SUCCESS' if parse_success else 'FAILED'} parse, "
    ...
    def record_system_health_metrics(
        ...
        self.record_metric(
            "system_network_bandwidth", network_bandwidth_kbps, "kbps", system_tags
        )
        self.record_metric(
            "system_db_queries", float(db_query_count), "count", system_tags
        )
        self.record_timing_metric(
            "system_db_query_time", db_query_time_ms / 1000, tags=system_tags
        )

        # Calculate system health score
        health_score = self._calculate_system_health_score(
            cpu_usage, memory_usage, db_query_time_ms
        )
        self.record_metric("system_health_score", health_score, "score", system_tags)

        logger.debug(
            f"System health recorded: {health_score:.1f} health score, "
    ...
    def _update_cache_hit_rate(self, cache_hit: bool, tier: str) -> None:
        ...
        if hit_rate < self.performance_baselines["cache_hit_rate_minimum"]:
            self._trigger_alert(
                "cache_hit_rate_low",
                f"Cache hit rate for {tier} is {hit_rate:.1f}%",
    ...
    def _update_ai_success_rate(self, success: bool, operation: str) -> None:
        ...
        self.record_percentage_metric(
            f"ai_success_rate_{operation}", success_rate, {"operation": operation}
        )

    def _calculate_system_health_score(
        self, cpu: float, memory: float, db_time: float
    ) -> float:
    ...
    def _check_optimization_alerts(
        self, compression_ratio: float, processing_time: float, opt_type: str
    ) -> None:
    ...
    def get_optimization_summary(self, time_window_hours: float = 1) -> dict[str, Any]:
        ...
        with self._optimization_lock:
            recent_optimizations = [
                opt for opt in self.optimization_metrics if opt.timestamp >= cutoff_time
            ]
        ...
        return {
            ...
            "optimization_performance": {
                ...
                "avg_compression_ratio": sum(compression_ratios)
            ...
            "performance_metrics": {
                "avg_processing_time_ms": self.get_metric_summary(
                    "response_processing_time", time_window_hours
                ),
                "cache_hit_rates": {
                    tier: self.get_metric_summary(
                        f"cache_hit_rate_{tier}", time_window_hours
                    )
                    for tier in ["global", "project", "branch", "task"]
                },
                "ai_success_rates": {
                    op: self.get_metric_summary(
                        f"ai_success_rate_{op}", time_window_hours
                    )
                    for op in ["hint_extraction", "response_parsing", "task_delegation"]
                },
            },
            "system_health": {
                "health_score": self.get_metric_summary(
                    "system_health_score", time_window_hours
                ),
                "resource_utilization": {
                    "cpu": self.get_metric_summary(
                        "system_cpu_usage", time_window_hours
                    ),
                    "memory": self.get_metric_summary(
                        "system_memory_usage", time_window_hours
                    ),
                },
            },
            "alerts": {
            ...
            "recommendations": self._generate_optimization_recommendations(
                recent_optimizations, recent_alerts
            ),
        }

    def _generate_optimization_recommendations(
        self, optimizations: list[OptimizationMetric], alerts: list[dict]
    ) -> list[str]:
        ...
        if not optimizations:
            recommendations.append(
                "No optimization data available - enable metrics collection"
            )
            return recommendations

        # Compression ratio recommendations
        avg_compression = sum(opt.compression_ratio for opt in optimizations) / len(
            optimizations
        )
        if avg_compression < 40:
            recommendations.append(
                "Consider implementing more aggressive response optimization strategies"
            )
        ...
        if profile_counts.get("DEBUG", 0) > profile_counts.get("MINIMAL", 0):
            recommendations.append(
                "High DEBUG profile usage detected - consider switching to MINIMAL for better performance"
            )
        ...
        if critical_alerts:
            recommendations.append(
                "Critical performance issues detected - review system resources and optimization settings"
            )
        ...
        if cache_alerts:
            recommendations.append(
                "Cache performance issues detected - consider increasing cache sizes or TTL values"
            )

        return recommendations

    def export_optimization_dashboard_data(
        self, time_window_hours: float = 24
    ) -> dict[str, Any]:
        ...
        dashboard_data = {
            "dashboard": {
                ...
                "time": {"from": f"now-{int(time_window_hours)}h", "to": "now"},
                "panels": [],
            },
            "metrics": {
                ...
                "system_health": {
                    ...
                    "type": "gauge",
                    "targets": [
                        {"expr": "system_health_score", "legendFormat": "Health Score"},
    ...
    async def generate_optimization_report(
        ...
        report = {
            ...
            "executive_summary": {
                "total_optimizations": summary.get("optimization_performance", {}).get(
                    "total_optimizations", 0
                ),
                "avg_compression_achieved": summary.get(
                    "optimization_performance", {}
                ).get("avg_compression_ratio", 0),
                "system_health_status": "healthy"
                if summary.get("system_health", {})
                and summary.get("system_health", {}).get("health_score", {})
                and summary.get("system_health", {})
                .get("health_score", {})
                .get("avg_value", 0)
                > 70
