...
class MetricsMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        self.collector.record_size_metric(
            "api_request_size", request_size, tags={"path": path, "method": method}
        )

        self.collector.record_size_metric(
            "api_response_size", response_size, tags={"path": path, "method": method}
        )
        ...
        if request_size > 0 and response_size > 0:
            if response_size < request_size:
                compression_ratio = (
                    (request_size - response_size) / request_size
                ) * 100
            ...
            try:
                ...
                if track_success_rate:
                    collector.record_metric(
                        f"{operation_name}_success", 1.0, "boolean", tags
                    )
                ...
                if track_errors:
                    ...
                    collector.record_metric(
                        f"{operation_name}_errors", 1.0, "count", error_tags
                    )

                # Track failure if success tracking enabled
                if track_success_rate:
                    collector.record_metric(
                        f"{operation_name}_success", 0.0, "boolean", tags
                    )
                ...
                if track_timing and start_time is not None:
                    ...
                    collector.record_timing_metric(
                        f"{operation_name}_duration", start_time, end_time, tags=tags
                    )
...
class MetricsCollectionService:
    ...
    async def start_metrics_collection(
        self, enable_reporting: bool = True, report_config: ReportConfig | None = None
    ):
    ...
    async def generate_on_demand_report(
        ...
        if not self.reporter:
            raise RuntimeError(
                "Reporting not enabled - start metrics collection with enable_reporting=True"
            )
...
def track_ai_operation(operation: str):
    ...
    return metrics_track(
        operation_name=f"ai_{operation}", custom_tags={"ai_operation": operation}
    )
