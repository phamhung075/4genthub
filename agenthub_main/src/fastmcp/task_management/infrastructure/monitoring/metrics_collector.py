...
class MetricPoint:
    ...
    def to_prometheus_format(self) -> str:
        ...
        tags_str = (
            ",".join([f'{k}="{v}"' for k, v in self.tags.items()]) if self.tags else ""
        )
...
class MetricsCollector:
    ...
    def record_timing_metric(
        ...
        self.record_metric(
            name=name, value=duration, unit="ms", tags=tags, category="timing"
        )

    def record_size_metric(
        self, name: str, size_bytes: int, tags: dict[str, str] | None = None
    ) -> None:
        """Record size metric in bytes."""
        self.record_metric(
            name=name, value=float(size_bytes), unit="bytes", tags=tags, category="size"
        )

    def record_percentage_metric(
        self, name: str, percentage: float, tags: dict[str, str] | None = None
    ) -> None:
    ...
    def record_rate_metric(
        self, name: str, rate: float, tags: dict[str, str] | None = None
    ) -> None:
        """Record rate metric (operations per second)."""
        self.record_metric(
            name=name, value=rate, unit="ops/sec", tags=tags, category="rate"
        )
    ...
    async def _process_metrics_batch(self, metrics: list[MetricPoint]) -> None:
        ...
        def write_metrics():
            with open(filename, "w") as f:
                json.dump(
                    [metric.to_dict() for metric in metrics], f, indent=2, default=str
                )
        ...
        while self._running:
            try:
                ...
                self.record_percentage_metric(
                    "system_cpu_usage", cpu_percent, {"source": "system"}
                )

                # Memory metrics
                memory = psutil.virtual_memory()
                self.record_percentage_metric(
                    "system_memory_usage", memory.percent, {"source": "system"}
                )
                self.record_size_metric(
                    "system_memory_available", memory.available, {"source": "system"}
                )
                ...
                self.record_size_metric(
                    "process_memory_rss", process_memory.rss, {"source": "process"}
                )
                self.record_size_metric(
                    "process_memory_vms", process_memory.vms, {"source": "process"}
                )

                # Disk I/O metrics
                disk_io = psutil.disk_io_counters()
                if disk_io:
                    self.record_rate_metric(
                        "system_disk_read_rate",
                        disk_io.read_bytes,
                        {"source": "system"},
                    )
                    self.record_rate_metric(
                        "system_disk_write_rate",
                        disk_io.write_bytes,
                        {"source": "system"},
                    )
    ...
    def get_metric_summary(
        self, metric_name: str, time_window_hours: float | None = None
    ) -> MetricSummary | None:
        ...
        with self._aggregation_lock:
            ...
            return MetricSummary(
                ...
                p95_value=statistics.quantiles(values, n=20)[18]
                if len(values) > 1
                else values[0],
                p99_value=statistics.quantiles(values, n=100)[98]
                if len(values) > 2
                else values[0],
        ...
        with self._aggregation_lock:
            for metric_name, metrics_list in self._aggregated_metrics.items():
                ...
                for metric in metrics_list:
                    ...
                    if (
                        tag_key not in latest_metrics
                        or metric.timestamp > latest_metrics[tag_key].timestamp
                    ):
    ...
    def generate_performance_report(
        ...
        if cpu_summary:
            report["system_health"]["cpu_usage"] = {
                ...
                "status": "healthy"
                if cpu_summary.avg_value < 80
                else "warning"
                if cpu_summary.avg_value < 95
                else "critical",
            }
        ...
        if memory_summary:
            report["system_health"]["memory_usage"] = {
                ...
                "status": "healthy"
                if memory_summary.avg_value < 80
                else "warning"
                if memory_summary.avg_value < 95
                else "critical",
            }

        # Performance target analysis
        response_time_summary = self.get_metric_summary(
            "mcp_response_time", time_window_hours
        )
        ...
        with self._aggregation_lock:
            for metric_name in list(self._aggregated_metrics.keys()):
                ...
                self._aggregated_metrics[metric_name] = [
                    m
                    for m in self._aggregated_metrics[metric_name]
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.start_time:
            self.collector.record_timing_metric(
                self.metric_name, self.start_time, tags=self.tags
            )
