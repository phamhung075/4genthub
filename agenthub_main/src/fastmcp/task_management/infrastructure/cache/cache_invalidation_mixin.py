...
class CacheInvalidationMixin:
    ...
    def invalidate_cache_for_entity(
        ...
        try:
            ...
            if not user_id:
                logger.warning(
                    f"No user_id available for cache invalidation of {entity_type}:{entity_id}"
                )
                return
            ...
            if entity_type == "context":
                self._invalidate_context_cache(
                    entity_id, level, user_id, operation, propagate
                )
                ...
                logger.warning(
                    f"Unknown entity type for cache invalidation: {entity_type}"
                )

            logger.debug(
                f"Cache invalidated for {entity_type}:{entity_id} after {operation.value}"
            )
    ...
    def _invalidate_context_cache(
        ...
        if not level:
            logger.warning(
                f"No level specified for context cache invalidation: {context_id}"
            )
            return

        # Invalidate the specific context
        self.cache.invalidate_context(
            user_id=user_id, level=level, context_id=context_id
        )
    ...
    def _invalidate_task_cache(
        self, task_id: str, user_id: str, operation: CacheOperation, propagate: bool
    ):
        """Invalidate task-related caches"""
        # Tasks might have associated contexts
        self.cache.invalidate_context(user_id=user_id, level="task", context_id=task_id)
    ...
    def _invalidate_project_cache(
        self, project_id: str, user_id: str, operation: CacheOperation, propagate: bool
    ):
        """Invalidate project-related caches"""
        self.cache.invalidate_context(
            user_id=user_id, level="project", context_id=project_id
        )
    ...
    def _invalidate_branch_cache(
        self, branch_id: str, user_id: str, operation: CacheOperation, propagate: bool
    ):
        """Invalidate branch-related caches"""
        self.cache.invalidate_context(
            user_id=user_id, level="branch", context_id=branch_id
        )
    ...
    def _propagate_context_invalidation(
        self, level: str, context_id: str, user_id: str
    ):
        ...
        if affected_levels:
            logger.debug(
                f"Propagating cache invalidation from {level}:{context_id} to {affected_levels}"
            )
            ...
            for affected_level in affected_levels:
                self.cache.invalidate_context(user_id=user_id, level=affected_level)
                self.cache.invalidate_inheritance(user_id=user_id, level=affected_level)

    def _invalidate_child_contexts(
        self, parent_level: str, parent_id: str, user_id: str
    ):
