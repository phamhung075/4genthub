...
class ContextCache:
    ...
    def get_inheritance_chain(
        ...
        key = self._make_key(
            "inheritance", level=level, context_id=context_id, user_id=user_id
        )
    ...
    def set_inheritance_chain(
        self, level: str, context_id: str, user_id: str, data: dict
    ):
    ...
    def invalidate_inheritance(
        self, user_id: str, level: str = None, context_id: str = None
    ):
    ...
    def get_context(self, level: str, context_id: str, user_id: str) -> dict | None:
        """Get cached context data"""
        key = self._make_key(
            "context", level=level, context_id=context_id, user_id=user_id
        )
        data = self.redis.get(key)
        if data:
            decompressed = self._decompress(data)
    ...
    def set_context(
        self, level: str, context_id: str, user_id: str, data: dict, ttl: int = None
    ):
        """Cache context data"""
        key = self._make_key(
            "context", level=level, context_id=context_id, user_id=user_id
    ...
    def get_multiple_contexts(
        self, contexts: list[dict[str, str]], user_id: str
    ) -> list[dict | None]:
    ...
    def set_multiple_contexts(
        self, contexts: list[dict], user_id: str, ttl: int = None
    ):
