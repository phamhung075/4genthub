...
class MetricsReporter:
    ...
    def __init__(
        self, metrics_collector: OptimizationMetricsCollector, config: ReportConfig
    ):
    ...
    async def generate_weekly_report(
        ...
        current_week_summary = self.metrics_collector.get_optimization_summary(
            24 * 7
        )  # 7 days
        previous_week_summary = self.metrics_collector.get_optimization_summary(
            24 * 14
        )  # 14 days for comparison

        # Calculate trends
        trends = self._calculate_weekly_trends(
            current_week_summary, previous_week_summary
        )

        # Generate weekly recommendations
        weekly_recommendations = self._generate_weekly_recommendations(
            trends, current_week_summary
        )
        ...
        report_data = {
            ...
            "total_optimizations": current_week_summary.get(
                "optimization_performance", {}
            ).get("total_optimizations", 0),
            "avg_compression": current_week_summary.get(
                "optimization_performance", {}
            ).get("avg_compression_ratio", 0),
            "avg_health": self._safe_get_health_score(current_week_summary),
            "total_alerts": current_week_summary.get("alerts", {}).get(
                "total_alerts", 0
            ),
            "trends": trends,
            "weekly_recommendations": weekly_recommendations,
            "compression_trend": "↗️ Improving"
            if trends.get("compression_ratio", {}).get("trend") == "up"
            else "↘️ Declining",
            "compression_trend_class": "trend-up"
            if trends.get("compression_ratio", {}).get("trend") == "up"
            else "trend-down",
            "health_trend": "↗️ Improving"
            if trends.get("system_health", {}).get("trend") == "up"
            else "↘️ Declining",
    ...
    async def generate_monthly_roi_report(
    ...
    def _calculate_weekly_trends(
        ...
        current_compression = current.get("optimization_performance", {}).get(
        ...
        trends["compression_ratio"] = {
            ...
            "change_percent": round(
                (
                    (current_compression - previous_compression)
                    / max(previous_compression, 1)
                )
                * 100,
                1,
            ),
        ...
        trends["system_health"] = {
            ...
            "change_percent": round(
                ((current_health - previous_health) / max(previous_health, 1)) * 100, 1
            ),
        }
    ...
    def _generate_weekly_recommendations(
        ...
        if trends.get("compression_ratio", {}).get("trend") == "down":
            recommendations.append(
                "Compression ratio declining - review optimization algorithms and consider profile adjustments"
            )

        # Health trend recommendations
        if trends.get("system_health", {}).get("trend") == "down":
            recommendations.append(
                "System health declining - monitor resource utilization and investigate performance bottlenecks"
            )
        ...
        if total_alerts > 10:
            recommendations.append(
                f"High alert volume ({total_alerts} alerts) - review alert thresholds and address root causes"
            )

        # Cache performance recommendations
        cache_performance = summary.get("performance_metrics", {}).get(
            "cache_hit_rates", {}
        )
        low_cache_tiers = [
            tier
            for tier, data in cache_performance.items()
            if data and data.get("latest_value", 0) < 60
        ]
        if low_cache_tiers:
            recommendations.append(
                f"Low cache hit rates detected in: {', '.join(low_cache_tiers)} - consider cache optimization"
            )

        if not recommendations:
            recommendations.append(
                "System performing well - maintain current optimization strategies"
            )
    ...
    def _calculate_roi_metrics(self, summary: dict) -> dict[str, Any]:
        ...
        avg_compression = summary.get("optimization_performance", {}).get(
            "avg_compression_ratio", 0
        )

        # Estimate cost savings based on compression and reduced bandwidth/storage
        estimated_bytes_saved = (
            total_optimizations * 5000 * (avg_compression / 100)
        )  # Rough estimate
        estimated_cost_savings = estimated_bytes_saved * 0.0001  # $0.0001 per KB saved

        # Calculate efficiency improvements
        avg_processing_time = summary.get("performance_metrics", {}).get(
            "avg_processing_time_ms", {}
        )
        processing_time_value = (
            avg_processing_time.get("avg_value", 100) if avg_processing_time else 100
        )

        efficiency_improvement = max(
            0, (200 - processing_time_value) / 200 * 100
        )  # Baseline 200ms
    ...
    async def _send_email_report(self, subject: str, html_content: str):
        ...
        try:
            ...
            logger.info(
                f"Email report sent to {len(self.config.email_recipients)} recipients"
            )
    ...
    async def _alert_monitor(self):
        ...
        while self._running:
            try:
                ...
                recent_summary = self.metrics_collector.get_optimization_summary(
                    5 / 60
                )  # 5 minutes in hours

                critical_alerts = recent_summary.get("alerts", {}).get(
                    "critical_alerts", 0
                )
        ...
        for alert in recent_alerts[:5]:  # Top 5 alerts
            if alert.get("severity") == "critical":
                alert_messages.append(
                    f"• {alert.get('message', 'Unknown alert')} at {alert.get('timestamp', 'Unknown time')}"
                )
...
def get_global_metrics_reporter(config: ReportConfig | None = None) -> MetricsReporter:
    ...
    if _global_metrics_reporter is None:
        ...
        _global_metrics_reporter = MetricsReporter(
            get_global_optimization_collector(), config
        )
