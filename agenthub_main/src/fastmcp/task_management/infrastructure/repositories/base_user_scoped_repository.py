...
class BaseUserScopedRepository:
    ...
    def __init__(self, session: Session, user_id: str | None = None):
        ...
        # When user_id is None, operate in system mode (no user filtering)
        self._is_system_mode = user_id is None

        if self._is_system_mode:
            logger.info(
                "Repository initialized in system mode during startup - no user filtering applied (expected behavior)"
            )
        ...
        try:
            ...
            if hasattr(query, "column_descriptions"):
                ...
                if hasattr(model, "user_id"):
                    ...
                    logger.warning(
                        f"Model {model.__name__} does not have user_id attribute"
                    )
                    return query
            else:
        ...
        if hasattr(entity, "user_id"):
            if entity.user_id != self.user_id:
                raise PermissionError(
                    f"Access denied: Entity does not belong to user {self.user_id}"
                )
        else:
            logger.warning(
                f"Entity {type(entity).__name__} does not have user_id attribute"
            )
    ...
    def log_access(
        self, operation: str, entity_type: str, entity_id: str | None = None
    ) -> None:
        """
        ...
        if self.user_id:
            logger.info(
                f"User {self.user_id} performed {operation} on {entity_type} {entity_id or 'new'}"
            )
        else:
            logger.info(
                f"System performed {operation} on {entity_type} {entity_id or 'new'}"
            )
