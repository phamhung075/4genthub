...
class BranchContextRepository(CacheInvalidationMixin, BaseORMRepository):
        ...
        if self._session:
            ...
            try:
                ...
                if "duplicate key" in str(ie).lower():
                    logger.error(
                        "Duplicate key constraint violation - record may already exist"
                    )
    ...
    def create(self, entity: BranchContext) -> BranchContext:
        ...
        with self.get_db_session() as session:
            try:
                ...
                query = session.query(BranchContextModel).filter(
                ...
                if "agent_assignments" in branch_settings:
                    branch_info["agent_assignments"] = branch_settings.get(
                        "agent_assignments", {}
                    )

                # Normalize project_id as well
                project_id = (
                    _normalize_uuid(entity.project_id) if entity.project_id else None
                )
                ...
                for key, value in entity.metadata.items():
                    if key not in predefined_metadata_keys:
                        ...
                        logger.debug(
                            f"Preserved custom metadata field '{key}' in branch context data"
                        )
                ...
                session.rollback()
                logger.error(
                    f"Integrity constraint violation creating branch context: {ie}"
                )
                if (
                    "duplicate key" in str(ie).lower()
                    or "unique constraint" in str(ie).lower()
                ):
                    # Handle duplicate key by returning existing record
                    existing = (
                        session.query(BranchContextModel)
                        .filter(BranchContextModel.id == normalized_id)
                        .first()
                    )
                    if existing:
                        logger.info(
                            f"Returning existing branch context after duplicate key error: {normalized_id}"
                        )
                        return self._to_entity(existing)
                raise ValueError(
                    f"Branch context creation failed due to constraint violation: {str(ie)}"
                )
    ...
    def get(self, context_id: str) -> BranchContext | None:
        ...
        with self.get_db_session() as session:
            query = session.query(BranchContextModel).filter(
                BranchContextModel.id == context_id
            )
    ...
    def update(self, context_id: str, entity: BranchContext) -> BranchContext:
        ...
        with self.get_db_session() as session:
            ...
            db_model.user_id = (
                self.user_id or entity.metadata.get("user_id") or db_model.user_id
            )  # CRITICAL FIX: Never fallback to 'system'
        ...
        with self.get_db_session() as session:
            ...
            if filters:
                if "project_id" in filters:
                    stmt = stmt.where(
                        BranchContextModel.parent_project_id == filters["project_id"]
                    )
    ...
    def _to_entity(self, db_model: BranchContextModel) -> BranchContext:
        ...
        branch_info = getattr(db_model, "branch_info", {}) or data_field.get(
            "branch_info", {}
        )
        git_branch_name = branch_info.get(
            "name", f"branch-{db_model.branch_id or db_model.id}"
        )
        ...
        return BranchContext(
            ...
            branch_info=getattr(db_model, "branch_info", {})
            or data_field.get("branch_info", {}),
            branch_workflow=getattr(db_model, "branch_workflow", {})
            or data_field.get("branch_workflow", {}),
            feature_flags=getattr(db_model, "feature_flags", {})
            or data_field.get("feature_flags", {}),
            discovered_patterns=getattr(db_model, "discovered_patterns", {})
            or data_field.get("discovered_patterns", {}),
            branch_decisions=getattr(db_model, "branch_decisions", {})
            or data_field.get("branch_decisions", {}),
            branch_settings=branch_settings,  # Keep for backwards compatibility
            metadata={
                "active_patterns": getattr(db_model, "active_patterns", {})
                or data_field.get("active_patterns", {}),
                "local_overrides": db_model.local_overrides
                or data_field.get("local_overrides", {}),
                "delegation_rules": db_model.delegation_rules
                or data_field.get("delegation_rules", {}),
