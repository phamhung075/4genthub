...
class ProjectContextRepository(CacheInvalidationMixin, BaseORMRepository):
    ...
    def create(self, entity: ProjectContext) -> ProjectContext:
        ...
        with self.get_db_session() as session:
            ...
            db_model = ProjectContextModel(
                ...
                technical_specifications=getattr(entity, "technical_specifications", {})
                or {},
    ...
    def update(self, context_id: str, entity: ProjectContext) -> ProjectContext:
        ...
        with self.get_db_session() as session:
            ...
            db_model.technical_specifications = (
                getattr(entity, "technical_specifications", {}) or {}
            )
        ...
        with self.get_db_session() as session:
            ...
            if filters:
                if "project_id" in filters:
                    stmt = stmt.where(
                        ProjectContextModel.project_id == filters["project_id"]
                    )
    ...
    def _to_entity(self, db_model: ProjectContextModel) -> ProjectContext:
        ...
        return ProjectContext(
            ...
            technical_specifications=getattr(db_model, "technical_specifications", {})
            or {},
            metadata={
