...
class ProjectRepositoryFactory:
    ...
    def create(
        ...
        **kwargs,
        ...
        logger.info(
            f"🔍 Project Repository Factory: Creating repository with user_id: {user_id}"
        )
        ...
        if user_id is None:
            logger.error(
                "❌ Project Repository Factory: No user_id provided - authentication is required"
            )
            raise UserAuthenticationRequiredError("Project repository creation")
        else:
            logger.info(
                f"✅ Project Repository Factory: Using provided user_id: {user_id}"
            )
        ...
        # Create new instance
        try:
            repository = cls._create_instance(
                repository_type, user_id, db_path, **kwargs
            )
        except Exception as e:
            logger.warning(
                f"Failed to create {repository_type} repository, falling back to mock: {e}"
            )
    ...
    def _get_default_type(cls) -> RepositoryType:
        ...
        logger.debug(
            f"[ProjectRepositoryFactory] Environment: {env}, Database Type: {db_type}"
        )

        # Test environment - use mock
        if env == "test":
            logger.info(
                "[ProjectRepositoryFactory] Test environment detected, using MOCK"
            )
    ...
    def _generate_cache_key(
        cls, repository_type: RepositoryType, user_id: str, db_path: str | None
    ) -> str:
        """Generate cache key for repository instance"""
        type_value = (
            repository_type.value
            if hasattr(repository_type, "value")
            else str(repository_type)
        )
        return f"{type_value}:{user_id}:{db_path if db_path else 'memory'}"
    ...
    @classmethod
    def register_type(
        cls, repository_type: RepositoryType, repository_class: type[ProjectRepository]
    ) -> None:
...
class GlobalRepositoryManager:
    ...
    def get_for_user(cls, user_id: str) -> ProjectRepository:
        ...
        if user_id not in cls._user_repositories:
            cls._user_repositories[user_id] = ProjectRepositoryFactory.create(
                user_id=user_id
            )
...
def create_project_repository(
    ...
    return ProjectRepositoryFactory.create(
        repository_type=repo_type, user_id=user_id, db_path=db_path
    )
...
def get_sqlite_repository(
    ...
    return ProjectRepositoryFactory.create(
        repository_type=RepositoryType.ORM, user_id=user_id, db_path=db_path
    )
