...
class UserScopedORMRepository(
    BaseORMRepository[ModelType], BaseUserScopedRepository, Generic[ModelType]
):
    """
    ...
    def __init__(
        self, model_class, session: Session | None = None, user_id: str | None = None
    ):
        """
        ...
        BaseUserScopedRepository.__init__(
            self, session or self.get_db_session(), user_id
        )
    ...
    def bulk_create(self, records: list[dict[str, Any]]) -> list[ModelType]:
        ...
        self.log_access(
            "bulk_create", self.model_class.__name__, f"count={len(models)}"
        )
