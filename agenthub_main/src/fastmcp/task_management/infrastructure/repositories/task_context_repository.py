...
class TaskContextRepository(CacheInvalidationMixin, BaseORMRepository):
    ...
    def _get_effective_user_id(self, entity) -> str:
        """Get effective user_id, raising error if not found."""
        user_id = self.user_id or entity.metadata.get("user_id")
        if not user_id:
            raise ValueError("user_id is required for task context operations")
        return user_id
    ...
    def create(self, entity: TaskContext) -> TaskContext:
        ...
        with self.get_db_session() as session:
            ...
            db_model = TaskContextModel(
                ...
                user_id=self._get_effective_user_id(entity),
                version=1,
            )
    ...
    def get(self, context_id: str) -> TaskContext | None:
        ...
        with self.get_db_session() as session:
            query = session.query(TaskContextModel).filter(
                TaskContextModel.id == context_id
            )
    ...
    def update(self, context_id: str, entity: TaskContext) -> TaskContext:
        ...
        with self.get_db_session() as session:
            ...
            db_model.implementation_notes = entity.metadata.get(
                "implementation_notes", {}
            )
            db_model.delegation_triggers = entity.metadata.get(
                "delegation_triggers", {}
            )
            db_model.inheritance_disabled = entity.metadata.get(
        ...
        with self.get_db_session() as session:
            ...
            if filters:
                if "branch_id" in filters:
                    stmt = stmt.where(
                        TaskContextModel.parent_branch_id == filters["branch_id"]
                    )
                if "inheritance_disabled" in filters:
                    stmt = stmt.where(
                        TaskContextModel.inheritance_disabled
                        == filters["inheritance_disabled"]
                    )
