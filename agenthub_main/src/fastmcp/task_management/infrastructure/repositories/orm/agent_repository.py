...
class ORMAgentRepository(
    ...
    def __init__(
        self, session=None, project_id: str | None = None, user_id: str | None = None
    ):
        """
    ...
    def _normalize_assigned_trees_to_set(self, assigned_trees_raw) -> set:
        ...
        """
        logger.debug(
            f"[NORMALIZE] Input type: {type(assigned_trees_raw)}, value: {assigned_trees_raw}"
        )

        if isinstance(assigned_trees_raw, str):
            # Single UUID stored as string
            logger.debug(
                "[NORMALIZE] Detected string type, returning set with single string"
            )
        ...
        elif hasattr(assigned_trees_raw, "__iter__") and not isinstance(
            assigned_trees_raw, str
        ):
        ...
        try:
            ...
            if agent.capabilities:
                for cap_str in agent.capabilities:
                    try:
                        ...
                        logger.warning(
                            f"Invalid capability '{cap_str}' for agent {agent.id}"
                        )
            ...
            if agent.status:
                try:
                    ...
                    logger.warning(
                        f"Invalid status '{agent.status}' for agent {agent.id}"
                    )
            ...
            assigned_projects = (
                set(assigned_projects_raw)
                if isinstance(assigned_projects_raw, list)
                else set()
            )
            ...
            active_tasks = (
                set(active_tasks_raw) if isinstance(active_tasks_raw, list) else set()
            )
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    logger.warning(
                        f"Duplicate key error during agent creation (race condition): {create_error}"
                    )
                    raise ValidationException(
                        message=(
            ...
            logger.info(
                f"Successfully registered agent '{agent.name}' (ID: {agent.id}) for project {project_id}"
            )
            return agent_entity
    ...
    def assign_agent_to_tree(
        ...
        logger.debug(
            f"[AGENT_REPO] assign_agent_to_tree called with project_id={project_id}, agent_id={agent_id}, git_branch_id={git_branch_id}"
        )
        logger.debug(
            f"[AGENT_REPO] Type of git_branch_id: {type(git_branch_id)}, value: {git_branch_id}"
        )
        ...
        try:
            ...
            if ":" in agent_id:
                ...
                agent_name = parts[1]
                logger.info(
                    f"Parsed agent_id format - UUID: {actual_agent_id}, Name: {agent_name}"
                )
            ...
            if not agent:
                ...
                logger.info(
                    f"Auto-registering agent {actual_agent_id} in project {project_id}"
                )
                try:
                    ...
                    if self.exists(id=actual_agent_id):
                        logger.info(
                            f"Agent {actual_agent_id} already exists, fetching it"
                        )
                        agent = self.get_by_id(actual_agent_id)
                        if agent:
                            auto_registered = False
                            logger.info(
                                f"Found existing agent {actual_agent_id} after exists check"
                            )
                        else:
                        ...
                        try:
                            ...
                            logger.info(
                                f"Successfully auto-registered agent {actual_agent_id} (name: {agent_name}) in project {project_id}"
                            )
                        except Exception as create_error:
                            # Handle duplicate key constraint violation (race condition)
                            if (
                                "UNIQUE constraint" in str(create_error)
                                or "duplicate key" in str(create_error).lower()
                            ):
                                logger.info(
                                    f"Agent {actual_agent_id} was created by another process, fetching it"
                                )
                                agent = self.get_by_id(actual_agent_id)
                                if agent:
                                    auto_registered = False
                                    logger.info(
                                        f"Found existing agent {actual_agent_id} after race condition"
                                    )
                                else:
                                    raise ResourceNotFoundException(
                    ...
                    logger.error(
                        f"Failed to auto-register agent {actual_agent_id}: {reg_error}"
                    )
                    raise ResourceNotFoundException(
            ...
            logger.debug(
                f"[AGENT_REPO] Checking if git_branch_id {git_branch_id} is in assigned_trees..."
            )
            ...
            logger.info(
                f"Assigned agent {actual_agent_id} to tree {git_branch_id} in project {project_id}"
            )
            return {
                "success": True,
            ...
            logger.error(
                f"Error assigning agent {actual_agent_id if 'actual_agent_id' in locals() else agent_id} to tree {git_branch_id}: {e}"
            )
            raise DatabaseException(
    ...
    def unassign_agent_from_tree(
        self, project_id: str, agent_id: str, git_branch_id: str = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Unassigned agent {agent_id} from {len(removed_assignments)} tree(s) in project {project_id}"
            )
            return {
    ...
    def list_agents(self, project_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"agents": agent_list, "total_agents": len(agent_list)}
    ...
    def rebalance_agents(self, project_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            for agent in agents:
                ...
                if assignments:
                    changes.append(
                        f"Agent '{agent.name}' has {len(assignments)} assignments"
                    )
    ...
    def find_by_name(self, name: str) -> Agent | None:
        ...
        try:
            ...
            if agent:
                logger.info(
                    f"Found agent by name with @: @{clean_name} -> ID: {agent.id}"
                )
                return agent
