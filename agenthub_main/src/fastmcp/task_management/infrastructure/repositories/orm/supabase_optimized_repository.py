...
class SupabaseOptimizedRepository(ORMTaskRepository):
    ...
    def __init__(self, git_branch_id: str = None):
        ...
        logger.info(
            f"Using Supabase-optimized repository for minimal latency, git_branch_id: {self.git_branch_id}"
        )
    ...
    def list_tasks_minimal(
        ...
        with self.get_db_session() as session:
            ...
            if assignee_id:
                filters.append(
                    "EXISTS (SELECT 1 FROM task_assignees WHERE task_id = tasks.id AND assignee_id = :assignee_id)"
                )
            ...
            logger.info(
                f"Fetched {len(tasks)} tasks with minimal query (Supabase optimized)"
            )
            return tasks

    def list_tasks_no_relations(
        self,
        status: str = None,
        priority: str = None,
        assignee_id: str = None,
        ...
        with self.get_db_session() as session:
            ...
            logger.info(
                f"Fetched {len(entities)} tasks without relationships (Supabase optimized)"
            )
            return entities
