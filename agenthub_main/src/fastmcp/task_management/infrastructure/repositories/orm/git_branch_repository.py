...
class ORMGitBranchRepository(
    BaseTimestampRepository[ProjectGitBranch], GitBranchRepository
):
    """
    ...
    def __init__(self, user_id: str | None = None, performance_mode: bool = False):
        ...
        logger.info(
            f"ORMGitBranchRepository initialized for user: {user_id}, performance_mode: {performance_mode}"
        )
    ...
    def with_user(self, user_id: str) -> ORMGitBranchRepository:
        ...
        return ORMGitBranchRepository(
            user_id=user_id, performance_mode=self.performance_mode
        )
    ...
    def _model_to_entity(self, model: ProjectGitBranch) -> GitBranch:
        ...
        with self.get_db_session() as session:
            ...
            tasks = session.query(Task).filter(Task.git_branch_id == model.id).all()
    ...
    async def find_by_id(
        ...
        try:
            with self.get_db_session() as session:
                ...
                if not self.user_id:
                    raise ValueError(
                        "User authentication required for branch operations"
                    )
        ...
        try:
            with self.get_db_session() as session:
                ...
                models = (
                    ...
                    .order_by(ProjectGitBranch.created_at.desc())
                    .all()
                )
    ...
    async def delete(self, project_id: str, branch_id: str) -> bool:
        ...
        try:
            with self.get_db_session() as session:
                ...
                project = (
                    ...
                    .filter(
                        and_(
                            ...
                            Project.user_id
                            == self.user_id,  # Verify user owns the project
                        )
                    )
                    .first()
                )
                ...
                if not project:
                    logger.warning(
                        f"User {self.user_id} attempted to delete branch in project {project_id} they don't own"
                    )
                    return False
        ...
        try:
            with self.get_db_session() as session:
                ...
                if self.user_id:
                    ...
                    project = (
                        ...
                        .filter(
                            and_(
                                ...
                                Project.user_id
                                == self.user_id,  # Verify user owns PROJECT
                            )
                        )
                        .first()
                    )
                    ...
                    if not project:
                        logger.warning(
                            f"User {self.user_id} attempted to delete branch in project they don't own"
                        )
                        return False

                logger.info(
                    f"Starting comprehensive cascade deletion for branch {branch_id}"
                )
                ...
                if self.user_id:
                    cache_filter = and_(
                        cache_filter, ContextInheritanceCache.user_id == self.user_id
                    )

                cache_count = (
                    session.query(ContextInheritanceCache).filter(cache_filter).delete()
                )
                logger.info(
                    f"Deleted {cache_count} ContextInheritanceCache records for branch {branch_id}"
                )
                ...
                delegation_filter = and_(
                    or_(
                        and_(
                            ContextDelegation.source_id == branch_id,
                            ContextDelegation.source_level == "branch",
                        ),
                        and_(
                            ContextDelegation.target_id == branch_id,
                            ContextDelegation.target_level == "branch",
                        ),
                    )
                )
                if self.user_id:
                    delegation_filter = and_(
                        delegation_filter, ContextDelegation.user_id == self.user_id
                    )

                delegation_count = (
                    session.query(ContextDelegation).filter(delegation_filter).delete()
                )
                logger.info(
                    f"Deleted {delegation_count} ContextDelegation records for branch {branch_id}"
                )
                ...
                if self.user_id:
                    branch_context_query = branch_context_query.filter(
                        BranchContext.user_id == self.user_id
                    )

                branch_context_ids = [row[0] for row in branch_context_query.all()]
                logger.info(
                    f"Found {len(branch_context_ids)} BranchContext records for branch {branch_id}"
                )

                # Step 4: Get Task IDs for this branch (needed for task-related cascades)
                from ...database.models import Task

                task_query = session.query(Task.id).filter(
                ...
                logger.info(
                    f"Found {len(task_ids)} Task records for branch {branch_id}"
                )
                ...
                if task_ids:
                    ...
                    if self.user_id:
                        subtask_filter = and_(
                            subtask_filter, Subtask.user_id == self.user_id
                        )

                    subtask_count = (
                        session.query(Subtask)
                        .filter(subtask_filter)
                        .delete(synchronize_session=False)
                    )
                    logger.info(
                        f"Deleted {subtask_count} Subtask records for branch {branch_id}"
                    )
                ...
                if task_ids:
                    ...
                    if self.user_id:
                        assignee_filter = and_(
                            assignee_filter, TaskAssignee.user_id == self.user_id
                        )
                    ...
                    logger.info(
                        f"Deleted {assignee_count} TaskAssignee records for branch {branch_id}"
                    )
                ...
                if task_ids:
                    ...
                    if self.user_id:
                        label_filter = and_(
                            label_filter, TaskLabel.user_id == self.user_id
                        )
                    ...
                    logger.info(
                        f"Deleted {label_count} TaskLabel records for branch {branch_id}"
                    )
                ...
                if task_ids:
                    ...
                    if self.user_id:
                        dependency_filter = and_(
                            dependency_filter, TaskDependency.user_id == self.user_id
                        )

                    dependency_count = (
                        session.query(TaskDependency)
                        .filter(dependency_filter)
                        .delete(synchronize_session=False)
                    )
                    logger.info(
                        f"Deleted {dependency_count} TaskDependency records for branch {branch_id}"
                    )
                ...
                if self.user_id:
                    task_context_filter = and_(
                        task_context_filter, TaskContext.user_id == self.user_id
                    )

                direct_task_context_count = (
                    session.query(TaskContext).filter(task_context_filter).delete()
                )
                logger.info(
                    f"Deleted {direct_task_context_count} direct TaskContext records for branch {branch_id}"
                )

                # Step 10: Delete TaskContext records referencing BranchContext records from this branch
                if branch_context_ids:
                    indirect_task_context_filter = (
                        TaskContext.parent_branch_context_id.in_(branch_context_ids)
                    )
                    if self.user_id:
                        indirect_task_context_filter = and_(
                            indirect_task_context_filter,
                            TaskContext.user_id == self.user_id,
                        )

                    indirect_task_context_count = (
                        session.query(TaskContext)
                        .filter(indirect_task_context_filter)
                        .delete(synchronize_session=False)
                    )
                    logger.info(
                        f"Deleted {indirect_task_context_count} indirect TaskContext records for branch {branch_id}"
                    )
                ...
                if self.user_id:
                    branch_context_filter = and_(
                        branch_context_filter, BranchContext.user_id == self.user_id
                    )

                branch_context_count = (
                    session.query(BranchContext).filter(branch_context_filter).delete()
                )
                logger.info(
                    f"Deleted {branch_context_count} BranchContext records for branch {branch_id}"
                )
                ...
                deleted_count = (
                    session.query(ProjectGitBranch).filter(branch_filter).delete()
                )
                logger.info(f"Deleted branch {branch_id}: {deleted_count > 0}")

                session.commit()
                logger.info(
                    f"Comprehensive cascade deletion completed for branch {branch_id}"
                )
                return deleted_count > 0

        except SQLAlchemyError as e:
            logger.error(
                f"Error in comprehensive cascade deletion for branch {branch_id}: {e}"
            )
            raise DatabaseException(
                message=f"Failed to delete git branch: {str(e)}",
            ...
            with self.get_db_session() as session:
                ...
                for field, value in updates.items():
                    if hasattr(model, field):
                        ...
                        logger.warning(
                            f"Field {field} does not exist on ProjectGitBranch model"
                        )
                ...
                if model:
                    ...
                    task_count_result = (
                        session.query(
                            ...
                            func.count(case((Task.status == "done", Task.id))).label(
                                "completed"
                            ),
                        )
    ...
    def find_by_project_id(self, project_id: str) -> list:
        ...
        try:
            with self.get_db_session() as session:
                models = (
                    session.query(ProjectGitBranch)
                    .filter(ProjectGitBranch.project_id == project_id)
                    .all()
                )
                ...
                for model in models:
                    ...
                    task_count_result = (
                        ...
                        .filter(Task.git_branch_id == model.id)
                        .first()
                    )
    ...
    async def find_by_assigned_agent(self, agent_id: str) -> list[GitBranch]:
        ...
        try:
            with self.get_db_session() as session:
                models = (
                    session.query(ProjectGitBranch)
                    .filter(ProjectGitBranch.assigned_agent_id == agent_id)
                    .order_by(ProjectGitBranch.created_at.desc())
                    .all()
                )
    ...
    async def find_available_for_assignment(self, project_id: str) -> list[GitBranch]:
        ...
        try:
            ...
            logger.error(
                f"Error finding available branches for project {project_id}: {e}"
            )
            raise DatabaseException(
    ...
    async def assign_agent(
        self, project_id: str, branch_id: str, agent_id: str
    ) -> bool:
            ...
            with self.get_db_session() as session:
                ...
                stats = (
                    session.query(
                        ...
                        func.sum(
                            case((ProjectGitBranch.status == "done", 1), else_=0)
                        ).label("completed_branches"),
                        func.sum(
                            case((ProjectGitBranch.status == "in_progress", 1), else_=0)
                        ).label("active_branches"),
                        func.sum(
                            case(
                                (ProjectGitBranch.assigned_agent_id.isnot(None), 1),
                                else_=0,
                            )
                        ).label("assigned_branches"),
                        func.sum(ProjectGitBranch.task_count).label("total_tasks"),
                        func.sum(ProjectGitBranch.completed_task_count).label(
                ...
                status_rows = (
                    ...
                    .filter(ProjectGitBranch.project_id == project_id)
                    .group_by(ProjectGitBranch.status)
                    .all()
                )
                ...
                if stats.total_tasks and stats.total_tasks > 0:
                    overall_progress = (
                        stats.total_completed_tasks / stats.total_tasks
                    ) * 100.0
    ...
    async def create_git_branch(
        ...
        try:
            git_branch = await self.create_branch(
                project_id, git_branch_name, git_branch_description
            )
            return {
                "success": True,
                "git_branch": {
            ...
            return {"success": False, "error": str(e), "error_code": "CREATE_FAILED"}
    ...
    async def get_git_branch_by_id(self, git_branch_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "error_code": "GET_FAILED"}
    ...
    async def list_git_branchs(self, project_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "git_branchs": branches, "count": len(branches)}
        except Exception as e:
    ...
    async def update_git_branch(
        ...
        try:
            ...
            return {"success": False, "error": str(e), "error_code": "UPDATE_FAILED"}
    ...
    async def assign_agent_to_branch(
        ...
        try:
            ...
            return {"success": False, "error": str(e), "error_code": "ASSIGN_FAILED"}
    ...
    async def unassign_agent_from_branch(
        ...
        try:
            ...
            return {"success": False, "error": str(e), "error_code": "UNASSIGN_FAILED"}
        ...
        try:
            with self.get_db_session() as session:
                ...
                task_count_result = (
                    ...
                    .filter(Task.git_branch_id == git_branch_id)
                    .first()
                )
    ...
    async def archive_branch(
        ...
        try:
            ...
            return {"success": False, "error": str(e), "error_code": "ARCHIVE_FAILED"}
            ...
            try:
                ...
                for row in result:
                    branch_data = {
                        ...
                        "is_completed": row.total_tasks > 0
                        and row.done_count == row.total_tasks,
                    }
                    branches.append(branch_data)

                logger.info(
                    f"Retrieved {len(branches)} branches with task counts for project {project_id}"
                )
                return branches
    ...
    async def check_name_exists_in_project(
        self, project_id: str, name: str, exclude_branch_id: str | None = None
    ) -> bool:
        """
    ...
    def find_by_ids(self, branch_ids: list[str]) -> dict[str, GitBranch]:
        ...
        try:
            with self.get_db_session() as session:
                ...
                models = (
                    session.query(ProjectGitBranch)
                    .filter(ProjectGitBranch.id.in_(branch_ids))
                    .all()
                )
