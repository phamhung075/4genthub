...
class CachedSubtaskRepository:
    ...
    def _init_redis(self) -> redis.Redis | None:
        ...
        try:
            ...
            client.ping()
            logger.info(
                "[CachedSubtaskRepository] Redis cache initialized successfully"
            )
            return client
        except (RedisError, Exception) as e:
            logger.warning(
                f"[CachedSubtaskRepository] Redis not available, caching disabled: {e}"
            )
            return None
    ...
    def get_by_task_id(self, task_id: str) -> list[Subtask]:
        ...
        if result:
            self._set_cached(
                cache_key, [s.__dict__ if hasattr(s, "__dict__") else s for s in result]
            )
    ...
    def find_by_parent_task_id(self, parent_task_id) -> list[Subtask]:
        ...
        task_id_str = (
            ...
            if hasattr(parent_task_id, "value")
            else str(parent_task_id)
        )
        ...
        # Fetch from base repo
        logger.info(
            f"[CachedSubtaskRepository] Cache miss, delegating find_by_parent_task_id to base repo for task: {task_id_str}"
        )
        ...
        if result is not None:
            ...
            logger.debug(
                f"[Cache] Cached {len(result)} subtasks for parent_task: {task_id_str}"
            )
    ...
    def update_progress(
        self, subtask_id: str, progress_percentage: int, progress_notes: str = None
    ) -> Subtask:
        """Update subtask progress with cache invalidation"""
        # Get subtask details for invalidation
        subtask = self.get_by_id(subtask_id)

        result = self.base_repo.update_progress(
            subtask_id, progress_percentage, progress_notes
        )
