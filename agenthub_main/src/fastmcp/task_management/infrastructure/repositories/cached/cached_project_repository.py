...
class CachedProjectRepository:
    ...
    def _init_redis(self) -> redis.Redis | None:
        ...
        try:
            ...
            client.ping()
            logger.info(
                "[CachedProjectRepository] Redis cache initialized successfully"
            )
            return client
        except (RedisError, Exception) as e:
            logger.warning(
                f"[CachedProjectRepository] Redis not available, caching disabled: {e}"
            )
            return None
    ...
    def get_all(self) -> list[Project]:
        ...
        if result:
            self._set_cached(
                cache_key, [p.__dict__ if hasattr(p, "__dict__") else p for p in result]
            )
