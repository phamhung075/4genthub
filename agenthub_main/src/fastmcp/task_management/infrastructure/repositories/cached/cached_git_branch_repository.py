...
class CachedGitBranchRepository:
    ...
    def _init_redis(self) -> Optional[redis.Redis]:
        ...
        try:
            ...
            # Test connection
            client.ping()
            logger.info("[CachedGitBranchRepository] Redis cache initialized successfully")
            return client
        except (RedisError, Exception) as e:
            logger.warning(f"[CachedGitBranchRepository] Redis not available, caching disabled: {e}")
            return None
    ...
    def get_by_project_id(self, project_id: str) -> List[GitBranch]:
        ...
        # Cache the result
        if result:
            self._set_cached(cache_key, [b.__dict__ if hasattr(b, '__dict__') else b for b in result])
