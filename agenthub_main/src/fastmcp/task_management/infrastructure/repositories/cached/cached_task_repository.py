...
class CachedTaskRepository:
    ...
    def _init_redis(self) -> redis.Redis | None:
        ...
        try:
            ...
            logger.warning(
                f"[CachedTaskRepository] Redis not available, caching disabled: {e}"
            )
            return None
    ...
    def _set_cached(self, key: str, value: Any):
        ...
        try:
            ...
            self.redis_client.setex(self._cache_key(key), self.ttl, serialized)
    ...
    async def create(self, task: Task) -> Task:
        ...
        if self.enabled:
            ...
            logger.info(
                f"[Cache] Invalidated caches after creating task: {task.id if hasattr(task, 'id') else 'new'}"
            )
    ...
    async def update(self, task: Task) -> Task:
        ...
        if self.enabled:
            ...
            self._invalidate_pattern("search:*")
            logger.info(
                f"[Cache] Invalidated caches after updating task: {task.id if hasattr(task, 'id') else 'unknown'}"
            )
    ...
    async def delete(self, task_id: TaskId) -> bool:
        ...
        if self.enabled:
            ...
            logger.info(
                f"[Cache] Invalidated all caches after deleting task: {task_id}"
            )
