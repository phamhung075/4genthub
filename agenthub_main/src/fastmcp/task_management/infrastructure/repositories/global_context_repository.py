...
class GlobalContextRepository(CacheInvalidationMixin, BaseUserScopedRepository):
    ...
    def __init__(self, session_factory, user_id: str | None = None):
        ...
        if user_id:
            ...
            logger.debug(
                "GlobalContextRepository initialized in system mode during startup - use with caution (expected behavior)"
            )
    ...
    def _normalize_context_id(self, context_id: str) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Failed to create user-specific UUID: {e}, falling back to standard UUID"
            )
            return GLOBAL_SINGLETON_UUID
    ...
    def create(self, entity: GlobalContext) -> GlobalContext:
        ...
        with self.get_db_session() as session:
            ...
            if self._is_system_mode:
                ...
                existing = (
                    session.query(GlobalContextModel)
                    .filter(
                        and_(
                            GlobalContextModel.id
                            == normalized_id,  # Use the normalized ID
            ...
            if existing:
                raise ValueError(
                    "Global context already exists for user. Use update instead."
                )

            # Ensure user_id is set unless in system mode
            if not self.user_id and not self._is_system_mode:
                raise ValueError(
                    "user_id is required for global context creation. Repository must be scoped to a user."
                )
            ...
            organization_standards = nested_structure_dict.get("organization", {}).get(
                "standards", {}
            )
            security_policies = nested_structure_dict.get("security", {}).get(
                "access_control", {}
            )
            compliance_requirements = nested_structure_dict.get("organization", {}).get(
                "compliance", {}
            )
            shared_resources = nested_structure_dict.get("operations", {}).get(
                "resources", {}
            )
            reusable_patterns = nested_structure_dict.get("development", {}).get(
                "patterns", {}
            )
            delegation_rules = nested_structure_dict.get("organization", {}).get(
                "policies", {}
            )
            ...
            logger.info(
                f"Creating global context with nested structure for user {self.user_id}"
            )
    ...
    def get(self, context_id: str) -> GlobalContext | None:
        ...
        with self.get_db_session() as session:
            ...
            if not db_model:
                logger.debug(
                    f"Global context not found for user {self.user_id}: {context_id}"
                )
            else:
    ...
    def update(self, context_id: str, entity: GlobalContext) -> GlobalContext:
        ...
        with self.get_db_session() as session:
            ...
            known_fields = {
                ...
                "delegation_rules",
                "user_preferences",
            }  # user_preferences is already mapped above
            custom_fields = {}
            ...
            organization_standards_nested = nested_structure_dict.get(
                "organization", {}
            ).get("standards", {})
            security_policies_nested = nested_structure_dict.get("security", {}).get(
                "access_control", {}
            )
            compliance_requirements_nested = nested_structure_dict.get(
                "organization", {}
            ).get("compliance", {})
            shared_resources_nested = nested_structure_dict.get("operations", {}).get(
                "resources", {}
            )
            reusable_patterns_nested = nested_structure_dict.get("development", {}).get(
                "patterns", {}
            )
            delegation_rules_nested = nested_structure_dict.get("organization", {}).get(
                "policies", {}
            )
            ...
            db_model.organization_standards = (
                organization_standards
                if organization_standards
                else organization_standards_nested
            )
            db_model.security_policies = (
                security_policies if security_policies else security_policies_nested
            )
            db_model.compliance_requirements = (
                compliance_requirements
                if compliance_requirements
                else compliance_requirements_nested
            )
            db_model.shared_resources = (
                shared_resources if shared_resources else shared_resources_nested
            )
            db_model.reusable_patterns = (
                reusable_patterns if reusable_patterns else reusable_patterns_nested
            )
            db_model.global_preferences = (
                global_preferences
                if global_preferences
                else combined_preferences_nested
            )
            db_model.delegation_rules = (
                delegation_rules if delegation_rules else delegation_rules_nested
            )
            ...
            logger.info(
                f"Updated unified_context_data with {len(merged_data)} keys for global context {context_id}"
            )
    ...
    def _to_entity(self, db_model: GlobalContextModel) -> GlobalContext:
        ...
        if unified_data:
            ...
            logger.info(
                f"Loading global context with {len(global_settings)} keys from unified_context_data"
            )
        else:
            global_settings = {}
        ...
        if (
            nested_structure_dict and schema_version == "2.0" and False
        ):  # Temporarily disabled complex merging
            ...
            if "preferences" in nested_structure_dict:
                ...
                if "user_interface" in prefs:
                    global_settings["user_preferences"] = prefs.get(
                        "user_interface", {}
                    )
                if "agent_behavior" in prefs:
                    global_settings["ai_agent_settings"] = {
                        "preferred_agents": [],
                        **prefs.get("agent_behavior", {}),
                    }
            ...
            if "organization" in nested_structure_dict:
                ...
                if "policies" in org and "agent_delegation" in org["policies"]:
                    global_settings["delegation_rules"] = org["policies"].get(
                        "agent_delegation", {}
                    )

            # Add schema version and custom categories
            if "_schema_version" in nested_structure_dict:
                global_settings["_schema_version"] = nested_structure_dict[
                    "_schema_version"
                ]
            if "_custom_categories" in nested_structure_dict:
                global_settings["_custom_categories"] = nested_structure_dict[
                    "_custom_categories"
                ]
            ...
            global_settings = {
                "organization_standards": getattr(
                    db_model, "organization_standards", None
                )
                or {},
                "security_policies": getattr(db_model, "security_policies", None) or {},
                "compliance_requirements": getattr(
                    db_model, "compliance_requirements", None
                )
                or {},
                ...
                "workflow_templates": getattr(db_model, "workflow_templates", None)
                or {},
            }
        ...
        if nested_structure_dict and schema_version == "2.0":
            try:
                entity._nested_data = GlobalContextNestedData.from_dict(
                    nested_structure_dict
                )
                entity._is_migrated = is_migrated
                logger.debug(
                    f"Loaded nested structure for global context {db_model.id}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to load nested structure for {db_model.id}: {e}"
                )
                # Fall back to migration from flat structure - but only if no custom data
                if not entity._is_only_nested_structure_data():
                    logger.info(
                        f"Preserving custom data in global_settings for {db_model.id}"
                    )
                else:
            ...
            if not entity._is_only_nested_structure_data():
                logger.info(
                    f"Preserving custom data, skipping nested structure init for {db_model.id}"
                )
            else:
                entity._ensure_nested_structure()
    ...
    def migrate_to_user_scoped(self) -> int:
        ...
        if not system_user_id:
            raise ValueError(
                "SYSTEM_USER_ID environment variable is required for migration"
            )
        migrated = 0
        ...
        with self.get_db_session() as session:
            ...
            contexts_to_migrate = (
                session.query(GlobalContextModel)
                .filter(GlobalContextModel.user_id is None)
                .all()
            )
