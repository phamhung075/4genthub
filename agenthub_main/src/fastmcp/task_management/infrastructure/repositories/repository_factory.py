...
class RepositoryFactory:
    ...
    def get_task_repository(
        ...
        logger.debug(
            f"[RepositoryFactory] Creating task repository with config: {config}"
        )
        ...
        if config["environment"] == "test":
            ...
            logger.info(
                "[RepositoryFactory] Using MockTaskRepository for test environment"
            )
        ...
        if config["database_type"] == "sqlite":
            ...
            logger.info(
                "[RepositoryFactory] SQLite configured, using ORMTaskRepository"
            )
            ...
            logger.info(
                "[RepositoryFactory] Supabase configured, using ORMTaskRepository"
            )
            ...
            logger.info(
                "[RepositoryFactory] PostgreSQL configured, using ORMTaskRepository"
            )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "CachedTaskRepository not available, using base repository"
                )
    ...
    def get_project_repository():
        ...
        logger.debug(
            f"[RepositoryFactory] Creating project repository with config: {config}"
        )
        ...
        if config["environment"] == "test":
            ...
            logger.info(
                "[RepositoryFactory] Using MockProjectRepository for test environment"
            )
        ...
        if config["database_type"] == "sqlite":
            ...
            logger.info(
                "[RepositoryFactory] SQLite configured, using ORMProjectRepository"
            )
            ...
            base_repository = ORMProjectRepository()
            logger.info(
                "[RepositoryFactory] Supabase configured, using ORMProjectRepository"
            )

        elif config["database_type"] == "postgresql":
            # PostgreSQL uses ORM repository directly - no separate implementation needed
            logger.info(
                "[RepositoryFactory] PostgreSQL configured, using ORMProjectRepository"
            )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "CachedProjectRepository not available, using base repository"
                )
    ...
    def get_git_branch_repository(user_id: str | None = None):
        ...
        logger.debug(
            f"[RepositoryFactory] Creating git branch repository with config: {config}"
        )
        ...
        if config["environment"] == "test":
            ...
            logger.info(
                "[RepositoryFactory] Using MockGitBranchRepository for test environment"
            )
        ...
        if config["database_type"] == "sqlite":
            ...
            logger.info(
                "[RepositoryFactory] SQLite configured, using ORMGitBranchRepository"
            )
            ...
            base_repository = ORMGitBranchRepository(user_id=user_id)
            logger.info(
                "[RepositoryFactory] Supabase configured, using ORMGitBranchRepository"
            )
            ...
            base_repository = ORMGitBranchRepository(user_id=user_id)
            logger.info(
                "[RepositoryFactory] PostgreSQL configured, using ORMGitBranchRepository"
            )
        ...
        if (
            ...
            try:
                ...
                logger.info(
                    "[RepositoryFactory] Wrapping with CachedGitBranchRepository"
                )
                return CachedGitBranchRepository(base_repository)
            except ImportError:
                logger.warning(
                    "CachedGitBranchRepository not available, using base repository"
                )
    ...
    def get_subtask_repository(user_id: str | None = None):
        ...
        logger.debug(
            f"[RepositoryFactory] Creating subtask repository with config: {config}, user_id: {user_id}"
        )
        ...
        if config["environment"] == "test":
            ...
            logger.info(
                "[RepositoryFactory] Using MockSubtaskRepository for test environment"
            )
        ...
        if config["database_type"] == "sqlite":
            ...
            logger.info(
                "[RepositoryFactory] SQLite configured, using ORMSubtaskRepository"
            )
            ...
            base_repository = ORMSubtaskRepository(user_id=user_id)
            logger.info(
                "[RepositoryFactory] Supabase configured, using ORMSubtaskRepository"
            )

        elif config["database_type"] == "postgresql":
            # PostgreSQL uses ORM repository directly - no separate implementation needed
            logger.info(
                "[RepositoryFactory] PostgreSQL configured, using ORMSubtaskRepository"
            )
        ...
        if not base_repository:
            ...
            base_repository = ORMSubtaskRepository(user_id=user_id)
            logger.info(
                f"[RepositoryFactory] Using ORMSubtaskRepository (fallback) with user_id: {user_id}"
            )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "CachedSubtaskRepository not available, using base repository"
                )
    ...
    def get_agent_repository():
        ...
        logger.debug(
            f"[RepositoryFactory] Creating agent repository with config: {config}"
        )
        ...
        if config["environment"] == "test":
            ...
            logger.info(
                "[RepositoryFactory] Using MockAgentRepository for test environment"
            )
        ...
        if config["database_type"] == "sqlite":
            ...
            logger.info(
                "[RepositoryFactory] SQLite configured, using ORMAgentRepository"
            )
            ...
            base_repository = ORMAgentRepository()
            logger.info(
                "[RepositoryFactory] Supabase configured, using ORMAgentRepository"
            )

        elif config["database_type"] == "postgresql":
            # PostgreSQL uses ORM repository directly - no separate implementation needed
            logger.info(
                "[RepositoryFactory] PostgreSQL configured, using ORMAgentRepository"
            )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "CachedAgentRepository not available, using base repository"
                )
    ...
    def get_context_repository():
        ...
        logger.debug(
            f"[RepositoryFactory] Creating context repository with config: {config}"
        )
        ...
        if config["environment"] == "test":
            try:
                ...
                logger.info(
                    "[RepositoryFactory] Using MockContextRepository for test environment"
                )
                return MockContextRepository()
            except ImportError:
                logger.warning(
                    "MockContextRepository not available, using mock task context"
                )
        ...
        if config["database_type"] == "sqlite":
            ...
            logger.info(
                "[RepositoryFactory] SQLite configured, using TaskContextRepository"
            )
            ...
            base_repository = TaskContextRepository(db_config.SessionLocal)
            logger.info(
                "[RepositoryFactory] Supabase configured, using TaskContextRepository"
            )

        elif config["database_type"] == "postgresql":
            # PostgreSQL uses TaskContextRepository directly - no separate implementation needed
            logger.info(
                "[RepositoryFactory] PostgreSQL configured, using TaskContextRepository"
            )
