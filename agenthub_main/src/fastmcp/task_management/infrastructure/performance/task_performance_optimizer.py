...
class TaskPerformanceOptimizer:
    ...
    def optimize_task_query(
        self, session: Session, base_query, filters: dict[str, Any]
    ) -> Any:
        ...
        optimized_query = optimized_query.execution_options(synchronize_session="fetch")
    ...
    def analyze_query_performance(self, session: Session) -> dict[str, Any]:
        ...
        """
        analysis = {"slow_queries": [], "missing_indexes": [], "recommendations": []}
    ...
    def optimize_response_payload(self, tasks: list[dict]) -> list[dict]:
        ...
        for task in tasks:
            ...
            optimized_task = {
                ...
                "is_blocked": any(
                    dep.get("status") != "done" for dep in task.get("dependencies", [])
                ),
            }
    ...
    def get_pagination_metadata(
        self, total_count: int, limit: int, offset: int
    ) -> dict:
