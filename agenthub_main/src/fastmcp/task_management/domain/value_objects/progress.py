...
class ProgressTimeline:
    ...
    def add_snapshot(self, snapshot: ProgressSnapshot) -> None:
        ...
        if snapshot.task_id != self.task_id:
            raise ValueError(
                f"Snapshot task_id {snapshot.task_id} doesn't match timeline task_id {self.task_id}"
            )
    ...
    def get_overall_progress(self) -> float:
        ...
        for snapshot in self.snapshots:
            if (
                snapshot.progress_type not in latest_by_type
                or snapshot.timestamp > latest_by_type[snapshot.progress_type].timestamp
            ):
    ...
    def add_milestone(self, name: str, percentage: float) -> None:
        ...
        if not 0 <= percentage <= 100:
            raise ValueError(
                f"Milestone percentage must be between 0 and 100, got {percentage}"
            )
...
class ProgressCalculationStrategy:
    ...
    def calculate_weighted_average(
        progress_values: dict[str, float], weights: dict[str, float] | None = None
    ) -> float:
        ...
        weighted_sum = sum(
            progress_values[k] * weights.get(k, 0) for k in progress_values.keys()
        )
    ...
    def calculate_from_subtasks(
        subtask_progress: list[dict[str, Any]], include_blocked: bool = False
    ) -> float:
        ...
        total_progress = sum(subtask.get("progress", 0.0) for subtask in valid_subtasks)

        return total_progress / len(valid_subtasks)

    @staticmethod
    def calculate_by_milestones(
        completed_milestones: list[str], all_milestones: dict[str, float]
    ) -> float:
        ...
        completed_values = [
            all_milestones[m] for m in completed_milestones if m in all_milestones
        ]
