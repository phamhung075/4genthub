...
def validate_user_id(user_id: str | None, operation: str = "This operation") -> str:
    ...
    if user_id is None:
        raise ValueError(
            f"{operation} requires user authentication. No user ID was provided."
        )
    ...
    try:
        ...
        logging.warning(
            f"UUID normalization failed for user_id '{user_id_str}': {e}. Using original value."
        )
        return user_id_str


def require_authenticated_user(
    user_id: str | None, operation: str = "This operation"
) -> str:
    """
