...
class Agent(BaseTimestampEntity):
    ...
    def complete_task(self, task_id: str, success: bool = True) -> None:
        ...
        if (
            ...
            and self.status == AgentStatus.BUSY
        ):
            self.status = AgentStatus.AVAILABLE
    ...
    def calculate_task_suitability_score(self, task_requirements: dict) -> float:
        ...
        workload_bonus = (
            1.0 - (self.current_workload / self.max_concurrent_tasks)
        ) * 10.0
    ...
    def check_availability(self) -> dict[str, Any]:
        ...
        if is_available:
            estimated_capacity = max(
                0, self.max_concurrent_tasks - self.current_workload
            )
        else:
