...
class CascadeDeletionService:
    ...
    def delete_task_cascade(
        self, task_id: str, scope: DeleteScope = DeleteScope.TASK_FULL
    ) -> dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Starting cascade deletion for task {task_id} with scope {scope.value}"
        )
        ...
        task_id_obj = (
            TaskId.from_string(task_id) if isinstance(task_id, str) else task_id
        )
        ...
        if success:
            ...
            self._dispatch_task_deleted_event(
                task_id, branch_id, task_status, task_title
            )
    ...
    def delete_branch_cascade(self, branch_id: str) -> dict[str, Any]:
        ...
        if (
            ...
            and hasattr(branch, "context_id")
            and branch.context_id
        ):
            try:
                self._context_repository.delete(branch.context_id)
                stats["contexts_deleted"] += 1
                logger.info(
                    f"Deleted context {branch.context_id} for branch {branch_id}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to delete branch context {branch.context_id}: {e}"
                )
        ...
        if success:
            ...
            logger.info(
                f"Successfully deleted branch {branch_id} with {stats['tasks_deleted']} tasks"
            )
        else:
    ...
    def delete_project_cascade(self, project_id: str) -> dict[str, Any]:
        ...
        if (
            self._context_repository
            and hasattr(project, "context_id")
            and project.context_id
        ):
            try:
                self._context_repository.delete(project.context_id)
                stats["contexts_deleted"] += 1
                logger.info(
                    f"Deleted context {project.context_id} for project {project_id}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to delete project context {project.context_id}: {e}"
                )
        ...
        if success:
            ...
            logger.info(
                f"Successfully deleted project {project_id} with {stats['branches_deleted']} branches"
            )
        else:
    ...
    def _dispatch_task_deleted_event(
        self, task_id: str, branch_id: str | None, status: str, title: str
    ) -> None:
        ...
        try:
            ...
            if branch_id:
                event = TaskDeletedEvent.create(
                    task_id=task_id, branch_id=branch_id, status=status, title=title
                )
    ...
    def _dispatch_branch_deleted_event(
        self, branch_id: str, project_id: str | None, name: str
    ) -> None:
        ...
        try:
            ...
            if project_id:
                event = BranchDeletedEvent.create(
                    branch_id=branch_id, project_id=project_id, name=name
                )
    ...
    def _dispatch_project_deleted_event(self, project_id: str, name: str) -> None:
        ...
        try:
            ...
            event = ProjectDeletedEvent.create(project_id=project_id, name=name)
