...
class GitBranchNameValidator:
    ...
    async def validate_unique_name(
        self, name: str, project_id: str, exclude_branch_id: str | None = None
    ) -> None:
        """
        ...
        existing_branches = await self._git_branch_repository.find_all_by_project(
            project_id
        )
        ...
        logger.debug(
            f"Branch name '{name}' validated successfully for project {project_id}"
        )
    ...
    async def validate_name_format(self, name: str) -> None:
        ...
        for char in forbidden_chars:
            if char in name:
                if char == " ":
                    raise ValidationException(
                        "Branch name cannot contain spaces. Use hyphens or underscores instead."
                    )
                else:
                    raise ValidationException(
                        f"Branch name cannot contain the character '{char}'"
                    )
    ...
    async def validate_branch_name(
        ...
        logger.info(
            f"Branch name '{name}' passed comprehensive validation for project {project_id}"
        )
