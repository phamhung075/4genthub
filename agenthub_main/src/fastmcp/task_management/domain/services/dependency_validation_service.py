...
class DependencyValidationService:
    ...
    def __init__(
        self, task_repository: TaskRepository, id_validator: IDValidator | None = None
    ):
        ...
        try:
            ...
            for dep_id in dependency_ids:
                ...
                if not dep_id_result.is_valid:
                    errors.append(
                        f"Invalid dependency ID format '{dep_id}': {dep_id_result.error_message}"
                    )
                    continue
            ...
            if orphaned_deps:
                errors.extend(
                    [
                        f"Dependency {dep_id} no longer exists"
    ...
    def _validate_single_dependency(
        self, task: Task, dependency_id: str, task_map: dict[str, Task]
    ) -> list[dict[str, Any]]:
        """
    ...
    def _find_dependency_across_states(self, dependency_id: str) -> Task | None:
        ...
        try:
            task_id = (
                TaskId.from_string(dependency_id)
    ...
    def _check_orphaned_dependencies(
        self, task: Task, task_map: dict[str, Task]
    ) -> list[str]:
        """
    ...
    def get_dependency_chain_status(self, task_id: TaskId) -> dict[str, Any]:
        ...
        try:
            ...
            completed_deps = sum(
                1 for dep in dependency_chain if dep.get("status") == "done"
            )
            blocked_deps = sum(
                1 for dep in dependency_chain if dep.get("status") == "blocked"
            )
            ...
            return {
                ...
                "chain_statistics": {
                    ...
                    "completion_percentage": (completed_deps / total_deps * 100)
                    if total_deps > 0
                    else 100,
                },
                "can_proceed": self._can_task_proceed(task, task_map),
    ...
    def _get_dependency_info(
        self, dependency_id: str, task_map: dict[str, Task]
    ) -> dict[str, Any]:
        """
        ...
        return {
            ...
            "status": dep_status,
            "found": True,
            "is_completed": dep_task.status.is_done()
            if hasattr(dep_task.status, "is_done")
            else dep_status == "done",
            "priority": str(dep_task.priority)
            if hasattr(dep_task, "priority")
            else "unknown",
        }
