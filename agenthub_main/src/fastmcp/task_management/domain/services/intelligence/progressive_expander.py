...
class ProgressiveExpander:
    ...
    def __init__(
        ...
        logger.info(
            f"ProgressiveExpander initialized (budget: {default_token_budget} tokens)"
        )
    ...
    def _calculate_expansion_priority(
        ...
        if context_id in self.context_access_patterns:
            ...
            access_frequency = pattern.get("access_count", 0) / max(
                1, pattern.get("total_sessions", 1)
            )
        ...
        recency_bonus = 0.0
        if context_id in self.context_access_patterns:
            last_accessed = self.context_access_patterns[context_id].get(
                "last_accessed"
            )
            if last_accessed:
                ...
                recency_bonus = max(
                    0.0, 0.2 * (1.0 - min(1.0, hours_ago / 24))
                )  # Decay over 24 hours
    ...
    def identify_expansion_candidates(
        ...
        for context in available_contexts:
            ...
            for trigger in triggers or [
                ExpansionTrigger.TOKEN_AVAILABLE
            ]:  # Default trigger
                ...
                candidate = ExpansionCandidate(
                    ...
                    trigger=trigger,
                    metadata={
                        "similarity_score": similarity_scores.get(context_id, 0.0)
    ...
    def expand_context_progressive(
        ...
        for candidate in expansion_candidates:
            ...
            if tokens_used + needed_tokens > available_budget:
                ...
                if candidate.priority_score > 0.8 and candidate.estimated_tokens < (
                    available_budget * 0.1
                ):
    ...
    def _has_dependency_relationship(
        self, candidate_context: dict[str, Any], current_context: dict[str, Any]
    ) -> bool:
        """Check if candidate has dependency relationship with current context."""
        # Task-level dependency checking
        candidate_id = candidate_context.get("id") or candidate_context.get(
            "context_id"
        )
    ...
    def get_expansion_stats(self) -> dict[str, Any]:
        ...
        avg_tokens = sum(exp["tokens_used"] for exp in recent_expansions) / len(
            recent_expansions
        )
        avg_contexts = sum(exp["contexts_expanded"] for exp in recent_expansions) / len(
            recent_expansions
        )
