...
class ContextPrioritizer:
    ...
    def score_context(
        ...
        # 8. Dependency Boost
        dependency_boost = self._calculate_dependency_boost(context_data, current_task)
    ...
    def score_contexts_batch(
        ...
        logger.info(
            f"Scored {len(scores)} contexts, top score: {scores[0].total_score:.3f}"
            if scores
            else "No contexts scored"
        )
        return scores

    def _calculate_semantic_score(
        self, context_data: dict[str, Any], query: str, similarity: float
    ) -> float:
    ...
    def _calculate_frequency_score(self, context_id: str) -> float:
        ...
        recent_accesses = [
            access
            for access in self.context_access_history[context_id]
            if access > cutoff
        ]

        # Normalize frequency (log scale to avoid dominance of very frequent items)
        frequency_score = math.log(len(recent_accesses) + 1) / math.log(
            self.frequency_window_days + 1
        )
    ...
    def _calculate_size_penalty(self, context_data: dict[str, Any]) -> float:
        ...
        penalty = min(
            0.5, excess_tokens / self.size_penalty_threshold
        )  # Cap at 50% penalty
    ...
    def _calculate_project_priority_score(
        self, context_data: dict[str, Any], project_context: dict[str, Any] | None
    ) -> float:
    ...
    def _calculate_dependency_boost(
        self, context_data: dict[str, Any], current_task: dict[str, Any] | None
    ) -> float:
    ...
    def record_context_access(
        self, context_id: str, access_time: datetime | None = None
    ) -> None:
    ...
    def get_scoring_stats(self) -> dict[str, Any]:
        ...
        total_accesses = sum(
            len(history) for history in self.context_access_history.values()
        )
        avg_accesses_per_context = total_accesses / total_contexts_tracked

        # Most frequently accessed contexts
        frequent_contexts = sorted(
            self.context_access_history.items(), key=lambda x: len(x[1]), reverse=True
        )[:5]
