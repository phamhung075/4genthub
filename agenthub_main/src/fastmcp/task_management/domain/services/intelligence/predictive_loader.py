...
class PredictiveLoader:
    ...
    def start_session(self, session_id: str, user_id: str | None = None) -> None:
        ...
        self.current_session = SessionContext(
            session_id=session_id, start_time=datetime.now(UTC), user_id=user_id
        )
    ...
    def end_session(self) -> dict[str, Any]:
        ...
        session_data = {
            ...
            "duration_minutes": (
                datetime.now(UTC) - self.current_session.start_time
            ).total_seconds()
            / 60,
        }
    ...
    def predict_next_contexts(
        ...
        current_context_id = current_context.get("id") or current_context.get(
            "context_id"
        )
        if current_context_id:
            transition_predictions = self._predict_from_context_transitions(
                current_context_id
            )
            for context_id, confidence in transition_predictions.items():
                ...
                prediction_reasons.append(
                    f"Context transition pattern suggests {context_id}"
                )
        ...
        for context_id, confidence in time_predictions.items():
            ...
            combined_confidence = max(
                existing_confidence, confidence * 0.7
            )  # Lower weight for time patterns
            predictions[context_id] = combined_confidence
            confidence_scores[context_id] = combined_confidence
        ...
        if session_context:
            ...
            for context_id, confidence in history_predictions.items():
                ...
                prediction_reasons.append(
                    f"Session history pattern suggests {context_id}"
                )
        ...
        logger.info(
            f"Predicted {len(predicted_contexts)} contexts with confidence >= {self.pattern_confidence_threshold}"
        )
        return result
        ...
        for window_size in [2, 3, 4]:
            if len(recent_tools) >= window_size:
                ...
                for pattern_seq, frequency in self.tool_sequences.items():
                    if (
                        len(pattern_seq) > len(sequence)
                        and pattern_seq[: len(sequence)] == sequence
                        and frequency >= self.pattern_min_frequency
                    ):
    ...
    def _predict_from_context_transitions(
        self, current_context_id: str
    ) -> dict[str, float]:
        ...
        for time_key, access_times in self.time_based_patterns.items():
            ...
            if len(parts) >= 2:
                ...
                if abs(current_hour - pattern_hour) <= 1:  # Within 1 hour
                    ...
                    confidence = min(
                        0.7, frequency / 15.0
                    )  # Lower confidence for time patterns
                    predictions[f"time_based_{context_type}"] = confidence

        return predictions

    def _predict_from_session_history(
        ...
        for historical_session in self.session_history[-20:]:  # Look at recent sessions
            ...
            tool_similarity = self._calculate_sequence_similarity(
                current_tools, hist_tools
            )
            context_similarity = self._calculate_sequence_similarity(
                current_contexts, hist_contexts
            )
            ...
            if tool_similarity > 0.5 or context_similarity > 0.5:
                ...
                for context_id in hist_contexts[len(current_contexts) :]:
                    ...
                    predictions[context_id] = max(
                        predictions.get(context_id, 0), confidence
                    )
    ...
    def validate_predictions(
        self, predictions: list[str], actual_contexts: list[str], pattern_ids: list[str]
    ) -> dict[str, float]:
        """
            ...
            if pattern_id in self.usage_patterns:
                ...
                if pattern.success_rate == 0.0:
                    ...
                    pattern.success_rate = (
                        alpha * hit_rate + (1 - alpha) * pattern.success_rate
                    )

                accuracy_by_pattern[pattern_id] = pattern.success_rate

        logger.info(
            f"Prediction validation: {len(correct_predictions)}/{len(predictions)} correct (hit rate: {hit_rate:.2f})"
        )
    ...
    def get_prediction_stats(self) -> dict[str, Any]:
        ...
        avg_success_rate = (
            sum(p.success_rate for p in self.usage_patterns.values()) / total_patterns
        )
        high_confidence_patterns = sum(
            1 for p in self.usage_patterns.values() if p.confidence > 0.7
        )
