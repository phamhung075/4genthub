    ...
    def extract_task_vector(self, task) -> TaskVector:
        ...
        try:
            ...
            return TaskVector(
                ...
                agents=getattr(task, "assignees", []).copy()
                if getattr(task, "assignees", [])
                else [],
                priority=task.priority.value
                if hasattr(task.priority, "value")
                else str(getattr(task, "priority", "medium")),
            ...
            self.logger.error(
                f"Error extracting features from task {getattr(task, 'id', 'unknown')}: {e}"
            )
...
class PatternLearner:
    ...
    def learn_from_project_history(
        ...
        for project in project_data:
            try:
                ...
                self.logger.error(
                    f"Error analyzing project {project.get('id', 'unknown')}: {e}"
                )
        ...
        self.logger.info(
            f"Learned {len(new_patterns)} new patterns from {len(project_data)} projects"
        )
        return new_patterns

    def _analyze_project_patterns(
    ...
    def _identify_pattern_type(
        ...
        if source.agents and target.agents:
            ...
            if (
                source_agents_set != target_agents_set
                and source_agents_set.intersection(target_agents_set)
            ):
    ...
    def _extract_pattern_features(self, vector: TaskVector) -> dict[str, Any]:
        ...
        features = {
            ...
            "file_types": list(
                set(
                    [ref.split(".")[-1] for ref in vector.file_references if "." in ref]
                )
            ),
    ...
    def _calculate_pattern_confidence(
        ...
        # Keyword similarity
        source_keywords = set(
            source_features["title_keywords"] + source_features["description_keywords"]
        )
        target_keywords = set(
            target_features["title_keywords"] + target_features["description_keywords"]
        )
        ...
        if source_keywords and target_keywords:
            ...
            keyword_similarity = (
                keyword_overlap / keyword_union if keyword_union > 0 else 0
            )
        ...
        if source_file_types and target_file_types:
            ...
            file_similarity = file_overlap / len(
                source_file_types.union(target_file_types)
            )
        ...
        if source_entities and target_entities:
            ...
            entity_similarity = entity_overlap / len(
                source_entities.union(target_entities)
            )
...
class PatternRecognitionEngine:
    ...
    def train_from_historical_data(
        ...
        # Learn patterns
        learned_patterns = self.pattern_learner.learn_from_project_history(
            project_history
        )
        ...
        avg_confidence = (
            np.mean([p.confidence for p in learned_patterns])
            if learned_patterns
            else 0.0
        )
    ...
    def predict_dependencies(
        self, task, available_tasks: list
        ...
        try:
            ...
            source_features = self.pattern_learner._extract_pattern_features(
                source_vector
            )
            ...
            for candidate_task in available_tasks:
                ...
                candidate_vector = self.feature_extractor.extract_task_vector(
                    candidate_task
                )
                candidate_features = self.pattern_learner._extract_pattern_features(
                    candidate_vector
                )
            ...
            self.logger.error(
                f"Error predicting dependencies for task {getattr(task, 'id', 'unknown')}: {e}"
            )
            return []
    ...
    def _find_matching_patterns(
        ...
        matching_patterns.sort(
            key=lambda p: p.metadata.get("current_match_score", 0), reverse=True
        )
    ...
    def _create_prediction(
        ...
        most_common_type = (
            Counter(pattern_types).most_common(1)[0][0] if pattern_types else "unknown"
        )

        reasoning = (
            f"Predicted based on {len(matching_patterns)} similar {most_common_type} pattern(s) "
        ...
        for pattern in matching_patterns[:3]:  # Top 3 patterns
            if pattern.metadata.get("current_match_score", 0) > 0.7:
                features_matched.append(
                    f"{pattern.pattern_type.value} (score: {pattern.metadata['current_match_score']:.2f})"
                )
        ...
        return PatternPrediction(
            ...
            pattern_evidence={
                ...
                "avg_support": total_support / len(matching_patterns)
                if matching_patterns
                else 0,
