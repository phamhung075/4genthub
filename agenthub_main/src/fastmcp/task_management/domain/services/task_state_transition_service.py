...
class TaskStateTransitionService:
    ...
    def can_transition_to(
        ...
        context: TransitionContext = TransitionContext.USER_INITIATED,
    ) -> tuple[bool, str | None]:
        """
        ...
        try:
            ...
            if target_status_str not in allowed_transitions:
                return (
                    False,
                    f"Cannot transition from '{current_status}' to '{target_status_str}'",
                )

            # Check context-specific prerequisites
            can_transition, reason = self._check_transition_prerequisites(
                task, target_status, context
            )
            ...
            logger.error(
                f"Error checking transition for task {task.id} to {target_status}: {e}"
            )
    ...
    def transition_to(
        ...
        try:
            ...
            can_transition, reason = self.can_transition_to(
                task, target_status, context
            )
            if not can_transition:
                return False, reason
            ...
            self._perform_post_transition_actions(
                task, old_status, target_status, context, metadata
            )

            logger.info(
                f"Task {task.id} successfully transitioned from '{old_status}' to '{target_status}' (context: {context.value})"
            )
        ...
        try:
            ...
            for target_status in allowed_transitions:
                ...
                result[target_status] = {
                    ...
                    "reason": reason,
                    "description": self._get_transition_description(
                        current_status, target_status
                    ),
                    "prerequisites": self._get_transition_prerequisites_description(
                        current_status, target_status
                    ),
                }
    ...
    def handle_dependency_completion(
        ...
        try:
            ...
            logger.error(
                f"Error handling dependency completion for task {completed_task.id}: {e}"
            )
            return []
    ...
    def _check_transition_prerequisites(
        self, task: Task, target_status: TaskStatus, context: TransitionContext
    ) -> tuple[bool, str | None]:
        ...
        if target_status_str == "done":
            ...
            if self._subtask_repository:
                ...
                if incomplete_subtasks:
                    return (
                        False,
                        f"Cannot complete task: {len(incomplete_subtasks)} subtasks are still incomplete",
                    )
    ...
    def _perform_pre_transition_actions(
        self,
        task: Task,
        target_status: TaskStatus,
    ...
    def _perform_post_transition_actions(
        self,
        task: Task,
        ...
        if new_status_str == "blocked":
            ...
            logger.info(
                f"Task {task.id} has been blocked. Reason: {metadata.get('blocking_reason', 'Not specified')}"
            )

    def _find_dependent_tasks(
        self, completed_task: Task, all_tasks: list[Task]
    ) -> list[Task]:
        """Find tasks that depend on the completed task."""
        dependent_tasks = []
        ...
        for dep_id in task.dependencies:
            ...
            for other_task in all_tasks:
                if str(other_task.id) == dep_id_str:
                    if (
                        other_task.status.is_done()
                        if hasattr(other_task.status, "is_done")
                        else str(other_task.status).lower() == "done"
                    ):
    ...
    def _get_transition_description(self, from_status: str, to_status: str) -> str:
        ...
        return descriptions.get(
            (from_status, to_status), f"Change status from {from_status} to {to_status}"
        )

    def _get_transition_prerequisites_description(
        self, from_status: str, to_status: str
    ) -> list[str]:
        """Get description of prerequisites for a transition."""
        prerequisites = {
            ("todo", "in_progress"): [
                "Task assignee available",
    ...
    def _get_alternative_suggestions(self, task: Task) -> list[str]:
        ...
        alternatives = {
            ...
            "in_progress": [
                "blocked",
                "todo",
            ],  # If can't progress, maybe blocked or need to restart
            "review": ["in_progress"],  # If review failed, back to in_progress
            "testing": [
                "review",
                "in_progress",
            ],  # If testing failed, back to review or rework
