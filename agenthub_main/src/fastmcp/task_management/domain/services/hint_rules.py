...
class ProgressBasedHintRule(HintRule):
    ...
    def get_latest_progress(
        self, task: Task, progress_type: ProgressType | None = None
    ) -> dict[str, Any] | None:
        ...
        if progress_type:
            filtered = [
                p
                for p in task.progress_timeline
...
class StalledProgressRule(ProgressBasedHintRule):
    ...
    def _create_blocker_hint(
        self, task: Task, stall_duration: timedelta
    ) -> WorkflowHint:
...
class ImplementationReadyForTestingRule(ProgressBasedHintRule):
    ...
    def evaluate(self, rule_context: RuleContext) -> WorkflowHint | None:
        ...
        if hasattr(task, "progress_breakdown"):
            impl_progress = task.progress_breakdown.get(
                ProgressType.IMPLEMENTATION.value
            )
...
class MissingContextRule(HintRule):
    ...
    def evaluate(self, rule_context: RuleContext) -> WorkflowHint | None:
        ...
        if status_value in ["done", "review"] and not context.data.get(
            "completion_summary"
        ):
    ...
    def _create_incomplete_context_hint(
        self, task: Task, missing_fields: list[str]
    ) -> WorkflowHint:
...
class CollaborationNeededRule(HintRule):
    ...
    def evaluate(self, rule_context: RuleContext) -> WorkflowHint | None:
        ...
        priority_value = (
            task.priority.label
            if hasattr(task.priority, "label")
            else str(task.priority)
        )
        if (
            priority_value in ["urgent", "critical"]
