...
class TaskValidationService:
    ...
    def validate_task_creation(
        self, task: Task, additional_context: dict[str, Any] | None = None
    ) -> list[str]:
        """
    ...
    def validate_task_relationships(self, task: Task) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            if hasattr(task, "git_branch_id") and task.git_branch_id:
                if (
                    self._git_branch_repository
                    and not self._git_branch_repository.exists(task.git_branch_id)
                ):
            ...
            if hasattr(task, "project_id") and task.project_id:
                if self._project_repository and not self._project_repository.exists(
                    task.project_id
                ):
    ...
    def validate_business_constraints(
        self, task: Task, operation_type: str = "general"
    ) -> list[str]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error validating business constraints for task {task.id}: {e}"
            )
        ...
        try:
            ...
            if task.title:
                ...
                if any(placeholder in title_lower for placeholder in placeholders):
                    errors.append(
                        "Task title appears to be placeholder text. Please provide a descriptive title."
                    )
            ...
            if hasattr(task, "description") and task.description:
                ...
                if len(desc_lower) < 10 and task.description.strip() not in [
                    "",
                    "n/a",
                    "na",
                    "none",
            ...
            if hasattr(task, "estimated_effort") and task.estimated_effort:
                ...
                if not any(pattern in effort for pattern in valid_patterns):
                    errors.append(
                        "Estimated effort should include time units (e.g., '2 hours', '1 day', '3 weeks')"
                    )
            ...
            logger.error(
                f"Error validating content appropriateness for task {task.id}: {e}"
            )
    ...
    def _validate_relationships(self, task: Task) -> list[str]:
        ...
        if hasattr(task, "git_branch_id"):
            if not task.git_branch_id:
                ...
                git_branch_result = self._id_validator.detect_id_type(
                    task.git_branch_id, "git_branch_id"
                )
                if not git_branch_result.is_valid:
                    errors.append(
                        f"Invalid git branch ID format: {git_branch_result.error_message}"
                    )
                ...
                if (
                    hasattr(task, "id")
                    and task.id
                    and str(task.id) == str(task.git_branch_id)
                ):
                    errors.append(
        ...
        if hasattr(task, "project_id") and task.project_id:
            project_result = self._id_validator.detect_id_type(
                task.project_id, "project_id"
            )
            if not project_result.is_valid:
                errors.append(
                    f"Invalid project ID format: {project_result.error_message}"
                )
            elif self._project_repository and not self._project_repository.exists(
                task.project_id
            ):
                errors.append(f"Project '{task.project_id}' does not exist")
    ...
    def _validate_creation_context(
        ...
        if similar_tasks:
            for similar_task in similar_tasks:
                if (
                    similar_task.get("title", "").lower().strip()
                    == task.title.lower().strip()
                ):
                    errors.append(
                        f"A task with similar title already exists: '{similar_task.get('title')}'"
                    )
                    break
    ...
    def _validate_update_constraints(
        self, current_task: Task, updated_task: Task
    ) -> list[str]:
    ...
    def validate_task_context_integrity(
        ...
        """
        try:
            result = self._id_validator.validate_task_context(
                task_id, expected_git_branch_id
            )
    ...
    def _validate_priority_status_combination(self, task: Task) -> list[str]:
        ...
        if status_str == "done" and priority_str in ["urgent", "critical"]:
            if not hasattr(task, "_completion_summary") or not task._completion_summary:
                errors.append(
                    "High priority completed tasks must include completion summary"
                )
