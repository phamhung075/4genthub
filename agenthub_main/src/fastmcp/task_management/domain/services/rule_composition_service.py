...
class RuleCompositionService(IRuleCompositionService):
    ...
    def __init__(
        self, conflict_strategy: ConflictResolution = ConflictResolution.MERGE
    ):
    ...
    def compose_rules(
        self,
        rules: list[RuleContent],
        output_format: RuleFormat = RuleFormat.MDC,
        ...
        strategy_func = self.composition_strategies.get(
            composition_strategy, self._intelligent_composition
        )
    ...
    def _intelligent_composition(
        self, rules: list[RuleContent], output_format: RuleFormat
                ...
                if section_name in merged_sections:
                    ...
                    if self.conflict_strategy == ConflictResolution.MERGE:
                        ...
                        conflicts_resolved.append(
                            f"Section '{section_name}' merged from {rule.rule_path}"
                        )
                    elif self.conflict_strategy == ConflictResolution.OVERRIDE:
                        merged_sections[section_name] = content
                        conflicts_resolved.append(
                            f"Section '{section_name}' overridden from {rule.rule_path}"
                        )
                    else:
                        # Default to merge
                        merged_content = self.merge_section_content(
            ...
            for var_name, value in rule.variables.items():
                if var_name in merged_variables:
                    if merged_variables[var_name] != value:
                        warnings.append(
                            f"Variable '{var_name}' conflict in {rule.rule_path}"
                        )
                        # Use latest value
                        merged_variables[var_name] = value
                        conflicts_resolved.append(
                            f"Variable '{var_name}' resolved from {rule.rule_path}"
                        )
                else:
                    merged_variables[var_name] = value
    ...
    def _priority_merge_composition(
        self, rules: list[RuleContent], output_format: RuleFormat
    ) -> tuple[str, list[str], list[str]]:
        ...
        for rule in rules[1:]:
            for section_name, content in rule.sections.items():
                if section_name not in merged_sections:
                    merged_sections[section_name] = content
                    conflicts_resolved.append(
                        f"Added section '{section_name}' from {rule.rule_path}"
                    )
            ...
            for var_name, value in rule.variables.items():
                if var_name not in merged_variables:
                    merged_variables[var_name] = value
                    conflicts_resolved.append(
                        f"Added variable '{var_name}' from {rule.rule_path}"
                    )
    ...
    def _detect_rule_conflicts(
        self, rule1: RuleContent, rule2: RuleContent
    ) -> list[dict[str, Any]]:
        ...
        if self.conflict_strategy == ConflictResolution.MERGE:
            if conflict["type"] == "section":
                resolved = self.merge_section_content(
                    conflict["rule1_content"], conflict["rule2_content"]
                )
    ...
    def _build_inheritance_chain(
        self, rules: list[RuleContent]
    ) -> list[RuleInheritance]:
