...
class TaskPriorityService:
    ...
    def calculate_priority_score(
        self, task: Task, context_factors: dict[str, Any] | None = None
    ) -> float:
        """
        ...
        try:
            ...
            # Blocking factor (20%)
            blocking_score = (
                self._calculate_blocking_score(task, context_factors) * 0.20
            )
            ...
            total_score = (
                base_score + urgency_score + blocking_score + age_score + progress_score
            )
    ...
    def order_tasks_by_priority(
        self, tasks: list[Task], context_factors: dict[str, Any] | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            return [
                {"task": task, "priority_score": 0.0, "error": str(e)} for task in tasks
            ]

    def get_next_task_recommendation(
        self, git_branch_id: str, exclude_statuses: list[str] | None = None
    ) -> dict[str, Any] | None:
        """
        ...
        if not self._task_repository:
            logger.warning(
                "Cannot get next task recommendation without task repository"
            )
            return None
        ...
        try:
            ...
            eligible_tasks = [
                ...
                not in [status.lower() for status in exclude_statuses]
            ]
            ...
            logger.error(
                f"Error getting next task recommendation for branch {git_branch_id}: {e}"
            )
            return None

    def adjust_priority_for_dependencies(
        self, task: Task, all_tasks: list[Task] | None = None
    ) -> float:
        """
        ...
        try:
            ...
            logger.error(
                f"Error adjusting priority for dependencies for task {task.id}: {e}"
            )
            return 1.0
    ...
    def _calculate_blocking_score(
        self, task: Task, context_factors: dict[str, Any]
    ) -> float:
    ...
    def _get_priority_factors(
        self, task: Task, context_factors: dict[str, Any] | None
    ) -> dict[str, Any]:
