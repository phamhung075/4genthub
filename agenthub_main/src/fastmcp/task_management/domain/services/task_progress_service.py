...
class TaskProgressService:
    ...
    def calculate_task_progress(self, task: Task) -> dict[str, Any]:
        ...
        try:
            ...
            subtask_progress = (
                self._calculate_subtask_progress(task)
            ...
            overall_progress = self._calculate_overall_progress(
                base_progress, subtask_progress
            )
            ...
            return {
                ...
                "can_complete": self._can_complete_based_on_progress(
                    overall_progress, subtask_progress
                ),
    ...
    def calculate_subtask_completion_percentage(self, task: Task) -> float:
        ...
        try:
            ...
            logger.error(
                f"Error calculating subtask completion percentage for task {task.id}: {e}"
            )
            return 0.0
    ...
    def get_subtask_summary(self, task: Task) -> dict[str, Any]:
        ...
        try:
                ...
                details.append(
                    {
                        ...
                        "progress_percentage": getattr(
                            subtask,
                            "progress_percentage",
                            0 if not subtask.is_completed else 100,
                        ),
                    }
                )
            ...
            return {
                ...
                "details": details,
                "incomplete_titles": [
                    s.title for s in incomplete_subtasks[:5]
                ],  # First 5 for UI display
                "completed_titles": [
                    s.title for s in completed_subtasks[:5]
                ],  # First 5 for UI display
            }
    ...
    def _calculate_base_task_progress(self, task: Task) -> dict[str, Any]:
        ...
        return {
            ...
            "is_completed": task.status.is_done()
            if hasattr(task.status, "is_done")
            else False,
    ...
    def _calculate_overall_progress(
        self, base_progress: dict[str, Any], subtask_progress: dict[str, Any] | None
    ) -> dict[str, Any]:
    ...
    def _can_complete_based_on_progress(
        self, overall_progress: dict[str, Any], subtask_progress: dict[str, Any] | None
    ) -> bool:
        ...
        )  # Allow completion at any progress level for now

    def _identify_blocking_factors(
        self, task: Task, subtask_progress: dict[str, Any] | None
    ) -> list[str]:
        ...
        if subtask_progress and subtask_progress["incomplete"] > 0:
            ...
            blocking_factors.append(
                f"{incomplete_count} of {total_count} subtasks incomplete"
            )
    ...
    def _create_error_progress_response(
        self, task: Task, error_msg: str
    ) -> dict[str, Any]:
