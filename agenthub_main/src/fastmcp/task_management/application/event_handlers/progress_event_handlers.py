...
class ProgressUpdatedHandler:
    ...
    async def handle(self, event: ProgressUpdated) -> None:
        ...
        """
        logger.info(
            f"Handling ProgressUpdated for task {event.task_id}: "
        ...
        try:
            ...
            if task and task.progress_timeline:
                ...
                for name, target in task.progress_timeline.milestones.items():
                    if (
                        event.old_percentage < target <= event.new_percentage
                        and not task.progress_timeline.is_milestone_reached(name)
                    ):
                        logger.info(
                            f"Auto-milestone '{name}' reached at {event.new_percentage}%"
                        )
...
class ProgressMilestoneReachedHandler:
    ...
    async def handle(self, event: ProgressMilestoneReached) -> None:
        ...
        """
        logger.info(
            f"Milestone '{event.milestone_name}' reached for task {event.task_id} "
...
class ProgressStalledHandler:
    ...
    async def handle(self, event: ProgressStalled) -> None:
        ...
        """
        logger.warning(
            f"Progress stalled for task {event.task_id} - "
...
class SubtaskProgressAggregatedHandler:
    ...
    def __init__(
        self, task_repository: TaskRepository, context_repository: ContextRepository
    ):
        """Initialize the handler."""
        self.task_repository = task_repository
        self.context_repository = context_repository
    ...
    async def handle(self, event: SubtaskProgressAggregated) -> None:
        ...
        """
        logger.info(
            f"Subtask progress aggregated for parent {event.parent_task_id}: "
...
class ProgressTypeCompletedHandler:
    ...
    async def handle(self, event: ProgressTypeCompleted) -> None:
        ...
        """
        logger.info(
            f"Progress type {event.progress_type.value} completed for task {event.task_id}"
        )
