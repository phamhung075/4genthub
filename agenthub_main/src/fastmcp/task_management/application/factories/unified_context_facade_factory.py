...
class UnifiedContextFacadeFactory:
    ...
    def __init__(self, session_factory=None):
        ...
        if session_factory is None:
            try:
                ...
                logger.warning(
                    f"Database not available, using mock context service: {e}"
                )
        ...
        try:
            ...
            logger.warning(
                f"Failed to initialize with database, using mock service: {e}"
            )
            self._create_mock_service()
            # Mark as initialized even with mock service
            UnifiedContextFacadeFactory._initialized = True
    ...
    def _create_mock_service(self):
        ...
        logger.warning(
            "Using MockUnifiedContextService - context operations will have limited functionality"
        )
    ...
    def auto_create_global_context(self, user_id: str | None = None) -> bool:
        ...
        try:
            ...
            if not user_id:
                logger.warning(
                    "Cannot auto-create global context: no user_id provided or available"
                )
                return False
            ...
            try:
                existing_result = facade.get_context(
                    level="global", context_id=global_context_id
                )
            ...
            result = facade.create_context(
                level="global", context_id=global_context_id, data=default_global_data
            )

            if result.get("success", False):
                logger.info(
                    f"Global context auto-created successfully for user {user_id}"
                )
                return True
            else:
                logger.warning(
                    f"Failed to auto-create global context for user {user_id}: {result.get('error', 'Unknown error')}"
                )
                return False
