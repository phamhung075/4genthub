...
class GitBranchFacadeFactory:
    ...
    def __init__(
        self, git_branch_repository_factory: GitBranchRepositoryFactory | None = None
    ):
        """
    ...
    def create_facade(
        ...
        if project_id is None:
            raise ValueError(
                "project_id is required for git branch facade creation (no fallback allowed for DDD compliance)"
            )

        return self.create_git_branch_facade(project_id=project_id, user_id=user_id)

    def create_git_branch_facade(
        self, project_id: str = None, user_id: str | None = None
    ) -> GitBranchApplicationFacade:
        """
        Create a git branch application facade with proper dependency injection.
        ...
        git_branch_service = GitBranchService(
            project_repo=project_repo, git_branch_repo=git_branch_repo, user_id=user_id
        )
    ...
    def get_cached_facade(
        self, project_id: str, user_id: str | None = None
    ) -> GitBranchApplicationFacade | None:
        """
