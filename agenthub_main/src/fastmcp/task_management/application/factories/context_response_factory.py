...
class ContextResponseFactory:
    ...
    def create_unified_context(
        ...
        try:
            ...
            elif "context" in context_data and isinstance(
            ...
            elif "metadata" in context_data and "task_id" in context_data.get(
                "metadata", {}
            ):
            ...
            # Case 4: Handle task_data structure from unified context system
                ...
                unified_context = ContextResponseFactory._convert_task_data_to_template(
                    context_data
                )
            ...
            if unified_context:
                ...
                return ContextResponseFactory._ensure_complete_structure(
                    unified_context
                )
    ...
    def _convert_task_data_to_template(context_data: dict[str, Any]) -> dict[str, Any]:
        ...
        template_context = {
            ...
            if isinstance(context_data.get("progress"), dict)
            else {
                "completion_percentage": context_data.get("progress", 0)
                if isinstance(context_data.get("progress"), (int, float))
                else 0
            },
        }
    ...
    def apply_to_task_response(task_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not task_data.get("context_data") and task_id:
            try:
                # Use FacadeService to maintain DDD boundaries
                from ..services.facade_service import FacadeService

                unified_facade = FacadeService.get_unified_context_facade()
                ...
                if context_response.get("success") and context_response.get("context"):
                    ...
                    logger.debug(
                        f"No context found for task {task_id} via manual fetch"
                    )
            except Exception as e:
                logger.warning(
                    f"Failed to manually fetch context for task {task_id}: {e}"
                )

        if "context_data" in task_data and task_data["context_data"]:
            unified_context = ContextResponseFactory.create_unified_context(
    ...
    def apply_to_next_response(next_response: dict[str, Any]) -> dict[str, Any]:
        ...
        if "task" in next_response:
            ...
            if "context_info" in task_data:
                ...
                if (
                    unified_context
                    and "metadata" in unified_context
                    and "task_id" in unified_context["metadata"]
                ):
                    task_data["context"] = unified_context["metadata"]["task_id"]
                elif (
                    "next_item" in task_data
                    and "task" in task_data["next_item"]
                    and "context_id" in task_data["next_item"]["task"]
                ):
