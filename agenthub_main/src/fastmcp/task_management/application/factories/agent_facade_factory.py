...
class AgentFacadeFactory:
    ...
    def create_agent_facade(
        ...
        if not user_id:
            raise ValueError(
                "user_id is required for agent facade creation (no fallback allowed for DDD compliance)"
            )
        ...
        try:
            ...
            if self._agent_repository_factory:
                ...
                agent_repository = AgentRepositoryFactory.create(user_id=user_id)
            else:
            ...
            logger.warning(
                f"Failed to create proper AgentApplicationFacade: {e}, using mock"
            )
    ...
    def create_facade(self, project_id: str) -> AgentApplicationFacade:
        ...
        if not project_id:
            raise ValueError(
                "Project ID is required. No fallback to default project allowed per DDD principles."
            )
    ...
    def create() -> AgentApplicationFacade:
        ...
        raise ValueError(
            "Cannot create agent facade: Project ID is required. No fallback to default project allowed per DDD principles."
        )
...
class MockAgentApplicationFacade:
    ...
    def update_agent(
        self, project_id: str, agent_id: str, name: str = None, call_agent: str = None
    ):
        return {
            "success": True,
            "agent": {
