...
class UpdateTaskUseCase:
    ...
    def execute(self, request: UpdateTaskRequest) -> UpdateTaskResponse:
        ...
        try:
            ...
            logger.warning(
                f"Context sync failed for task {task.id} but task update succeeded: {e}"
            )
        ...
        try:
            ...
            logger.info(
                f"Task update WebSocket payload size: {payload_size} bytes for task {task.id.value}"
            )
            ...
            logger.info(
                f"Sent WebSocket notification for task {task.id.value} update with complete payload ({len(task_payload)} fields)"
            )
    ...
    def _sync_task_context_after_update(self, task) -> None:
        ...
        try:
            ...
            logger.debug(
                f"[UpdateTaskUseCase] Task details - status: {task.status}, git_branch_id: {git_branch_id}"
            )
            ...
            try:
                ...
                asyncio.create_task(
                    self._context_sync_service.sync_task_metadata(task_id_str, task)
                )
                logger.info(
                    f"⏭️ Scheduled metadata sync for updated task {task_id_str} (in async context)"
                )
            except RuntimeError:
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️ Failed to sync metadata for updated task {task_id_str}: {sync_error}"
                    )
            except Exception as sync_error:
                logger.warning(
            ...
            logger.warning(
                f"[UpdateTaskUseCase] Failed to sync context for task {task.id} after update: {e}"
            )
            logger.debug(
                "[UpdateTaskUseCase] Context sync error details", exc_info=True
            )
