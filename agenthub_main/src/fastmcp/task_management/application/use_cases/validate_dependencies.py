...
class ValidateDependenciesUseCase:
    ...
    def validate_task_dependencies(self, task_id: str | int) -> dict[str, Any]:
        ...
        try:
            ...
            validation_result = self._validation_service.validate_dependency_chain(
                domain_task_id
            )

            # Add use case specific information
            validation_result.update(
                {"use_case": "validate_dependencies", "requested_task_id": str(task_id)}
            )
            ...
            return {"valid": False, "error": str(e), "task_id": str(task_id)}
    ...
    def get_dependency_chain_analysis(self, task_id: str | int) -> dict[str, Any]:
        ...
        try:
            ...
            chain_status = self._validation_service.get_dependency_chain_status(
                domain_task_id
            )
            ...
            return {"error": f"Analysis failed: {str(e)}", "task_id": str(task_id)}
    ...
    def _suggest_next_actions(self, chain_status: dict[str, Any]) -> list:
        ...
        if chain_status.get("can_proceed", False):
            ...
            for dep in dependency_chain:
                if (
                    not dep.get("is_completed", False)
                    and dep.get("status") != "blocked"
                ):
                    actions.append(
                        {
