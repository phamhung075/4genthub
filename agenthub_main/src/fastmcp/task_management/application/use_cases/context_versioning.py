...
class ContextVersioningService:
    ...
    async def create_version(
        ...
        logger.info(
            f"Created version {version_id} for {context_level.value}:{context_id}"
        )
    ...
    async def get_version(self, version_id: str) -> ContextVersion | None:
    ...
    async def get_diff(
        ...
        for key, value in to_version.data.items():
            if key not in from_version.data:
                ...
                modified[key] = {"from": from_version.data[key], "to": value}
    ...
    async def rollback(
        ...
        if (
            target_version.context_level != context_level
            or target_version.context_id != context_id
        ):
        ...
        logger.info(
            f"Rolled back {context_level.value}:{context_id} to version {target_version_id}"
        )
    ...
    async def tag_version(self, version_id: str, tags: list[str]):
    ...
    async def get_milestone_versions(
        self, context_level: ContextLevel, context_id: str
    ) -> list[ContextVersion]:
    ...
    async def export_version_history(
        ...
        versions = await self.get_version_history(
            context_level=context_level, context_id=context_id, limit=1000
        )
    ...
    def get_storage_stats(self) -> dict[str, Any]:
        ...
        total_size = sum(len(json.dumps(v.data)) for v in self.versions.values())
