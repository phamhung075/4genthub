    ...
    def _check_project_health(self, project) -> dict[str, Any]:
        ...
        if project.cross_tree_dependencies:
            if self._has_circular_dependencies(project.cross_tree_dependencies):
                issues.append(
                    "Circular dependencies detected in cross-tree dependencies"
                )
        ...
        for git_branch_name, agent_id in project.agent_assignments.items():
            if agent_id not in project.registered_agents:
                issues.append(
                    f"Agent '{agent_id}' assigned to tree '{git_branch_name}' but not registered"
                )

            if git_branch_name not in project.git_branchs:
                issues.append(
                    f"Agent '{agent_id}' assigned to non-existent tree '{git_branch_name}'"
                )
        ...
        for session_id, session in project.active_work_sessions.items():
            if session.agent_id not in project.registered_agents:
                issues.append(
                    f"Active work session '{session_id}' references unregistered agent '{session.agent_id}'"
                )
        ...
        for resource, agent_id in project.resource_locks.items():
            if agent_id not in project.registered_agents:
                issues.append(
                    f"Resource '{resource}' locked by unregistered agent '{agent_id}'"
                )
        ...
        return {
            ...
            "cross_tree_dependencies": sum(
                len(deps) for deps in project.cross_tree_dependencies.values()
            ),
        }
