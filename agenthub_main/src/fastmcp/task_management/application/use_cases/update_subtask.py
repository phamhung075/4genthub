...
class UpdateSubtaskUseCase:
    ...
    def execute(self, request: UpdateSubtaskRequest) -> SubtaskResponse:
        ...
        if self._subtask_repository:
            ...
            if not subtask:
                raise ValueError(
                    f"Subtask {request.id} not found in task {request.task_id}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"Parent task context sync failed for task {task.id} but subtask update succeeded: {e}"
                )
        else:
    ...
    def _sync_parent_task_context_after_subtask_update(self, parent_task) -> None:
        ...
        try:
            ...
            task_id_str = str(
                parent_task.id.value
                if hasattr(parent_task.id, "value")
                else parent_task.id
            )
            ...
            logger.info(
                f"[UpdateSubtaskUseCase] Auto-syncing parent task context for task {task_id_str} after subtask update"
            )
            logger.debug(
                f"[UpdateSubtaskUseCase] Parent task details - status: {parent_task.status}, git_branch_id: {git_branch_id}"
            )
            ...
            if self._context_sync_service is None:
                ...
                self._context_sync_service = TaskContextSyncService(
            ...
            try:
                ...
                logger.debug(
                    f"[UpdateSubtaskUseCase] Running in async context, context sync triggered for parent task {task_id_str}"
                )
                logger.info(
                    f"[UpdateSubtaskUseCase] Parent task context sync triggered for task {task_id_str}"
                )

            except RuntimeError:
                # No event loop, we can create one
                logger.debug(
                    "[UpdateSubtaskUseCase] No async context, creating new event loop for parent task context sync"
                )

                async def sync_context():
                    if not project_id:
                        raise ValueError("project_id is required for subtask update")
                ...
                if result:
                    logger.info(
                        f"[UpdateSubtaskUseCase] Successfully synced parent task context for task {task_id_str}"
                    )
                else:
                    logger.warning(
                        f"[UpdateSubtaskUseCase] Parent task context sync returned None for task {task_id_str}"
                    )

        except Exception as e:
            # Don't fail the subtask update operation if context sync fails
            logger.warning(
                f"[UpdateSubtaskUseCase] Failed to sync parent task context for task {parent_task.id} after subtask update: {e}"
            )
            logger.debug(
                "[UpdateSubtaskUseCase] Parent task context sync error details",
                exc_info=True,
            )
