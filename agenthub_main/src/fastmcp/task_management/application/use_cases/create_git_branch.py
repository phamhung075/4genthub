...
class CreateGitBranchUseCase:
    ...
    async def execute(
        ...
        try:
            ...
            # Save updated project (using save to persist git branches properly)
            ...
            try:
                ...
                context_facade = factory.create_facade(
                    user_id=user_id, project_id=project.id, git_branch_id=git_branch.id
                )
                ...
                context_response = context_facade.create_context(
                    level="branch", context_id=git_branch.id, data=context_data
                )
                ...
                if not context_response["success"]:
                    ...
                    logger.warning(
                        f"Failed to create branch context for {git_branch.id}: {context_response.get('error')}"
                    )
                ...
                logger = logging.getLogger(__name__)
                logger.warning(
                    f"Error creating branch context for {git_branch.id}: {context_error}"
                )
