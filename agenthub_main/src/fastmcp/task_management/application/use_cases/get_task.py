...
class GetTaskUseCase:
    ...
    async def execute(
        ...
        try:
            ...
            if include_context:
                try:
                    ...
                    if self._context_service:
                        logger.debug(
                            f"Attempting to fetch context for task {task_id} using context service: {type(self._context_service)}"
                        )
                        # Check if it's a UnifiedContextFacade (sync) or old-style async service
                        if hasattr(
                            self._context_service, "get_context"
                        ) and not inspect.iscoroutinefunction(
                            self._context_service.get_context
                        ):
                            # UnifiedContextFacade - sync method
                            logger.debug(
                                f"Using UnifiedContextFacade sync method for task {task_id}"
                            )
                            context_response = self._context_service.get_context(
                                level="task", context_id=task_id, include_inherited=True
                            )
                            logger.debug(
                                f"Context response received: {context_response}"
                            )
                        else:
                            # Old-style async service
                            logger.debug(
                                f"Using old-style async service for task {task_id}"
                            )
                            ...
                            logger.debug(
                                f"Async context response received: {context_response}"
                            )

                        if isinstance(context_response, dict):
                            # UnifiedContextFacade returns dict
                            logger.debug(
                                f"Processing dict response for task {task_id}: keys={list(context_response.keys())}"
                            )
                            if context_response.get("success") and context_response.get(
                                "context"
                            ):
                                # Context data is in 'context' field for UnifiedContextService
                                context_data = context_response["context"]
                                logger.debug(
                                    f"Context data fetched from context field for task {task_id}"
                                )
                            elif context_response.get(
                                "success"
                            ) and context_response.get("data"):
                                # Check if context data is in 'data' field (legacy)
                                data_field = context_response["data"]
                                if (
                                    isinstance(data_field, dict)
                                    and "context_data" in data_field
                                ):
                                    context_data = data_field["context_data"]
                                    logger.debug(
                                        f"Context data fetched from data.context_data for task {task_id}"
                                    )
                                else:
                                    context_data = data_field
                                    logger.debug(
                                        f"Context data fetched from data field for task {task_id}"
                                    )
                            else:
                                logger.warning(
                                    f"No context data found for task {task_id}. Response: {context_response}"
                                )
                        else:
                            ...
                            if context_response.success and context_response.context:
                                ...
                                logger.debug(
                                    f"Context data fetched for task {task_id} from data field"
                                )
                            else:
                                logger.debug(
                                    f"No context data found for task {task_id}"
                                )
                    else:
                        logger.warning(
                            f"No context service available for task {task_id}"
                        )

                except Exception as e:
                    logger.warning(
                        f"Failed to fetch context data for task {task_id}: {e}"
                    )
                    import traceback
            ...
            _event = TaskRetrieved(task_id=str(task.id))
