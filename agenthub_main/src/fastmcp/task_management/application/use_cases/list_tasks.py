...
class ListTasksUseCase:
    ...
    def execute(self, request: ListTasksRequest) -> TaskListResponse:
        ...
        if request.git_branch_id:
            ...
            logger.debug(
                f"[USE_CASE] Added git_branch_id to filters: {request.git_branch_id}"
            )
        else:
            logger.debug(
                "[USE_CASE] No git_branch_id in request, not filtering by branch"
            )
        ...
        logger.debug(
            f"[USE_CASE] Repository returned {len(tasks) if tasks else 0} tasks"
        )
