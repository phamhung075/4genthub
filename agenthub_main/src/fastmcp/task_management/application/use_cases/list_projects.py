    ...
    async def execute(self, include_branches: bool = True) -> dict[str, Any]:
        ...
        for project in projects:
            try:
                ...
                logger.error(
                    f"Error calculating project aggregations for {project.name}: {e}"
                )
            ...
            if include_branches and project.git_branchs:
                ...
                for branch_id, branch in project.git_branchs.items():
                    ...
                    # Count by status
                    branch_completed = sum(
                        1
                        for t in branch.all_tasks.values()
                        if isinstance(t, dict) and t.get("status") == "done"
                    )
                    branch_in_progress = sum(
                        1
                        for t in branch.all_tasks.values()
                    ...
                    branch_progress = int(
                        (branch_completed / branch_task_count * 100)
                    ...
                    git_branchs_dict[branch_id] = {
                        ...
                        if hasattr(branch, "git_branch_name") and branch.git_branch_name
                        else branch.name,  # Required by BranchDTO
                        "description": branch.description
                        if hasattr(branch, "description")
                        else "",
                        "created_at": branch.created_at.isoformat()
                        if hasattr(branch, "created_at")
                        else "",
                        "updated_at": branch.updated_at.isoformat()
                        if hasattr(branch, "updated_at")
                        else "",
                        ...
                        "agent_assignments": len(branch.agent_assignments)
                        if hasattr(branch, "agent_assignments")
                        else 0,
                    }
