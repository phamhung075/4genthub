...
class BatchContextOperations:
    ...
    async def _execute_transactional(
        self, operations: list[BatchOperation], stop_on_error: bool
    ) -> list[BatchOperationResult]:
        ...
        if stop_on_error:
            for i, result in enumerate(results):
                if not result.success:
                    ...
                    logger.warning(
                        f"Operation {i} failed, but parallel execution continued"
                    )
                    break
    ...
    async def _execute_single_operation(self, op: BatchOperation) -> dict[str, Any]:
        ...
        if op.operation == BatchOperationType.CREATE:
            ...
            return await self.context_service.delete_context(
                context_level=op.level, context_id=op.context_id, user_id=op.user_id
            )
    ...
    async def bulk_create(
        ...
        return await self.execute_batch(
            operations=operations, transaction=transaction, user_id=user_id
        )
    ...
    async def copy_contexts(
        ...
        return await self.execute_batch(
            operations=operations, transaction=True, user_id=user_id
        )
