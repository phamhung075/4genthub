...
class NextTaskUseCase:
    ...
    async def execute(
        ...
        Args:
            ...
            user_id: User identifier for context creation (required - no default)
        ...
        if user_id is None:
            user_id = getattr(self._task_repository, "user_id", None)
            if not user_id:
                raise ValueError(
        ...
        if not filtered_tasks:
            return NextTaskResponse(
                has_next=False, message="No tasks match the specified filters."
            )
        ...
        active_tasks = [
            ...
            if task.status
            and hasattr(task.status, "value")
            and task.status.value in actionable_statuses
        ]
        ...
        if not active_tasks:
            ...
            completed_tasks = [
                ...
                for task in filtered_tasks
                if task.status
                and hasattr(task.status, "is_done")
                and task.status.is_done()
            ]
        ...
        for task in sorted_tasks:
            ...
            if next_subtask:
                    ...
                    try:
                        ...
                        try:
                            ...
                            context_result = context_service.resolve_context(
                                level="task", context_id=task.id.value
                            )
                        except PermissionError as e:
                            logger.warning(
                                f"Permission denied during context resolution for task {task.id.value}: {e}"
                            )
                        except FileNotFoundError:
                            logger.debug(
                                f"No existing context file for task {task.id.value}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"Context resolution failed for task {task.id.value}: {e}"
                            )
                        ...
                        if not context_result or not context_result.get("success"):
                            try:
                                ...
                                context_service.create_context(
                                    ...
                                    data={
                                        ...
                                        "task_data": {
                                            ...
                                            if task.priority
                                            and hasattr(task.priority, "value")
                                ...
                                logger.warning(
                                    f"Could not create context for task {task.id.value}: {e}"
                                )
                        ...
                        logger.warning(
                            f"Context management failed for task {task.id}: {e}"
                        )
                ...
                return NextTaskResponse(
                    ...
                    next_item={
                        "type": "task",
                        "task": self._task_to_dict(
                            task, include_context, user_id, project_id, git_branch_id
                        ),
                        "context": self._get_task_context(task, all_tasks),
                    },
                    context=task.context_id
                    or str(task.id.value),  # Use task's context_id instead of null
        ...
        return NextTaskResponse(has_next=False, message="No actionable tasks found.")
    ...
    def _sort_tasks_by_priority(self, tasks: list) -> list:
        ...
        def safe_sort_key(task):
            ...
            try:
                ...
                status_val = (
                    task.status.value
                    if task.status and hasattr(task.status, "value")
                    else str(task.status)
                    if task.status
                    else "todo"
                )
                ...
                logger.warning(
                    f"Error sorting task {getattr(task, 'id', 'unknown')}: {e}"
                )
        ...
        if include_context:
            try:
                ...
                try:
                    # context_service methods are synchronous, no need for await or timeout
                    context_result = context_service.resolve_context(
                        level="task", context_id=str(task.id.value)
                    )
                except PermissionError as e:
                    logger.warning(
                        f"Permission denied during context resolution for task {task.id.value}: {e}"
                ...
                logger.warning(
                    f"Failed to fetch context data for task {task.id.value}: {e}"
                )
                task_dict["context_data"] = None
                task_dict["context_available"] = False
        else:
    ...
    def _validate_task_context_alignment(self, tasks: list) -> list[dict[str, Any]]:
        ...
        try:
                ...
                try:
                    ...
                    if context:
                        ...
                        if not isinstance(context_data, dict):
                            ...
                            logger.debug(
                                f"Context data for task {task.id.value} is not a dictionary (type: {type(context_data)}), skipping validation"
                            )
                            continue
                        ...
                        if task_status == "done" and context_data.get(
                            "subtasks", {}
                        ).get("items"):
                            incomplete_subtasks = [
                                subtask
                                for subtask in context_data["subtasks"]["items"]
                                if not subtask.get("completed", False)
                            ]
                    ...
                    logger.warning(
                        f"Context validation failed for task {task.id.value}: {e}"
                    )
                    continue
            ...
            logger.warning(
                f"Context manager initialization failed during validation: {e}"
            )
