...
class AITaskCreationUseCase:
    ...
    def __init__(
        self, task_repository: TaskRepository, task_facade: TaskApplicationFacade
    ):
    ...
    async def execute(self, request: AITaskCreationRequest) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"AI-enhanced task creation completed: {result.get('success', False)}"
            )
            return result
    ...
    async def create_full_ai_plan(
        ...
        try:
            ...
            return {"success": False, "error": f"AI plan creation failed: {str(e)}"}

    async def enhance_existing_task(
        self, task_id: str, enhancement_options: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            enhanced_result = (
                await self.ai_integration_service.add_ai_insights_to_task_response(
                    task_result, action="enhance"
                )
            )
            ...
            return {"success": False, "error": f"Task enhancement failed: {str(e)}"}
