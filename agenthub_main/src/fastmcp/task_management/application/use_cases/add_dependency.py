...
class AddDependencyUseCase:
    ...
    def _find_dependency_task(self, dependency_id: TaskId) -> Task | None:
        ...
        if hasattr(self._task_repository, "find_by_id_all_states"):
            ...
            if dependency_task:
                logger.debug(
                    f"Found dependency task {dependency_id} with status {dependency_task.status}"
                )
        ...
        logger.warning(
            f"Dependency task {dependency_id} not found in any context or state"
        )
        return None
    ...
    def execute(self, request: AddDependencyRequest) -> DependencyResponse:
        ...
        if not dependency_task:
            raise TaskNotFoundError(
                f"Dependency task {request.depends_on_task_id} not found in active, completed, or archived tasks"
            )
        ...
        dependency_status = (
            ...
            if hasattr(dependency_task.status, "value")
            else str(dependency_task.status)
        )
        status_info = ""
        ...
        if dependency_task.status.is_done():
            ...
            status_info = (
                f" (dependency is {dependency_status} - task will wait for completion)"
            )
