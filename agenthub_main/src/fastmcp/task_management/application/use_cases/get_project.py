...
class GetProjectUseCase:
        ...
        for tree_id, tree in git_branchs.items():
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Branch {tree_id} tree_status type: {type(tree_status)}"
                    )
                except Exception as status_err:
                    logger.error(
                        f"Error getting tree_status for branch {tree_id}: {status_err}",
                        exc_info=True,
                    )
                ...
                result[tree_id] = {
                    ...
                    "name": tree.name,
                    "git_branch_name": tree.git_branch_name
                    if hasattr(tree, "git_branch_name") and tree.git_branch_name
                    else tree.name,  # Required by BranchDTO
                    "description": tree.description,
                    "created_at": tree.created_at.isoformat(),
                    "updated_at": tree.updated_at.isoformat()
                    if hasattr(tree, "updated_at")
                    else tree.created_at.isoformat(),
                    "status": str(tree.status)
                    if hasattr(tree, "status")
                    else "active",  # Required by BranchDTO
    ...
    async def execute(self, project_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            task_count = sum(
                tree.get_task_count() for tree in project.git_branchs.values()
            )
            ...
            logger.info(
                f"🔍 project.git_branchs keys: {list(project.git_branchs.keys())}"
            )
            for tree_id, tree in project.git_branchs.items():
                logger.info(
                    f"🔍 Branch {tree_id}: type={type(tree)}, all_tasks count={len(tree.all_tasks)}, get_task_count()={tree.get_task_count()}"
                )
        except Exception as e:
        ...
        return {
            ...
            "project": {
                ...
                "git_branchs": self._build_git_branchs_dict(
                    project.git_branchs, logger
                ),
                "registered_agents": {
                    agent_id: {
                        "id": agent.id,
