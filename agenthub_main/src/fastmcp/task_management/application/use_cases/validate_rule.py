...
class ValidateRuleUseCase:
    ...
    async def execute(self, rule_path: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to validate rules: {str(e)}"}
    ...
    async def _validate_single_rule(self, rule_path: str) -> dict[str, Any]:
        ...
        if not await self._rule_repository.rule_exists(rule_path):
            return {"success": False, "error": f"Rule not found at path: {rule_path}"}
    ...
    async def _has_circular_dependency(
        self, rule_path: str, visited: set = None
    ) -> bool:
