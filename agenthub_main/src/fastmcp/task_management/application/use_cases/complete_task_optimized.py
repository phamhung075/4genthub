    ...
    async def _execute_async(self, request: CompleteTaskRequest) -> dict[str, Any]:
        ...
        try:
            ...
            if self._context_manager:
                context_result = await self._context_manager.get_context(
                    str(request.task_id)
                )
                if context_result.get("success"):
                    ...
                    if "template_context" in context_result:
                        ...
                        if "full_context" in template_context:
                            ...
                            if (
                                "metadata" in full_context
                                and "updated_at" in full_context["metadata"]
                            ):
                                context_updated_at = datetime.fromisoformat(
                                    full_context["metadata"]["updated_at"]
                                )
                        elif (
                            "task_metadata" in template_context
                            and "updated_at" in template_context["task_metadata"]
                        ):
                            context_updated_at = datetime.fromisoformat(
                                template_context["task_metadata"]["updated_at"]
                            )
            ...
            if self._context_manager and request.completion_summary and context_data:
                try:
                    ...
                    await self._context_manager.update_context(
                        str(request.task_id), context_update, merge_mode=True
                    )
            ...
            if (
                hasattr(task, "parent_task_id")
                and task.parent_task_id
                and self._progress_service
            ):
                try:
