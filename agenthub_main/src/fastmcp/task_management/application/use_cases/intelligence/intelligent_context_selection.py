...
class IntelligentContextSelectionUseCase:
    ...
    async def execute(
        self, request: IntelligentSelectionRequest
        ...
        try:
            ...
            if request.session_id:
                self.intelligent_selector.start_session(
                    request.session_id, request.user_id
                )

            # Load available contexts
            await self._ensure_contexts_loaded(
                project_id=request.project_id, git_branch_id=request.git_branch_id
            )
            ...
            if request.user_preferences:
                user_preferences = UserPreferences(
                    preferred_context_types=request.user_preferences.get(
                        "preferred_context_types", []
                    ),
                    max_context_size=request.user_preferences.get(
                        "max_context_size", 2000
                    ),
                    priority_boost_keywords=request.user_preferences.get(
                        "priority_boost_keywords", []
                    ),
                    penalty_keywords=request.user_preferences.get(
                        "penalty_keywords", []
                    ),
                    agent_preferences=request.user_preferences.get(
                        "agent_preferences", {}
                    ),
                )
            ...
            return IntelligentSelectionResponse(
                ...
                performance_metrics={
                    ...
                    "contexts_considered": selection_result.metadata.get(
                        "contexts_considered", 0
                    ),
                    "semantic_matches": selection_result.metadata.get(
                        "semantic_matches", 0
                    ),
                    "expansion_path": selection_result.metadata.get(
                        "expansion_path", []
                    ),
                },
    ...
    async def _ensure_contexts_loaded(
        self, project_id: str | None = None, git_branch_id: str | None = None
    ) -> None:
        """Ensure contexts are loaded into the intelligent selector."""
        # Check if we need to refresh contexts
        should_refresh = (
            not self._contexts_loaded
            or (
                self._last_context_refresh
                and (datetime.now(UTC) - self._last_context_refresh).total_seconds()
                > 300
            )  # 5 minutes
        )
        ...
        logger.info(
            f"Loaded {len(available_contexts)} contexts for intelligent selection"
        )
    ...
    def _generate_recommendations(
        self, selection_result: SelectionResult, request: IntelligentSelectionRequest
    ) -> list[str]:
    ...
    async def _record_selection_for_learning(
        self, request: IntelligentSelectionRequest, result: SelectionResult
    ) -> None:
        ...
        try:
            ...
            if request.session_id:
                ...
                for context in result.selected_contexts:
                    ...
                    self.intelligent_selector.record_tool_usage(
                        "context_selection", context_id
                    )
