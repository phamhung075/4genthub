...
class ListAgentsUseCase:
    ...
    def execute(self, request: ListAgentsRequest) -> ListAgentsResponse:
        ...
        try:
            ...
            agents = [
                AgentResponse.from_dict(agent_data)
                for agent_data in result.get("agents", [])
            ]

            return ListAgentsResponse(
                success=True, agents=agents, total_agents=result.get("total_agents", 0)
            )

        except ProjectNotFoundError as e:
            logger.warning(f"Project not found: {e}")
            return ListAgentsResponse(success=False, error=str(e))
