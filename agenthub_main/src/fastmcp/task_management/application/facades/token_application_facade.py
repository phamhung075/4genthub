...
class TokenApplicationFacade:
    ...
    async def generate_mcp_token_from_user(
        ...
        try:
            ...
            mcp_token_obj = (
                await self.mcp_token_service.generate_mcp_token_from_user_id(
                    user_id=user_id,
                    email=email,
            ...
            return {
                ...
                "expires_at": mcp_token_obj.expires_at.isoformat()
                if mcp_token_obj.expires_at
                else None,
            ...
            if created_token:
                return {
                    ...
                    "token": {
                        ...
                        if created_token.created_at
                        else None,
                        "expires_at": created_token.expires_at.isoformat()
    ...
    async def list_user_tokens(
        self, user_id: str, session: Session, skip: int = 0, limit: int = 100
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": True, "tokens": token_list, "total": total}

        except Exception as e:
            logger.error(f"Error listing user tokens: {e}")
            return {"success": False, "error": str(e), "tokens": [], "total": 0}

    async def get_token_details(
        self, token_id: str, user_id: str, session: Session
    ) -> dict[str, Any]:
        """
    ...
    async def reactivate_token(
        ...
        try:
            ...
            if success:
                return {"success": True, "message": "Token reactivated successfully"}
    ...
    async def rotate_token(
        ...
        try:
            ...
            if new_result["success"]:
                return {"success": True, "token_data": new_result["token"]}
    ...
    async def validate_token(self, token: str, session: Session) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if not db_token or not db_token.is_active:
                return {"success": False, "error": "Token is invalid or revoked"}
