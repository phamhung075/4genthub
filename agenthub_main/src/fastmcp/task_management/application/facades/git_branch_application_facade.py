...
class GitBranchApplicationFacade:
    def __init__(
        self,
        git_branch_service: GitBranchService | None = None,
    ...
    async def create_tree(
        self, project_id: str, tree_name: str, description: str = ""
    ) -> dict[str, Any]:
        ...
        try:
            ...
            git_branch_repo = repo_provider.get_git_branch_repository(
        ...
        return await self._git_branch_service.create_git_branch(
            project_id, tree_name, description
        )
    ...
    def create_git_branch(
        ...
        try:
            ...
            logger.info(
                f"Creating git branch: project_id={project_id}, name={git_branch_name}"
            )
                    ...
                    try:
                        WebSocketNotificationService.sync_broadcast_branch_event(
                            ...
                            branch_data={
                                ...
                                "git_branch_name": branch_data.get("git_branch_name")
                                or branch_data.get("name", git_branch_name),
                                "name": branch_data.get("name"),
                                "description": branch_data.get(
                        ...
                        logger.info(
                            f"✅ WebSocket notification sent for branch creation: {branch_data.get('id')}"
                        )
                    except Exception as ws_error:
                        logger.warning(
                            f"Failed to send WebSocket notification for branch creation: {ws_error}"
                        )
                ...
                if branch_result.get("success"):
                    ...
                    WebSocketNotificationService.sync_broadcast_branch_event(
                        ...
                        branch_data={
                            ...
                            or branch_data.get("git_branch_name")
                            or branch_data.get("name", ""),
                            "name": branch_data.get("name"),
                            "description": git_branch_description
                            or branch_data.get("description", ""),
                            "project_id": str(actual_project_id),
                        },
                    )
                    logger.info(
                        f"✅ WebSocket notification sent for branch update: {git_branch_id}"
                    )
                else:
                    logger.warning(
                        f"Could not retrieve branch data for WebSocket notification: {git_branch_id}"
                    )
            except Exception as ws_error:
                logger.warning(
                    f"Failed to send WebSocket notification for branch update: {ws_error}"
                )
    ...
    def get_git_branch(self, project_id: str, git_branch_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.info(
                f"Getting git branch: project_id={project_id}, git_branch_id={git_branch_id}"
            )
            ...
            if (
                result.get("success")
                and result.get("git_branch", {}).get("project_id") != project_id
            ):
                logger.warning(
                    f"Project ID mismatch: expected {project_id}, found {result.get('git_branch', {}).get('project_id')}"
                )
    ...
    def delete_git_branch(
        self, git_branch_id: str, project_id: str | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                def run_in_thread():
                    ...
                    try:
                        result = asyncio.run(
                            self._git_branch_service.delete_git_branch(
                                target_project_id, git_branch_id
                            )
                        )
                    except Exception as e:
                        exception = e
    ...
    def assign_agent(
        self, git_branch_id: str, agent_id: str, project_id: str | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            agent_facade = facade_service.get_agent_facade(
                project_id=project_id or self._project_id, user_id=self._user_id
            )
            ...
            logger.info(
                f"Agent {agent_id} successfully assigned to git branch {git_branch_id}"
            )
            return result
    ...
    def unassign_agent(
        ...
        try:
            ...
            logger.info(
                f"Agent {agent_id} successfully unassigned from git branch {git_branch_id}"
            )
            return result
    ...
    def get_statistics(self, project_id: str, git_branch_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger = logging.getLogger(__name__)
            logger.info(
                f"Getting statistics for git branch {git_branch_id} in project {project_id}"
            )
            ...
            task_objs = (
                task_repo.get_tasks_by_git_branch_id(git_branch_id)
                if hasattr(task_repo, "get_tasks_by_git_branch_id")
                else []
            )
            ...
            def run_in_thread():
                ...
                try:
                    result = asyncio.run(
                        self._get_branch_entity(git_branch_id, git_branch_repo)
                    )
                except Exception as e:
                    exception = e
            ...
            logger.info(
                f"🎯 Using trigger-maintained counts: {total_tasks} total, {completed_tasks} completed"
            )
            in_progress_tasks = len(
                [t for t in tasks if get_task_status(t) == "in_progress"]
            )
            ...
            progress_percentage = (
                (total_progress / total_tasks) if total_tasks > 0 else 0.0
            )
            ...
            if tasks:
                last_updates = [
                    task.get("updated_at") for task in tasks if task.get("updated_at")
                ]
                if last_updates:
    ...
    def get_branches_with_task_counts(self, project_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            for branch in branches:
                ...
                task_repo = task_repo_factory.create_repository(
                    project_id=project_id,
                    git_branch_name=branch.get("name", "main"),
                    user_id=self._user_id,
                )
                # Use repository method to get tasks for branch
                # FIXED: Use correct method name get_tasks_by_git_branch_id
                task_objs = (
                    task_repo.get_tasks_by_git_branch_id(branch_id)
                    if hasattr(task_repo, "get_tasks_by_git_branch_id")
                    else []
                )
                ...
                logger.info(
                    f"🎯 Branch {branch.get('name')}: Using trigger-maintained counts: {total_tasks} total, {completed_tasks} completed"
                )
                ...
                # Sum all task progress percentages and divide by total task count
                total_progress = sum(
                    task.get("progress_percentage", 0) for task in tasks
                )
                progress_percentage = (
                    (total_progress / total_tasks) if total_tasks > 0 else 0.0
                )
                ...
                enhanced_branch = {
                    ...
                    "git_branch_name": branch.get("git_branch_name")
                    or branch.get("name"),
    ...
    def get_project_branch_summary(self, project_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Generated project summary: {total_branches} branches, {total_tasks} tasks"
            )
    ...
    def get_branch_summary(self, branch_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"🎯 Branch summary: Using trigger-maintained counts: {total_tasks} total, {completed_tasks} completed"
            )
            ...
            logger.info(
                f"Generated branch summary: {total_tasks} tasks, {progress_percentage:.1f}% complete"
            )

            return {"success": True, "branch": branch_summary}
