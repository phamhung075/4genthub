...
class TaskApplicationService:
    ...
    def __init__(
        ...
        self._create_task_use_case = CreateTaskUseCase(
            self._get_user_scoped_repository(), git_branch_repo
        )
        self._get_task_use_case = GetTaskUseCase(
            self._get_user_scoped_repository(), context_service, git_branch_repo
        )
        ...
        self._search_tasks_use_case = SearchTasksUseCase(
        ...
        self._complete_task_use_case = CompleteTaskUseCase(
            self._get_user_scoped_repository(), None, None
        )
    ...
    def _get_user_scoped_repository(self) -> TaskRepository:
        ...
        logger = logging.getLogger(__name__)
        logger.debug(
            f"_get_user_scoped_repository: self._user_id={self._user_id}, "
        ...
        if hasattr(self._task_repository, "with_user") and self._user_id:
            ...
            logger.debug(
                f"Creating user-scoped repository with user_id={self._user_id}"
            )
            scoped_repo = self._task_repository.with_user(self._user_id)
            logger.debug(
                f"Scoped repo created with user_id={getattr(scoped_repo, 'user_id', 'N/A')}"
            )
            return scoped_repo
            ...
            if self._user_id and self._task_repository.user_id != self._user_id:
                ...
                if hasattr(self._task_repository, "session"):
                    return repo_class(
                        self._task_repository.session, user_id=self._user_id
                    )
    ...
    def with_user(self, user_id: str) -> TaskApplicationService:
        ...
        return TaskApplicationService(
            self._task_repository, self._context_service, user_id
        )
