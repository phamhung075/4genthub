...
class AgentProfile:
    ...
    def should_escalate(self) -> bool:
        ...
        if self.operations_count >= 20:
            recent_compliance = (
                self.compliance_history[-10:]
                if len(self.compliance_history) >= 10
                else self.compliance_history
            )
            compliance_rate = (
    ...
    def deescalate_level(self):
        ...
        self.manually_set_level = (
            False  # Clear manual flag after automatic deescalation
        )
...
class ProgressiveEnforcementService:
    ...
    def __init__(
        ...
        """
        self.enforcement_service = enforcement_service or ParameterEnforcementService(
            default_level
        )
        self.default_level = default_level
        self.agent_profiles: dict[str, AgentProfile] = {}
        logger.info(
            f"ProgressiveEnforcementService initialized with default level: {default_level.value}"
        )

    def enforce_with_progression(
        self, action: str, provided_params: dict[str, any], agent_id: str
    ) -> EnforcementResult:
    ...
    def _get_or_create_profile(self, agent_id: str) -> AgentProfile:
        ...
        if agent_id not in self.agent_profiles:
            ...
            logger.info(
                f"New agent profile created for {agent_id} with {self.default_level.value} enforcement"
            )
    ...
    def set_agent_level(self, agent_id: str, level: EnforcementLevel):
        ...
        logger.info(
            f"Manually changed agent {agent_id} from {old_level.value} to {level.value}"
        )
    ...
    def get_enforcement_stats(self) -> dict[str, any]:
        ...
        for profile in self.agent_profiles.values():
            ...
            if profile.compliance_history:
                compliance_rate = sum(profile.compliance_history) / len(
