...
class ContentAnalyzer:
    ...
    def analyze_task_content(self, task) -> list[DependencyHint]:
        ...
        try:
            ...
            # Analyze keywords
            keyword_hints = self._analyze_dependency_keywords(
                task, content_text, all_tasks
            )
            ...
            logger.info(
                f"Content analysis found {len(hints)} dependency hints for task {task.id}"
            )
    ...
    def _analyze_dependency_keywords(
        self, task, content_text: str, all_tasks: list
    ) -> list[DependencyHint]:
                ...
                for other_task in all_tasks:
                    ...
                    if other_title_lower in content_text:
                        ...
                        if abs(keyword_pos - title_pos) < 100:  # Within 100 characters
                            proximity_score = max(
                                0.5, 1.0 - abs(keyword_pos - title_pos) / 100
                            )
    ...
    def _analyze_agent_dependencies(
        self, task, all_tasks: list
    ) -> list[DependencyHint]:
            ...
            if shared_agents:
                ...
                hint = DependencyHint(
                    ...
                    evidence={
                        ...
                        "agent_overlap": len(shared_agents)
                        / len(task_agents.union(other_agents)),
                    },
                )
                hints.append(hint)
...
class DependencyManagementEngine:
    ...
    def with_user(self, user_id: str) -> DependencyManagementEngine:
        ...
        return DependencyManagementEngine(
            self.dependency_resolver.with_user(user_id), self.task_repository, user_id
        )

    async def resolve_dependencies_with_ai(
        self, task_id: str
    ) -> EnhancedDependencyRelationships:
        """
        ...
        try:
            ...
            optimization_score = self._calculate_optimization_score(
                basic_relationships, ai_suggestions
            )
    ...
    def suggest_dependencies(self, task_id: str) -> list[DependencySuggestion]:
        ...
        try:
            ...
            for hint in content_hints:
                ...
                try:
                    dep_task = self.task_repository.find_by_id(
                        TaskId(hint.suggested_dependency_id)
                    )
                    if dep_task:
                        dep_info = DependencyInfo(
                            ...
                            assignees=dep_task.assignees.copy()
                            if dep_task.assignees
                            else [],
                            updated_at=dep_task.updated_at,
                        )

                        suggestion = DependencySuggestion(
                            hint=hint, target_task_info=dep_info
                        )
                        suggestions.append(suggestion)
                except Exception as e:
                    logger.warning(
                        f"Could not get info for suggested dependency {hint.suggested_dependency_id}: {e}"
                    )
    ...
    def _calculate_optimization_score(
        ...
        high_confidence_suggestions = [
            s for s in ai_suggestions if s.hint.confidence_score > 0.7
        ]
        existing_deps = len(basic_relationships.depends_on)

        # Score factors
        suggestion_score = sum(
            s.hint.confidence_score for s in high_confidence_suggestions
        ) / len(ai_suggestions)
        coverage_score = min(
            1.0, len(high_confidence_suggestions) / max(1, existing_deps)
        )
    ...
    def _generate_suggestion_summary(
        ...
        medium_conf = len(
            [s for s in suggestions if 0.4 < s.hint.confidence_score <= 0.7]
        )
