...
class ResponseOptimizer:
    ...
    DEBUG_INDICATORS = ["debug", "trace", "verbose", "development", "test"]
    ...
    def __init__(self):
        ...
        self.metrics = {
            ...
            "profile_usage": {"minimal": 0, "standard": 0, "detailed": 0, "debug": 0},
        }
    ...
    def optimize_response(
        ...
        logger.debug(
            f"Response optimized with {profile.value} profile: {original_size} -> {optimized_size} bytes "
    ...
    def auto_select_profile(
        self, response: dict[str, Any], request_context: dict[str, Any] | None = None
    ) -> ResponseProfile:
        """
        ...
        for indicator in self.AI_AGENT_INDICATORS:
            if (
                indicator in str(assignees).lower()
                or indicator in user_agent
                or indicator in str(params).lower()
            ):
        ...
        for indicator in self.DEBUG_INDICATORS:
            if indicator in str(headers).lower() or indicator in str(params).lower():
                return ResponseProfile.DEBUG
    ...
    def remove_duplicates(self, response: dict[str, Any]) -> dict[str, Any]:
        ...
        if "confirmation" in response and "operation_details" in response.get(
            "confirmation", {}
        ):
        ...
        if "confirmation" in response and "operation_completed" in response.get(
            "confirmation", {}
        ):
            if (
                response.get("success")
                == response["confirmation"]["operation_completed"]
            ):
    ...
    def flatten_structure(self, response: dict[str, Any]) -> dict[str, Any]:
        ...
        if "confirmation" in response:
            ...
            if (
                len(confirmation) <= 3
                and "data_persisted" in confirmation
                and confirmation.get("partial_failures", []) == []
            ):
                # Move data_persisted to meta
                if "meta" not in response:
                    response["meta"] = {}
                response["meta"]["persisted"] = confirmation.get(
            ...
            for key, value in data.items():
                ...
                if cleaned_value is not None:
                    ...
                    if isinstance(cleaned_value, str) and cleaned_value == "":
                        continue
                    if (
                        isinstance(cleaned_value, (list, dict))
    ...
    def apply_profile(
        self, response: dict[str, Any], profile: ResponseProfile
    ) -> dict[str, Any]:
        """
        ...
        if profile == ResponseProfile.MINIMAL:
            ...
            if "workflow_guidance" in filtered:
                filtered["hints"] = self._simplify_workflow_guidance(
                    filtered["workflow_guidance"]
                )
        ...
        if "next_steps" in guidance:
            ...
            if isinstance(next_steps, dict):
                if "recommendations" in next_steps and next_steps["recommendations"]:
                    hints["next"] = (
                        next_steps["recommendations"][0]
                        if isinstance(next_steps["recommendations"], list)
                        else str(next_steps["recommendations"])
                    )
    ...
    def _update_metrics(
        self, original_size: int, optimized_size: int, profile: ResponseProfile
    ):
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "most_used_profile": max(
                self.metrics["profile_usage"], key=self.metrics["profile_usage"].get
            )
            if self.metrics["total_optimized"] > 0
            else None,
        }
