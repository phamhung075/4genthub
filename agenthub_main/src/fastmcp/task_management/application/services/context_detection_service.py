...
class ContextDetectionService:
    ...
    def detect_id_type(self, context_id: str) -> tuple[str, str | None]:
        ...
        try:
            ...
            try:
                ...
                if branch:
                    logger.debug(
                        f"ID {context_id} identified as git branch ID with project {branch.project_id}"
                    )
            ...
            try:
                ...
                if task:
                    ...
                    if branch:
                        logger.debug(
                            f"ID {context_id} identified as task ID with project {branch.project_id}"
                        )
    ...
    def get_context_level_for_id(self, context_id: str) -> str:
        ...
        if id_type == "project":
            ...
            logger.warning(
                f"Unknown ID type for {context_id}, defaulting to task level"
            )
            return "task"
