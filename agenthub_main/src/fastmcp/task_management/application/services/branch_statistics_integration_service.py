...
class BranchStatisticsIntegrationService:
    ...
    def register_event_handlers(self) -> None:
        ...
        dispatcher.register_handler("task_created", self._handle_task_created)

        # Register handler for task updates
        dispatcher.register_handler("task_updated", self._handle_task_updated)

        # Register handler for task deletion
        dispatcher.register_handler("task_deleted", self._handle_task_deleted)
        ...
        dispatcher.register_handler(
            "task_moved_to_branch", self._handle_task_moved_to_branch
        )
    ...
    def _handle_task_created(self, event: TaskCreatedEvent) -> None:
        ...
        try:
            ...
            self._branch_statistics_service.on_task_created(
                task_id=event.task_id, branch_id=event.branch_id, status=event.status
            )
