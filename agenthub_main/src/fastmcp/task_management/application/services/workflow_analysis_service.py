...
class WorkflowAnalysisService:
    ...
    def with_user(self, user_id: str) -> WorkflowAnalysisService:
        ...
        return WorkflowAnalysisService(
            self.task_repository, self.context_repository, self.event_store, user_id
        )

    async def analyze_task_workflow(
        self, task_id: UUID, include_related: bool = True
    ) -> WorkflowAnalysis:
        """
    ...
    async def _get_related_tasks(self, task: Task) -> list[Task]:
        ...
        if hasattr(task, "labels") and task.labels:
            for label in task.labels[:2]:
                tasks = await self.task_repository.list(labels=[label], limit=20)
    ...
    async def _detect_patterns(
        self, task: Task, context: TaskContext | None, related_tasks: list[Task]
    ...
    def _detect_completion_patterns(
        self, task: Task, related_tasks: list[Task]
    ) -> list[WorkflowPattern]:
        ...
        if len(completed_similar) >= 3:
            ...
            if hasattr(task, "created_at"):
                current_duration = (
                    datetime.now(UTC) - task.created_at
                ).total_seconds() / 3600
    ...
    def _detect_collaboration_patterns(
        self, task: Task, context: TaskContext | None
    ) -> list[WorkflowPattern]:
        ...
        if len(indicators) >= 2:
            pattern = WorkflowPattern(
                ...
                metrics={"indicators": indicators, "indicator_count": len(indicators)},
            )
            patterns.append(pattern)
        ...
        if breakdown:
            ...
            if progress_values:
                ...
                if max_progress - min_progress > 0.6:
                    lagging_types = [
                        pt
                        for pt, progress in breakdown.items()
    ...
    async def _identify_bottlenecks(
        ...
        if hasattr(task, "progress_timeline") and task.progress_timeline:
            last_update = datetime.fromisoformat(
                task.progress_timeline[-1]["timestamp"]
            )
    ...
    async def _find_optimization_opportunities(
        self, task: Task, context: TaskContext | None, patterns: list[WorkflowPattern]
    ) -> list[dict[str, Any]]:
        ...
        if not hasattr(task, "subtasks") or not task.subtasks:
            if hasattr(task, "estimated_effort"):
                ...
                if effort_str and (
                    effort_str.endswith("d") or effort_str.endswith("h")
                ):
    ...
    async def _predict_completion_time(
        ...
        completed_similar = [
            ...
            and hasattr(t, "created_at")
            and hasattr(t, "updated_at")
            and hasattr(t, "estimated_effort")
            and t.estimated_effort == getattr(task, "estimated_effort", None)
        ]
        ...
        completion_times = [t.updated_at - t.created_at for t in completed_similar]

        avg_completion = sum(ct.total_seconds() for ct in completion_times) / len(
            completion_times
        )
    ...
    def _assess_risk_factors(
        ...
        if (
            task.priority in ["urgent", "critical"]
            and getattr(task, "progress", 0) < 0.3
        ):
    ...
    async def get_workflow_recommendations(self, task_id: UUID) -> list[dict[str, Any]]:
        """
        ...
        for opportunity in analysis.optimization_opportunities:
            impact_priority = {"high": "high", "medium": "medium", "low": "low"}
            priority = impact_priority.get(opportunity["potential_impact"], "medium")
