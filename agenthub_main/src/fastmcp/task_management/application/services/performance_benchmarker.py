    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "compression": {
                ...
                "median_compression_ratio_percent": statistics.median(
                    compression_ratios
                ),
...
class PerformanceBenchmarker:
    ...
    def start_suite(self, name: str) -> BenchmarkSuite:
        ...
        self.current_suite = BenchmarkSuite(suite_name=name, started_at=datetime.now())
    ...
    def benchmark_context(
        self, name: str, category: BenchmarkCategory, input_data: Any = None
    ):
    ...
    def benchmark_response_optimization(
        self, optimizer: Any, test_responses: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        for i, response in enumerate(test_responses):
            with self.benchmark(
                ...
                try:
                    benchmark.output_size_bytes = len(
                        json.dumps(optimized, default=str).encode("utf-8")
                    )
                except Exception:
        ...
        for context in test_contexts:
            for field_set in field_sets:
                for operation in operations:
                    ...
                    with self.benchmark(
                        test_name, BenchmarkCategory.CONTEXT_SELECTION, context
                    ) as benchmark:
                        # Get template
                        template = template_manager.get_template(operation)

                        # Get minimal context
                        minimal = template_manager.get_minimal_context(
                            operation, context
                        )
                        ...
                        try:
                            benchmark.output_size_bytes = len(
                                json.dumps(minimal, default=str).encode("utf-8")
                            )
                        except Exception:
    ...
    def benchmark_cache_performance(
        self, cache_optimizer: Any, test_data: list[tuple[str, Any]]
    ) -> dict[str, Any]:
        """Benchmark cache performance"""

        # Warm up cache
        with self.benchmark(
            "cache_warmup", BenchmarkCategory.CACHE_PERFORMANCE
        ) as benchmark:
            warmed = 0
        ...
        # Test cache hits
        with self.benchmark(
            "cache_hit_performance", BenchmarkCategory.CACHE_PERFORMANCE
        ) as benchmark:
            hits = 0
        ...
        # Test cache misses
        with self.benchmark(
            "cache_miss_performance", BenchmarkCategory.CACHE_PERFORMANCE
        ) as benchmark:
            misses = 0
        ...
        with self.benchmark(
            "cache_optimization", BenchmarkCategory.CACHE_PERFORMANCE
        ) as benchmark:
    ...
    def benchmark_end_to_end(
        ...
        for i, scenario in enumerate(test_scenarios):
            with self.benchmark(
                f"end_to_end_scenario_{i + 1}", BenchmarkCategory.END_TO_END, scenario
            ) as benchmark:
                ...
                if template and context_data:
                    minimal_context = template_manager.get_minimal_context(
                        op_enum, context_data
                    )
                else:
                ...
                try:
                    ...
                    original_size = len(
                        json.dumps(response, default=str).encode("utf-8")
                    )
                    ...
                    benchmark.metadata = {
                        ...
                        "compression_achieved": (
                            1 - benchmark.output_size_bytes / original_size
                        )
                        * 100,
    ...
    def _get_category_summary(self, category: BenchmarkCategory) -> dict[str, Any]:
        ...
        category_results = [
            r for r in self.current_suite.results if r.category == category
        ]
        ...
        compression_ratios = [
            r.compression_ratio for r in successful if r.input_size_bytes > 0
        ]
        ...
        if compression_ratios:
            summary.update(
                {
                    "avg_compression_ratio_percent": statistics.mean(
                        compression_ratios
                    ),
                    "median_compression_ratio_percent": statistics.median(
                        compression_ratios
    ...
    def generate_report_old(
        ...
        report = {
            ...
            "completed_at": target_suite.completed_at.isoformat()
            if target_suite.completed_at
            else None,
        ...
        for category in categories:
            report["category_summaries"][category.value] = self._get_category_summary(
                category
            )
    ...
    def _compare_with_baseline(self) -> dict[str, Any]:
        ...
        avg_output_size = statistics.mean(
            [r.output_size_bytes for r in successful_results]
        )
        compression_ratios = [
            r.compression_ratio for r in successful_results if r.input_size_bytes > 0
        ]
        avg_compression = (
        ...
        return {
            ...
            "improvements": {
                "execution_time_improvement_percent": max(
                    0,
                    (baseline["response_processing_ms"] - avg_execution_time)
                    / baseline["response_processing_ms"]
                    * 100,
                ),
                "size_reduction_percent": max(
                    0,
                    (baseline["response_size_bytes"] - avg_output_size)
    ...
    def _generate_recommendations(self, suite: BenchmarkSuite) -> list[str]:
        ...
        if avg_time > 50:
            recommendations.append(
                "Consider optimizing response processing - average time exceeds 50ms"
            )

        if max_time > 200:
            recommendations.append(
                "Some operations are very slow (>200ms) - investigate bottlenecks"
            )
        ...
        if compression_ratios:
            ...
            if avg_compression < 50:
                recommendations.append(
                    "Compression ratio below 50% - review optimization strategies"
                )
            elif avg_compression > 80:
                recommendations.append(
                    "Excellent compression ratio achieved - current optimization is effective"
                )
        ...
        if BenchmarkCategory.CACHE_PERFORMANCE in categories:
            cache_results = [
                ...
                if r.category == BenchmarkCategory.CACHE_PERFORMANCE
            ]
            ...
            if avg_cache_time > 10:
                recommendations.append(
                    "Cache operations are slow - consider cache optimization"
                )

        return (
            recommendations
            if recommendations
            else ["Performance looks good - no specific recommendations"]
        )
    ...
    def check_target(self, result: BenchmarkResult, target: PerformanceTarget) -> bool:
        ...
        if target.percentile_targets and result.all_times:
            for percentile, max_time in target.percentile_targets.items():
                percentile_value = (
                    statistics.quantiles(result.all_times, n=100)[percentile - 1]
                    if len(result.all_times) >= 100
                    else max(result.all_times)
                )
    ...
    def compare(
        ...
        speedup = (
            result1.mean_time / result2.mean_time
        ...
        return BenchmarkComparison(
            speedup=speedup, winner=winner, statistical_significance=significance
        )
    ...
    def profile(
        ...
        profile_data = {"function_calls": {}, "time_per_line": {}, "hotspots": []}
    ...
    def calculate_regression_severity(
        self, current: BenchmarkResult, historical: list[BenchmarkResult]
    ) -> float:
    ...
    def measure(self, name: str):
        ...
        start = time.perf_counter()
        context = type("Context", (), {"elapsed_time": 0, "result_stored": False})()
        ...
        report = {
            "summary": {
                ...
                "mean_execution_time": statistics.mean(
                    [r.mean_time for r in self.results]
                )
                if self.results
                else 0,
            },
    ...
    def _generate_recommendations_for_report(self) -> list[str]:
        ...
        if slowest.mean_time > 0.1:
            recommendations.append(
                f"Optimize '{slowest.name}' - it's the slowest operation at {slowest.mean_time:.3f}s"
            )
        ...
        if high_variance:
            recommendations.append(
                f"{len(high_variance)} operations have high variance - investigate stability"
            )
