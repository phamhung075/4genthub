...
class AITaskIntegrationService:
    ...
    async def create_ai_enhanced_task_plan(
        self,
        requirements: str,
        title: str,
        description: str,
        ...
        try:
            ...
            task_plan = await self.ai_planning_service.create_intelligent_plan(
                planning_request
            )
            ...
            if auto_create_tasks:
                created_tasks = await self._create_mcp_tasks_from_plan(
                    task_plan, git_branch_id, user_id
                )
    ...
    async def enhance_task_creation(
        ...
        try:
            ...
            return {"success": False, "error": f"AI enhancement failed: {str(e)}"}

    async def add_ai_insights_to_task_response(
        self, task_data: dict[str, Any], action: str = "get"
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            insights = await self._generate_task_insights(
                task_title, task_description, action
            )
    ...
    def _parse_requirements(self, requirements: str) -> list[RequirementItem]:
        ...
        try:
            ...
            requirement_items = [
                RequirementItem(id="req_1", description=requirements, priority="medium")
            ]

        return requirement_items

    async def _create_mcp_tasks_from_plan(
        self, task_plan: TaskPlan, git_branch_id: str, user_id: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        for planned_task in root_tasks:
            try:
                ...
                create_request = CreateTaskRequest(
                    ...
                    git_branch_id=git_branch_id,
                    assignees=[planned_task.agent_assignment.agent_id]
                    if planned_task.agent_assignment
                    else [],
                    estimated_effort=f"{planned_task.estimated_hours} hours"
                    if planned_task.estimated_hours
                    else None,
                ...
                if result.get("success"):
                    ...
                    created_tasks.append(
                        {
                            ...
                            "title": planned_task.title,
                            "agent_assignment": planned_task.agent_assignment.agent_id
                            if planned_task.agent_assignment
                            else None,
                    ...
                    logger.info(
                        f"Created MCP task {mcp_task_id} from planned task {planned_task.id}"
                    )
                else:
                    logger.error(
                        f"Failed to create MCP task for planned task {planned_task.id}: {result.get('error')}"
                    )

            except Exception as e:
                logger.error(
                    f"Error creating MCP task from planned task {planned_task.id}: {e}"
                )

        return created_tasks

    async def _generate_task_breakdown(
        self, description: str, parent_task_id: str, git_branch_id: str
    ) -> dict[str, Any]:
    ...
    async def _suggest_optimal_agents(self, task_content: str) -> dict[str, Any]:
        ...
        if re.search(
            r"\b(test|testing|qa|suite|spec|unit|integration)\b", content_lower
        ):
            suggestions.append("test-orchestrator-agent")
        # UI-related keywords
        elif re.search(
            r"\b(ui|frontend|component|interface|dashboard|view|screen)\b",
            content_lower,
        ):
            suggestions.append("shadcn-ui-expert-agent")
        # Security-related keywords
        elif re.search(
            r"\b(security|auth|authentication|login|permission|access)\b", content_lower
        ):
    ...
    async def _generate_task_insights(
        self, title: str, description: str, action: str
    ) -> dict[str, Any]:
    ...
    def _identify_risks(self, title: str, description: str) -> list[str]:
        ...
        if any(
            keyword in content for keyword in ["integration", "migration", "refactor"]
        ):
        ...
        if any(
            keyword in content for keyword in ["security", "auth", "authentication"]
        ):
