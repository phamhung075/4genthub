...
class ContextValidationService:
    ...
    def __init__(
        self, user_id: str | None = None, id_validator: IDValidator | None = None
    ):
    ...
    def validate_completion_context(
        ...
        if context.metadata.task_id != str(task.id):
            errors.append(
                f"Context task_id {context.metadata.task_id} does not match task {task.id}"
            )

            # Additional check for ID confusion
            context_id_result = self._id_validator.detect_id_type(
                context.metadata.task_id, "task_id"
            )
            if not context_id_result.is_valid:
                errors.append(
                    f"Invalid context task_id format: {context_id_result.error_message}"
                )
            elif context.metadata.task_id == str(task.id):
                errors.append(
                    "CRITICAL: Context task_id and task ID mismatch may indicate ID confusion"
                )
        ...
        logger.info(
            f"Validating completion context for task {task.id}: "
    ...
    def validate_context_update(
        self, context: TaskContext, update_data: dict[str, Any]
    ) -> tuple[bool, list[str]]:
        """
    ...
    def ensure_completion_summary_in_context(
        self,
        context: TaskContext,
        completion_summary: str,
        testing_notes: str | None = None,
    ...
    def validate_progress_update(
        self, progress_type: str, details: str, percentage: float | None = None
    ) -> tuple[bool, list[str]]:
        """
        ...
        if progress_type not in valid_types:
            errors.append(
                f"Invalid progress_type. Must be one of: {', '.join(valid_types)}"
            )
    ...
    def validate_context_data(
        self, level: ContextLevel, data: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        if level == ContextLevel.TASK:
            ...
            if "priority" in data and data["priority"] not in [
                "low",
                "medium",
                "high",
                "urgent",
