...
class AgentInheritanceService:
    ...
    def apply_agent_inheritance(
        self, subtask: Subtask, parent_task: Task | None = None
    ) -> Subtask:
            ...
            if parent_task:
                ...
                if parent_assignees:
                    try:
                        ...
                        logger.info(
                            f"Applying agent inheritance: subtask {subtask.id} inheriting {assignee_count} assignees from parent task {parent_task.id}"
                        )
                    except (TypeError, AttributeError):
                        # Handle cases where parent_assignees might be invalid (e.g., Mock objects in tests)
                        logger.info(
                            f"Applying agent inheritance: subtask {subtask.id} inheriting assignees from parent task {parent_task.id}"
                        )
                    subtask.inherit_assignees_from_parent(parent_assignees)
                else:
                    logger.info(
                        f"No assignees to inherit: parent task {parent_task.id} has no assignees"
                    )
            else:
                logger.warning(
                    f"Cannot apply inheritance: parent task {subtask.parent_task_id} not found"
                )
        else:
            logger.debug(
                f"Subtask {subtask.id} already has assignees, no inheritance needed"
            )
    ...
    def apply_inheritance_to_all_subtasks(self, task_id: TaskId) -> list[Subtask]:
        ...
        for subtask in subtasks:
            if subtask.should_inherit_assignees():
                ...
                original_assignees = (
                    getattr(subtask, "assignees", []).copy()
                    if hasattr(subtask, "assignees")
                    else []
                )
                ...
                if current_assignees != original_assignees:
                    ...
                    logger.info(
                        f"Updated subtask {subtask.id} with inherited assignees: {current_assignees}"
                    )

        logger.info(
            f"Applied inheritance to {len(updated_subtasks)} subtasks for task {task_id}"
        )
    ...
    def validate_agent_assignments(self, assignees: list[str]) -> list[str]:
        ...
        dummy_task = Task(
            title="dummy", description="dummy"
        )  # Temporary task for validation
