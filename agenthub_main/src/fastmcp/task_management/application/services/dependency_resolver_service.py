...
class DependencyResolverService:
        ...
        if hasattr(self.task_repository, "with_user") and self._user_id:
            ...
            if self._user_id and self.task_repository.user_id != self._user_id:
                ...
                if hasattr(self.task_repository, "session"):
                    return repo_class(
                        self.task_repository.session, user_id=self._user_id
                    )
    ...
    def resolve_dependencies(self, task_id: str) -> DependencyRelationships:
        ...
        try:
            ...
            depends_on = self._resolve_direct_dependencies(
                main_task.get_dependency_ids()
            )
            ...
            completed_dependencies = sum(
                1 for dep in depends_on if dep.status == "done"
            )
            blocked_dependencies = sum(
                1 for dep in depends_on if dep.status == "blocked"
            )
        ...
        for dep_task in dependent_tasks:
            ...
            try:
                ...
                if task:
                    ...
                    if chain_tasks:
                        completed_tasks = sum(
                            1 for t in chain_tasks if t.status == "done"
                        )
                        blocked_tasks = sum(
                            1 for t in chain_tasks if t.status == "blocked"
                        )
    ...
    def _generate_dependency_summary(
        self, depends_on: list[DependencyInfo], blocks: list[DependencyInfo]
    ) -> str:
        ...
        if depends_on:
            ...
            summary_parts.append(
                f"Depends on {total} task(s) ({completed}/{total} completed)"
            )
    ...
    def _generate_next_actions(
        ...
        if can_start:
            ...
            if incomplete_deps:
                actions.append(
                    f"⏳ Wait for {len(incomplete_deps)} dependencies to complete"
                )
                ...
                if todo_deps:
                    actions.append(
                        f"💡 Consider working on {len(todo_deps)} unstarted dependencies"
                    )

        if blocks:
            actions.append(
                f"🚧 Completing this task will unblock {len(blocks)} other task(s)"
            )
