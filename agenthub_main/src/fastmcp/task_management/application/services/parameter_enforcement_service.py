...
class ParameterEnforcementService:
    ...
    PARAMETER_TEMPLATES = {
        ...
        "insights_found": [
            "Found existing utility for token generation",
            "Database index needed for performance",
        ],
    }
    ...
    def __init__(
        ...
        logger.info(
            f"ParameterEnforcementService initialized with level: {enforcement_level.value}"
        )
    ...
    def enforce(
        ...
        if level == EnforcementLevel.DISABLED:
            return EnforcementResult(
                allowed=True, level=level, message="Parameter enforcement disabled"
            )
        ...
        missing_required = [
            ...
            if p not in provided_params or not provided_params[p]
        ]
        missing_recommended = [
            p
            for p in recommended_params
            if p not in provided_params or not provided_params[p]
        ]
        ...
        if agent_id:
            self._track_enforcement(
                agent_id,
                is_compliant,
                level == EnforcementLevel.STRICT and not is_compliant,
            )
        ...
        if level == EnforcementLevel.SOFT:
            ...
            if missing_required or missing_recommended:
                logger.info(
                    f"[SOFT] Missing parameters for {action}: required={missing_required}, recommended={missing_recommended}"
                )
            return self._create_soft_result(
                missing_required, missing_recommended, agent_id
            )
            ...
            if missing_required:
                logger.warning(
                    f"[WARNING] Missing required parameters for {action}: {missing_required}"
                )
            return self._create_warning_result(
                action, missing_required, missing_recommended, agent_id
            )
            ...
            if missing_required:
                logger.error(
                    f"[STRICT] Blocking {action} due to missing required parameters: {missing_required}"
                )
                return self._create_strict_result(
                    action, missing_required, missing_recommended, agent_id
                )
            else:
                ...
                return EnforcementResult(
                    ...
                    hints=[f"✅ All required parameters provided for {action}"]
        ...
        return EnforcementResult(
            allowed=True, level=level, message="All required parameters provided"
        )
    ...
    def _create_warning_result(
        ...
        if missing_required:
            hints.append(
                f"⚠️ Missing required parameters: {', '.join(missing_required)}"
            )
        ...
        message = (
            "Operation allowed with warnings"
            if missing_required
    ...
    def set_enforcement_level(self, level: EnforcementLevel):
        ...
        logger.info(
            f"Enforcement level changed from {self.enforcement_level.value} to {level.value}"
        )
    ...
    def get_parameter_hints(self, action: str) -> dict[str, Any]:
        ...
        all_params = requirements.get("strict", []) + requirements.get(
            "recommended", []
        )
