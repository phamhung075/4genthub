...
class UnifiedContextService:
    ...
    def with_user(self, user_id: str) -> UnifiedContextService:
        ...
        global_repo = self._get_user_scoped_repository_for_user(
            self.repositories[ContextLevel.GLOBAL], user_id
        )
        project_repo = self._get_user_scoped_repository_for_user(
            self.repositories[ContextLevel.PROJECT], user_id
        )
        branch_repo = self._get_user_scoped_repository_for_user(
            self.repositories[ContextLevel.BRANCH], user_id
        )
        task_repo = self._get_user_scoped_repository_for_user(
            self.repositories[ContextLevel.TASK], user_id
        )
    ...
    def _normalize_global_context_id(
        self, context_id: str, user_id: str | None = None
    ) -> str:
        """
        ...
        if not context_id or context_id.lower() == "global":
            ...
            if effective_user_id:
                ...
                logger.info(
                    f"Normalized '{context_id or 'None'}' to user-specific UUID: {normalized_id} for user: {effective_user_id}"
                )
                return normalized_id
            else:
                logger.error("Cannot normalize global context_id without user_id")
                raise ValueError(
                    "user_id is required for global context normalization (no fallback allowed for DDD compliance)"
                )
    ...
    def create_context(
        ...
        try:
            ...
            if context_level == ContextLevel.GLOBAL:
                context_id = self._normalize_global_context_id(
                    context_id, user_id or self._user_id
                )
            ...
            if (
                ...
                try:
                    ...
                    git_branch_factory = (
                        DomainServiceFactory.get_git_branch_repository_factory()
                    )

                    # Check if git_branch_factory is None before trying to use it
                    if git_branch_factory is None:
                        logger.debug(
                            "Git branch repository factory not available, skipping auto-detection"
                        )
                    else:
                        ...
                        if branch and hasattr(branch, "project_id"):
                            ...
                            logger.info(
                                f"Auto-detected project_id '{branch.project_id}' for branch context '{context_id}'"
                            )
            ...
            # Ensure data is not None and is a dict before checking its contents
                ...
                and not any(
                    data.get(key)
                    for key in ["branch_id", "parent_branch_id", "git_branch_id"]
                )
            ):
                ...
                try:
                    ...
                    if task_factory is None:
                        logger.debug(
                            "Task repository factory not available, skipping auto-detection"
                        )
                    else:
                        ...
                        if hasattr(task_repo, "get"):
                            ...
                            logger.warning(
                                "Task repository doesn't have expected methods 'get' or 'find_by_id'"
                            )
                            task = None

                        if (
                            task
                            and hasattr(task, "git_branch_id")
                            and task.git_branch_id
                        ):
                            data["git_branch_id"] = task.git_branch_id
                            logger.info(
                                f"Auto-detected git_branch_id '{task.git_branch_id}' for task context '{context_id}'"
                            )
                        else:
                            logger.warning(
                                f"Task found but no git_branch_id available: task={task}, has_attr={hasattr(task, 'git_branch_id') if task else 'No task'}"
                            )
                except Exception as e:
                    logger.warning(
                        f"Could not auto-detect git_branch_id from task: {e}",
                        exc_info=True,
                    )
            ...
            if auto_create_parents and context_level != ContextLevel.GLOBAL:
                ...
                if not auto_creation_result["success"]:
                    logger.warning(
                        f"Failed to ensure parent contexts: {auto_creation_result.get('error')}"
                    )
                    # Continue anyway - some contexts may still be valid

            # Validate hierarchy requirements after auto-creation
            is_valid, error_msg, guidance = (
                self.hierarchy_validator.validate_hierarchy_requirements(
                    level=context_level, context_id=context_id, data=data
                )
            )
            ...
            if not is_valid:
                ...
                if not auto_create_parents:
                    logger.info(
                        f"Validation failed but auto-creation disabled for {level}:{context_id}"
                    )
                    ...
                    if self._should_allow_orphaned_creation(
                        context_level, context_id, data
                    ):
                        logger.info(
                            f"Allowing orphaned creation for {level}:{context_id} due to special conditions"
                        )
                        is_valid = True
                        error_msg = None
                    else:
            ...
            if effective_user_id and hasattr(base_repository, "with_user"):
                ...
                logger.debug(
                    f"Created user-scoped repository for user: {effective_user_id}"
                )
            else:
                repo = base_repository
                logger.warning(
                    f"Using unscoped repository - no user_id available for {level} context creation"
                )
            ...
            try:
                ...
                if existing_context:
                    logger.info(
                        f"Context already exists: {level}:{context_id}, returning existing"
                    )
                    return {
                        "success": True,
            ...
            logger.info(
                f"Created entity with ID: {getattr(context_entity, 'id', 'NO_ID')}"
            )

            # Save to repository
            logger.info(
                f"About to create {level} context in repository with entity: {context_entity}"
            )
            saved_context = repo.create(context_entity)
            logger.info(
                f"Successfully created {level} context in repository: {saved_context}"
            )
            ...
            try:
                ...
                if context_level != ContextLevel.GLOBAL:
                    parent_level, parent_id = self._get_parent_info(
                        context_level, saved_context
                    )
                    if parent_level and parent_id:
                        ...
                        logger.debug(
                            f"Invalidated parent cache for {parent_level.value}:{parent_id}"
                        )
            except Exception as cache_error:
                logger.warning(
                    f"Cache invalidation failed (non-critical): {cache_error}"
                )
    ...
    def get_context(
        ...
        try:
            ...
            if not context_id:
                return {"success": False, "error": "Context ID is required"}
            ...
            if include_inherited:
                context_data = self._resolve_inheritance_sync(
                    context_level, context_entity, context_data
                )
    ...
    def update_context(
        ...
        try:
            ...
            repository = self._get_user_scoped_repository(
                self.repositories.get(context_level)
            )
            if not repository:
                return {
                    "success": False,
            ...
            if context_level == ContextLevel.GLOBAL:
                ...
                logger.info(
                    f"DEBUG: GlobalContext update - existing_dict keys: {existing_dict.keys()}"
                )
                ...
                if "global_settings" in existing_dict:
                    ...
                    updated_global_settings = self._merge_context_data(
                        existing_data=existing_dict["global_settings"], new_data=data
                    )
                    logger.info(
                        f"DEBUG: GlobalContext update - merged global_settings: {updated_global_settings}"
                    )
                ...
                logger.info(
                    f"DEBUG: GlobalContext update - final updated_data: {updated_data}"
                )
            else:
                updated_data = self._merge_context_data(
                    existing_data=self._entity_to_dict(existing), new_data=data
                )
            ...
            updated_entity = self._update_context_entity(
                existing_entity=existing, new_data=updated_data
            )
            ...
            try:
                ...
                if propagate_changes:
                    self._invalidate_child_caches(
                        context_level, context_id, effective_user_id, cache
                    )

                logger.debug(
                    f"Cache invalidated for updated context {level}:{context_id}"
                )
            except Exception as cache_error:
                logger.warning(
                    f"Cache invalidation failed (non-critical): {cache_error}"
    ...
    def delete_context(
        self, level: str, context_id: str, user_id: str | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Invalidate the specific context and its inheritance chain
                cache.invalidate_context(
                    user_id=user_id, level=context_level.value, context_id=context_id
                )
                cache.invalidate_inheritance(
                    user_id=user_id, level=context_level.value, context_id=context_id
                )
                ...
                logger.debug(
                    f"Cache invalidated for deleted context {level}:{context_id}"
                )
            except Exception as cache_error:
                logger.warning(
                    f"Cache invalidation failed (non-critical): {cache_error}"
            ...
            return {"success": result, "level": level, "context_id": context_id}
    ...
    def delegate_context(
        ...
        try:
            ...
            delegation_result = {
                "success": True,
                "message": "Delegation skipped in sync mode",
            }
    ...
    def add_insight(
        ...
        try:
            ...
            return self.update_context(
                level=level, context_id=context_id, data={"insights": insights}
            )
    ...
    def _create_context_entity(
        ...
        if level == ContextLevel.GLOBAL:
            ...
            if custom_fields:
                ...
                logger.info(
                    f"Routed {len(custom_fields)} custom fields to local_standards._custom: {list(custom_fields.keys())}"
                )
            ...
            if custom_fields:
                ...
                logger.info(
                    f"Preserved {len(custom_fields)} custom fields in branch metadata: {list(custom_fields.keys())}"
                )
            ...
            branch_id = (
                data.get("branch_id")
                or data.get("parent_branch_id")
                or data.get("git_branch_id")
            )
            if not branch_id:
                raise ValueError(
                    "Task context requires branch_id, parent_branch_id, or git_branch_id"
                )
    ...
    def _entity_to_dict(self, entity) -> dict[str, Any]:
        ...
        if hasattr(entity, "dict") and callable(getattr(entity, "dict")):
            ...
            result = vars(entity)
        logger.info(
            f"DEBUG: _entity_to_dict - converting entity type {type(entity).__name__} to dict with {len(result)} keys"
        )
        return result
    ...
    def _merge_context_data(
        ...
        if existing_data is None:
            logger.warning(
                "🚨 CONTEXT_FIX: existing_data is None, initializing to empty dict"
            )
            existing_data = {}
        if new_data is None:
            logger.warning(
                "🚨 CONTEXT_FIX: new_data is None, initializing to empty dict"
            )
            new_data = {}
        ...
        try:
            ...
            if existing_data is None:
                logger.warning(
                    "🚨 CONTEXT_FIX: _serialize_for_json returned None for existing_data, using empty dict"
                )
                existing_data = {}
        except Exception as e:
        ...
        try:
            ...
            if new_data is None:
                logger.warning(
                    "🚨 CONTEXT_FIX: _serialize_for_json returned None for new_data, using empty dict"
                )
                new_data = {}
        except Exception as e:
        ...
        if not isinstance(existing_data, dict):
            logger.warning(
                f"🚨 CONTEXT_FIX: existing_data is not a dict after serialization: {type(existing_data)}, converting"
            )
            existing_data = {}
        if not isinstance(new_data, dict):
            logger.warning(
                f"🚨 CONTEXT_FIX: new_data is not a dict after serialization: {type(new_data)}, converting"
            )
            new_data = {}
        ...
        if new_data is not None and isinstance(new_data, dict):
            for key, value in new_data.items():
                if (
                    ...
                    and key not in replace_list_fields
                ):
            ...
            logger.debug(
                "🚨 CONTEXT_FIX: new_data is None or not a dict, skipping merge iteration"
            )
    ...
    def _update_context_entity(self, existing_entity, new_data: dict[str, Any]):
        ...
        if isinstance(existing_entity, TaskContext):
            ...
            return TaskContext(
                ...
                execution_context=new_data.get(
                    "execution_context", existing_entity.execution_context
                ),
                discovered_patterns=new_data.get(
                    "discovered_patterns", existing_entity.discovered_patterns
                ),
                implementation_notes=new_data.get(
                    "implementation_notes", existing_entity.implementation_notes
                ),
            ...
            return BranchContext(
                ...
                git_branch_name=new_data.get(
                    "git_branch_name", existing_entity.git_branch_name
                ),
                branch_info=new_data.get(
                    "branch_info", getattr(existing_entity, "branch_info", {})
                ),
                branch_workflow=new_data.get(
                    "branch_workflow", getattr(existing_entity, "branch_workflow", {})
                ),
                feature_flags=new_data.get(
                    "feature_flags", getattr(existing_entity, "feature_flags", {})
                ),
                discovered_patterns=new_data.get(
                    "discovered_patterns",
                    getattr(existing_entity, "discovered_patterns", {}),
                ),
                branch_decisions=new_data.get(
                    "branch_decisions", getattr(existing_entity, "branch_decisions", {})
                ),
                branch_settings=new_data.get(
                    "branch_settings", existing_entity.branch_settings
                ),
                metadata=new_data.get("metadata", existing_entity.metadata),
            )
        elif isinstance(existing_entity, ProjectContext):
            # Extract all fields (matching CONTEXT_DATA_MODELS.md)
            project_info = new_data.get(
                "project_info", getattr(existing_entity, "project_info", {})
            )
            team_preferences = new_data.get(
                "team_preferences", getattr(existing_entity, "team_preferences", {})
            )
            technology_stack = new_data.get(
                "technology_stack", getattr(existing_entity, "technology_stack", {})
            )
            project_workflow = new_data.get(
                "project_workflow", getattr(existing_entity, "project_workflow", {})
            )
            local_standards = new_data.get(
                "local_standards", getattr(existing_entity, "local_standards", {})
            )
            project_settings = new_data.get(
                "project_settings", getattr(existing_entity, "project_settings", {})
            )
            technical_specifications = new_data.get(
                "technical_specifications",
                getattr(existing_entity, "technical_specifications", {}),
            )
            ...
            if new_data is not None and isinstance(new_data, dict):
                ...
                logger.debug(
                    "🚨 CONTEXT_FIX: new_data is None or not a dict in _update_context_entity, skipping custom fields merge"
                )
            ...
            # Merge metadata
            existing_metadata = (
                existing_entity.metadata.copy() if existing_entity.metadata else {}
            )
            # CRITICAL FIX: Add additional null checks for metadata merge
            if (
                new_data is not None
                and isinstance(new_data, dict)
                and "metadata" in new_data
                and isinstance(new_data["metadata"], dict)
                and new_data["metadata"] is not None
            ):
                for key, value in new_data["metadata"].items():
                    existing_metadata[key] = value
            else:
                logger.debug(
                    "🚨 CONTEXT_FIX: Skipping metadata merge due to None or invalid metadata structure"
                )
            ...
            logger.info(
                f"DEBUG: _update_context_entity GlobalContext - new_data keys: {new_data.keys()}"
            )
            ...
            if "global_settings" in new_data:
                ...
                logger.info(
                    "DEBUG: _update_context_entity GlobalContext - using global_settings from new_data"
                )
            else:
                # The entire new_data is the global settings update
                global_settings = new_data
                logger.info(
                    "DEBUG: _update_context_entity GlobalContext - using entire new_data as global_settings"
                )

            logger.info(
                f"DEBUG: _update_context_entity GlobalContext - global_settings: {global_settings}"
            )

            return GlobalContext(
                id=new_data.get("id", existing_entity.id),
                organization_name=new_data.get(
                    "organization_name", existing_entity.organization_name
                ),
    ...
    def auto_create_context_if_missing_old(
        ...
        try:
            ...
            if create_result["success"]:
                logger.info(
                    f"Successfully auto-created context for {level}:{context_id}"
                )
            else:
                logger.warning(
                    f"Failed to auto-create context for {level}:{context_id}: {create_result.get('error')}"
                )
            ...
            logger.error(
                f"Error in auto_create_context_if_missing for {level}:{context_id}: {e}"
            )
            return {"success": False, "error": f"Auto-creation failed: {str(e)}"}

    def _resolve_inheritance_sync(
        self, level: ContextLevel, context_entity: Any, context_data: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
                    ...
                    if branch_id:
                        ...
                        if branch_repo:
                            try:
                                ...
                                if branch_entity:
                                    project_id = getattr(
                                        branch_entity, "project_id", None
                                    )
                            except Exception as e:
                                logger.warning(
                                    f"Could not fetch branch for project lookup: {e}"
                                )
                    ...
                    if project_repo:
                        try:
                            ...
                            if project_entity:
                                ...
                                logger.debug(
                                    f"Added project context {project_id} to inheritance chain"
                                )
                        except Exception as e:
                            logger.warning(
                                f"Could not fetch project context {project_id}: {e}"
                            )
                    ...
                    if branch_repo:
                        try:
                            ...
                            if branch_entity:
                                ...
                                logger.debug(
                                    f"Added branch context {branch_id} to inheritance chain"
                                )
                        except Exception as e:
                            logger.warning(
                                f"Could not fetch branch context {branch_id}: {e}"
                            )
            ...
            current_level_in_chain = any(
                item["level"] == level.value for item in inheritance_chain
            )
    ...
    def _merge_inheritance_chain(
        self, inheritance_chain: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
        ...
        for i in range(1, len(inheritance_chain)):
            ...
            if current_level == "project" and self.inheritance_service:
                ...
                merged = self.inheritance_service.inherit_project_from_global(
                    merged, current_data
                )
            elif current_level == "branch" and self.inheritance_service:
                # Use branch inheritance logic
                merged = self.inheritance_service.inherit_branch_from_project(
                    merged, current_data
                )
            elif current_level == "task" and self.inheritance_service:
                # Use task inheritance logic
                merged = self.inheritance_service.inherit_task_from_branch(
                    merged, current_data
                )
            else:
    ...
    def _build_default_context_data(
        ...
        if level == "global":
            return {
                "organization_name": base_data.get(
                    "organization_name", "Default Organization"
                ),
                "global_settings": base_data.get(
                    "global_settings",
                    {"default_timezone": "UTC", "auto_create_contexts": True},
                ),
            ...
            return {
                "project_name": base_data.get(
                    "project_name", f"Project {context_id[:8]}"
                ),
                "project_settings": base_data.get(
                    "project_settings",
                    {"auto_context_creation": True, "default_branch": "main"},
                ),
                "metadata": {**default_metadata, **base_data.get("metadata", {})},
            }
            ...
            return {
                ...
                "branch_settings": base_data.get(
                    "branch_settings",
                    {"auto_created": True, "workflow_type": "standard"},
                ),
                "metadata": {**default_metadata, **base_data.get("metadata", {})},
            }
            ...
            return {
                ...
                or base_data.get("branch_id")
                or base_data.get("parent_branch_id"),
                "task_data": base_data.get(
                    "task_data",
                    {
                        "title": base_data.get("title", f"Task {context_id[:8]}"),
                        "description": base_data.get(
                            "description", "Auto-created task context"
                        ),
                        "auto_created": True,
                    },
                ),
    ...
    def _ensure_parent_contexts_exist(
        ...
        try:
            logger.info(
                f"Ensuring parent contexts exist for {target_level} context: {context_id}"
            )
            ...
            if target_level == ContextLevel.PROJECT:
                ...
                if not branch_project_id:
                    logger.warning(
                        f"Branch context creation: project_id={project_id}, data.project_id={data.get('project_id') if data else 'None'}"
                    )
                    ...
                    if context_id and "-" in context_id:
                        ...
                        logger.warning(
                            f"Attempting to extract project_id from context_id pattern: {context_id}"
                        )
                ...
                project_result = self._ensure_project_context_exists(
                    branch_project_id, effective_user_id
                )
                ...
                # Ensure project context
                project_result = self._ensure_project_context_exists(
                    task_project_id, effective_user_id
                )
                if project_result["success"]:
                    if project_result.get("created", False):
                        created_contexts.append("project")
                ...
                branch_result = self._ensure_branch_context_exists(
                    branch_id, task_project_id, effective_user_id
                )
            ...
            logger.error(
                f"Exception during parent context creation: {e}", exc_info=True
            )
            return {
                "success": False,
    ...
    def _create_hierarchy_atomically(self, contexts_to_create: list[tuple]) -> bool:
        ...
        try:
            logger.info(
                f"Creating context hierarchy atomically: {len(contexts_to_create)} contexts"
            )
            ...
            try:
                for level, context_id, data in contexts_to_create:
                    ...
                    if success:
                        ...
                        logger.debug(
                            f"Successfully ensured context exists: {level.value}/{context_id}"
                        )
                    else:
                        logger.warning(
                            f"Failed to ensure context exists: {level.value}/{context_id}"
                        )
                ...
                logger.info(
                    f"Hierarchy creation completed. Processed: {len(contexts_to_create)} contexts, Created/Verified: {len(created_contexts)}"
                )
                return True
            ...
            logger.error(
                f"Failed to create context hierarchy atomically: {e}", exc_info=True
            )
            return False

    def _create_context_atomically(
        self, level: ContextLevel, context_id: str, data: dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            if effective_user_id and hasattr(base_repository, "with_user"):
                ...
                logger.debug(
                    f"Atomic creation: Using user-scoped repository for user: {effective_user_id}"
                )
            else:
                repository = base_repository
                logger.warning(
                    f"Atomic creation: Using unscoped repository - no user_id available for {level.value} context"
                )
            ...
            try:
                ...
                logger.debug(
                    f"Context {level.value}/{context_id} does not exist, will create: {e}"
                )
                pass
            ...
            if level == ContextLevel.GLOBAL:
                ...
                if context_id and "_" in context_id and len(context_id.split("_")) == 2:
                    logger.info(
                        "Atomic flow: Found concatenated global context ID, converting to UUID"
                    )
                    import uuid
                    ...
                    if len(parts) == 2:
                        ...
                        context_id = proper_uuid
                        logger.info(
                            f"Atomic flow: Converted global context ID to proper UUID: {proper_uuid}"
                        )
            ...
            try:
                ...
                if saved_context:
                    logger.info(
                        f"Successfully created context atomically: {level.value}/{context_id}"
                    )
                    return True
                else:
                    logger.warning(
                        f"Repository create returned None for {level.value}/{context_id}"
                    )
                    return False
                ...
                if "duplicate" in error_msg or "unique constraint" in error_msg:
                    logger.info(
                        f"Context {level.value}/{context_id} already exists (duplicate key), returning success"
                    )
                    # Context already exists, this is acceptable for idempotent operations
                    return True
                else:
                    logger.error(
                        f"Integrity error creating context {level.value}/{context_id}: {ie}"
                    )
            ...
            logger.error(
                f"Failed atomic context creation for {level.value}/{context_id}: {e}",
                exc_info=True,
            )
            # Try to rollback the session if available
            if hasattr(repository, "session") and repository.session:
    ...
    def _resolve_project_id_from_branch(self, branch_id: str) -> str | None:
        ...
        try:
            ...
            try:
                ...
                with get_session() as session:
                    ...
                    if git_branch and git_branch.project_id:
                        logger.info(
                            f"✅ Found project_id '{git_branch.project_id}' for branch '{branch_id}'"
                        )
                        return git_branch.project_id
                    else:
                        logger.warning(
                            f"⚠️ Git branch '{branch_id}' not found or has no project_id"
                        )
                        return None
            ...
            logger.error(
                f"❌ Error in _resolve_project_id_from_branch for branch '{branch_id}': {e}",
                exc_info=True,
            )
            return None

    def _should_allow_orphaned_creation(
        self, context_level: ContextLevel, context_id: str, data: dict[str, Any]
    ) -> bool:
        """
        ...
        if context_level == ContextLevel.PROJECT:
            ...
            if (
                data.get("auto_created")
                or data.get("project_name", "").startswith("Test")
                or data.get("allow_orphaned_creation")
            ):
                return True
    ...
    def _get_parent_info(self, context_level: ContextLevel, context_entity) -> tuple:
        ...
        if context_level == ContextLevel.PROJECT:
            ...
            namespace = uuid.UUID("a47ae7b9-1d4b-4e5f-8b5a-9c3e5d2f8a1c")
            parent_id = (
                str(uuid.uuid5(namespace, self._user_id)) if self._user_id else None
            )
            ...
            parent_id = getattr(context_entity, "git_branch_id", None) or getattr(
                context_entity, "branch_id", None
            )

        return parent_level, parent_id

    def _invalidate_child_caches(
        self, context_level: ContextLevel, context_id: str, user_id: str, cache
    ):
    ...
    def _ensure_global_context_exists(
        ...
        try:
            ...
            global_repo = self._get_user_scoped_repository(
    ...
    def _ensure_project_context_exists(
        ...
        try:
            ...
            project_repo = self._get_user_scoped_repository(
            ...
            project_data = {
                ...
                "project_settings": {"auto_created": True, "default_branch": "main"},
                "metadata": {
                    "auto_created": True,
                    "created_by": "context_bootstrap",
            ...
            # Use atomic creation to avoid validation cycles
            success = self._create_context_atomically(
                level=ContextLevel.PROJECT, context_id=project_id, data=project_data
            )
    ...
    def _ensure_branch_context_exists(
        self, branch_id: str, project_id: str, user_id: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"🔍 CONTEXT_FIX: Ensuring branch context exists for branch_id: {branch_id}, project_id: {project_id}"
            )

            # Check if branch context exists
            branch_repo = self._get_user_scoped_repository(
            ...
            if branch_repo:
                try:
                    ...
                    if existing:
                        logger.info(
                            f"✅ CONTEXT_FIX: Branch context already exists for {branch_id}"
                        )
                        return {
                            "success": True,
                            "created": False,
                    ...
                    logger.debug(
                        f"⚠️ CONTEXT_FIX: Branch context check failed (will create): {check_error}"
                    )

            # Get git branch info to populate branch data properly
            # Extract UUID string from GitBranchId value object
            branch_id_str = (
                branch_id.value if hasattr(branch_id, "value") else str(branch_id)
            )
            ...
            try:
                ...
                with get_session() as session:
                    git_branch = (
                        session.query(ProjectGitBranch)
                        .filter(ProjectGitBranch.id == branch_id_str)
                        .first()
                    )
                    ...
                    if git_branch:
                        ...
                        if (
                            ...
                            and git_branch.project_id != project_id
                        ):
                            logger.warning(
                                f"⚠️ CONTEXT_FIX: Project ID mismatch - provided: {project_id}, git branch has: {git_branch.project_id}"
                            )
                            # Use the git branch's project_id as it's authoritative
                            project_id = git_branch.project_id
                        logger.info(
                            f"✅ CONTEXT_FIX: Found git branch info - name: {git_branch_name}, project_id: {project_id}"
                        )
                    else:
                        logger.warning(
                            f"⚠️ CONTEXT_FIX: Git branch {branch_id} not found in database, using defaults"
                        )

            except Exception as git_lookup_error:
                logger.debug(
                    f"⚠️ CONTEXT_FIX: Git branch lookup failed, using defaults: {git_lookup_error}"
                )
            ...
            logger.info(
                f"🔧 CONTEXT_FIX: Creating branch context with data: {branch_data}"
            )

            # Use atomic creation to avoid validation cycles
            success = self._create_context_atomically(
                level=ContextLevel.BRANCH, context_id=branch_id, data=branch_data
            )

            if success:
                logger.info(
                    f"✅ CONTEXT_FIX: Successfully created branch context for {branch_id}"
                )
                return {"success": True, "created": True, "context_id": branch_id}
            else:
                logger.error(
                    f"❌ CONTEXT_FIX: Failed to create branch context for {branch_id}"
                )
                return {"success": False, "error": "Atomic context creation failed"}

        except Exception as e:
            logger.error(
                f"❌ CONTEXT_FIX: Exception in _ensure_branch_context_exists for branch {branch_id}: {e}",
                exc_info=True,
            )
            return {"success": False, "error": str(e)}
    ...
    def bootstrap_context_hierarchy(
        ...
        try:
            ...
            if global_result["success"]:
                ...
                errors.append(
                    f"Global context creation failed: {global_result.get('error')}"
                )
            ...
            if project_id:
                ...
                if project_result["success"]:
                    ...
                    errors.append(
                        f"Project context creation failed: {project_result.get('error')}"
                    )

            # Step 3: Create branch context if requested
            if branch_id and project_id:
                branch_result = self._ensure_branch_context_exists(
                    branch_id, project_id, user_id
                )
                if branch_result["success"]:
                    ...
                    errors.append(
                        f"Branch context creation failed: {branch_result.get('error')}"
                    )
            ...
            # Add usage guidance
            result["usage_guidance"] = self._generate_bootstrap_usage_guidance(
                created_contexts
            )
    ...
    def _generate_bootstrap_usage_guidance(
        ...
        if "branch" in created_contexts:
            guidance["next_steps"].append(
                "Branch context is ready for branch-specific workflows"
            )
            guidance["examples"].append(
                {
    ...
    def auto_create_context_if_missing(
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Context {level}/{context_id} does not exist, will create"
                )
                pass
            ...
            if result.get("success", False):
                ...
                logger.warning(
                    f"Failed to auto-create context {level}/{context_id}: {result.get('error', 'Unknown error')}"
                )
                return result
