...
class ContextCacheOptimizer:
    ...
    def get(self, key: str, context_type: str = "unknown") -> Any | None:
        """
        Get cached data
    ...
    def put(
        ...
        with self._lock:
            ...
            logger.debug(
                f"Cached {key} ({context_type}, {size_bytes} bytes, TTL: {entry_ttl}s)"
            )
    ...
    def _get_adaptive_ttl(self, context_type: str, operation_type: str) -> int:
        """
    ...
    def _evict_entry(self) -> bool:
        ...
        if self.strategy == CacheStrategy.LRU:
            ...
            expired = [k for k, entry in self._cache.items() if entry.is_expired()]
            if expired:
    ...
    def _track_access(self, key: str, context_type: str) -> None:
        ...
        self._access_patterns[context_type] = [
            access for access in self._access_patterns[context_type] if access > cutoff
        ]
            ...
            stats = CacheStatsDict(
                {
                    ...
                    "current_size_mb": round(
                        self._metrics["size_bytes"] / (1024 * 1024), 6
                    ),
                    ...
                    "storage": {
                        ...
                        "size_mb": round(
                            self._metrics["size_bytes"] / (1024 * 1024), 2
                        ),
    ...
    def warm_cache(
        self, common_data: dict[str, Any], context_type: str = "warmup"
    ) -> int:
        """
    ...
    def load_from_disk(self, file_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Cache loaded from {file_path}, {len(self._cache)} entries restored"
            )
            return True
