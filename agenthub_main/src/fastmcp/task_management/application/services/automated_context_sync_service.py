...
class AutomatedContextSyncService:
    ...
    def with_user(self, user_id: str) -> AutomatedContextSyncService:
        ...
        return AutomatedContextSyncService(
            self._task_repository, self._subtask_repository, user_id
        )
    ...
    async def sync_task_context_after_update(
        self, task: Task, operation_type: str = "update"
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"[AutomatedContextSync] Syncing task context for {task_id_str} after {operation_type}"
            )
            ...
            if result:
                logger.info(
                    f"[AutomatedContextSync] Successfully synced context for task {task_id_str}"
                )
                return True
            else:
                logger.warning(
                    f"[AutomatedContextSync] Context sync returned None for task {task_id_str}"
                )
                return False

        except Exception as e:
            logger.error(
                f"[AutomatedContextSync] Failed to sync context for task {task.id}: {e}"
            )
            return False

    def sync_task_context_after_update_sync(
        self, task: Task, operation_type: str = "update"
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"[AutomatedContextSync] Context sync triggered for task {task.id} (async context)"
                )
                return True
            except RuntimeError:
                # No event loop - create one
                return asyncio.run(
                    self.sync_task_context_after_update(task, operation_type)
                )

        except Exception as e:
            logger.warning(
                f"[AutomatedContextSync] Sync wrapper failed for task {task.id}: {e}"
            )
            return False
    ...
    async def sync_parent_context_after_subtask_update(
        self,
        parent_task: Task,
        subtask: Subtask | None = None,
        operation_type: str = "subtask_update",
    ) -> bool:
        ...
        try:
            ...
            success = await self.sync_task_context_after_update(
                parent_task, f"parent_{operation_type}"
            )

            if success and progress_summary:
                logger.info(
                    f"[AutomatedContextSync] Updated parent task {parent_task.id} context with subtask progress: {progress_summary}"
                )
            ...
            logger.error(
                f"[AutomatedContextSync] Failed to sync parent context for task {parent_task.id}: {e}"
            )
            return False

    def sync_parent_context_after_subtask_update_sync(
        self,
        parent_task: Task,
        subtask: Subtask | None = None,
        operation_type: str = "subtask_update",
        ...
        try:
            ...
            try:
                ...
                # Already in async context - just log and return
                logger.info(
                    f"[AutomatedContextSync] Parent context sync triggered for task {parent_task.id} (async context)"
                )
                return True
            except RuntimeError:
                # No event loop - create one
                return asyncio.run(
                    self.sync_parent_context_after_subtask_update(
                        parent_task, subtask, operation_type
                    )
                )

        except Exception as e:
            logger.warning(
                f"[AutomatedContextSync] Parent sync wrapper failed for task {parent_task.id}: {e}"
            )
            return False
    ...
    async def _calculate_subtask_progress(
        ...
        try:
            ...
            logger.error(
                f"[AutomatedContextSync] Failed to calculate subtask progress for task {parent_task.id}: {e}"
            )
            return None
        ...
        for task_id in task_ids:
            try:
                ...
                if task:
                    ...
                    logger.warning(
                        f"[AutomatedContextSync] Task {task_id} not found for batch sync"
                    )
                    results[task_id] = False

            except Exception as e:
                logger.error(
                    f"[AutomatedContextSync] Failed to sync task {task_id} in batch: {e}"
                )
                results[task_id] = False

        successful_syncs = sum(1 for success in results.values() if success)
        logger.info(
            f"[AutomatedContextSync] Batch sync completed: {successful_syncs}/{len(task_ids)} successful"
        )
