...
class ContextHierarchyValidator:
        ...
        try:
            ...
            if not global_context:
                ...
                if global_contexts and len(global_contexts) > 0:
                    ...
                    logger.debug(
                        f"Found global context via list() method for user {self.user_id}"
                    )

            # If no global context found, allow creation with warning
            if not global_context:
                logger.warning(
                    f"No global context found for user {self.user_id} during project context validation - allowing creation with auto-creation"
                )
            ...
            logger.warning(
                f"Global context check error for user {self.user_id}: {e} - allowing permissive creation"
            )
    ...
    def _validate_branch_requirements(
        self, branch_id: str, data: dict[str, Any]
    ) -> tuple[bool, str | None, dict[str, Any] | None]:
            ...
            try:
                ...
                if hasattr(self, "branch_repo") and self.branch_repo:
                    ...
                    if git_branch and hasattr(git_branch, "project_id"):
                        ...
                        logger.info(
                            f"Auto-resolved project_id '{project_id}' from git_branch_id '{branch_id}'"
                        )
                        # Add the resolved project_id to the data for consistency
                        data["project_id"] = project_id
            except Exception as e:
                logger.warning(
                    f"Failed to auto-resolve project_id from git_branch_id '{branch_id}': {e}"
                )
        ...
        if not project_id:
            ...
            is_auto_creation = (
                data.get("source") == "task_creation_auto_create"
                or data.get("auto_created") is True
            )

            if is_auto_creation:
                logger.info(
                    f"Allowing branch context creation without project_id for auto-creation scenario (branch_id={branch_id})"
                )
                return (
                    True,
                    None,
            ...
            return (
                False,
                "Branch context requires project_id",
                {
        ...
        try:
            ...
            if not project_context:
                logger.warning(
                    f"Project context '{project_id}' not found - allowing branch creation with auto-creation"
                )
            ...
            return (
                False,
                "Project context must exist first",
                {
    ...
    def _validate_task_requirements(
        ...
        try:
            ...
            git_branch = (
                self.branch_repo.get(branch_id)
                if hasattr(self, "branch_repo") and self.branch_repo
                else None
            )
            ...
            if git_branch:
                branch_exists = True
                project_id = (
                    git_branch.project_id if hasattr(git_branch, "project_id") else None
                )
                logger.debug(
                    f"✅ Branch {branch_id} exists with project_id: {project_id}"
                )
            else:
            ...
            if not branch_exists:
                ...
                logger.warning(
                    f"Branch '{branch_id}' not found in database - allowing task creation with auto-creation"
                )
            ...
            logger.warning(
                f"Branch context check error: {e} - allowing permissive task creation"
            )
            # Be more permissive - allow creation with warning instead of blocking
            return (
                True,
