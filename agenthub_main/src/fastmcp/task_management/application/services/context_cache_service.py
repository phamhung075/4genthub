...
class ContextCacheService:
    ...
    def __init__(
        self, repository=None, default_ttl_hours: int = 1, user_id: str | None = None
    ):
        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    logger.debug(
                        f"Event loop running, skipping cache for {level}:{context_id}"
                    )
                    return None
                else:
                    return loop.run_until_complete(
                        self.get_cached_context(level, context_id)
                    )
            except RuntimeError:
        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    logger.debug(
                        f"Event loop running, skipping cache set for {level}:{context_id}"
                    )
                    return
                else:
                    loop.run_until_complete(
                        self.cache_resolved_context(
                            level,
                            context_id,
                            context,
                            dependencies_hash,
                ...
                asyncio.run(
                    self.cache_resolved_context(
                        level, context_id, context, dependencies_hash, resolution_path
                    )
                )
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    logger.debug(
                        f"Event loop running, skipping cache invalidation for {level}:{context_id}"
                    )
                    return
                else:
                    loop.run_until_complete(
                        self.invalidate_context_cache(level, context_id)
                    )
            except RuntimeError:
                # No event loop exists, create one
            ...
            logger.warning(
                f"Error in sync cache invalidation for {level}:{context_id}: {e}"
            )
    ...
    async def invalidate(
        self, level: str, context_id: str, reason: str = "manual_invalidation"
    ) -> bool:
    ...
    async def _update_hit_stats(self, level: str, context_id: str) -> None:
        ...
        try:
            await self.repository.update_cache_stats(
                level,
                context_id,
                {
    ...
    async def cache_resolved_context(
        self,
        level: str,
        context_id: str,
        ...
        try:
            ...
            logger.debug(
                f"Cached context {level}:{context_id} (size: {cache_size} bytes, expires: {expires_at})"
            )
            return True
    ...
    async def invalidate_dependent_caches(
        self, level: str, context_id: str, reason: str = "dependency_changed"
    ) -> list[str]:
        """
        ...
        try:
            ...
            if level == "global":
                ...
                project_caches = await self.repository.get_cache_entries_by_level(
                    "project"
                )
                ...
                task_caches = await self.repository.get_task_caches_by_project(
                    context_id
                )

                for cache in task_caches:
                    await self.invalidate_context_cache(
                        "task", cache["context_id"], reason
                    )
                    invalidated.append(f"task:{cache['context_id']}")

            logger.info(
                f"Invalidated {len(invalidated)} dependent caches for {level}:{context_id}"
            )
            return invalidated
        ...
        try:
            ...
            for entry in expired_entries:
                await self.repository.remove_cache_entry(
                    entry["context_level"], entry["context_id"]
                )
                removed_count += 1
            ...
            if removed_count > 0:
                logger.info(
                    f"Cleaned up {removed_count} expired cache entries, freed {total_size_freed} bytes"
                )
    ...
    async def cleanup_invalidated(self) -> dict[str, Any]:
        ...
        try:
            ...
            if removed_count > 0:
                logger.info(
                    f"Cleaned up {removed_count} invalidated cache entries, freed {total_size_freed} bytes"
                )
    ...
    async def clear_all_cache(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                f"Cleared all cache entries: {removed_count} entries removed"
            )
            return result
    ...
    async def optimize_cache(self) -> dict[str, Any]:
        ...
        try:
            ...
            if cleanup_result.get("success"):
                ...
                optimization_result["space_freed_bytes"] += cleanup_result.get(
                    "size_freed_bytes", 0
                )
                optimization_result["entries_removed"] += cleanup_result.get(
                    "removed_count", 0
                )
            ...
            if invalidated_result.get("success"):
                ...
                optimization_result["space_freed_bytes"] += invalidated_result.get(
                    "size_freed_bytes", 0
                )
                optimization_result["entries_removed"] += invalidated_result.get(
                    "removed_count", 0
                )
            ...
            if total_entries > 500:  # Cache pressure threshold
                low_hit_entries = await self.repository.get_low_hit_cache_entries(
                    hit_threshold=2, limit=50
                )
                ...
                for entry in low_hit_entries:
                    ...
                    removed_low_hit += 1
                    optimization_result["space_freed_bytes"] += entry.get(
                        "cache_size_bytes", 0
                    )

                if removed_low_hit > 0:
                    optimization_result["actions_taken"].append(
                        f"removed_{removed_low_hit}_low_hit_entries"
                    )
                    optimization_result["entries_removed"] += removed_low_hit

            optimization_result["optimization_completed"] = datetime.now(
                UTC
            ).isoformat()
    ...
    async def warm_cache(
        self, contexts_to_warm: list[dict[str, str]]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            for context_spec in contexts_to_warm:
                ...
                try:
                    ...
                    failed_count += 1
                    logger.warning(
                        f"Failed to warm cache for {level}:{context_id}: {e}"
                    )
    ...
    def get_cache_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            health_metrics = {
                ...
                "cache_hit_rate": stats.get("performance_metrics", {}).get(
                    "hit_rate_estimated", 0.0
                ),
                "cache_miss_rate": 1.0
                - stats.get("performance_metrics", {}).get("hit_rate_estimated", 0.0),
                "average_resolution_time_ms": stats.get(
                    "average_resolution_time_ms", 0.0
                ),
                "expired_entries": stats.get("health_indicators", {}).get(
                    "expired_entries", 0
                ),
            }
            ...
            return {
                "cache_entries": {"global": 0, "project": 0, "branch": 0, "task": 0},
                "cache_hit_rate": 0.0,
                "cache_miss_rate": 0.0,
                "average_resolution_time_ms": 0.0,
                "expired_entries": 0,
