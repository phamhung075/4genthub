...
class ProgressTrackingService:
    ...
    def with_user(self, user_id: str) -> ProgressTrackingService:
        ...
        return ProgressTrackingService(
            self.task_repository, self.context_repository, self.event_bus, user_id
        )
    ...
    async def update_progress(
        ...
        await self._update_context_progress(
            task, progress_type, percentage, description
        )
    ...
    async def batch_update_progress(self, updates: list[dict[str, Any]]) -> list[Task]:
        """
        ...
        for update in updates:
            try:
                ...
                logger.error(
                    f"Failed to update progress for task {update.get('task_id')}: {e}"
                )
    ...
    async def get_progress_timeline(
        self, task_id: str, hours: int = 24, progress_type: ProgressType | None = None
    ...
    async def set_progress_milestone(
        self, task_id: str, milestone_name: str, percentage: float
    ) -> Task:
        """
    ...
    async def infer_progress_from_context(self, task_id: str) -> float | None:
        ...
        if context.insights:
            recent_text += " ".join(
                [i.get("content", "") for i in context.insights[-5:]]
            )
        if context.progress:
            recent_text += " ".join(
                [p.get("content", "") for p in context.progress[-5:]]
            )
        ...
        if any(word in recent_text for word in progress_indicators["completed"]):
            ...
            word in recent_text for word in progress_indicators["near_completion"]
        ):
            return 85.0
    ...
    async def suggest_progress_update(self, task_id: str) -> dict[str, Any]:
        ...
        suggestion = {"task_id": task_id, "suggested_updates": []}
        ...
        if (
            "implement" in task_content
            or "develop" in task_content
            or "code" in task_content
        ):
            suggestion["suggested_updates"].append(
                {
    ...
    async def _update_context_progress(
        self,
        task: Task,
        progress_type: ProgressType,
        percentage: float,
        ...
        try:
            ...
            if context:
                ...
                progress_entry = {
                    ...
                    "percentage": percentage,
                    "description": description
                    or f"Updated {progress_type.value} progress to {percentage}%",
                }
    ...
    async def _aggregate_subtask_progress(self, parent_task: Task) -> None:
        ...
        for subtask in parent_task.subtasks:
            ...
            if subtask_id:
                try:
                    ...
                    subtask_entity = await self.task_repository.get_by_id(
                        TaskId(subtask_id)
                    )
                    if subtask_entity:
        ...
        new_progress = ProgressCalculationStrategy.calculate_from_subtasks(
            subtask_details
        )
