...
class ProjectApplicationService:
    ...
    def __init__(
        self, project_repository: ProjectRepository, user_id: str | None = None
    ):
        ...
        self._get_project_use_case = GetProjectUseCase(
            self._get_user_scoped_repository()
        )
        self._list_projects_use_case = ListProjectsUseCase(
            self._get_user_scoped_repository()
        )
        self._update_project_use_case = UpdateProjectUseCase(
            self._get_user_scoped_repository()
        )
        self._create_git_branch_use_case = CreateGitBranchUseCase(
            self._get_user_scoped_repository()
        )
        self._project_health_check_use_case = ProjectHealthCheckUseCase(
            self._get_user_scoped_repository()
        )
        ...
        if hasattr(self._project_repository, "with_user") and self._user_id:
            ...
            if self._user_id and self._project_repository.user_id != self._user_id:
                ...
                if hasattr(self._project_repository, "session"):
                    return repo_class(
                        self._project_repository.session, user_id=self._user_id
                    )
    ...
    async def create_project(
        ...
        return await self._create_project_use_case.execute(
    ...
    async def update_project(
        ...
        return await self._update_project_use_case.execute(
            project_id, name, description
        )
    ...
    async def create_git_branch(
        ...
        return await self._create_git_branch_use_case.execute(
    ...
    async def assign_agent_to_tree(
