...
class GitBranchServiceWrapper:
    ...
    def __init__(self, git_branch_service: GitBranchService | None = None):
    ...
    def create_git_branch(
        ...
        return self._run_async(
            self._git_branch_service.create_git_branch(
    ...
    def list_git_branchs(self, project_id: str) -> dict[str, Any]:
        ...
        return self._run_async(self._git_branch_service.list_git_branchs(project_id))
    ...
    def update_git_branch(
        ...
        return self._run_async(
            self._git_branch_service.update_git_branch(
                git_branch_id, git_branch_name, git_branch_description
            )
        )
    ...
    def assign_agent_to_branch(
        self, project_id: str, agent_id: str, git_branch_name: str
    ) -> dict[str, Any]:
        """Assign an agent to a git branch"""
        return self._run_async(
            self._git_branch_service.assign_agent_to_branch(
                project_id, agent_id, git_branch_name
            )
        )

    def unassign_agent_from_branch(
        self, project_id: str, agent_id: str, git_branch_name: str
    ) -> dict[str, Any]:
        """Unassign an agent from a git branch"""
        return self._run_async(
            self._git_branch_service.unassign_agent_from_branch(
                project_id, agent_id, git_branch_name
            )
        )
