        ...
        for variable in self.variables:
            if variable.required and variable.name not in context:
                raise TemplateValidationError(
                    f"Required variable '{variable.name}' missing from context"
                )
        return True
...
class ContextTemplateManager:
    ...
    def _apply_inheritance(
        self, operation: OperationType, template: dict[str, list[str]]
    ) -> dict[str, list[str]]:
        """
    ...
    def load_custom_templates(self, templates_path: str) -> None:
        ...
        try:
            ...
            if path.exists():
                ...
                if "templates" in custom_data:
                    for op_name, template in custom_data["templates"].items():
                        try:
                            ...
                            logger.warning(
                                f"Unknown operation type in custom templates: {op_name}"
                            )
                ...
                logger.info(
                    f"Loaded {len(self.custom_templates)} custom templates from {templates_path}"
                )
    ...
    def save_templates(self, output_path: str) -> None:
        ...
        try:
            templates_dict = {
                ...
                "templates": {
                    op.value: template for op, template in self.templates.items()
                },
            }
    ...
    def validate_template(
        self, operation: OperationType, required_contexts: list[str]
    ) -> bool:
        """
        ...
        for context_type in required_contexts:
            if context_type not in template:
                logger.warning(
                    f"Template for {operation.value} missing required context: {context_type}"
                )
                return False
    ...
    def get_minimal_context(
        self, operation: OperationType, available_data: dict[str, dict[str, Any]]
    ...
    def suggest_template_improvements(
        self, operation: OperationType, actual_usage: dict[str, list[str]]
    ) -> dict[str, Any]:
        """
        ...
        total_unused = sum(
            len(fields) for fields in suggestions["unused_fields"].values()
        )
        total_fields = sum(
            len(fields) if fields != ["*"] else 50 for fields in template.values()
        )
    ...
    def estimate_savings(self) -> dict[str, float]:
        ...
        total_possible = (
            self._metrics["fields_requested"] + self._metrics["fields_saved"]
        )
        field_reduction = (
            (self._metrics["fields_saved"] / total_possible) * 100
            if total_possible > 0
            else 0
        )
        ...
        return {
            ...
            "cache_hit_rate": round(
                ...
                if self._metrics["templates_used"] > 0
                else 0,
                1,
            ),
        }
