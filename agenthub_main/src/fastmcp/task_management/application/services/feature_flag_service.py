...
class FeatureFlagService:
    ...
    def _load_flags(self) -> None:
        ...
        try:
            if os.path.exists(self._config_path):
                ...
                logger.info(
                    f"Loaded {len(self._flags)} feature flags from {self._config_path}"
                )
            else:
                logger.info(
                    f"No existing feature flags file found at {self._config_path}"
                )
    ...
    def _save_flags(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Saved {len(self._flags)} feature flags to {self._config_path}"
            )
    ...
    def enable_flag(
        self, flag_name: str, metadata: dict[str, Any] | None = None
    ) -> bool:
    ...
    def _update_flag(
        self, flag_name: str, enabled: bool, metadata: dict[str, Any] | None = None
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Feature flag '{flag_name}' changed from {old_state} to {enabled}"
            )
            return True
    ...
    def rollback_migration(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"ROLLBACK: Disabled all migration flags ({len(migration_flags)} flags)"
            )
            return True
