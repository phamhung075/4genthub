    ...
    def update_task_progress_from_subtasks(self, task_id: str) -> float | None:
        ...
        try:
            ...
            if not subtasks:
                ...
                logger.debug(
                    f"Task {task_id} has no subtasks, keeping progress at current value"
                )
            ...
            for subtask in subtasks:
                ...
                if subtask.is_completed:
                    ...
                    hasattr(subtask, "status") and str(subtask.status) == "in_progress"
                ):
            ...
            progress_percentage = (
                total_progress / total_subtasks if total_subtasks > 0 else 0
            )
            ...
            with get_session() as session:
                ...
                if task_model:
                    ...
                    session.commit()
                    logger.info(
                        f"Updated task {task_id} progress to {progress_percentage}%"
                    )
                else:
