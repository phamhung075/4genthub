...
class DistributionPlan:
    ...
    assignments: list[tuple[str, str, str]] = field(
    ...
    def add_assignment(
        self, task_id: str, agent_id: str, role: str = "assignee"
    ) -> None:
...
class WorkDistributionService:
    ...
    async def _distribute_round_robin(
        self, tasks: list[Task], agents: list[Agent], plan: DistributionPlan
    ) -> None:
    ...
    async def _distribute_priority_based(
        ...
        sorted_tasks = sorted(
            ...
            key=lambda t: priority_order.get(
                ...
                else (t.priority if t.priority else "none"),
                1,
            ),
            reverse=True,
        )
        ...
        sorted_agents = sorted(
            agents, key=lambda a: self._get_agent_performance_score(a), reverse=True
        )
        ...
        for task in sorted_tasks:
            ...
            for agent in sorted_agents:
                if (
                    agent.is_available()
                    and agent_assignments[agent.id] < agent.max_concurrent_tasks
                ):
                    plan.add_assignment(
                        task.id.value if hasattr(task.id, "value") else task.id,
                        agent.id,
            ...
            if not assigned:
                plan.mark_unassignable(
                    task.id.value if hasattr(task.id, "value") else task.id,
                    "No suitable agent for priority task",
                )
    ...
    async def _distribute_hybrid(
        ...
        for task in tasks:
            ...
            if task_priority_value and task_priority_value in [
                PriorityLevel.CRITICAL.label,
                PriorityLevel.HIGH.label,
            ]:
        ...
        for task in priority_tasks:
            for agent in best_agents:
                if (
                    agent.is_available()
                    and agent_utilization[agent.id] < agent.max_concurrent_tasks
                ):
                    plan.add_assignment(
                        task.id.value if hasattr(task.id, "value") else task.id,
                        agent.id,
        ...
        for task in skilled_tasks:
            ...
            if matching_agents:
                ...
                plan.mark_unassignable(
                    task.id.value if hasattr(task.id, "value") else task.id,
                    "No agent with required skills available",
                )
        ...
        for task in regular_tasks:
            ...
            available_agents = [
                a for a in agents if agent_utilization[a.id] < a.max_concurrent_tasks
            ]

            if available_agents:
                least_loaded = min(
                    available_agents, key=lambda a: agent_utilization[a.id]
                )
                plan.add_assignment(
                    task.id.value if hasattr(task.id, "value") else task.id,
                    least_loaded.id,
    ...
    def _agent_matches_requirements(
        self, agent: Agent, requirements: TaskRequirements
    ) -> bool:
    ...
    async def _execute_distribution_plan(self, plan: DistributionPlan) -> None:
        ...
        for task_id, agent_id, role in plan.assignments:
            try:
                ...
                logger.info(
                    f"Assigned task {task_id} to agent {agent_id} with role {role}"
                )
    ...
    def _record_distribution(
        self, plan: DistributionPlan, strategy: DistributionStrategy
    ) -> None:
    ...
    async def get_distribution_analytics(self) -> dict[str, Any]:
        ...
        strategy_success_rates = {
            strategy: (data["assigned"] / data["total"] * 100)
            if data["total"] > 0
            else 0
        ...
        return {
            ...
            "overall_assignment_rate": (total_assigned / total_tasks * 100)
            if total_tasks > 0
            else 0,
            ...
            "recent_unassignable_reasons": [
                reason
                for record in self.distribution_history[-10:]
