...
class AgentCoordinationService:
    ...
    async def assign_agent_to_task(
        ...
        if not agent.is_available():
            raise AgentCoordinationException(
                f"Agent {agent_id} is not available for new assignments"
            )
    ...
    async def request_work_handoff(
        ...
        logger.info(
            f"Work handoff requested from {from_agent_id} to {to_agent_id} for task {task_id}"
        )

        return handoff

    async def accept_handoff(
        self, handoff_id: str, agent_id: str, notes: str | None = None
    ) -> None:
        ...
        if handoff.to_agent_id != agent_id:
            raise AgentCoordinationException(
                f"Agent {agent_id} is not the target of this handoff"
            )
    ...
    async def detect_and_resolve_conflict(
        ...
        if resolution_strategy:
            await self.resolve_conflict(
                ...
                resolved_by="agent_coordination_service",
    ...
    async def rebalance_workload(
        ...
        workload_before = {agent.id: len(agent.active_tasks) for agent in agents}

        # Find overloaded and underutilized agents
        overloaded = [a for a in agents if a.get_workload_percentage() > 80]
        underutilized = [
            a for a in agents if a.get_workload_percentage() < 40 and a.is_available()
        ]
        ...
        workload_after = {agent.id: len(agent.active_tasks) for agent in agents_after}
        ...
        event = AgentWorkloadRebalanced(
            ...
            agents_affected=list(
                set(list(workload_before.keys()) + list(tasks_reassigned.values()))
            ),
        ...
        logger.info(
            f"Workload rebalanced for project {project_id}: {len(tasks_reassigned)} tasks reassigned"
        )
    ...
    async def get_agent_workload(self, agent_id: str) -> dict[str, Any]:
        ...
        active_assignments = [
            ...
            and assignment.task_id in agent.active_tasks
        ]
        ...
        pending_handoffs_to = [
            ...
            if handoff.to_agent_id == agent_id
            and handoff.status == HandoffStatus.PENDING
        ]
