...
class AuditService:
    ...
    def log_operation(
        ...
        if isinstance(compliance_level, ComplianceLevel):
            ...
            logger.warning(
                f"Received string compliance level instead of ComplianceLevel enum: {compliance_level}"
            )
            ...
            if compliance_level_value not in valid_levels:
                logger.warning(
                    f"Invalid compliance level '{compliance_level_value}', using 'low' as default"
                )
    ...
    def generate_compliance_report(self) -> dict[str, Any]:
        ...
        try:
            ...
            recent_violations = [
                entry
                for entry in self._audit_log[-100:]  # Last 100 entries
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        if compliance_rate < 85:
            recommendations.append(
                "Overall compliance below target (85%). Review failed operations."
            )

        if self._compliance_metrics["violations"] > 10:
            recommendations.append(
                "High number of violations detected. Implement additional validation."
            )

        # Analyze common failure patterns
        recent_failures = [
            entry for entry in self._audit_log[-50:] if not entry["success"]
        ]
        if len(recent_failures) > 5:
            recommendations.append(
                "Frequent failures detected. Review system stability."
            )
