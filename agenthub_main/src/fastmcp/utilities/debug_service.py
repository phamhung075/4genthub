...
class DebugService:
    ...
    def __init__(self):
        ...
        self.debug_enabled = (
            os.getenv("DEBUG_SERVICE_ENABLED", "false").lower() == "true"
        )
        ...
        self.debug_frontend = (
            os.getenv("DEBUG_FRONTEND_ISSUES", "false").lower() == "true"
        )

        # Debug output controls
        self.debug_verbose = os.getenv("DEBUG_VERBOSE", "false").lower() == "true"
        self.debug_stack_traces = (
            os.getenv("DEBUG_STACK_TRACES", "false").lower() == "true"
        )

        # Create debug loggers
        self.logger = logging.getLogger("agenthub.debug.service")
        self.http_logger = logging.getLogger("agenthub.debug.http")
        self.api_logger = logging.getLogger("agenthub.debug.api")
        self.auth_logger = logging.getLogger("agenthub.debug.auth")
        self.db_logger = logging.getLogger("agenthub.debug.database")
        ...
        if body and self.debug_verbose:
            try:
                ...
                self.http_logger.debug(
                    f"📦 Request Body (raw): {body[:200]}... [Parse error: {e}]"
                )
    ...
    def log_response(
        ...
        self.http_logger.debug(
            f"{status_icon} RESPONSE: {status_code} ({duration:.3f}s)"
        )
        ...
        if body and self.debug_verbose:
            try:
                ...
                self.http_logger.debug(
                    f"📦 Response Body (raw): {body[:200]}... [Parse error: {e}]"
                )

        self.http_logger.debug("=" * 80)

    def log_api_v2_request(
        self, endpoint: str, user_id: str | None, method: str, params: dict[str, Any]
    ):
        ...
        self.api_logger.debug(
            f"📋 Parameters: {json.dumps(params, indent=2) if params else 'None'}"
        )

    def log_api_v2_response(
        self,
        endpoint: str,
        success: bool,
        ...
        if success and data:
            if "tasks" in data:
                ...
                if task_count > 0 and self.debug_verbose:
                    ...
                    self.api_logger.debug(
                        f"📝 First task: {first_task.get('title', 'N/A')} [{first_task.get('status', 'N/A')}]"
                    )
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self.logger.debug(
                        f"✅ COMPLETED: {func.__name__} ({duration:.3f}s)"
                    )
                    return result
                except Exception as e:
                    duration = time.time() - start_time
                    self.logger.debug(
                        f"❌ FAILED: {func.__name__} ({duration:.3f}s) - {str(e)}"
                    )
                    if self.debug_stack_traces:
                        import traceback
