...
This script sets up the agenthub task management system for any project location.
...
def setup_project_structure(project_path: Path) -> bool:
    ...
    try:
        ...
        files_to_create = {
            ...
            cursor_rules_dir
            / "brain"
            / "projects.json": None,  # Handle specially below
            cursor_rules_dir
...
This project uses the agenthub task management system.
...
def create_mcp_config(project_path: Path, agenthub_path: Path | None = None) -> bool:
    ...
    Args:
        project_path: Path to the project root
        agenthub_path: Path to agenthub_main directory (optional)
    ...
    try:
        # Auto-detect agenthub_main if not provided
        if agenthub_path is None:
            # Look for agenthub_main in the project or parent directories
            search_paths = [
                project_path / "agenthub_main",
                project_path.parent / "agenthub_main",
                Path.cwd() / "agenthub_main",
            ]
            ...
            if agenthub_path is None:
                print("❌ Could not find agenthub_main directory")
                print(
                    "   Please ensure agenthub_main is in the project or parent directory"
                )
                return False

        print(f"Using agenthub_main at: {agenthub_path}")

        # Check if virtual environment exists
        venv_python = agenthub_path / ".venv" / "bin" / "python"
        if not venv_python.exists():
            print(f"❌ Virtual environment not found at: {venv_python}")
            print(
                "   Please run: cd agenthub_main && python -m venv .venv && source .venv/bin/activate && pip install -e ."
            )
            return False
        ...
        mcp_config = {
            "mcpServers": {
                "agenthub": {
                    ...
                    "env": {
                        "PYTHONPATH": str(agenthub_path / "src"),
                        "PROJECT_ROOT_PATH": ".",
