...
os.environ["AGENTHUB_AUTH_ENABLED"] = "false"
...
def diagnose_environment():
    ...
    print(
        f"AGENTHUB_AUTH_ENABLED: {os.environ.get('AGENTHUB_AUTH_ENABLED', 'NOT SET')}"
    )
    print(
        f"AGENTHUB_DISABLE_CURSOR_TOOLS: {os.environ.get('AGENTHUB_DISABLE_CURSOR_TOOLS', 'NOT SET')}"
    )
    print()
...
def diagnose_server_creation():
    ...
    try:
        ...
        print(
            f"Task management enabled: {getattr(server, '_enable_task_management', 'Unknown')}"
        )
        ...
        core_tools = [
            t for t in tools if t in ["health_check", "get_server_capabilities"]
        ]
        ...
        if "health_check" in tools:
            print("\n=== HEALTH CHECK ===")
            health_tool = server._tools.get("health_check") or server.tools.get(
                "health_check"
            )
            if health_tool:
                try:
...
def main():
    print("AGENTHUB MCP TOOLS DIAGNOSTIC")
    print("=" * 50)
