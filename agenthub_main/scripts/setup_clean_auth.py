...
class AuthenticationSetup:
    ...
    def validate_keycloak_config(self) -> dict[str, Any]:
        ...
        if missing_vars:
            ...
            logger.info(
                "Please update your .env file with actual Keycloak cloud values"
            )
            return {}
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    realm_url, timeout=10.0, follow_redirects=True
                )
            ...
            for _ in range(30):
                result = subprocess.run(
                    [
                        ...
                        "pg_isready",
                        "-U",
                        os.getenv("DATABASE_USER", "agenthub_user"),
                    ],
                    cwd=self.project_root,
                    capture_output=True,
                    check=False,
    ...
    async def run(self) -> bool:
        ...
        if not self.check_prerequisites():
            logger.error(
                "Prerequisites not met. Please install Docker and Docker Compose."
            )
            return False
