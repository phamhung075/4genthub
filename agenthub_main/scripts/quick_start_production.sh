...
echo "=================================================="
echo "agenthub Production Quick Start"
...
echo "   - Keycloak Realm: ${KEYCLOAK_REALM:-agenthub}"
...
for i in {1..30}; do
    if docker-compose exec postgres pg_isready -U ${DATABASE_USER:-agenthub_user} -d ${DATABASE_NAME:-agenthub_prod} &>/dev/null; then
...
echo "     python agenthub_main/scripts/test_keycloak_mcp_integration.py"
echo ""
echo "  2. View logs:"
echo "     docker-compose logs -f mcp-server"
echo ""
echo "  3. Access PostgreSQL:"
echo "     docker-compose exec postgres psql -U ${DATABASE_USER:-agenthub_user} -d ${DATABASE_NAME:-agenthub_prod}"
echo ""
