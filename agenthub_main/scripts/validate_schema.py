...
class ColumnMismatch:
    ...
    column_name: str
    issue_type: (
        str  # 'missing_in_db', 'missing_in_model', 'type_mismatch', 'nullable_mismatch'
    )
...
class SchemaValidator:
    ...
    def validate_all_tables(self) -> list[ColumnMismatch]:
        ...
        logger.info(
            f"Schema validation completed. Found {len(self.mismatches)} issues."
        )
    ...
    def print_report(self):
        ...
        if errors:
            print(
                "1. Fix critical errors immediately - they may cause application failures"
            )
        if user_id_issues:
            print(
                "2. Run the user isolation migration: 004_fix_user_isolation_missing_columns.sql"
            )
        if warnings:
    ...
    with engine.connect() as conn:
        for issue in issues:
            if issue.issue_type == "missing_in_db" and issue.column_name == "user_id":
                ...
                issue.issue_type == "nullable_mismatch"
                and issue.column_name == "user_id"
            ):
                try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate database schema against SQLAlchemy models"
    )
    parser.add_argument(
        "--fix",
        action="store_true",
        help="Attempt to fix critical issues automatically",
    )
