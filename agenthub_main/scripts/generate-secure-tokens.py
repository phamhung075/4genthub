...
def main():
    print("🔐 agenthub Secure Token Generator")
    print("=" * 50)
    print(f"Generated on: {datetime.now().isoformat()}")
    print()
    
    # Generate new tokens
    new_agenthub_token = generate_mcp_token()
    new_api_token = generate_api_token()
    new_jwt_secret = generate_jwt_secret()
    
    print("🔑 GENERATED SECURE TOKENS:")
    print("-" * 30)
    print(f"AGENTHUB_TOKEN={new_agenthub_token}")
    print(f"AGENTHUB_API_TOKEN={new_api_token}")  
    ...
    with open(env_file, 'w') as f:
        ...
        f.write(f"AGENTHUB_TOKEN={new_agenthub_token}\n")
        f.write(f"AGENTHUB_API_TOKEN={new_api_token}\n")
