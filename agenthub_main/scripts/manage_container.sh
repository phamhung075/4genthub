...
start_container() {
    log "🚀 Starting agenthub container..."
...
stop_container() {
    log "🛑 Stopping agenthub container..."
...
restart_container() {
    log "🔄 Restarting agenthub container..."
    stop_container
    sleep 2
    start_container
}
...
cleanup_duplicates() {
    ...
    # Find all containers with agenthub in the name
    DUPLICATE_CONTAINERS=$(docker ps -a --filter "name=agenthub" --format "{{.Names}}" | grep -v "^${CONTAINER_NAME}$" || true)
...
reset_container() {
    log "🔄 Resetting agenthub container..."
...
show_help() {
    echo "agenthub Container Management"
    echo "================================"
    echo ""
