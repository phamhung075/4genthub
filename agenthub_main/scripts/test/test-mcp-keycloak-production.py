        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"   Error: {error_data.get('error_description', response.text)}"
                    )
                    return False
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"   Database: {health_data.get('database', {}).get('status')}"
                    )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"   ✅ Projects listed: {len(projects.get('projects', []))} found"
                    )
                else:
                    logger.error(
                        f"   ❌ Failed to list projects: {response.status_code}"
                    )
                    return False
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 201]:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(
                            f"   ⚠️  Task update returned: {response.status_code}"
                        )
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(
                            f"   ⚠️  Task completion returned: {response.status_code}"
                        )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 201]:
                    ...
                    logger.warning(
                        f"   ⚠️  Global context creation returned: {response.status_code}"
                    )
                ...
                response = await client.post(
                    ...
                    json={
                        "action": "update",
                        "level": "global",
                        "data": {"last_test": time.time(), "test_status": "successful"},
                    },
                    headers=headers,
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        f"   ⚠️  Context update returned: {response.status_code}"
                    )
                    return False
...
async def main():
    ...
    parser.add_argument("--username", help="Keycloak username for testing")
    parser.add_argument("--password", help="Keycloak password for testing")
    ...
    # Run tests
    tester = MCPKeycloakTester(username=args.username, password=args.password)
