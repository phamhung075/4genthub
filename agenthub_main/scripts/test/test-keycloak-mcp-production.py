...
class KeycloakMCPTester:
    ...
    async def test_keycloak_connection(self) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(
                verify=False
            ) as client:  # Disable SSL verify for testing
                ...
                if response.status_code == 200:
                    ...
                    print_error(
                        f"Failed to get Keycloak configuration: {response.status_code}"
                    )
                    return False
        ...
        try:
            async with httpx.AsyncClient(verify=False) as client:
                ...
                if response.status_code == 200:
                    ...
                    print_info(
                        f"Access token received (expires in {data.get('expires_in', 'N/A')} seconds)"
                    )
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                response = await client.get(
                    f"{self.mcp_url}/mcp/tools", headers=headers
                )
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    print_info(
                        f"New token expires in {data.get('expires_in', 'N/A')} seconds"
                    )
                    return True
    ...
    async def run_all_tests(self):
        ...
        print_header("agenthub Keycloak Integration Test Suite")
