        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"❌ MCP authentication failed: {response.status_code}"
                    )
                    logger.error(f"  Response: {response.text}")
                    return False
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await client.post(
                    ...
                    json={"action": "get", "level": "global", "context_id": "global"},
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"❌ MCP token authentication failed: {response.status_code}"
                    )
                    logger.error(f"  Response: {response.text}")
                    return False
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for token in tokens[:3]:  # Show first 3 tokens
                        logger.info(
                            f"  - {token.get('name')}: Created {token.get('created_at')}"
                        )
    ...
    async def test_revoke_token(self) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                tokens = response.json()
                test_token = next(
                    (t for t in tokens if t["name"] == "Test Token"), None
                )
