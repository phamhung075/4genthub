    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print_error(
                    f"Failed to connect to Keycloak (HTTP {response.status_code})"
                )
                return False
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print_error(
                    f"Login failed: {error_data.get('error_description', 'Unknown error')}"
                )
                return None
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print_error(
                    f"MCP Server health check failed (HTTP {response.status_code})"
                )
                return False
...
async def run_all_tests():
    ...
    print_colored("\n" + "=" * 60, BLUE)
    print_colored("agenthub Authentication Test Suite", BLUE)
