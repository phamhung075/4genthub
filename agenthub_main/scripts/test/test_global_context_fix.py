...
def test_global_context_normalization():
    ...
    try:
        ...
        if result.get("success"):
            ...
            print(
                f"   Retrieved data: {context_data.get('global_settings', {}).get('test', 'N/A')}"
            )
        else:
            print(f"   ❌ FAILED: {result.get('error')}")
            return False
        ...
        result = facade.update_context(
            level="global",
            context_id="global",  # Using "global" instead of UUID
            data={"updated": "This field was updated using 'global' as context_id"},
        )
        ...
        if result.get("success"):
            ...
            print(
                f"   Has 'updated' field: {'updated' in resolved_data.get('global_settings', {})}"
            )
        else:
            print(f"   ❌ FAILED: {result.get('error')}")
            return False
        ...
        print(
            "   It gets automatically translated to the user's actual global context UUID"
        )
