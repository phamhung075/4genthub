...
sys.path.insert(0, str(Path(__file__).parent / "agenthub_main" / "src"))
...
async def main():
    ...
    print(
        f"   Database: PostgreSQL @ {os.getenv('DATABASE_HOST')}:{os.getenv('DATABASE_PORT')}"
    )
    ...
    if keycloak_ok and postgres_ok and mcp_ok:
        ...
        if not postgres_ok:
            print("\n📌 PostgreSQL Setup:")
            print(
                "   1. Run: docker-compose -f docker-compose.production.yml up -d postgres"
            )
