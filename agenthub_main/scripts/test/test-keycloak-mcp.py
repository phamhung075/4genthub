...
# Configuration
MCP_SERVER_URL = (
    os.getenv("MCP_HOST", "http://localhost") + ":" + os.getenv("MCP_PORT", "8001")
)
...
class KeycloakMCPTester:
    ...
    def test_keycloak_connection(self) -> bool:
        ...
        if KEYCLOAK_URL == "https://your-keycloak.cloud.provider.com":
            ...
            self.print_warning(
                "Please update KEYCLOAK_URL with your actual Keycloak instance"
            )
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_error(
                    f"Failed to connect to Keycloak: {response.status_code}"
                )
                return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(token_parts) >= 2:
                    ...
                    print(
                        f"  Client roles: {', '.join(client_roles) if client_roles else 'None'}"
                    )
    ...
    def test_mcp_auth(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.print_warning(
                    "User needs 'mcp-user' or 'mcp-tools' role in Keycloak"
                )
                return False
            else:
        ...
        for tool in tools_to_test:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.print_error(
                        f"  {tool['name']} - Failed ({response.status_code})"
                    )
                    all_passed = False
    ...
    def run_tests(self):
        ...
        if results["keycloak_connection"]:
            ...
            if username and password:
                ...
                self.print_warning(
                    "Skipping authentication test - no credentials provided"
                )
