...
async def test_postgresql_connection():
    ...
    try:
        ...
        config.database_password = os.getenv(
...
async def test_keycloak_auth_provider():
    ...
    try:
        ...
        if config:
            ...
            print_info(
                f"Authorization endpoint: {config.get('authorization_endpoint')}"
            )
            print_info(f"Token endpoint: {config.get('token_endpoint')}")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.post(
                f"{mcp_url}/mcp/manage_task", json={"action": "list"}, timeout=5.0
            )
            ...
            if response.status_code == 401:
                ...
                print_warning(
                    f"Unexpected response for invalid token: {response.status_code}"
                )
