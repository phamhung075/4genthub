...
async def test_email_service():
    ...
    try:
        ...
        if email_test["success"]:
            ...
            print(
                f"   ❌ Enhanced auth service error: {email_test.get('error_message')}"
            )
        ...
        is_valid = TokenManager.validate_token(
            token_data["token"], "test@example.com", token_data["hash"]
        )
...
async def test_real_email_send():
    ...
    try:
        ...
        result = await email_service.send_verification_email(
            email=test_email, user_name="Test User"
        )
