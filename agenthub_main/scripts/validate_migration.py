...
class MigrationValidator:
    ...
    def validate_data_integrity(self, engine) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                integrity_results["foreign_key_constraints"].append(
                    "✅ Foreign key relationships working"
                )
                integrity_results["relationship_validation"].append(
                    "✅ ORM relationships working"
                )

            except Exception as e:
                integrity_results["foreign_key_constraints"].append(
                    f"❌ Foreign key constraint error: {e}"
                )
            ...
            try:
                ...
                retrieved_project = (
                    ...
                    .filter_by(name="Integrity Test Project")
                    .first()
                )
                assert retrieved_project.metadata == complex_metadata

                integrity_results["json_field_validation"].append(
                    "✅ JSON fields working correctly"
                )

            except Exception as e:
                integrity_results["json_field_validation"].append(
                    f"❌ JSON field error: {e}"
                )
    ...
    def validate_repository_functionality(self, database_type: str) -> dict[str, Any]:
        ...
        functionality_results = {"database_type": database_type, "repository_tests": []}
            ...
            with patch.dict(os.environ, {"DATABASE_TYPE": database_type}):
                ...
                try:
                    ...
                    if repo_type in project_repo.__class__.__name__:
                        functionality_results["repository_tests"].append(
                            f"✅ ProjectRepository {repo_type} working"
                        )
                    else:
                        functionality_results["repository_tests"].append(
                            f"❌ ProjectRepository wrong type: {project_repo.__class__.__name__}"
                        )
                except Exception as e:
                    functionality_results["repository_tests"].append(
                        f"❌ ProjectRepository error: {e}"
                    )
                ...
                try:
                    ...
                    if repo_type in agent_repo.__class__.__name__:
                        functionality_results["repository_tests"].append(
                            f"✅ AgentRepository {repo_type} working"
                        )
                    else:
                        functionality_results["repository_tests"].append(
                            f"❌ AgentRepository wrong type: {agent_repo.__class__.__name__}"
                        )
                except Exception as e:
                    functionality_results["repository_tests"].append(
                        f"❌ AgentRepository error: {e}"
                    )
                ...
                try:
                    ...
                    if repo_type in task_repo.__class__.__name__:
                        functionality_results["repository_tests"].append(
                            f"✅ TaskRepository {repo_type} working"
                        )
                    else:
                        functionality_results["repository_tests"].append(
                            f"❌ TaskRepository wrong type: {task_repo.__class__.__name__}"
                        )
                except Exception as e:
                    functionality_results["repository_tests"].append(
                        f"❌ TaskRepository error: {e}"
                    )

        except Exception as e:
            functionality_results["repository_tests"].append(
                f"❌ Repository functionality error: {e}"
            )
    ...
    def validate_performance(self, engine) -> dict[str, Any]:
        ...
        try:
            ...
            performance_results["connection_test"] = (
                f"✅ Connection established in {connection_time:.3f}s"
            )
    ...
    def compare_sqlite_postgresql_schemas(self) -> dict[str, Any]:
        ...
        try:
            ...
            comparison_results["sqlite_schema"] = self.validate_database_schema(
                sqlite_engine
            )
            ...
            try:
                postgresql_url = os.getenv(
                    "DATABASE_URL",
                    "postgresql://dev_user:dev_password@localhost:5432/agenthub_dev",
                )
                postgresql_engine = create_engine(postgresql_url)
                Base.metadata.create_all(postgresql_engine)
                comparison_results["postgresql_schema"] = self.validate_database_schema(
                    postgresql_engine
                )

                # Compare schemas
                sqlite_tables = set(
                    comparison_results["sqlite_schema"]["existing_tables"]
                )
                postgresql_tables = set(
                    comparison_results["postgresql_schema"]["existing_tables"]
                )

                if sqlite_tables == postgresql_tables:
                    comparison_results["differences"].append(
                        "✅ Table names match between SQLite and PostgreSQL"
                    )
                else:
                    comparison_results["differences"].append(
                        f"❌ Table mismatch: SQLite={sqlite_tables}, PostgreSQL={postgresql_tables}"
                    )

            except Exception as e:
                comparison_results["differences"].append(
                    f"⚠️ PostgreSQL not available: {e}"
                )
    ...
    def run_full_validation(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.results["schema_validation"]["sqlite"] = self.validate_database_schema(
                sqlite_engine
            )
            self.results["data_integrity"]["sqlite"] = self.validate_data_integrity(
                sqlite_engine
            )
            self.results["functionality_tests"]["sqlite"] = (
                self.validate_repository_functionality("sqlite")
            )
            self.results["performance_tests"]["sqlite"] = self.validate_performance(
                sqlite_engine
            )
        ...
        try:
            ...
            self.results["schema_validation"]["postgresql"] = (
                self.validate_database_schema(postgresql_engine)
            )
            self.results["data_integrity"]["postgresql"] = self.validate_data_integrity(
                postgresql_engine
            )
            self.results["functionality_tests"]["postgresql"] = (
                self.validate_repository_functionality("postgresql")
            )
            self.results["performance_tests"]["postgresql"] = self.validate_performance(
                postgresql_engine
            )
    ...
    def generate_report(self) -> str:
        ...
        for db_type, schema_info in self.results["schema_validation"].items():
            ...
            if schema_info["missing_tables"]:
                report.append(
                    f"  ❌ Missing: {', '.join(schema_info['missing_tables'])}"
                )
            else:
