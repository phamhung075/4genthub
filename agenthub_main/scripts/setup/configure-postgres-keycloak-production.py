...
BACKEND_ROOT = PROJECT_ROOT / "agenthub_main"
FRONTEND_ROOT = PROJECT_ROOT / "agenthub-frontend"

# Files that may contain Supabase references
SUPABASE_FILES_TO_CHECK = [
    # Backend files
    "agenthub_main/src/fastmcp/server/routes/auth_starlette.py",
    "agenthub_main/src/fastmcp/server/routes/auth_keycloak_enhanced.py",
    "agenthub_main/src/fastmcp/server/routes/task_routes.py",
    "agenthub_main/src/fastmcp/server/manage_connection_tool.py",
    "agenthub_main/src/fastmcp/task_management/infrastructure/database/database_config.py",
    "agenthub_main/src/fastmcp/task_management/infrastructure/database/database_initializer.py",
    "agenthub_main/src/fastmcp/task_management/infrastructure/database/connection_pool.py",
    "agenthub_main/src/fastmcp/task_management/infrastructure/repositories/repository_factory.py",
    # Test files
    "agenthub_main/src/tests/auth/test_email_service_integration.py",
    "agenthub_main/src/tests/unit/auth/token_validator_test.py",
    "agenthub_main/src/tests/unit/auth/middleware/dual_auth_middleware_test.py",
    # Frontend files
    "agenthub-frontend/src/contexts/AuthContext.tsx",
    "agenthub-frontend/src/services/api.ts",
]
...
FILES_TO_REMOVE = [
    ...
    "agenthub_main/scripts/test_supabase_auth.py",
    "agenthub_main/scripts/run_supabase_migration.py",
]
...
class ProductionConfigurator:
    ...
    def _clean_auth_context(self, content: str) -> str:
        ...
        content = re.sub(r"const\s+supabase\s*=.*?;", "", content, flags=re.DOTALL)

        # Remove Supabase-specific authentication methods
        content = re.sub(
            r"const\s+loginWithSupabase.*?};", "", content, flags=re.DOTALL
        )
...
DATABASE_URL=postgresql://agenthub_user:CHANGE_ME_SECURE_PASSWORD@postgres:5432/agenthub_prod?sslmode=prefer
...
"""
    ...
    def validate_postgresql_connection(self) -> bool:
        ...
        try:
            ...
            db_name = os.getenv("DATABASE_NAME", "agenthub_prod")
        ...
        try:
            ...
            with httpx.Client() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"✅ Keycloak connected: Realm '{realm_info.get('realm')}' is accessible"
                    )
                    return True
                else:
                    logger.error(
                        f"❌ Keycloak realm not accessible: {response.status_code}"
                    )
                    return False
...
def main():
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run validation tests after configuration"
    )
    ...
    if not env_path.exists():
        ...
        if template_path.exists():
            ...
            logger.info(
                "✅ Created .env from template. Please update it with your values."
            )
        else:
            logger.error(
                "❌ No .env or .env.template found. Configuration cannot proceed."
            )
            sys.exit(1)
    ...
    # Run configuration
    configurator = ProductionConfigurator(clean_backups=args.clean, run_tests=args.test)
