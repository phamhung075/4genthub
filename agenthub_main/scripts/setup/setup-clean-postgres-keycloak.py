...
===============================================================
This script configures the agenthub system to use:
...
class SetupManager:
    def __init__(self):
        ...
        self.src_dir = self.root_dir / "agenthub_main" / "src"
...
DATABASE_URL=postgresql://postgres:postgres_secure_password_2025@localhost:5432/agenthub

# PostgreSQL Docker Configuration
POSTGRES_CONTAINER_NAME=agenthub-postgres
POSTGRES_DATA_VOLUME=agenthub_postgres_data
POSTGRES_BACKUP_VOLUME=agenthub_postgres_backup
...
KEYCLOAK_TOKEN_ISSUER=https://your-keycloak-domain.com/realms/agenthub
...
# DOCKER
# =============================================================================
COMPOSE_PROJECT_NAME=agenthub-clean
DOCKER_NETWORK=agenthub_clean_network
...
# =============================================================================
PGADMIN_EMAIL=admin@agenthub.com
...
services:
  ...
  postgres:
    ...
    container_name: ${POSTGRES_CONTAINER_NAME:-agenthub-postgres}
    environment:
      POSTGRES_DB: ${DATABASE_NAME:-agenthub}
    ...
    networks:
      - agenthub_network
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U ${DATABASE_USER:-postgres} -d ${DATABASE_NAME:-agenthub}"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  backend:
    ...
    environment:
      ...
      DATABASE_PORT: 5432
      DATABASE_NAME: ${DATABASE_NAME:-agenthub}
      DATABASE_USER: ${DATABASE_USER:-postgres}
      DATABASE_PASSWORD: ${DATABASE_PASSWORD:-postgres_secure_password_2025}
      DATABASE_URL: postgresql://${DATABASE_USER:-postgres}:${DATABASE_PASSWORD:-postgres_secure_password_2025}@postgres:5432/${DATABASE_NAME:-agenthub}
...
networks:
  agenthub_network:
    driver: bridge
    name: ${DOCKER_NETWORK:-agenthub_clean_network}

volumes:
  postgres_data:
    name: ${POSTGRES_DATA_VOLUME:-agenthub_postgres_data}
  postgres_backup:
    name: ${POSTGRES_BACKUP_VOLUME:-agenthub_postgres_backup}
  pgadmin_data:
...
echo "=========================================="
echo " Starting agenthub Clean Setup"
echo "=========================================="
...
docker-compose -f docker-compose.clean.yml exec postgres psql -U postgres -c "CREATE DATABASE agenthub;" 2>/dev/null || echo "Database already exists"
...
"""
    ...
    def run(self):
        ...
        self.print_header("agenthub Clean Setup - PostgreSQL + Keycloak")
