#!/usr/bin/env python3
"""
PostgreSQL + Keycloak Setup Script for agenthub
...
class PostgresKeycloakSetup:
    """Configure agenthub for PostgreSQL + Keycloak"""
    ...
    def create_env_file(self) -> bool:
        ...
        env_content = """# agenthub Environment Configuration
...
# Connection URL
DATABASE_URL=postgresql://agenthub_user:SecurePassword2025!@localhost:5432/agenthub_prod?sslmode=prefer
...
# ===================================================================
DOCKER_NETWORK=agenthub_network
...
"""
    ...
    def remove_supabase_references(self) -> int:
        ...
        for root, dirs, files in os.walk(self.project_root / "agenthub_main"):
    ...
    def setup_database_init_script(self) -> bool:
        ...
        init_sql_path = self.project_root / "agenthub_main" / "scripts" / "init.sql"
        init_sql_path.parent.mkdir(parents=True, exist_ok=True)
        
        init_sql = """-- PostgreSQL Initialization Script for agenthub

-- Create database if not exists (run as superuser)
SELECT 'CREATE DATABASE agenthub_prod'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'agenthub_prod')\\gexec
...
GRANT ALL ON ALL TABLES IN SCHEMA public TO agenthub_user;
GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO agenthub_user;
...
"""
    ...
    def run_database_migrations(self) -> bool:
        ...
        migration_script = self.project_root / "agenthub_main" / "src" / "fastmcp" / "task_management" / "infrastructure" / "database" / "init_database.py"
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                cwd=self.project_root / "agenthub_main"
            )
    ...
    def run_setup(self):
        ...
        print("=" * 60)
        print("agenthub PostgreSQL + Keycloak Setup")
        print("=" * 60)
        ...
        print("  Connect to PostgreSQL: psql -h localhost -U agenthub_user -d agenthub_prod")
