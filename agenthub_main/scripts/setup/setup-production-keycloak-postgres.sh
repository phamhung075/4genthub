...
# ============================================================================
# This script sets up agenthub with:
...
echo -e "${BLUE}============================================================================${NC}"
echo -e "${BLUE}agenthub Production Setup - PostgreSQL Docker + Keycloak Cloud${NC}"
echo -e "${BLUE}============================================================================${NC}"
...
cat > config/pgadmin/servers.json << 'EOF'
{
    "Servers": {
        "1": {
            ...
            "SSLMode": "prefer",
            "Comment": "agenthub Production Database"
        }
    }
}
EOF
...
for i in {1..30}; do
    if docker-compose -f "$DOCKER_COMPOSE_FILE" exec postgres pg_isready -U agenthub_user -d agenthub_prod &>/dev/null; then
...
from agenthub_main.src.fastmcp.task_management.infrastructure.database.database_initializer import DatabaseInitializer
...
if [ "$start_pgadmin" = "y" ]; then
    ...
    echo -e "                Email: admin@agenthub.com"
...
echo -e "  DB shell:     ${BLUE}docker-compose -f $DOCKER_COMPOSE_FILE exec postgres psql -U agenthub_user -d agenthub_prod${NC}"
