#!/usr/bin/env python3
"""
Enhanced Test Coverage Analysis for agenthub - DDD Architecture Focus
...
class EnhancedTestCoverageAnalyzer:
    ...
    def __init__(
        self,
        src_path: str = "agenthub_main/src",
        test_path: str = "agenthub_main/src/tests",
    ):
    ...
    def _analyze_source_files(self):
        ...
        for py_file in self.src_path.rglob("*.py"):
            ...
            if (
                py_file.name in ["__init__.py", "__main__.py"]
                or "__pycache__" in str(py_file)
                or "migrations" in str(py_file).lower()
            ):
                continue
    ...
    def _analyze_single_file(self, file_path: Path) -> FileAnalysis:
        ...
        try:
            ...
            has_test = any(
                module_path.endswith(existing) for existing in self.existing_tests
            )
    ...
    def _calculate_priorities(self):
        ...
        for file_analysis in self.files:
            ...
            component_priority = self.COMPONENT_PRIORITIES.get(
                file_analysis.component, 1
            )
            ...
            domain_bonus = (
                10
                if file_analysis.domain == "task_management"
                else 5
                if file_analysis.domain in ["auth", "connection_management"]
                else 0
            )
    ...
    def _generate_comprehensive_report(self) -> dict:
        ...
        report = {
            "summary": {
                ...
                "coverage_percentage": len(tested_files) / len(self.files) * 100
                if self.files
                else 0,
            },
    ...
    def _calculate_layer_stats(self) -> dict:
        ...
        layer_stats = defaultdict(
            lambda: {"total": 0, "tested": 0, "untested": 0, "components": Counter()}
        )
    ...
    def _calculate_domain_stats(self) -> dict:
        ...
        domain_stats = defaultdict(
            lambda: {"total": 0, "tested": 0, "untested": 0, "layers": Counter()}
        )
    ...
    def _print_detailed_report(self, report: dict):
        ...
        print(
            f"   Files with tests:   {summary['tested_files']:>6} ({summary['tested_files'] / summary['total_files'] * 100:5.1f}%)"
        )
        print(
            f"   Files without tests: {summary['untested_files']:>5} ({summary['untested_files'] / summary['total_files'] * 100:5.1f}%)"
        )
        ...
        for layer in ["domain", "application", "infrastructure", "interface"]:
            if layer in layer_stats:
                ...
                priority_indicator = (
                    "🔴" if coverage < 30 else "🟡" if coverage < 60 else "🟢"
                )

                print(
                    f"   {priority_indicator} {layer.upper():<15}: {stats['tested']:3d}/{stats['total']:3d} ({coverage:5.1f}%)"
                )
                ...
                if missing_components:
                    ...
                    components_str = ", ".join(
                        [f"{comp}({count})" for comp, count in top_missing]
                    )
        ...
        for domain, stats in sorted(
            domain_stats.items(), key=lambda x: -x[1]["untested"]
        ):
            coverage = (
                stats["tested"] / stats["total"] * 100 if stats["total"] > 0 else 0
            )
            priority_indicator = (
                "🔴"
                if stats["untested"] > 50
                else "🟡"
                if stats["untested"] > 20
                else "🟢"
            )

            print(
                f"   {priority_indicator} {domain.upper():<20}: {stats['tested']:3d}/{stats['total']:3d} ({coverage:5.1f}%) - {stats['untested']} missing"
            )
        ...
        for i, file_analysis in enumerate(top_files, 1):
            priority_icon = (
                "🔴"
                if file_analysis.priority > 150
                else "🟡"
                if file_analysis.priority > 100
                else "🟠"
            )

            print(f"   {i:2d}. {priority_icon} {file_analysis.path}")
            print(
                f"       Domain: {file_analysis.domain} | Layer: {file_analysis.layer} | Component: {file_analysis.component}"
            )
            print(
                f"       Priority: {file_analysis.priority} | LOC: {file_analysis.lines_of_code} | Classes: {file_analysis.class_count} | Methods: {file_analysis.method_count}"
            )
        ...
        print("   " + "-" * 50)
        print(
            "   1. 🎯 IMMEDIATE FOCUS: Domain entities and services (business logic core)"
        )
        print(
            "   2. 🔄 NEXT PHASE: Application use cases and facades (workflow orchestration)"
        )
        print("   3. 🔌 INFRASTRUCTURE: Repositories and data access patterns")
        print("   4. 🌐 INTERFACE: Controllers (focus on integration tests)")
        print(
            "   5. 📊 METRICS: Aim for 80%+ coverage in Domain, 70%+ in Application layers"
        )
        ...
        quick_wins = [
            f
            for f in top_files
            if f.component in ["entities", "value_objects", "exceptions"]
            and f.lines_of_code < 200
        ][:10]

        for i, file_analysis in enumerate(quick_wins, 1):
            test_path = f"agenthub_main/src/tests/unit/{file_analysis.path.replace('.py', '_test.py')}"
        ...
        for i, file_analysis in enumerate(top_files[:10], 1):
            ...
            print(
                f"   # {i}. {file_analysis.component.upper()} - {file_analysis.domain}"
            )
