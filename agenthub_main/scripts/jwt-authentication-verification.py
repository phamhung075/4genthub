...
class JWTAuthenticationTester:
    ...
    def test_configuration(self) -> bool:
        ...
        if not jwt_secret:
            ...
            if jwt_secret == "default-secret-key-change-in-production":
                ...
                issues.append(
                    f"JWT_SECRET_KEY is too short ({len(jwt_secret)} chars, recommend 32+)"
                )
        ...
        if not supabase_secret:
            ...
            self.log(
                f"✅ SUPABASE_JWT_SECRET defined ({len(supabase_secret)} characters)"
            )
        ...
        if jwt_secret and supabase_secret:
            if jwt_secret == supabase_secret:
                ...
                self.log(
                    "⚠️  JWT secrets differ - may cause authentication issues", "warning"
                )
                self.log(
                    f"   Backend uses: JWT_SECRET_KEY ({len(jwt_secret)} chars)",
                    "warning",
                )
                self.log(
                    f"   Frontend uses: SUPABASE_JWT_SECRET ({len(supabase_secret)} chars)",
                    "warning",
                )
    ...
    def test_token_generation(self) -> dict[str, Any]:
        ...
        if supabase_secret and supabase_secret != jwt_secret:
            try:
                ...
                generation_results["supabase_jwt_secret"] = {
                    ...
                    "token_preview": supabase_token[:50] + "..."
                    if len(supabase_token) > 50
                    else supabase_token,
                }
                self.log(
                    f"✅ Token generated with SUPABASE_JWT_SECRET ({len(supabase_token)} chars)"
                )
                ...
                self.log(
                    f"❌ Token generation with SUPABASE_JWT_SECRET failed: {e}", "error"
                )
        ...
        if (
            "jwt_secret_key" in generation_results
            and generation_results["jwt_secret_key"]["status"] == "success"
        ):
            return jwt_token
        elif (
            "manual_jwt" in generation_results
            and generation_results["manual_jwt"]["status"] == "success"
        ):
            return manual_token
        else:
            return None
    ...
    def test_token_validation(self, token: str) -> bool:
        ...
        try:
            ...
            if payload:
                ...
                self.log(
                    f"✅ Token validated with JWT_SECRET_KEY (user: {payload.get('sub') or payload.get('user_id')})"
                )
            else:
                validation_results["jwt_secret_key"] = {
                    "status": "failed",
        ...
        if supabase_secret and supabase_secret != jwt_secret:
            try:
                supabase_jwt_service = JWTService(secret_key=supabase_secret)
                payload = supabase_jwt_service.verify_token(
                ...
                if payload:
                    ...
                    self.log(
                        f"✅ Token validated with SUPABASE_JWT_SECRET (user: {payload.get('sub') or payload.get('user_id')})"
                    )
                else:
                    validation_results["supabase_jwt_secret"] = {
                        "status": "failed",
                ...
                self.log(
                    f"❌ Token validation with SUPABASE_JWT_SECRET failed: {e}", "error"
                )
        ...
        for secret_name, secret_value in [
            ...
            try:
                ...
                self.log(
                    f"⚠️  Direct PyJWT validation with {secret_name}: token expired",
                    "warning",
                )
                ...
                self.log(
                    f"❌ Direct PyJWT validation with {secret_name} failed: {e}",
                    "error",
                )
            ...
            try:
                ...
                if method == "GET":
                    ...
                    response = requests.post(
                        url, headers=test_headers, json={}, timeout=10
                    )
                else:
                    continue
                ...
                if response.status_code < 400:
                    ...
                    self.log(
                        f"❌ {method} {path} - 401 Unauthorized (authentication failed)",
                        "error",
                    )
                elif response.status_code == 404:
                    self.log(
                        f"⚠️  {method} {path} - 404 Not Found (endpoint may not exist)",
                        "warning",
                    )
                else:
    ...
    def test_middleware_compatibility(self, token: str) -> bool:
        ...
        for secret_name, secret_value in secrets_to_try:
            try:
                ...
                for token_type in ["api_token", "access"]:
                    try:
                        ...
                        if payload:
                            ...
                            self.log(
                                f"✅ Middleware simulation: {secret_name} + {token_type} type = SUCCESS"
                            )
                            break
                    except Exception as e:
                        middleware_results["secret_attempts"].append(
                        ...
                        self.log(
                            f"❌ Middleware simulation: {secret_name} + {token_type} type = FAILED: {e}",
                            "debug",
                        )
                ...
                self.log(
                    f"❌ Middleware simulation: {secret_name} service creation failed: {e}",
                    "error",
                )
        ...
        successes = [
            a
            for a in middleware_results["secret_attempts"]
            if a.get("status") == "success"
        ]
        if successes:
            self.log(
                f"✅ Middleware compatibility test passed ({len(successes)} successful attempts)"
            )
            self.results["summary"]["passed"] += 1
            return True
        else:
            self.log(
                "❌ Middleware compatibility test failed - no successful validation attempts",
                "error",
            )
            self.results["summary"]["failed"] += 1
            return False
    ...
    def generate_summary(self):
        ...
        if summary["failed"] == 0:
            ...
            self.log(
                "🚨 Some tests failed. Review the errors above and check your configuration.",
                "error",
            )
        ...
        if not config.get("secrets_match", True):
            ...
            self.log(
                "   Set JWT_SECRET_KEY and SUPABASE_JWT_SECRET to the same value",
                "info",
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="JWT Authentication End-to-End Verification"
    )
    parser.add_argument(
        "--base-url",
    ...
    if JWTService is None:
        print("❌ Cannot run full test suite without agenthub modules")
