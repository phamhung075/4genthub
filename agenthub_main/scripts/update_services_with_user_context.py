...
def update_service_file(filepath: Path, analysis: dict) -> bool:
    ...
    if not analysis["has_user_id"]:
        ...
        init_body_replacement = (
            r"\1        self._user_id = user_id  # Store user context\n"
        )
    ...
    if not analysis["has_with_user"]:
        ...
        if "_get_user_scoped_repository" in content:
            pattern = (
                r"(def _get_user_scoped_repository[^:]+:[^}]+return repository\s*\n)"
            )
...
def generate_summary_report(analyses: list[dict]) -> str:
    ...
    needs_update = [
        a for a in analyses if not (a["has_user_id"] and a["has_with_user"])
    ]
    ...
    report.append(
        "    def __init__(self, repo1=None, repo2=None, user_id: Optional[str] = None):"
    )
