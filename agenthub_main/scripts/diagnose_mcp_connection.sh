...
if [[ -f ".cursor/mcp.json" ]]; then
    ...
    echo "📋 agenthub configuration:"
    jq '.mcpServers.agenthub' .cursor/mcp.json 2>/dev/null || echo "❌ Error reading JSON"
else
...
if [[ -f "agenthub_main/.venv/bin/python" ]]; then
    echo "✅ Virtual environment exists"
    echo "📋 Python version: $(agenthub_main/.venv/bin/python --version)"
else
    echo "❌ Virtual environment not found at agenthub_main/.venv/bin/python"
fi
echo ""
...
if agenthub_main/.venv/bin/python -c "
import sys
...
" 2>/dev/null; then
    ...
    agenthub_main/.venv/bin/python -c "
import sys
sys.path.insert(0, 'agenthub_main/src')
from fastmcp.server.mcp_entry_point import create_agenthub_server
server = create_agenthub_server()
...
echo "2. After restart, check if agenthub tools appear in the MCP tools list"
