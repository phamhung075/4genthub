#!/bin/bash
# agenthub Project Cleanup Script
# Version: 1.0
# Date: 2025-09-08
...
BACKUP_DIR="/tmp/agenthub-cleanup-backup-$(date +%Y%m%d_%H%M%S)"
...
show_header() {
    echo -e "${BLUE}"
    echo "╔══════════════════════════════════════════════════════════════════════════════╗"
    echo "║                    agenthub Project Cleanup Script                         ║"
    echo "║                                                                              ║"
...
create_backup() {
    ...
    cp "$PROJECT_ROOT/agenthub_main/logs/agenthub.log" "$BACKUP_DIR/" 2>/dev/null || log "Note: agenthub.log not found"
    cp "$PROJECT_ROOT/agenthub_main/logs/agenthub_errors.log" "$BACKUP_DIR/" 2>/dev/null || log "Note: agenthub_errors.log not found"
...
safety_checks() {
    ...
    if [ ! -f "$PROJECT_ROOT/CLAUDE.md" ]; then
        log "${RED}Error: Not in agenthub project root${NC}"
        exit 1
    fi
    ...
    if lsof +D "$PROJECT_ROOT/agenthub_main/logs/" 2>/dev/null | grep -q .; then
        log "${RED}Error: Active processes are using log files:${NC}"
        lsof +D "$PROJECT_ROOT/agenthub_main/logs/" 2>/dev/null
    ...
    # Check Docker status
    if docker ps 2>/dev/null | grep -q agenthub; then
        log "${YELLOW}Warning: agenthub Docker containers are running${NC}"
...
cleanup_logs() {
    ...
    for i in {2..9}; do
        local log_file="$PROJECT_ROOT/agenthub_main/logs/backend.log.$i"
        if [ -f "$log_file" ]; then
...
show_summary() {
    ...
    local logs_size=$(du -sh "$PROJECT_ROOT/agenthub_main/logs/" 2>/dev/null | cut -f1 || echo "N/A")
...
rollback() {
    ...
    if [ ! -d "$BACKUP_DIR" ]; then
        ...
        ls -la /tmp/agenthub-cleanup-backup-* 2>/dev/null || echo "No backups found"
        exit 1
    fi
    
    # Restore critical logs
    if [ -f "$BACKUP_DIR/agenthub.log" ]; then
        cp "$BACKUP_DIR/agenthub.log" "$PROJECT_ROOT/agenthub_main/logs/"
        log "Restored agenthub.log"
    fi
    
    if [ -f "$BACKUP_DIR/agenthub_errors.log" ]; then
        cp "$BACKUP_DIR/agenthub_errors.log" "$PROJECT_ROOT/agenthub_main/logs/"
        log "Restored agenthub_errors.log"
    fi
...
verify_cleanup() {
    ...
    if [ -f "$PROJECT_ROOT/CLAUDE.md" ] && [ -f "$PROJECT_ROOT/agenthub_main/src/fastmcp/__init__.py" ]; then
...
main() {
    ...
    log "${GREEN}Starting agenthub Project Cleanup${NC}"
...
dry_run() {
    ...
    find "$PROJECT_ROOT/agenthub_main/logs/" -name "backend.log.[2-9]" 2>/dev/null | while read -r file; do
...
case "${1:-}" in
    ...
    --rollback)
        ...
        if [ -n "$backup_path" ]; then
            ...
            BACKUP_DIR=$(ls -td /tmp/agenthub-cleanup-backup-* 2>/dev/null | head -1)
        fi
        rollback
        exit 0
        ;;
