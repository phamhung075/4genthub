...
# MCP Inspector Launcher for agenthub Server
# This script starts the MCP inspector with the correct configuration

echo "🚀 Starting MCP Inspector for agenthub Server..."
echo "📍 Working directory: $(pwd)"

# Check if we're in the right directory
if [[ ! -d ".venv" ]]; then
    echo "❌ Error: .venv directory not found. Please run this script from agenthub_main directory"
    exit 1
fi
