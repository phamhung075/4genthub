...
# =============================================================================
# agenthub Unified Docker Management Script
...
FRONTEND_CONTAINER="agenthub-frontend"
POSTGRES_CONTAINER="agenthub-postgres"
...
print_banner() {
    echo -e "${CYAN}=================================================${NC}"
    echo -e "${CYAN}🚀 agenthub Unified Docker Manager${NC}"
    echo -e "${CYAN}=================================================${NC}"
    echo ""
}
...
prepare_environment() {
    ...
    case "$mode" in
        "production")
            ...
            export AGENTHUB_AUTH_ENABLED="true"
            export AGENTHUB_MVP_MODE="false"
            ;;
        "local"|"dev")
            export DATABASE_TYPE="postgresql"
            export AGENTHUB_AUTH_ENABLED="false"
...
cmd_start() {
    ...
    print_banner
    log_info "Starting agenthub in $mode mode..."
...
cmd_stop() {
    ...
    log_info "Stopping agenthub containers..."
...
cmd_health() {
    ...
    if check_container_running "$CONTAINER_NAME"; then
        ...
        if check_container_running "$POSTGRES_CONTAINER"; then
            if docker exec $POSTGRES_CONTAINER pg_isready -U agenthub_user -d agenthub > /dev/null 2>&1; then
...
cmd_clean() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        docker volume rm agenthub_data agenthub_logs postgres_data redis_data 2>/dev/null || true
        
        # Remove images (optional)
        read -p "Remove Docker images too? (y/N): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker rmi agenthub/mcp-server:latest agenthub/frontend:latest 2>/dev/null || true
...
cmd_rebuild() {
    ...
    log_info "Rebuilding agenthub environment..."
